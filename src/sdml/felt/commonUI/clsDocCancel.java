...
public class clsDocCancel {
    ...
    public boolean LoadData(int pCompanyID) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM D_COM_DOC_CANCEL_REQUEST WHERE COMPANY_ID=" + pCompanyID + " AND REQ_DATE>='" + SDMLERPGLOBAL.FinFromDateDB + "' AND REQ_DATE<='" + SDMLERPGLOBAL.FinToDateDB + "' ORDER BY REQ_DATE");
            Ready = true;
            MoveLast();
            return true;
        } catch (Exception e) {
    ...
    public boolean LoadData(int pCompanyID, int pModuleID) {
        ...
        try {
            Conn = data.getConn();
            Stmt = Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rsResultSet = Stmt.executeQuery("SELECT * FROM D_COM_DOC_CANCEL_REQUEST WHERE COMPANY_ID=" + pCompanyID + " AND REQ_DATE>='" + SDMLERPGLOBAL.FinFromDateDB + "' AND REQ_DATE<='" + SDMLERPGLOBAL.FinToDateDB + "' AND MODULE_ID=" + pModuleID + " ORDER BY REQ_DATE");
            Ready = true;
            MoveLast();
            return true;
        } catch (Exception e) {
    ...
    public static HashMap getHistoryList(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT * FROM D_COM_DOC_CANCEL_REQUEST_H WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND REQ_NO='" + pDocNo + "'");
    ...
    public boolean Insert() {
        ...
        try {
            ...
            /*java.sql.Date FinFromDate=java.sql.Date.valueOf(SDMLERPGLOBAL.FinFromDateDB);
             java.sql.Date FinToDate=java.sql.Date.valueOf(SDMLERPGLOBAL.FinToDateDB);
            ...
            setAttribute("REQ_NO", clsFirstFree.getNextFreeNo((int) SDMLERPGLOBAL.gCompanyID, 44, (int) getAttribute("FFNO").getVal(), true));
            //-------------------------------------------------
            ...
            rsHistory.updateString("ENTRY_DATE", SDMLERPGLOBAL.getCurrentDateDB());
            ...
            rsHistory.updateString("CHANGED_DATE", SDMLERPGLOBAL.getCurrentDateDB());
...
//              clsModuleInterface.CancelDocument(SDMLERPGLOBAL.gCompanyID, (int)getAttribute("MODULE_ID").getVal(),(String)getAttribute("DOC_NO").getObj());
//          }
            if (AppStatus.equals("F")) {
                //if ((int)getAttribute("MODULE_ID").getVal() == clsSalesInvoice.ModuleID) {
                //    clsSalesInvoice.CancelDoc(SDMLERPGLOBAL.gCompanyID, (String)getAttribute("DOC_NO").getObj(),(String)getAttribute("DOC_DATE").getObj());
                //}
                // else {   
                clsModuleInterface.CancelDocument(SDMLERPGLOBAL.gCompanyID, (int) getAttribute("MODULE_ID").getVal(), (String) getAttribute("DOC_NO").getObj());
                //}
    ...
    public boolean Update() {
        ...
        try {
            ...
            //stHeader=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            //rsHeader=stHeader.executeQuery("SELECT * FROM D_INV_GATEPASS_REQ_HEADER WHERE COMPANY_ID="+SDMLERPGLOBAL.gCompanyID+" AND TRIM(GATEPASS_REQ_NO)='"+theDocNo+"'");
            //rsHeader.first();
            ...
            //========= Inserting Into History =================//
            //Get the Maximum Revision No.
            int RevNo = SDMLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM D_COM_DOC_CANCEL_REQUEST_H WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND REQ_NO='" + (String) getAttribute("REQ_NO").getObj() + "'");
            RevNo++;
            ...
            if (AStatus.equals("R")) {
                ...
                //data.Execute("UPDATE D_INV_INDENT_HEADER SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+SDMLERPGLOBAL.gCompanyID+" AND INDENT_NO='"+ObjFlow.DocNo+"'");
                //Remove Old Records from D_COM_DOC_DATA
                //data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID="+SDMLERPGLOBAL.gCompanyID+" AND MODULE_ID=3 AND DOC_NO='"+ObjFlow.DocNo+"'");
            ...
            if (IsRejected) {
                //Remove the Rejected Flag First
                data.Execute("UPDATE D_COM_DOC_CANCEL_REQUEST SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND REQ_NO='" + ObjFlow.DocNo + "'");
                //Remove Old Records from D_COM_DOC_DATA
                data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND MODULE_ID=44 AND DOC_NO='" + ObjFlow.DocNo + "'");
            ...
            if (AppStatus.equals("F")) {
                /*if (getAttribute("MODULE_ID").getInt() == clsSalesInvoice.ModuleID) {
                 ...
                 clsSales_Party.CancelParty(SDMLERPGLOBAL.gCompanyID, getAttribute("DOC_NO").getString(),getAttribute("MAIN_CODE").getString());
                 } else {   
                 */
                clsModuleInterface.CancelDocument(SDMLERPGLOBAL.gCompanyID, (int) getAttribute("MODULE_ID").getVal(), (String) getAttribute("DOC_NO").getObj());
                //}
    ...
    public boolean Filter(String pCondition, long pCompanyID) {
        ...
        try {
            ...
            if (rsResultSet.getRow() <= 0) {
                strSql = "SELECT * FROM D_COM_DOC_CANCEL_REQUEST WHERE COMPANY_ID=" + Long.toString(pCompanyID) + " AND REQ_DATE>='" + SDMLERPGLOBAL.FinFromDateDB + "' AND REQ_DATE<='" + SDMLERPGLOBAL.FinToDateDB + "' ORDER BY REQ_NO";
                rsResultSet = Stmt.executeQuery(strSql);
                Ready = true;
                MoveLast();
    ...
    public static HashMap getPendingApprovals(int pCompanyID, int pUserID, int pOrder, int FinYearFrom) {
        ...
        try {
            if (pOrder == SDMLERPGLOBAL.OnRecivedDate) {
            ...
            if (pOrder == SDMLERPGLOBAL.OnDocDate) {
            ...
            if (pOrder == SDMLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT D_COM_DOC_CANCEL_REQUEST.REQ_NO,D_COM_DOC_CANCEL_REQUEST.REQ_DATE,RECEIVED_DATE,D_COM_DOC_CANCEL_REQUEST.DEPT_ID,D_COM_DOC_CANCEL_REQUEST.MODULE_ID FROM D_COM_DOC_CANCEL_REQUEST,D_COM_DOC_DATA WHERE D_COM_DOC_CANCEL_REQUEST.REQ_NO=D_COM_DOC_DATA.DOC_NO AND D_COM_DOC_CANCEL_REQUEST.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_COM_DOC_CANCEL_REQUEST.COMPANY_ID=" + Integer.toString(pCompanyID) + " AND D_COM_DOC_DATA.USER_ID=" + pUserID + " AND D_COM_DOC_DATA.STATUS='W' AND D_COM_DOC_DATA.MODULE_ID=44 ORDER BY D_COM_DOC_CANCEL_REQUEST.REQ_NO";
            }
            ...
            Counter = 0;
            while (!rsTmp.isAfterLast()) {
                if (SDMLERPGLOBAL.isWithinDate(rsTmp.getString("REQ_DATE"), FinYearFrom)) {
                    Counter = Counter + 1;
                    clsDocCancel ObjDoc = new clsDocCancel();
