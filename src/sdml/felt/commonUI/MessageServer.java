...
public class MessageServer {
    ...
    public MessageServer() {
        SDMLERPGLOBAL.Clients = new HashMap(); //Erase all previous clients
    }

    public void startService() {
        //Starts the server to listen for clients
        try {
            SDMLERPGLOBAL.theServer = new ServerSocket(SDMLERPGLOBAL.Port);
    ...
    public boolean sendMessage(int ClientSrNo, String Message) {
        try {
            Client ObjClient = (Client) SDMLERPGLOBAL.Clients.get(Integer.toString(ClientSrNo));
            ...
            public void run() {
                while (true) {
                    ...
                    try {
                        ...
                        NewClient.clSocket = SDMLERPGLOBAL.theServer.accept();
                        ...
                        SDMLERPGLOBAL.Clients.put(Integer.toString(SDMLERPGLOBAL.Clients.size() + 1), NewClient);
                    } catch (Exception e) {
                    }
    ...
    private void ListenMessages() {
        new Thread() {
            public void run() {
                boolean done = false;
                while (true) {
                    Iterator iclients = SDMLERPGLOBAL.Clients.keySet().iterator();

                    while (iclients.hasNext()) {
                        String key = (String) iclients.next();
                        Client ObjClient = (Client) SDMLERPGLOBAL.Clients.get(key);
                        ...
                        if (ClientIsConnected(ObjClient)) {
                            ...
                            try {
                                ...
                                Client theUser = (Client) SDMLERPGLOBAL.Clients.get(key);
                                SDMLERPGLOBAL.Clients.remove(key);
    ...
    private void parseMessage(Client pClient, String pMessage, String cSrNo) {
        ...
        if (Header.trim().equals("##USER.INFO##")) {
            ...
            SDMLERPGLOBAL.Clients.put(cSrNo, pClient); //client information saved to the list
        ...
        if (Header.trim().equals("##MSG##")) {
            ...
            if (ToUser.trim().equals("##SERVER##")) {
                ...
                theMessage = SDMLERPGLOBAL.padRight(20, FromUser.trim(), " ") + "Message received from " + FromUser.trim() + " On " + SDMLERPGLOBAL.getCurrentDate() + " " + SDMLERPGLOBAL.getCurrentTime() + "\n\n" + theMessage;
            ...
            if (ToUser.trim().equals("##EVERYBODY##")) {
                ...
                while (iclients.hasNext()) {
                    ...
                    Client ObjClient = (Client) SDMLERPGLOBAL.Clients.get(key);
                    String newMessage = SDMLERPGLOBAL.padRight(20, "##MSG##", " ") + SDMLERPGLOBAL.padRight(20, FromUser, " ") + SDMLERPGLOBAL.padRight(20, ObjClient.UserName, " ") + actualMessage;
