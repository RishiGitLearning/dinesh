...
public class frmUser extends javax.swing.JApplet {
    ...
    public frmUser() {
        ...
        if(ObjUser.LoadData(SDMLERPGLOBAL.gCompanyID)) {
...
//        }
    ...
    private void TreeMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TreeMouseClicked
        ...
        if(evt.getClickCount()==2 && (EditMode==SDMLERPGLOBAL.EDIT || EditMode==SDMLERPGLOBAL.ADD))
        {
            try
            {
    ...
    private void cmdRevokeAllActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdRevokeAllActionPerformed
      ...
      for(int i=1;i<=colFunctionList.size();i++)
      {
            ...
            SDMLTreeModel aObject=(SDMLTreeModel) aNode.getUserObject();
            String Key=aObject.Key;
  ...
  private void DisplayData()
  {
      ...
      txtPassExpDate.setText(SDMLERPGLOBAL.formatDate(ObjUser.getAttribute("PASSWORD_EXPIRY_DATE").getString()));
      
      SDMLERPGLOBAL.setComboIndex(cmbDept,(int)ObjUser.getAttribute("DEPT_ID").getVal());
      SDMLERPGLOBAL.setComboIndex(cmbUser,(int)ObjUser.getAttribute("SUPERIOR_ID").getVal());
  ...
  private void SetData() {
      ...
      if(chkResetPassword.isSelected() || EditMode == SDMLERPGLOBAL.ADD) {
          ObjUser.setAttribute("PASSWORD",MBMencode.MBMen("erp123".getBytes())); 
      } else {
          ObjUser.setAttribute("PASSWORD",txtPassword.getText()); 
      }
      
      ObjUser.setAttribute("DEPT_ID",SDMLERPGLOBAL.getComboCode(cmbDept)); 
      ObjUser.setAttribute("SUPERIOR_ID",SDMLERPGLOBAL.getComboCode(cmbUser)); 
      ...
      if(txtPassExpDate.getText().trim().equals("")) {
          //ObjUser.setAttribute("PASSWORD_EXPIRY_DATE",SDMLERPGLOBAL.formatDate(clsCalcInterest.addMonthToDate(SDMLERPGLOBAL.getCurrentDateDB(),2))); 
          ObjUser.setAttribute("PASSWORD_EXPIRY_DATE",SDMLERPGLOBAL.formatDate(SDMLERPGLOBAL.addDaysToDate(SDMLERPGLOBAL.getCurrentDateDB(),60,"yyyy-MM-dd")));
      } else {
      ...
      //Audit Trail Fields
      if(EditMode==SDMLERPGLOBAL.ADD) {
          ObjUser.setAttribute("CREATED_BY",SDMLERPGLOBAL.gUserID);
          ObjUser.setAttribute("CREATED_DATE",SDMLERPGLOBAL.formatDateDB(SDMLERPGLOBAL.getCurrentDate()));
          ObjUser.setAttribute("MODIFIED_BY",SDMLERPGLOBAL.gUserID);
          ObjUser.setAttribute("MODIFIED_DATE",SDMLERPGLOBAL.formatDateDB(SDMLERPGLOBAL.getCurrentDate()));
      }
      ...
      for(int i=1;i<=colFunctionList.size();i++) {
            ...
            if(Prefix.equals("G")) {
                ...
                ObjRight.setAttribute("COMPANY_ID",SDMLERPGLOBAL.gCompanyID);
...
//      }
  ...
  private void GenerateTree()
  {
    ...
    List=ObjMenu.getMenuList(SDMLERPGLOBAL.gCompanyID,"FELT_SALES");
    
    //First create Root Node
    DefaultMutableTreeNode Root=new DefaultMutableTreeNode(new SDMLTreeModel("0","Root"));
    ...
    for(int i=1;i<=List.size();i++)
    {
        ...
        fList.clear();
        fList=ObjMenu.getFunctionList(SDMLERPGLOBAL.gCompanyID,MenuID);
  ...
  private void Find()
  {
      Loader ObjLoader=new Loader(this,"sdml.felt.commonUI.frmUserFind",true);
      frmUserFind ObjReturn= (frmUserFind) ObjLoader.getObj();
      
      if(ObjReturn.Cancelled==false)
      {
          if(!ObjUser.Filter(ObjReturn.strQuery,SDMLERPGLOBAL.gCompanyID))
          {
