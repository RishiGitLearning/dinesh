...
public class clsFeltProductionApprovalFlow {
    ...
    public static boolean IsCreator(int pModuleID,String pDocNo) {
        ...
        try {
            Conn=data.getConn();
            stTmp=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            strSQL="SELECT STATUS FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID="+pModuleID+" AND DOC_NO='"+pDocNo+"' AND TYPE='C' AND USER_ID="+SDMLERPGLOBAL.gUserID;
            rsTmp=stTmp.executeQuery(strSQL);
            rsTmp.first();
            if(rsTmp.getRow()>0) {
                Creator=true;
    ...
    public boolean UpdateFlow() {
        ...
        try {
            ...
            rsTmp=data.getResult("SELECT SR_NO FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID="+ModuleID+" AND DOC_NO='"+DocNo+"' AND USER_ID="+SDMLERPGLOBAL.gNewUserID);
            rsTmp.first();
            ...
            if(Status.equals("F")){ //Final Approval
                //Set the A Flag for From User ID
                strSQL="UPDATE PRODUCTION.FELT_PROD_DOC_DATA SET STATUS='F',REMARKS='"+Remarks+"',ACTION_DATE=CURRENT_TIMESTAMP,CHANGED=1,CHANGED_DATE=CURRENT_TIMESTAMP WHERE MODULE_ID="+ModuleID+" AND DOC_NO='"+DocNo+"' AND USER_ID="+SDMLERPGLOBAL.gNewUserID;
                data.Execute(strSQL);
