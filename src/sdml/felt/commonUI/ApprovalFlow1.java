...
import java.util.*;
//import RS.Logging.*;
//import RS.Finance.FinanceGlobal;

/**
 *
 * @author 
 */
public class ApprovalFlow1 {

    public int CompanyID = 1; //Company ID
        ...
        try {
            ...
            for (int i = 1; i <= pUsers.size(); i++) {
                ...
                if (!UserFound) {
                    ...
                    rsDocData.updateString("RECEIVED_DATE", null);
                    rsDocData.updateString("ACTION_DATE", null);
    ...
    public static boolean IsCreator(int pModuleID, String pDocNo) {
        ...
        try {
            Conn = data.getConn();
            stTmp = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
            strSQL = "SELECT STATUS FROM D_COM_DOC_DATA WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + pModuleID + " AND DOC_NO='" + pDocNo + "' AND TYPE='C' AND USER_ID=" + SDMLERPGLOBAL.gUserID;
            rsTmp = stTmp.executeQuery(strSQL);
            rsTmp.first();
            if (rsTmp.getRow() > 0) {
                Creator = true;
    ...
    public static boolean IsOnceRejectedDoc(int pCompanyID, int pModuleID, String pDocNo) {
        ...
        try {
            ...
            ResultSet rsTmp;
            System.out.println("SELECT REJECTED FROM " + TableName + " WHERE "+FieldName+" = "+ pDocNo + "");
    ...
    public boolean UpdateFlow() {
        ...
        try {
            ...
            strSQL = "SELECT A.COMPANY_ID,A.HIERARCHY_ID,A.MODULE_ID,B.USER_ID,B.APPROVAL_SEQUENCE FROM D_COM_HIERARCHY A,D_COM_HIERARCHY_RIGHTS B WHERE A.COMPANY_ID=B.COMPANY_ID AND A.HIERARCHY_ID=B.HIERARCHY_ID AND A.COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND A.MODULE_ID=" + ModuleID + " AND A.HIERARCHY_ID=" + HierarchyID + " ORDER BY B.APPROVAL_SEQUENCE";
            ...
            if ((HierarchyID == 837 && SDMLERPGLOBAL.gCompanyID == 2) || (HierarchyID == 681 && SDMLERPGLOBAL.gCompanyID == 3)) {
            ...
            rsTmp = data.getResult("SELECT SR_NO FROM D_COM_DOC_DATA WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + ModuleID + " AND DOC_NO='" + DocNo + "' AND USER_ID=" + SDMLERPGLOBAL.gNewUserID);
            rsTmp.first();
            ...
            if (Status.equals("A")) //Approve and Forward
            {
                ...
                rsTmp = data.getResult("SELECT * FROM D_COM_DOC_DATA WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + ModuleID + " AND DOC_NO='" + DocNo + "' AND STATUS='W'");
                rsTmp.first();
                ...
                if (rsTmp.getRow() > 0) {
                    ...
                    data.Execute("UPDATE D_COM_DOC_DATA SET STATUS='W' WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + ModuleID + " AND DOC_NO='" + DocNo + "' AND SR_NO=" + CurrentUserSrNo);
                }
                //============== Check Completed =======================//
            ...
            {
                //Set the A Flag for From User ID
                strSQL = "UPDATE D_COM_DOC_DATA SET STATUS='F',REMARKS='" + Remarks + "',ACTION_DATE=CURDATE(),CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID=" + Integer.toString(CompanyID) + " AND MODULE_ID=" + Integer.toString(ModuleID) + " AND DOC_NO='" + DocNo + "' AND USER_ID=" + SDMLERPGLOBAL.gNewUserID;
                data.Execute(strSQL);

                //Set Approved Flag in Table Specified
                strSQL = "UPDATE " + TableName + " SET APPROVED=1,APPROVED_DATE='" + SDMLERPGLOBAL.getCurrentDateDB() + "',CHANGED=1,CHANGED_DATE=CURDATE() WHERE  " + FieldName + "='" + DocNo + "' AND COMPANY_ID=" + Integer.toString(CompanyID);
                ...
                if (UseSpecifiedURL) {
                    if (data.getIntValueFromDB(reCheck, SpecificURL) == 0) {
                        objLog.CompanyID = SDMLERPGLOBAL.gCompanyID;
                        objLog.ModuleName = "ApprovalFlow";
                        objLog.Method = "UpdateFlow";
                        objLog.Trace = " Logged on " + SDMLERPGLOBAL.getCurrentDate() + ": Update to table did not work. Approval Flag not set by query. Query was " + strSQL;
                        objLog.Remarks = "";
                        objLog.LogEvent();
            ...
            if (Status.equals("R")) //Rejected
            {
                ...
                if (ExplicitSendTo) {
                    rsTmp = data.getResult("SELECT * FROM D_COM_DOC_DATA WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + Integer.toString(ModuleID) + " AND DOC_NO='" + DocNo + "' AND USER_ID=" + To);
                    rsTmp.first();
                ...
                if (ExplicitSendTo) {
                    ...
                    System.out.println("SELECT * FROM D_COM_DOC_DATA WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + Integer.toString(ModuleID) + " AND DOC_NO='" + DocNo + "' AND USER_ID=" + To);
                    rsTmp = data.getResult("SELECT * FROM D_COM_DOC_DATA WHERE COMPANY_ID=" + SDMLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + Integer.toString(ModuleID) + " AND DOC_NO='" + DocNo + "' AND USER_ID=" + To);
                    rsTmp.first();
                ...
                //Set Approved Flag in Table Specified
                strSQL = "UPDATE " + TableName + " SET APPROVED=0,REJECTED=1,REJECTED_DATE='" + SDMLERPGLOBAL.getCurrentDateDB() + "',CHANGED=1,CHANGED_DATE=CURDATE() WHERE  " + FieldName + "='" + DocNo + "' AND COMPANY_ID=" + Integer.toString(CompanyID);
    ...
    public boolean UpdateFlow(String dbURL) {
        ...
        ResultSet rsHierarchy, rsTmp;
        Statement stHierarchy = null, stTmp = null;
        ...
        try {
            ...
            rsTmp = data.getResult("SELECT SR_NO FROM D_COM_DOC_DATA WHERE COMPANY_ID=" + CompanyID + " AND MODULE_ID=" + ModuleID + " AND DOC_NO='" + DocNo + "' AND USER_ID=" + SDMLERPGLOBAL.gNewUserID);
            rsTmp.first();
            ...
            if (Status.equals("F")) //Final Approval
            {
                ...
                if (UseSpecifiedURL) {
                    data.Execute(strSQL, SpecificURL);
                    strSQL = "UPDATE " + TableName + "_H SET APPROVED=1,APPROVED_DATE='" + SDMLERPGLOBAL.getCurrentDateDB() + "',CHANGED=1,CHANGED_DATE=CURDATE() WHERE  " + FieldName + "='" + DocNo + "' AND COMPANY_ID=" + Integer.toString(CompanyID);
                    data.Execute(strSQL, SpecificURL);
                } else {
                    data.Execute(strSQL);
                    strSQL = "UPDATE " + TableName + "_H SET APPROVED=1,APPROVED_DATE='" + SDMLERPGLOBAL.getCurrentDateDB() + "',CHANGED=1,CHANGED_DATE=CURDATE() WHERE  " + FieldName + "='" + DocNo + "' AND COMPANY_ID=" + Integer.toString(CompanyID);
