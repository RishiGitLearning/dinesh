 ...
 *
 */
public class FrmShiftSchChange extends javax.swing.JApplet {
    ...
    private clsSftChange ShiftChange;
    ...
    private void DisplayData() {
        ...
        try {
            ...
            for (int i = 1; i <= ShiftChange.colMRItems.size(); i++) {

                clsShiftSchChangeItem ObjItem = (clsShiftSchChangeItem) ShiftChange.colMRItems.get(Integer.toString(i));
                Object[] rowData = new Object[50];
                rowData[0] = i;
                rowData[1] = (String) ObjItem.getAttribute("SSCC_EMPID").getObj();
                System.out.println("SELECT EMP_NAME FROM SDMLATTPAY.ATTPAY_EMPMST WHERE PAY_EMP_NO='" + (String) ObjItem.getAttribute("SSCC_EMPID").getObj() + "'");
                rowData[2] = data.getStringValueFromDB("SELECT EMP_NAME FROM SDMLATTPAY.ATTPAY_EMPMST WHERE PAY_EMP_NO='" + (String) ObjItem.getAttribute("SSCC_EMPID").getObj() + "'");
                rowData[3] = data.getStringValueFromDB("SELECT DPTNAME FROM SDMLATTPAY.ATTPAY_EMPMST LEFT JOIN (SELECT DPTID,COALESCE(NAME,'') AS DPTNAME FROM SDMLATTPAY.ATT_DEPARTMENT_MASTER) AS D ON EMP_DEPARTMENT=DPTID WHERE PAY_EMP_NO='" + (String) ObjItem.getAttribute("SSCC_EMPID").getObj() + "'");
                rowData[4] = EITLERPGLOBAL.formatDate((String) ObjItem.getAttribute("SSCC_SHIFT_CHANGE_DATE").getObj());
                rowData[5] = (String) ObjItem.getAttribute("SSCC_CURRENT_SHIFT").getObj();
                rowData[6] = (String) ObjItem.getAttribute("SSCC_CHANGE_SHIFT").getObj();
        ...
        try {
            ...
            HashMap History = clsSftChange.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsSftChange ObjHistory = (clsSftChange) History.get(Integer.toString(i));
    ...
    private void SetMenuForRights() {
        // --- Add Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10031, 100311)) { //7008,70081
        ...
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10031, 100312)) { //7008,70082
            cmdEdit.setEnabled(true);
        } else {
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10031, 100313)) {
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10031, 100314)) {
    ...
    private void initComponents() {
        ...
        jScrollPane1.setBounds(10, 70, 900, 250);
        ...
        jLabel1.setText("Press F1 for EmpCode,Change Shift and Date in [DD/MM/YYYY] format");
        jPanel1.add(jLabel1);
        ...
        if (evt.getKeyCode() == 112) //F1 Key pressed
        {
            if (Table.getSelectedColumn() == 1) {
                ...
                if (aList.ShowLOV()) {
                    ...
                    Table.setValueAt(data.getStringValueFromDB("SELECT DPTNAME FROM SDMLATTPAY.ATTPAY_EMPMST LEFT JOIN (SELECT DPTID,COALESCE(NAME,'') AS DPTNAME FROM SDMLATTPAY.ATT_DEPARTMENT_MASTER) AS D ON EMP_DEPARTMENT=DPTID WHERE PAY_EMP_NO='" + aList.ReturnVal + "'"), Table.getSelectedRow(), 3);
                }
            }
...
//                        if (mmnth == tchkmnth.getInt("VLDMONTH")) {
//                            mvld = 1;
//                        }
//                        tchkmnth.next();
//                    }
                    ...
                    if (mvld == 0) {
                        ...
                        String mcsft;
                        System.out.println("SELECT SDMLATTPAY.ATT_SHIFT_SCHEDULE.SSC_" + Integer.parseInt(mdy) + " FROM SDMLATTPAY.ATT_SHIFT_SCHEDULE WHERE SSC_EMPID='" + Table.getValueAt(Table.getSelectedRow(), 1) + "' AND SSC_YEAR=" + myr + " AND SSC_MONTH=" + mmnth);
                        mcsft = data.getStringValueFromDB("SELECT SDMLATTPAY.ATT_SHIFT_SCHEDULE.SSC_" + Integer.parseInt(mdy) + " FROM SDMLATTPAY.ATT_SHIFT_SCHEDULE WHERE SSC_EMPID='" + Table.getValueAt(Table.getSelectedRow(), 1) + "' AND SSC_YEAR=" + myr + " AND SSC_MONTH=" + mmnth);
                        Table.setValueAt(mcsft, Table.getSelectedRow(), 5);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void FindWaiting() {
        ShiftChange.Filter(" DOC_NO IN (SELECT DISTINCT SDMLATTPAY.ATT_SHIFT_SCH_CHANGE.DOC_NO FROM SDMLATTPAY.ATT_SHIFT_SCH_CHANGE, SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.ATT_SHIFT_SCH_CHANGE.DOC_NO=SDMLATTPAY.D_COM_DOC_DATA.DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=" + ModuleId + " AND CANCELED=0) ");
        SetMenuForRights();
        DisplayData();
    }
...
//        OpgFinal.setSelected(true);
    ...
    private void FormatGrid() {
        ...
        DataModel.addColumn("Curr Shift");   //5
        DataModel.addColumn("Change Shift");   //6
    ...
    private void Save() {
        ...
        for (int k = 0; k <= Table.getRowCount() - 1; k++) {
            ...
            if (Table.getValueAt(k, 5).toString().equalsIgnoreCase("")) {
                JOptionPane.showMessageDialog(this, "Invalid Current Shift at Row " + (k + 1), "ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
            if (Table.getValueAt(k, 6).toString().equalsIgnoreCase("")) {
                JOptionPane.showMessageDialog(this, "Enter Change Shift at Row " + (k + 1), "ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
        for (int k = 0; k <= Table.getRowCount() - 1; k++) {
            for (int l = k; l <= Table.getRowCount() - 1; l++) {
                if (l != k && ((String) Table.getValueAt(k, 1)).trim().equals(((String) Table.getValueAt(l, 1)).trim())
                        && (l != k && ((String) Table.getValueAt(k, 4)).trim().equals(((String) Table.getValueAt(l, 4)).trim()))) {
                    JOptionPane.showMessageDialog(this, "Same EmpCode And Date at Row " + (k + 1) + " and " + (l + 1), "ERROR", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }
        }
...
// find rate update by doc no
    public void Find(String docNo) {
        ShiftChange.Filter(" SSCC_DOC_NO='" + docNo + "'");
        SetMenuForRights();
        DisplayData();
    }
...
//      
        ShiftChange.setAttribute("PREFIX", SelPrefix);
        ShiftChange.setAttribute("SUFFIX", SelSuffix);
        ShiftChange.setAttribute("FFNO", FFNo);
        ...
        for (int i = 0; i < Table.getRowCount(); i++) {
            ...
            myr = Integer.parseInt(dt.substring(6, 10));
            mmnth = Integer.parseInt(dt.substring(3, 5));
            clsShiftSchChangeItem ObjItem = new clsShiftSchChangeItem();
            ObjItem.setAttribute("SSCC_DOC_NO", lbldoc.getText());
            ObjItem.setAttribute("SSCC_DOC_DATE", docdate1.getText());
            ObjItem.setAttribute("SSCC_EMPID", Table.getValueAt(i, 1).toString());
            ObjItem.setAttribute("SSCC_YEAR", String.valueOf(myr).trim());
            ObjItem.setAttribute("SSCC_MONTH", String.valueOf(mmnth).trim());
            ObjItem.setAttribute("SSCC_SHIFT_CHANGE_DATE", Table.getValueAt(i, 4).toString());
            ObjItem.setAttribute("SSCC_CURRENT_SHIFT", Table.getValueAt(i, 5).toString());
            ObjItem.setAttribute("SSCC_CHANGE_SHIFT", Table.getValueAt(i, 6).toString());
            ShiftChange.colMRItems.put(Integer.toString(ShiftChange.colMRItems.size() + 1), ObjItem);
        }
