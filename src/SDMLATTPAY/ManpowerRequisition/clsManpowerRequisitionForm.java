 ...
 * @author Vivek Kumar
 */
public class clsManpowerRequisitionForm {
     ...
     */
    public clsManpowerRequisitionForm() {
        LastError = "";
        props = new HashMap();
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT * FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA GROUP BY DOC_NO ORDER BY CREATED_DATE,DOC_NO");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
...
//            else setData();
    ...
    public boolean Insert() {
        ...
        try {
            ...
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.MANPOWER_REQUISITION_DATA WHERE DOC_NO='1'");

            // Production data history connection
            statementHistory = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetHistory = statementHistory.executeQuery("SELECT * FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA_H WHERE DOC_NO='1'");

            setAttribute("DOC_NO", getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 865, 379, true));

            //Now Insert records into production tables
            for (int i = 1; i <= hmDailyAttDataDetails.size(); i++) {
                clsManpowerRequisitionFormDetails ObjDailyAttDataDetails = (clsManpowerRequisitionFormDetails) hmDailyAttDataDetails.get(Integer.toString(i));
...
//                resultSetHistory.updateString("EMP_ESI",ObjDailyAttDataDetails.getAttribute("EMP_ESI").getString());               
            ...
            ObjFlow.TableName = "SDMLATTPAY.MANPOWER_REQUISITION_DATA";
    ...
    public boolean Update() {
        ...
        try {
            ...
            resultSetHistory = statementHistory.executeQuery("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA_H WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
            ...
            statementHistory.execute("DELETE FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
...
//                resultSetHistory.updateString("EMP_ESI",ObjDailyAttDataDetails.getAttribute("EMP_ESI").getString());       
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE SDMLATTPAY.MANPOWER_REQUISITION_DATA SET REJECTED=0,CHANGED=1 WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
    ...
    public boolean CanDelete(String documentNo, String stringProductionDate, int userID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA WHERE DOC_NO='" + documentNo + "' AND APPROVED=" + 1;
            ...
            if (rsTmp.getInt("COUNT") > 0) {  //Item is Approved
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=865 AND USER_ID=" + userID + " AND DOC_NO='" + documentNo + "' AND STATUS='W'";
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();

                if (rsTmp.getInt("COUNT") > 0) {
                    strSQL = "DELETE FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA WHERE DOC_NO='" + documentNo + "'";
    ...
    public boolean Filter(String stringFindQuery) {
        Ready = false;
        try {
            String strSql = "SELECT * FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA WHERE 1=1 " + stringFindQuery + " GROUP BY DOC_NO ORDER BY DOC_NO ";
            connection = data.getConn();
            statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean setData() {
        ...
        try {
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            String str = "SELECT * FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA WHERE DOC_NO = '" + docNo + "' AND DOC_DATE = '" + docDate + "' ORDER BY SR_NO";
            ...
            while (!resultSetTemp.isAfterLast()) {
                serialNoCounter++;
                clsManpowerRequisitionFormDetails ObjDailyAttDataDetails = new clsManpowerRequisitionFormDetails();
                setAttribute("REVISION_NO", 0);
...
//                ObjDailyAttDataDetails.setAttribute("EMP_ESI", resultSetTemp.getString("EMP_ESI"));
    ...
    public static HashMap getHistoryList(String DocumentNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT * FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA_H WHERE DOC_NO='" + DocumentNo + "' GROUP BY REVISION_NO");

            while (rsTmp.next()) {
                clsManpowerRequisitionForm ObjDailyAttData = new clsManpowerRequisitionForm();
    ...
    public boolean ShowHistory(String pProductionDate, String pDocNo) {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT * FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA_H WHERE DOC_NO='" + pDocNo + "'");
            Ready = true;
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            ...
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT DISTINCT MA.DOC_NO,MA.DOC_DATE,SUBSTRING(RECEIVED_DATE,1,10) AS RECEIVED_DATE FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA MA,SDMLATTPAY.D_COM_DOC_DATA D WHERE MA.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=865 AND MA.CANCELED=0 ORDER BY RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT DISTINCT MA.DOC_NO,MA.DOC_DATE,SUBSTRING(RECEIVED_DATE,1,10) AS RECEIVED_DATE FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA MA,SDMLATTPAY.D_COM_DOC_DATA D WHERE MA.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=865 AND MA.CANCELED=0 ORDER BY MA.DOC_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT DISTINCT MA.DOC_NO,MA.DOC_DATE,SUBSTRING(RECEIVED_DATE,1,10) AS RECEIVED_DATE FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA MA,SDMLATTPAY.D_COM_DOC_DATA D WHERE MA.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=865 AND MA.CANCELED=0 ORDER BY MA.DOC_NO";
            }
            ...
            while (rsTmp.next()) {
                ...
                Counter = Counter + 1;
                clsManpowerRequisitionForm ObjDoc = new clsManpowerRequisitionForm();
    ...
    public static void CancelDoc(String pDocNo) {
        ...
        if (CanCancel(pDocNo)) {
            ...
            try {
                rsTmp = data.getResult("SELECT APPROVED FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA WHERE DOC_NO='" + pDocNo + "' ");
                rsTmp.first();
                ...
                if (!Approved) {
                    data.Execute("DELETE FROM SDMLATTPAY.D_COM_DOC_DATA WHERE DOC_NO='" + pDocNo + "' AND MODULE_ID=865");
                }
                data.Execute("UPDATE SDMLATTPAY.MANPOWER_REQUISITION_DATA SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE DOC_NO='" + pDocNo + "'");
    ...
    public static boolean CanCancel(String pDocNo) {
        ...
        try {
            ...
            System.out.println("SELECT DOC_NO FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA WHERE DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
            rsTmp = stTmp.executeQuery("SELECT DOC_NO FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA WHERE DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
