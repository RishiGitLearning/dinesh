...
public class frmManpowerRequisitionForm extends javax.swing.JApplet {
    ...
    private clsManpowerRequisitionForm ObjDailyAttData;
    ...
    public void init() {
        ...
        ObjDailyAttData = new clsManpowerRequisitionForm();
        lblTitle.setForeground(Color.WHITE);
...
//        GenerateDeptCombo();
        ...
        if (ObjDailyAttData.LoadData()) {
            ...
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Error occured while Loading Data. Error is " + ObjDailyAttData.LastError, "DATA LOADING ERROR", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void initComponents() {
        ...
        lblTitle.setText("Excess Manpower Requisition - ");
        ...
        lblDeptCmb1.setText("Request Dept : ");
        Tab1.add(lblDeptCmb1);
        lblDeptCmb1.setBounds(200, 40, 120, 20);
        ...
        Tab1.add(txtAttDate);
        txtAttDate.setBounds(90, 40, 100, 20);
...
//        EITLERPGLOBAL.PAGE_BREAK = true;
    ...
    private void cmdShowRemarksActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdShowRemarksActionPerformed
        if (TableUpdateHistory.getRowCount() > 0 && TableUpdateHistory.getSelectedRow() >= 0) {
            ...
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Select a row from Document Update History");
        }
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            GenerateRejectedSendToCombo();
            if (SDMLATTPAY.ApprovalFlow.IsOnceRejectedDoc(EITLERPGLOBAL.gCompanyID, 865, txtDocNo.getText())) {
                cmbSendTo.setEnabled(true);
            } else {
                cmbSendTo.setEnabled(false);
            }
    ...
    private void cmdDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdDeleteActionPerformed
        if (JOptionPane.showConfirmDialog(frmManpowerRequisitionForm.this, "Are you sure want to delete this record ?", "DELETE RECORD", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            Delete();
        }
...
//            ComboData 
    ...
    private void DisplayData() {
        ...
        if (EITLERPGLOBAL.gAuthorityUserID != EITLERPGLOBAL.gUserID) {
            if (clsAuthority.IsAuthorityGiven(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gUserID, EITLERPGLOBAL.gAuthorityUserID, 865)) {
        ...
        try {
            ...
            lblTitle.setText("Excess Manpower Requisition - " + docNo);
...
//            EITLERPGLOBAL.setComboIndex(cmbDept, Integer.parseInt(attDept));
            ...
            for (int i = 1; i <= ObjDailyAttData.hmDailyAttDataDetails.size(); i++) {
                clsManpowerRequisitionFormDetails ObjDailyAttDataDetails = (clsManpowerRequisitionFormDetails) ObjDailyAttData.hmDailyAttDataDetails.get(Integer.toString(i));
...
//                }
            ...
            hmList = SDMLATTPAY.ApprovalFlow.getDocumentFlow(EITLERPGLOBAL.gCompanyID, 865, docNo);
            ...
            FormatGridUpdateHistory();
            HashMap hmApprovalHistory = clsManpowerRequisitionForm.getHistoryList(txtDocNo.getText());
            for (int i = 1; i <= hmApprovalHistory.size(); i++) {
                clsManpowerRequisitionForm ObjDailyAttData = (clsManpowerRequisitionForm) hmApprovalHistory.get(Integer.toString(i));
...
////            Table.getColumnModel().getColumn(13).setMaxWidth(60);
    ...
    private void GenerateHierarchyCombo() {
        ...
        hmHierarchyList = clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=865 ");

        if (EditMode == EITLERPGLOBAL.EDIT) {
            hmHierarchyList = clsHierarchy.getList(" WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=865 ");
        }
    ...
    private void GenerateSendToCombo() {
        ...
        try {
            ...
            if (EditMode == EITLERPGLOBAL.ADD) {
                ...
                hmSendToList = SDMLATTPAY.ApprovalFlow.getRemainingUsers(EITLERPGLOBAL.gCompanyID, 865, ObjDailyAttData.getAttribute("DOC_NO").getString());
                for (int i = 1; i <= hmSendToList.size(); i++) {
                    clsUser ObjUser = (clsUser) hmSendToList.get(Integer.toString(i));
                    ComboData aData = new ComboData();
                    aData.Code = (int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void GenerateRejectedSendToCombo() {
        ...
        for (int i = 1; i <= hmRejectedSendToList.size(); i++) {
            ...
            if (EditMode == EITLERPGLOBAL.EDIT) {
                if (OpgApprove.isSelected()) {
                    IncludeUser = SDMLATTPAY.ApprovalFlow.IncludeUserInApproval(EITLERPGLOBAL.gCompanyID, 865, txtDocNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }

                if (OpgReject.isSelected()) {
                    IncludeUser = SDMLATTPAY.ApprovalFlow.IncludeUserInRejection(EITLERPGLOBAL.gCompanyID, 865, txtDocNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            int Creator = SDMLATTPAY.ApprovalFlow.getCreator(EITLERPGLOBAL.gCompanyID, 865, txtDocNo.getText());
    ...
    private void SetupApproval() {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = SDMLATTPAY.ApprovalFlow.getFromID(EITLERPGLOBAL.gCompanyID, 865, ObjDailyAttData.getAttribute("DOC_NO").getString());
            lnFromUserId = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = SDMLATTPAY.ApprovalFlow.getFromRemarks(EITLERPGLOBAL.gCompanyID, 865, FromUserID, ObjDailyAttData.getAttribute("DOC_NO").getString());
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            cmbHierarchy.setEnabled(false);
            if (SDMLATTPAY.ApprovalFlow.IsCreator(865, txtDocNo.getText())) {
                OpgReject.setEnabled(false);
            }
        }
    ...
    private void SetMenuForRights() {
        // --- Add Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10089, 100891)) {
            cmdNew.setEnabled(true);
        } else {
            cmdNew.setEnabled(false);
        }
        ...
        // --- Edit Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10089, 100892)) {
            cmdEdit.setEnabled(true);
        } else {
            cmdEdit.setEnabled(false);
        }
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10089, 100893)) {
            cmdDelete.setEnabled(true);
        } else {
            cmdDelete.setEnabled(false);
        }
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10089, 100895)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        } else {
            cmdPreview.setEnabled(false);
    ...
    private void Add() {
        ...
        txtDocNo.setText(clsManpowerRequisitionForm.getNextFreeNo(EITLERPGLOBAL.gCompanyID, 865, FFNo, false));
        txtDocDate.setText(EITLERPGLOBAL.getCurrentDate());

        lblTitle.setText("Excess Manpower Requisition - " + txtDocNo.getText());
        lblTitle.setBackground(Color.GRAY);
    ...
    private void Edit() {
        ...
        if (ObjDailyAttData.IsEditable(productionDocumentNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if (SDMLATTPAY.ApprovalFlow.IsCreator(865, productionDocumentNo)) {
                SetFields(false);
            } else {
                EnableApproval();
            }
...
//            Table.setEnabled(false);
            ...
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "You cannot edit this record. It is either approved/rejected or waiting approval for other user", "EDITING ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void Delete() {
        if (ObjDailyAttData.CanDelete(txtDocNo.getText(), txtDocDate.getText(), EITLERPGLOBAL.gNewUserID)) {
            ...
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, ObjDailyAttData.LastError, "DELETION ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
...
//        attDept = Integer.toString(cmbDept.getSelectedIndex());
        ...
        if (txtAttDate.getText().trim().equals("")) {
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Select the Att Date.", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (cmbDept.getSelectedItem().equals("Select Dept")) {
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Select the Department.", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (cmbShift.getSelectedItem().equals("Select Shift")) {
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Select the Shift.", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }

        //Check the no. of items in table
        if (Table.getRowCount() <= 0) {
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Enter Attendance Details Before Saving.", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }
...
//                }
            ...
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Enter Correct Details at Row " + (i + 1) + " and Column " + (j + 1) + ". Error is " + nfe.getMessage(), "ERROR", JOptionPane.ERROR_MESSAGE);
            j = 0;
            nfe.printStackTrace();
            return;
        }
        ...
        if (cmbHierarchy.getSelectedIndex() == -1) {
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Select the hierarchy.", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if ((!OpgApprove.isSelected()) && (!OpgReject.isSelected()) && (!OpgFinal.isSelected()) && (!OpgHold.isSelected())) {
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Select the Approval Action.", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (OpgReject.isSelected() && txtToRemarks.getText().trim().equals("")) {
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Enter the remarks for rejection", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if ((OpgApprove.isSelected() || OpgReject.isSelected()) && cmbSendTo.getItemCount() <= 0) {
            JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Select the user, to whom rejected document to be send", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            if (ObjDailyAttData.Insert()) {
                if (OpgFinal.isSelected()) {
                    data.Execute("UPDATE SDMLATTPAY.DAILY_ATT_FORM_EXCESS_LIST E, SDMLATTPAY.MANPOWER_REQUISITION_DATA F SET E.ASSIGNED_IND=1,ASSIGNED_DEPT=F.ATT_DEPT,E.ASSIGNED_REMARK=F.EMP_REMARK WHERE E.EMP_ID=F.EMP_ID AND E.ATT_DATE=F.ATT_DATE AND E.ASSIGNED_IND=0 AND F.EMP_EXCESS=0 AND F.DOC_NO='" + txtDocNo.getText().trim() + "' ");                    
                }
                DisplayData();
            } else {
                JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Error occured while saving. Error is " + ObjDailyAttData.LastError, "SAVING ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            if (ObjDailyAttData.Update()) {
                ...
                JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, "Error occured while saving editing. Error is " + ObjDailyAttData.LastError, "SAVING ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
    ...
    private void Find() {
        Loader ObjLoader = new Loader(this, "SDMLATTPAY.ManPowerRequisition.frmFindManPowerRequisitionForm", true);
        frmFindManPowerRequisitionForm ObjFindDailyAttData = (frmFindManPowerRequisitionForm) ObjLoader.getObj();

        if (ObjFindDailyAttData.Cancelled == false) {
            if (!ObjDailyAttData.Filter(ObjFindDailyAttData.stringFindQuery)) {
                JOptionPane.showMessageDialog(frmManpowerRequisitionForm.this, " No records found.", "Find Manpower Requisition Details", JOptionPane.YES_OPTION);
            }
            MoveLast();
        }
    }
    ...
    public void FindWaiting() {
        ObjDailyAttData.Filter(" AND DOC_NO IN (SELECT DISTINCT MA.DOC_NO FROM SDMLATTPAY.MANPOWER_REQUISITION_DATA MA, SDMLATTPAY.D_COM_DOC_DATA D WHERE MA.DOC_NO=D.DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=865 AND CANCELED=0) ");
        SetMenuForRights();
        DisplayData();
    }
