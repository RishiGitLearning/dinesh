...
public class clsLeaveAmend {
    ...
    public static int ModuleID = 813; //    
    ...
    public clsLeaveAmend() {
        ...
        props.put("LVT_AMEND_NO", new Variant(""));
        props.put("LVT_AMEND_DATE",new Variant(""));
        props.put("LVT_AMEND_SR_NO",new Variant(0));
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            //rsResultSet = Stmt.executeQuery("SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY ORDER BY LVT_AMEND_DATE,LVT_AMEND_NO  ");
            String str="SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY GROUP BY LVT_AMEND_NO";
            rsResultSet = Stmt.executeQuery(str);
            //rsResultSet = Stmt.executeQuery("SELECT DISTINCT LVT_AMEND_NO FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND LVT_AMEND_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY LVT_AMEND_DATE");
            
            connection = data.getConn();            
            //statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);            
            //resultSet=statement.executeQuery("SELECT DISTINCT LVT_AMEND_NO FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND LVT_AMEND_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY LVT_AMEND_DATE");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public Object getObject(int pCompanyID,String pLeaveNo) {
        String strCondition = " LVT_AMEND_NO='"+pLeaveNo+"' ";
        
        clsLeaveAmend ObjLeave = new clsLeaveAmend();
    ...
    public boolean Filter(String pCondition,int pCompanyID) {
        Ready=false;
        try {
            String strSql = "SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY " + pCondition +" ORDER BY LVT_AMEND_DATE";
            Conn=data.getConn();
            Stmt=Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean Insert() {
        ...
        ResultSet  resultSetTemp,resultSetHistory,rsTmp1;
        Statement  statementTemp, statementHistory,stTmp1;
        try {
            
            //Leave data connection
            
            statementTemp = Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_NO=''");
            ...
            resultSetHistory=statementHistory.executeQuery("SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY_H WHERE LVT_AMEND_NO=''");
           ...
           /* // ---- History Related Changes ------ //
            ...
            rsHistory = stHistory.executeQuery("SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY_H WHERE LVT_AMEND_NO='1'"); // '1' for restricting all data retrieval
            rsHistory.first();
            */
            ...
            String AmendNo="";
            int AmendSrNo=0;
            LeaveNo = (String) getAttribute("LVT_DOC_NO").getObj();
            //--------- Generate New Amendment No.- LeaveNo./Amendment Sr.no. ------------//
            //stTmp1 = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            //rsTmp1 = stTmp1.executeQuery("SELECT COALESCE(MAX(LVT_AMEND_SR_NO),0) AS MAXNO FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_DOC_NO='"+LeaveNo+"'");
            //rsTmp1.first();
            int MAXNO=data.getIntValueFromDB("SELECT COALESCE(MAX(LVT_AMEND_SR_NO),0) AS MAXNO FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_DOC_NO='"+LeaveNo+"'"); 
            //if (rsTmp1.getRow() > 0) {
            //if(MAXNO>0){
                //String AmendNo = LeaveNo + "/" + (rsTmp1.getInt("MAXNO") + 1);
                //int AmendSrNo = rsTmp1.getInt("MAXNO") + 1;
                AmendNo = LeaveNo + "/" + (MAXNO + 1);
                AmendSrNo = MAXNO + 1;
                //setAttribute("LVT_AMEND_NO", AmendNo);
                //setAttribute("LVT_AMEND_SR_NO", AmendSrNo);
...
/*
            if (Status.equals("F")) {
                ...
                data.Execute("UPDATE D_PUR_AMEND_HEADER SET LAST_AMENDMENT=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE PO_NO='" + LeaveNo);
                data.Execute("UPDATE D_PUR_AMEND_DETAIL SET LAST_AMENDMENT=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE PO_NO='" + LeaveNo);
                //------------------------------------------------------------------------//
            }
  */          
            ...
            clsLeaveAmendDetails objcolLeaveDetails=(clsLeaveAmendDetails) hmcolLeaveDetails.get(Integer.toString(i));
            ...
            resultSetTemp.updateString("LVT_YEAR", getAttribute("LVT_YEAR").getString());
            ...
            resultSetTemp.updateString("LVT_AMEND_NO", AmendNo);
            resultSetTemp.updateString("LVT_AMEND_DATE", EITLERPGLOBAL.formatDateDB((String) getAttribute("LVT_AMEND_DATE").getObj()));
            resultSetTemp.updateInt("LVT_AMEND_SR_NO",AmendSrNo);
            ...
            resultSetTemp.updateString("AMEND_TYPE", objcolLeaveDetails.getAttribute("AMEND_TYPE").getString());
            ...
            int RevNo = EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY_H WHERE LVT_AMEND_NO='" + (String) getAttribute("LVT_AMEND_NO").getObj() + "'");
            RevNo++;
            ...
            resultSetHistory.updateString("LVT_YEAR", getAttribute("LVT_YEAR").getString());
            resultSetHistory.updateString("LVT_EMPID", getAttribute("LVT_EMPID").getString());
            
//            resultSetHistory.updateString("LVT_AMEND_NO", (String) getAttribute("LVT_AMEND_NO").getObj());
//            resultSetHistory.updateString("LVT_AMEND_DATE", (String) getAttribute("LVT_AMEND_DATE").getObj());
//            resultSetHistory.updateInt("LVT_AMEND_SR_NO", (int) getAttribute("LVT_AMEND_SR_NO").getVal());
            
            resultSetHistory.updateString("LVT_AMEND_NO", AmendNo);
            resultSetHistory.updateString("LVT_AMEND_DATE", EITLERPGLOBAL.formatDateDB((String) getAttribute("LVT_AMEND_DATE").getObj()));
            resultSetHistory.updateInt("LVT_AMEND_SR_NO",AmendSrNo);
            ...
            resultSetHistory.updateString("AMEND_TYPE", objcolLeaveDetails.getAttribute("AMEND_TYPE").getString());
            ...
            try {            
                    if ("F".equals(getAttribute("APPROVAL_STATUS").getString())) {
                        if (objcolLeaveDetails.getAttribute("AMEND_TYPE").getString().equals("UPDATE")) {
                            String strUpdate="UPDATE SDMLATTPAY.ATT_LEAVE_TRN SET "                                        
                                    + " LVT_LEAVE_CODE='"+objcolLeaveDetails.getAttribute("LVT_LEAVE_CODE").getString()+"',"
                                    + " LVT_DAYS='"+objcolLeaveDetails.getAttribute("LVT_DAYS").getVal()+"',"
                                    + " LVT_FROMDATE='"+EITLERPGLOBAL.formatDateDB(objcolLeaveDetails.getAttribute("LVT_FROMDATE").getString())+"',"
                                    + "LVT_TODATE='"+EITLERPGLOBAL.formatDateDB(objcolLeaveDetails.getAttribute("LVT_TODATE").getString())+"',"
+ "LVT_MENTION_TIME='"+objcolLeaveDetails.getAttribute("LVT_MENTION_TIME").getString()+"',"
                                    + "AMEND_TYPE='UPDATE' "
                                    +" WHERE LVT_DOC_NO='"+getAttribute("LVT_DOC_NO").getString()+"' AND"
                                    +" SR_NO="+i+" AND LVT_LEAVE_TYPE=3 ";                                    
                                    
                            System.out.println(strUpdate);
                            data.Execute(strUpdate); 
                            
                            String strUpdateInLeaveAppln = "UPDATE SDMLATTPAY.ATT_LEAVE_ENTRY SET "
                                    + " LVT_LEAVE_CODE='" + objcolLeaveDetails.getAttribute("LVT_LEAVE_CODE").getString() + "',"
                                    + " LVT_DAYS='" + objcolLeaveDetails.getAttribute("LVT_DAYS").getVal() + "',"
                                    + " LVT_FROMDATE='" + EITLERPGLOBAL.formatDateDB(objcolLeaveDetails.getAttribute("LVT_FROMDATE").getString()) + "',"
                                    + "LVT_TODATE='" + EITLERPGLOBAL.formatDateDB(objcolLeaveDetails.getAttribute("LVT_TODATE").getString()) + "',"
                            ...
                            System.out.println(strUpdateInLeaveAppln);
                            data.Execute(strUpdateInLeaveAppln);                                                        
                        }
                        if (objcolLeaveDetails.getAttribute("AMEND_TYPE").getString().equals("DELETE")) {
                            String strDelete = "UPDATE SDMLATTPAY.ATT_LEAVE_TRN SET "
                                    + " LVT_LEAVE_CODE='" + objcolLeaveDetails.getAttribute("LVT_LEAVE_CODE").getString() + "',"
                                    + " LVT_LEAVE_TYPE=9,"
                                    + " LVT_DAYS='" + objcolLeaveDetails.getAttribute("LVT_DAYS").getVal() + "',"
                                    + " LVT_FROMDATE='" + EITLERPGLOBAL.formatDateDB(objcolLeaveDetails.getAttribute("LVT_FROMDATE").getString()) + "',"
                            ...
                            data.Execute(strDelete);  
                            
                            String strDeleteInLeaveAppln="DELETE FROM SDMLATTPAY.ATT_LEAVE_ENTRY "                                                                            
                                    +" WHERE LVT_DOC_NO='"+getAttribute("LVT_DOC_NO").getString()+"' AND"
                                    +" SR_NO="+i+" AND LVT_LEAVE_TYPE=3 ";                                    
                                    
                            System.out.println(strDeleteInLeaveAppln);
                            data.Execute(strDeleteInLeaveAppln);                                                        
                        }
                        
                        if (objcolLeaveDetails.getAttribute("AMEND_TYPE").getString().equals("ADD")) {
                            ...
                            String strInsertInLeaveAppln="INSERT INTO SDMLATTPAY.ATT_LEAVE_ENTRY (LVT_YEAR,LVT_EMPID,LVT_PAY_EMPID,LVT_LEAVE_CODE,LVT_LEAVE_TYPE,LVT_DAYS,LVT_FROMDATE,LVT_TODATE,LVT_REMARK,CREATED_BY,CREATED_DATE,"
                                    //+ "MODIFIED_BY,MODIFIED_DATE,"
                                    + "APPROVED,APPROVED_DATE,CANCELED,HIERARCHY_ID,CHANGED,CHANGED_DATE,FROM_IP,LVT_DOC_NO,LVT_DOC_DATE,SR_NO,LVT_MENTION_TIME)"                                        
                                    + " VALUES ('" + objcolLeaveDetails.getAttribute("LVT_YEAR").getString() + "',"
                                    + "'" + getAttribute("LVT_EMPID").getString() + "',"
                                    + "'" + getAttribute("LVT_PAY_EMPID").getString() + "',"
                                    + "'" + objcolLeaveDetails.getAttribute("LVT_LEAVE_CODE").getString() + "', 3,"
                                    ...
                                    + "'" + EITLERPGLOBAL.formatDateDB(objcolLeaveDetails.getAttribute("LVT_TODATE").getString()) + "', "
                                    + "'" + getAttribute("LVT_REMARK").getString().replaceAll("'", "''") + "',"
                                    + "'" + getAttribute("CREATED_BY").getInt() + "',"
                                    + "'" + getAttribute("CREATED_DATE").getString() + "',"
                                    //+ "'" + getAttribute("MODIFIED_BY").getInt() + "',"
                                    //+ "'" + getAttribute("MODIFIED_DATE").getString() + "',"
                                    + "1,"
                                    + "'" + EITLERPGLOBAL.getCurrentDateDB() + "',0,"
                                    + "'" + (int) getAttribute("HIERARCHY_ID").getVal() + "',1,'" + EITLERPGLOBAL.getCurrentDateDB() + "',"
                                    + "'" + str_split[1] + "',"
                                    + "'" + getAttribute("LVT_DOC_NO").getString() + "',"
                                    + "'" + EITLERPGLOBAL.formatDateDB(getAttribute("LVT_DOC_DATE").getString()) + "',"
                                    + "" + i + ",'" + objcolLeaveDetails.getAttribute("LVT_MENTION_TIME").getString() + "')";                                            
                            System.out.println(strInsertInLeaveAppln);
                            data.Execute(strInsertInLeaveAppln);                                                        
                        }
            ...
            ObjFlow.ModuleID = clsLeaveAmend.ModuleID;
            //ObjFlow.DocNo = (String) getAttribute("LVT_AMEND_NO").getObj();
            ObjFlow.DocNo = AmendNo;
            //ObjFlow.DocDate = (String) getAttribute("LVT_AMEND_DATE").getObj();
            ObjFlow.DocDate =EITLERPGLOBAL.formatDateDB((String) getAttribute("LVT_AMEND_DATE").getObj());
            ObjFlow.From = (int) getAttribute("FROM").getVal();
            ObjFlow.To = (int) getAttribute("TO").getVal();
            ObjFlow.Status = (String) getAttribute("APPROVAL_STATUS").getObj();
            ObjFlow.TableName = "SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY";
            ObjFlow.IsCreator = true;
            ObjFlow.HierarchyID = (int) getAttribute("HIERARCHY_ID").getVal();
            ObjFlow.Remarks = (String) getAttribute("REJECTED_REMARKS").getObj();
            ObjFlow.FieldName = "LVT_AMEND_NO";
    ...
    public boolean Update() {       
        ...
        try {
            ...
            */
            ...
            resultSetHistory=statementHistory.executeQuery("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY_H WHERE LVT_AMEND_NO='"+getAttribute("LVT_AMEND_NO").getString()+"'");
            ...
            statementHistory.execute("DELETE FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_NO='"+getAttribute("LVT_AMEND_NO").getString()+"'");
            
            resultSetHistory=statementHistory.executeQuery("SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY_H WHERE LVT_AMEND_NO=''");              
            
            
            //String theDocNo = getAttribute("LVT_AMEND_NO").getString();
           for(int i=1;i<=hmcolLeaveDetails.size();i++) {
            //clsLeaveApplicationDetails objcolLeaveDetails=(clsLeaveApplicationDetails) hmcolLeaveDetails.get(Integer.toString(i));
            clsLeaveAmendDetails objcolLeaveDetails=(clsLeaveAmendDetails) hmcolLeaveDetails.get(Integer.toString(i));
            resultSetTemp.moveToInsertRow();
            ...
            resultSetTemp.updateString("LVT_AMEND_NO", (String) getAttribute("LVT_AMEND_NO").getObj());
            resultSetTemp.updateString("LVT_AMEND_DATE", EITLERPGLOBAL.formatDateDB((String) getAttribute("LVT_AMEND_DATE").getObj()));
            resultSetTemp.updateInt("LVT_AMEND_SR_NO", (int) getAttribute("LVT_AMEND_SR_NO").getVal());
            ...
            //========= Inserting Into History =================//
            //Get the Maximum Revision No.
            int RevNo = EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.LVT_LEAVE_TRN_H WHERE LVT_AMEND_NO='" + (String) getAttribute("LVT_AMEND_NO").getObj() + "'");
            RevNo++;
            String RevDocNo = (String) getAttribute("LVT_AMEND_NO").getObj();
            ...
            resultSetHistory.updateString("LVT_DOC_NO", getAttribute("LVT_DOC_NO").getString());
            resultSetHistory.updateString("LVT_DOC_DATE", EITLERPGLOBAL.formatDateDB(getAttribute("LVT_DOC_DATE").getString()));
...
+ " LVT_LEAVE_TYPE=9,"
                        ...
                        if (objcolLeaveDetails.getAttribute("AMEND_TYPE").getString().equals("ADD")) {
                            String strInsert = "INSERT INTO SDMLATTPAY.ATT_LEAVE_TRN (LVT_YEAR,LVT_EMPID,LVT_PAY_EMPID,LVT_LEAVE_CODE,LVT_LEAVE_TYPE,LVT_DAYS,LVT_FROMDATE,LVT_TODATE,LVT_REMARK,LVT_DOC_NO,LVT_DOC_DATE,SR_NO,LVT_MENTION_TIME,AMEND_TYPE,LVT_TIME1,LVT_TIME2)"
                                    + " VALUES ('" + objcolLeaveDetails.getAttribute("LVT_YEAR").getString() + "',"
                                    + "'" + getAttribute("LVT_EMPID").getString() + "',"
                                    + "'" + getAttribute("LVT_PAY_EMPID").getString() + "',"
                                    + "'" + objcolLeaveDetails.getAttribute("LVT_LEAVE_CODE").getString() + "', 3,"
                                    ...
                                    + "'" + getAttribute("LVT_DOC_NO").getString() + "',"
                                    + "'" + EITLERPGLOBAL.formatDateDB(getAttribute("LVT_DOC_DATE").getString()) + "',"
                                    + "" + i + ",'" + objcolLeaveDetails.getAttribute("LVT_MENTION_TIME").getString() + "','ADD','','')";
                            System.out.println(strInsert);
                            try{
                            data.Execute(strInsert);
                            }catch(Exception e){
                            ...
                            String strInsertInLeaveAppln="INSERT INTO SDMLATTPAY.ATT_LEAVE_ENTRY (LVT_YEAR,LVT_EMPID,LVT_PAY_EMPID,LVT_LEAVE_CODE,LVT_LEAVE_TYPE,LVT_DAYS,LVT_FROMDATE,LVT_TODATE,LVT_REMARK,CREATED_BY,CREATED_DATE,MODIFIED_BY,MODIFIED_DATE,APPROVED,APPROVED_DATE,CANCELED,HIERARCHY_ID,CHANGED,CHANGED_DATE,FROM_IP,LVT_DOC_NO,LVT_DOC_DATE,SR_NO,LVT_MENTION_TIME)"                                        
                                    + " VALUES ('" + objcolLeaveDetails.getAttribute("LVT_YEAR").getString() + "',"
                                    + "'" + getAttribute("LVT_EMPID").getString() + "',"
                                    + "'" + getAttribute("LVT_PAY_EMPID").getString() + "',"
                                    + "'" + objcolLeaveDetails.getAttribute("LVT_LEAVE_CODE").getString() + "', 3,"
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE LVT_AMEND_NO='" + ObjFlow.DocNo + "' ");

                //Remove Old Records from FELT_DOC_DATA
                data.Execute("DELETE FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=" + clsLeaveAmend.ModuleID + " AND DOC_NO='" + ObjFlow.DocNo + "'");
    ...
    public boolean setData() {
        ...
        try {
            ...
            setAttribute("LVT_AMEND_NO", rsResultSet.getString("LVT_AMEND_NO"));
            setAttribute("LVT_AMEND_DATE", rsResultSet.getString("LVT_AMEND_DATE"));            
            setAttribute("LVT_AMEND_SR_NO", rsResultSet.getInt("LVT_AMEND_SR_NO"));            
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);            
            //statementTemp = Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);            
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_NO='"+rsResultSet.getString("LVT_AMEND_NO")+"' ORDER BY SR_NO");
...
//                setAttribute("REJECTED_REMARKS", rsResultSet.getString("REJECTED_REMARKS"));
                ...
                clsLeaveAmendDetails objcolLeaveDetails = new clsLeaveAmendDetails();
                ...
                objcolLeaveDetails.setAttribute("AMEND_TYPE",resultSetTemp.getString("AMEND_TYPE"));
    ...
    public boolean IsEditable(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_NO='" + pDocNo + "' AND (APPROVED=1)";
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=813 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getHistoryList(int CompanyID, String DocNo) {
        ...
        try {
            //String strSQL = "SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY_H WHERE LVT_AMEND_NO='" + DocNo + "' ";
            String strSQL = "SELECT DISTINCT REVISION_NO,LVT_AMEND_NO,LVT_AMEND_DATE,UPDATED_BY "
                    + ",APPROVAL_STATUS,ENTRY_DATE,APPROVER_REMARKS,FROM_IP FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY_H WHERE LVT_AMEND_NO='" + DocNo + "' ";
            ...
            if (rsTmp.getRow() > 0) {
                while (!rsTmp.isAfterLast()) {
                    clsLeaveAmend objParty = new clsLeaveAmend();
                    ...
                    objParty.setAttribute("LVT_AMEND_NO", rsTmp.getString("LVT_AMEND_NO"));
                    objParty.setAttribute("LVT_AMEND_DATE", rsTmp.getString("LVT_AMEND_DATE"));
    ...
    public boolean Filter(String Condition) {
        ...
        try {
            String strSQL = "";
            strSQL += "SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE " + Condition + " ";
            Conn = data.getConn();
            Stmt = Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rsResultSet = Stmt.executeQuery(strSQL);

            if (!rsResultSet.first()) {
                strSQL = "SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_DATE>='" + EITLERPGLOBAL.FinFromDateDB + "' AND LVT_AMEND_DATE<='" + EITLERPGLOBAL.FinToDateDB + "'     ORDER BY LVT_AMEND_NO";
                rsResultSet = Stmt.executeQuery(strSQL);
                Ready = true;
                MoveLast();
                return false;
    ...
    public boolean CanDelete(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            //First check that document is approved or not
            strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.LVT_LEAVE_ENTRY WHERE LVT_AMEND_NO='" + pDocNo + "' AND (APPROVED=1)";
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=813 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND TYPE='C' AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean Delete(int pUserID) {
        try {
            ...
            String lDocNo = (String) getAttribute("LVT_AMEND_NO").getObj();
            String strSQL = "";

            //First check that record is editable
            if (CanDelete(lCompanyID, lDocNo, pUserID)) {
                String strQry = "DELETE FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_NO='" + lDocNo + "'";
                data.Execute(strQry);
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT DISTINCT SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY.LVT_AMEND_NO,SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY.LVT_AMEND_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY.LVT_AMEND_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=813 ORDER BY SDMLATTPAY.D_COM_DOC_DATA.RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT DISTINCT SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY.LVT_AMEND_NO,SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY.LVT_AMEND_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY.LVT_AMEND_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=813 ORDER BY SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY.LVT_AMEND_DATE";
            }
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                ...
                Counter = Counter + 1;
                clsLeaveAmend ObjItem = new clsLeaveAmend();

                //------------- Header Fields --------------------//
                ObjItem.setAttribute("DOC_NO", rsTmp.getString("LVT_AMEND_NO"));
                ObjItem.setAttribute("DOC_DATE", rsTmp.getString("LVT_AMEND_DATE"));
    ...
    public boolean ShowHistory(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY_H WHERE LVT_AMEND_NO='" + pDocNo + "'   ORDER BY REVISION_NO");
            Ready = true;
            HistoryView = true;
            MoveFirst();
            return true;
    ...
    public static boolean IsAmendmentUnderProcess(int pCompanyID, String pLeaveNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT * FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE COMPANY_ID=" + pCompanyID + " AND PO_NO='" + pLeaveNo + "' AND APPROVED=0 AND CANCELLED=0");
            rsTmp.first();
    ...
    public static int getMaxAmendSrNo(int pCompanyID, String pLeaveNo) {
        ...
        try {
            tmpConn = data.getConn();
            stTmp = tmpConn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            rsTmp = stTmp.executeQuery("SELECT MAX(AMEND_SR_NO) AS MAXNO FROM SDMLATTTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE COMPANY_ID=" + pCompanyID + " AND LVT_AMEND_NO='" + pLeaveNo + "' ");
            rsTmp.first();
    ...
    public static boolean CanCancel(String pDocNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT LVT_AMEND_NO FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_NO='"+pDocNo+"'  AND APPROVED=0 AND CANCELED=0");
    ...
    ResultSet rsTmp=null;
        ...
        if(CanCancel(pAmendNo)) {
            ...
            try {
                rsTmp=data.getResult("SELECT APPROVED FROM SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY WHERE LVT_AMEND_NO='"+pAmendNo+"' ");
                rsTmp.first();
                ...
                if(!Approved) {
                    data.Execute("DELETE FROM SDMLATTPAY.D_COM_DOC_DATA WHERE DOC_NO='"+pAmendNo+"' AND MODULE_ID=813");
                }
                data.Execute("UPDATE SDMLATTPAY.ATT_LEAVE_UPDATION_ENTRY SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE LVT_AMEND_NO='"+pAmendNo+"'");
