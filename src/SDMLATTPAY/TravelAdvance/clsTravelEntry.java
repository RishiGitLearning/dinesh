...
public class clsTravelEntry {
    ...
    public static int ModuleID = 858; //72
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL  GROUP BY TSD_DOC_NO");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHistory = stHistory.executeQuery("SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL_H WHERE TSD_DOC_NO='1'"); // '1' for restricting all data retrieval
            rsHistory.first();
            rsHDetail = stHDetail.executeQuery("SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL_H WHERE TSD_DOC_NO='1'");
            rsHDetail.first();
            //------------------------------------//

            //--------- Generate Gatepass requisition no.  ------------
            setAttribute("TSD_DOC_NO", clsFirstFree.getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 858, (int) getAttribute("FFNO").getVal(), true));
            //-------------------------------------------------
            ...
            rsTmp = tmpStmt.executeQuery("SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL WHERE TSD_DOC_NO='1'");
            ...
            ObjFlow.ModuleID = clsTravelEntry.ModuleID;
            ...
            ObjFlow.TableName = "SDMLATTPAY.TRAVEL_SANCTION_DETAIL";
    ...
    public boolean Update() {
        ...
        try {
            ...
            int RevNo = EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL_H WHERE TSD_DOC_NO='" + (String) getAttribute("TSD_DOC_NO").getObj() + "'");
            RevNo++;
            ...
            if (ApprovalFlow.IsCreator(ModuleID, (String) getAttribute("TSD_DOC_NO").getObj())) {
                ...
                data.Execute("DELETE FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL WHERE TSD_DOC_NO='" + mDocNo + "'");

                Statement tmpStmt;

                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
                ResultSet rsTmp1 = tmpStmt.executeQuery("SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL WHERE TSD_DOC_NO='1'");
                ...
                data.Execute("UPDATE SDMLATTPAY.TRAVEL_SANCTION_DETAIL "
                        + "SET TSD_ADVANCE=" + getAttribute("TSD_ADVANCE").getString() + ",TSD_AUDIT_REMARK='" + getAttribute("TSD_AUDIT_REMARK").getString() + "' "
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE SDMLATTPAY.TRAVEL_SANCTION_DETAIL SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE TSD_DOC_NO='" + ObjFlow.DocNo + "' ");

                //Remove Old Records from FELT_DOC_DATA
                data.Execute("DELETE FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=" + clsTravelEntry.ModuleID + " AND DOC_NO='" + ObjFlow.DocNo + "'");
    ...
    public boolean setData() {
        ...
        try {
            ...
            if (HistoryView) {
                rsTmp = tmpStmt.executeQuery("SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL_H WHERE TSD_DOC_NO='" + mDocNo + "' AND REVISION_NO=" + RevNo + " ");
            } else {
                rsTmp = tmpStmt.executeQuery("SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL WHERE TSD_DOC_NO='" + mDocNo + "' ");
            }
            rsTmp.first();
    ...
    public boolean IsEditable(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL WHERE TSD_DOC_NO='" + pDocNo + "' AND (APPROVED=1)";
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=858 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getHistoryList(int CompanyID, String DocNo) {
        ...
        try {
            String strSQL = "SELECT TSD_DOC_NO,TSD_DOC_DATE,REVISION_NO,UPDATED_BY,APPROVAL_STATUS,DATE_FORMAT(ENTRY_DATE,'%d/%m/%Y %H:%i:%S') AS ENTRY_DATE,REJECTED_REMARKS AS APPROVER_REMARKS,FROM_IP FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL_H WHERE TSD_DOC_NO='" + DocNo + "' GROUP BY TSD_DOC_NO,REVISION_NO";
            rsTmp = data.getResult(strSQL);
            rsTmp.first();

            if (rsTmp.getRow() > 0) {
                while (!rsTmp.isAfterLast()) {
                    clsTravelEntry objParty = new clsTravelEntry();
    ...
    public boolean Filter(String Condition) {
        ...
        try {
            String strSQL = "";
            strSQL += "SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL WHERE " + Condition + "  GROUP BY TSD_DOC_NO";
            ...
            if (!rsResultSet.first()) {
                strSQL = "SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL   GROUP BY TSD_DOC_NO";
                rsResultSet = Stmt.executeQuery(strSQL);
                Ready = true;
                MoveLast();
                return false;
    ...
    public boolean CanDelete(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=858 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND TYPE='C' AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO,SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=858 GROUP BY SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO ORDER BY SDMLATTPAY.D_COM_DOC_DATA.RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO,SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=858 GROUP BY SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO  ORDER BY SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO,SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=858 GROUP BY SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO  ORDER BY SDMLATTPAY.TRAVEL_SANCTION_DETAIL.TSD_DOC_NO";
            }
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                ...
                Counter = Counter + 1;
                clsTravelEntry ObjItem = new clsTravelEntry();
    ...
    public boolean ShowHistory(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL_H WHERE TSD_DOC_NO='" + pDocNo + "' GROUP BY TSD_DOC_NO ORDER BY REVISION_NO ");
            Ready = true;
            HistoryView = true;
            MoveFirst();
            return true;
    ...
    private void UpdateLeave() {
        ...
        if (getAttribute("TSD_CATEGORY").getObj().toString().equalsIgnoreCase("Within Gujarat [One Day Trip]")) {
            ...
            sql = sql + "'ON DUTY','',APPROVED,APPROVED_DATE,TSD_DOC_NO,TSD_DOC_DATE FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL "
            ...
            sh = data.getIntValueFromDB("SELECT COUNT(*) FROM ("
                    + "SELECT TSD_START_DATE,TSD_START_TIME,TSD_PAY_EMP_NO,EMP_SHIFT_ID,TSD_INFO FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL "
                    ...
                    + " WHERE TSD_INFO='CURRENT' AND TSD_START_TIME>=TIME(ADDTIME(CONCAT('2021-01-01 ',SHIFT_IN_TIME,':00'),\"2:00:00\"))  "
                    + " AND TSD_START_TIME<=TIME(ADDTIME(CONCAT('2021-01-01 ',SHIFT_OUT_TIME,':00'),\"-2:00:00\")) ");
            fh = data.getIntValueFromDB("SELECT COUNT(*) FROM ("
                    + "SELECT TSD_PAY_EMP_NO,TSD_ARRIVAL_TIME,TSD_INFO FROM (SELECT * FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL "
                    + "WHERE TSD_DOC_NO='" + (String) getAttribute("TSD_DOC_NO").getObj() + "' "
                    + "ORDER BY TSD_SR_NO DESC) AS D "
                    + "GROUP BY TSD_DOC_NO) AS DD "
                    + "LEFT JOIN SDMLATTPAY.ATTPAY_EMPMST  "
                    + "ON PAY_EMP_NO=TSD_PAY_EMP_NO  "
                    + "LEFT JOIN SDMLATTPAY.ATT_SHIFT S ON S.SHIFT_ID=EMP_SHIFT_ID "
                    + "WHERE TSD_INFO='CURRENT' AND TSD_ARRIVAL_TIME>=TIME(ADDTIME(CONCAT('2021-01-01 ',SHIFT_IN_TIME,':00'),\"2:00:00\"))  "
            ...
            if (sh > 0) {
                sql = "INSERT INTO SDMLATTPAY.ATT_LEAVE_TRN "
                        ...
                        + "'2ND HALF',APPROVED,APPROVED_DATE,TSD_DOC_NO,TSD_DOC_DATE FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL "
            ...
            if (fh > 0) {
                sql = "INSERT INTO SDMLATTPAY.ATT_LEAVE_TRN "
                        ...
                        + "'1ST HALF',APPROVED,APPROVED_DATE,TSD_DOC_NO,TSD_DOC_DATE FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL "
                        + "WHERE TSD_INFO='CURRENT' AND TSD_DOC_NO='" + (String) getAttribute("TSD_DOC_NO").getObj() + "' "
    ...
    public static void CancelDoc(String pDocNo) {
        ...
        if (CanCancel(pDocNo)) {
            ...
            try {
                rsTmp = data.getResult("SELECT APPROVED FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL WHERE TSD_DOC_NO='" + pDocNo + "' ");
                rsTmp.first();
                ...
                data.Execute("UPDATE SDMLATTPAY.TRAVEL_SANCTION_DETAIL SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE TSD_DOC_NO='" + pDocNo + "'");
    ...
    public static boolean CanCancel(String pDocNo) {
        ...
        try {
            ...
            System.out.println("SELECT TSD_DOC_NO FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL WHERE TSD_DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
            rsTmp = stTmp.executeQuery("SELECT TSD_DOC_NO FROM SDMLATTPAY.TRAVEL_SANCTION_DETAIL WHERE TSD_DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
