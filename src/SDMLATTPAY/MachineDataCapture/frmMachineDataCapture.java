 ...
 * @author Dharmendra
 */
public class frmMachineDataCapture extends javax.swing.JApplet {
...
//                txtEmpName.setText(data.getStringValueFromDB("SELECT EMP_NAME FROM SDMLATTPAY.ATTPAY_EMPMST WHERE PAY_EMP_NO='" + txtEmpNo.getText() + "'"));
    ...
    private void GenerateReport() {
        ...
        ResultSet rsTmpRemote;
        Statement stremote = null;
...
//            strSQL = "SELECT serialno,DEVICENAME,machinesn,CHECKTIME,USERID,EMP_NAME,DESIGNATION,DEPARTMENT,MAIN_CATEGORY,CATEGORY FROM "
            ...
            strSQL += " ) AS AD "
                    ...
                    + "ON EMPID+0 = USERID "
                    + "LEFT JOIN "
                    + "(SELECT DEVICE_IP,DEVICE_DESC FROM SDMLATTPAY.ATT_PUNCHING_MACHINE_MASTER) AS PMD ON DEVICE_IP = DEVICENAME "
                    + "WHERE 1=1 ";
            ...
            rsTmpRemote = stremote.executeQuery(strSQL);

            ResultSetMetaData rsInfo = rsTmpRemote.getMetaData();
            ...
            rsTmpRemote.first();
            System.out.println("Row no." + rsTmpRemote.getRow());
            if (rsTmpRemote.getRow() > 0) {
                while (!rsTmpRemote.isAfterLast()) {
                    Object[] rowData = new Object[100];
                    for (int m = 1; m < i; m++) {
                        rowData[m - 1] = rsTmpRemote.getString(m);
                    }
    ...
    private void GenerateDeviceCombo() {
        ...
        try {
            ...
            Statement stTmp = Conn.createStatement();
            int counter = 1;
            ResultSet rsTmp = stTmp.executeQuery("SELECT DISTINCT TRIM(DEVICE_DESC) AS DEVICE_DESC FROM SDMLATTPAY.ATT_PUNCHING_MACHINE_MASTER ORDER BY DEVICE_DESC ");
            while (rsTmp.next()) {
                aData.Code = counter;
