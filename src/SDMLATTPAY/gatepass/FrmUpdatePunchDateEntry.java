 ...
 *
 */
public class FrmUpdatePunchDateEntry extends javax.swing.JApplet {
    ...
    private int FFNo = 0;
    private int ModuleId = 822;
    private String DOC_NO = "";
    private clsUpdatePunchDateEntry updatepunchdate;
    ...
    public void init() {
        ...
        updatepunchdate = new clsUpdatePunchDateEntry();
        //boolean load = updatepunchdate.LoadData(EITLERPGLOBAL.gCompanyID);
        boolean load = updatepunchdate.LoadData();
        if (load) {
            DisplayData();
            MoveLast();
        } else {
            JOptionPane.showMessageDialog(this, "Error occured while Loading Data. Error is " + updatepunchdate.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
        }
        SetFields(false);
    ...
    private void DisplayData() {
        ...
        try {
            ...
            if ("1".equals(updatepunchdate.getAttribute("APPROVED").getString())) {
                lblTitle.setBackground(Color.BLUE);
                lblTitle.setForeground(Color.WHITE);
            }

            if ("0".equals(updatepunchdate.getAttribute("APPROVED").getString())) {
                lblTitle.setBackground(Color.GRAY);
                lblTitle.setForeground(Color.BLACK);
            }

            if ("1".equals(updatepunchdate.getAttribute("CANCELED").getString())) {
...
/*
        try {
            if (EditMode == 0) {
                if (updatepunchdate.getAttribute("APPROVED").getInt() == 1) {
                    lblTitle.setBackground(Color.BLUE);
                }
                ...
                if (updatepunchdate.getAttribute("CANCELLED").getInt() == 1) {
                    lblTitle.setBackground(Color.RED);
                }
            }
        } catch (Exception c) {
...
*/
        ...
        try {
            ...
            DOC_NO = updatepunchdate.getAttribute("UPD_DOC_NO").getString();
            correctionType = updatepunchdate.getAttribute("CORRECTION_TYPE").getString();
            lblTitle.setText("UPDATE PUNCH DATE ENTRY  - " + DOC_NO);
            lbldoc.setText(DOC_NO);
            docdate.setText(EITLERPGLOBAL.formatDate(updatepunchdate.getAttribute("UPD_DOC_DATE").getString()));
            lblCorrectionType.setText(updatepunchdate.getAttribute("CORRECTION_TYPE").getString());
            ...
            FormatGrid();
            //Now Generate Table
            for (int i = 1; i <= updatepunchdate.hmTimeCorrectionDetails.size(); i++) {
                clsTimeCorrectionDetails objTimeCorrectionDetails = (clsTimeCorrectionDetails) updatepunchdate.hmTimeCorrectionDetails.get(Integer.toString(i));
                ...
                rowData[2]=empname;
                rowData[3]=empdept;                                                    
                rowData[4] = EITLERPGLOBAL.formatDate(objTimeCorrectionDetails.getAttribute("A_DATE").getString());
                rowData[5] = EITLERPGLOBAL.formatDateTime(objTimeCorrectionDetails.getAttribute("PUNCH").getString());
                //rowData[5] = EITLERPGLOBAL.formatDate(objTimeCorrectionDetails.getAttribute("PUNCH").getString().substring(0, 10))+" "+objTimeCorrectionDetails.getAttribute("PUNCH").getString().substring(11, 19);
                rowData[6] = EITLERPGLOBAL.formatDate(objTimeCorrectionDetails.getAttribute("UPDATED_PUNCH_DATE").getString());
        ...
        try {
            ...
            String DocNo = updatepunchdate.getAttribute("UPD_DOC_NO").getString();
            ...
            FormatGridHS();
            //HashMap History = updatepunchdate.getHistoryList(EITLERPGLOBAL.gCompanyID + "", DocNo);
            HashMap History = clsUpdatePunchDateEntry.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsUpdatePunchDateEntry ObjHistory = (clsUpdatePunchDateEntry) History.get(Integer.toString(i));
    ...
    private void SetMenuForRights() {
        // --- Add Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10037, 100371)) { //7008,70081
        ...
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10037, 100372)) { //7008,70082
            cmdEdit.setEnabled(true);
        } else {
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10037, 100373)) {
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10037, 100374)) {
    ...
    private void SetupApproval() {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = SDMLATTPAY.ApprovalFlow.getFromID(EITLERPGLOBAL.gCompanyID, ModuleId, updatepunchdate.getAttribute("UPD_DOC_NO").getString());
            //lnFromUserId = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = SDMLATTPAY.ApprovalFlow.getFromRemarks(EITLERPGLOBAL.gCompanyID, ModuleId, FromUserID, updatepunchdate.getAttribute("UPD_DOC_NO").getString());
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            cmbHierarchy.setEnabled(false);
            if (SDMLATTPAY.ApprovalFlow.IsCreator(ModuleId, updatepunchdate.getAttribute("UPD_DOC_NO").getString() + "")) {
                OpgReject.setEnabled(false);
            }
        }
    ...
    private void initComponents() {
        ...
        btnRemove.setBounds(210, 60, 81, 27);
        ...
        lblTitle.setText("UPDATE PUNCH DATE ENTRY");
    ...
    private void cmdViewHistoryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdViewHistoryActionPerformed
        ...
        //updatepunchdate.ShowHistory(DocNo);
        MoveLast();
    }//GEN-LAST:event_cmdViewHistoryActionPerformed

    private void cmdNormalViewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdNormalViewActionPerformed
        //updatepunchdate.HistoryView = false;
        //updatepunchdate.LoadData();
        MoveLast();
...
if(EditMode==EITLERPGLOBAL.ADD || EditMode==EITLERPGLOBAL.EDIT){
        if (evt.getKeyCode() == 112) //F1 Key pressed
        {
            if (Table.getSelectedColumn() == 1) {
                ...
                //setText(aList.ReturnVal);                               
                    ...
                    Table.getModel().setValueAt(empname, Table.getSelectedRow(), 2);
                    Table.getModel().setValueAt(empdept, Table.getSelectedRow(), 3);
            ...
            String empcode = Table.getModel().getValueAt(Table.getSelectedRow(), 1).toString();
            String attendancedate=EITLERPGLOBAL.formatDateDB(Table.getValueAt(Table.getSelectedRow(),4).toString());
            ...
            //aList.SQL = "SELECT EMP_CODE,LC_TIME FROM SDMLATTPAY.ATT_DATA WHERE EMP_CODE='"+txtempcd.getText().substring(3)+"'";
                ...
                aList.SQL = "SELECT A_DATE,DATE_FORMAT(A_DATETIME,'%d/%m/%Y %H:%i:%s') AS PUNCH_DATE,P_TIME AS PUNCH_TIME FROM SDMLATTPAY.ATT_DATA WHERE EMP_CODE='" + Table.getModel().getValueAt(Table.getSelectedRow(), 1).toString() + "' AND A_DATE='" + dtbefore + "' "
                        + "UNION ALL "
                        + "SELECT A_DATE,DATE_FORMAT(A_DATETIME,'%d/%m/%Y %H:%i:%s') AS PUNCH_DATE,P_TIME AS PUNCH_TIME FROM SDMLATTPAY.ATT_DATA WHERE EMP_CODE='" + Table.getModel().getValueAt(Table.getSelectedRow(), 1).toString() + "' AND A_DATE='" + attendancedate + "' "
                        + "UNION ALL "
                        + "SELECT A_DATE,DATE_FORMAT(A_DATETIME,'%d/%m/%Y %H:%i:%s') AS PUNCH_DATE,P_TIME AS PUNCH_TIME FROM SDMLATTPAY.ATT_DATA WHERE EMP_CODE='" + Table.getModel().getValueAt(Table.getSelectedRow(), 1).toString() + "' AND A_DATE='" + dtafter + "' ";
                aList.ReturnCol = 2;
                aList.ShowReturnCol = true;
                aList.DefaultSearchOn = 1;
                aList.SecondCol = 3;
                ...
                if (aList.ShowLOV()) {
                    Table.getModel().setValueAt(aList.ReturnVal, Table.getSelectedRow(), 5);
                ...
                Table.changeSelection(Table.getSelectedRow(), 6, false, false);
            }
        }
}
    ...
    private void TableKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_TableKeyReleased
        ...
        if(Table.getSelectedColumn()==5){
            ...
            attendancedate= Table.getModel().getValueAt(Table.getSelectedRow(),4).toString();
...
//        }
    ...
    private void MoveFirst() {
        updatepunchdate.MoveFirst();
        DisplayData();
    }

    private void MovePrevious() {
        updatepunchdate.MovePrevious();
        DisplayData();
    }

    private void MoveNext() {
        updatepunchdate.MoveNext();
        DisplayData();
    }

    private void MoveLast() {
        updatepunchdate.MoveLast();
        DisplayData();
    }
    ...
    Loader ObjLoader=new Loader(this,"SDMLATTPAY.gatepass.FrmTimeCorrectionEntryFind",true);
        ...
        if(ObjTimeCorrectionFind.Cancelled==false) {
            if(!updatepunchdate.Filter(ObjTimeCorrectionFind.stringFindQuery)) {
                JOptionPane.showMessageDialog(FrmUpdatePunchDateEntry.this," No records found.","Find Time Correction Details",JOptionPane.YES_OPTION);
            }
            MoveLast();
        }
    }
    ...
    public void FindWaiting() {
        updatepunchdate.Filter(" UPD_DOC_NO IN (SELECT DISTINCT SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY.UPD_DOC_NO FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY, SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY.UPD_DOC_NO=SDMLATTPAY.D_COM_DOC_DATA.DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=" + ModuleId + " AND CANCELED=0) ");
        SetMenuForRights();
        DisplayData();
    }
    ...
    private void Add() {
        ...
        correctionType = "UPDATE PUNCH DATE";
        ...
        EITLERP.FeltSales.common.SelectFirstFree aList = new EITLERP.FeltSales.common.SelectFirstFree();
        aList.ModuleID = ModuleId;
        aList.FirstFreeNo = 326;
        FFNo = aList.FirstFreeNo;
        lbldoc.setText(clsFirstFree.getNextFreeNo(EITLERPGLOBAL.gCompanyID, ModuleId, FFNo, false));
        lblTitle.setText("UPDATE PUNCH DATE ENTRY - " + lbldoc.getText());
    ...
    private void FormatGrid() {
        ...
        DataModel.addColumn("Punch");
        DataModel.addColumn("Updated Punch");
    ...
    private void Save() {
        ...
        //updatepunchdate.LoadData();
        if (EditMode == EITLERPGLOBAL.ADD) {
            if (updatepunchdate.Insert()) {
                clsFirstFree.getNextFreeNo(EITLERPGLOBAL.gCompanyID, ModuleId, FFNo, true);

                DisplayData();
            } else {
                JOptionPane.showMessageDialog(this, "Error occured while saving. Error is " + updatepunchdate.LastError, " SAVING ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }

        if (EditMode == EITLERPGLOBAL.EDIT) {
            if (updatepunchdate.Update()) {
                DisplayData();
            } else {
                JOptionPane.showMessageDialog(this, "Error occured while saving editing. Error is " + updatepunchdate.LastError, "SAVING ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
    ...
    private void Edit() {
        ...
        //String DocumentNo = updatepunchdate.getAttribute("ATT_TIME_CORRRECTION_DOC_NO").getString();
        //String DocumentNo = lbldoc.getText();
        String DocumentNo = DOC_NO;

        //if (updatepunchdate.IsEditable(EITLERPGLOBAL.gCompanyID, DocumentNo, EITLERPGLOBAL.gNewUserID)) {
        if (updatepunchdate.IsEditable(DocumentNo, EITLERPGLOBAL.gNewUserID)) {
...
// find rate update by doc no
    public void Find(String docNo) {
        updatepunchdate.Filter(" DOC_NO='" + docNo + "'");
        SetMenuForRights();
        DisplayData();
    }

    public void FindEx(int pCompanyID, String docno) {
        updatepunchdate.Filter(" UPD_DOC_NO='" + docno + "'");
        updatepunchdate.MoveFirst();
        DisplayData();
    }
...
//      
        updatepunchdate.setAttribute("UPD_DOC_NO", lbldoc.getText());
        DOC_NO = lbldoc.getText();
        //updatepunchdate.setAttribute("UPD_DOC_NO", DOC_NO);
        updatepunchdate.setAttribute("UPD_DOC_DATE", docdate.getText());
        updatepunchdate.setAttribute("CORRECTION_TYPE", lblCorrectionType.getText());

        updatepunchdate.setAttribute("MODULE_ID", ModuleId);
        updatepunchdate.setAttribute("USER_ID", EITLERPGLOBAL.gNewUserID);

        updatepunchdate.setAttribute("REJECTED_REMARKS", txtToRemarks.getText());
        updatepunchdate.setAttribute("REMARKS", "");
        updatepunchdate.setAttribute("APPROVAL_STATUS", "");
        updatepunchdate.setAttribute("APPROVER_REMARKS", txtFromRemarks.getText());
        updatepunchdate.setAttribute("ENTRY_DATE", EITLERPGLOBAL.getCurrentDateDB());

        //----- Update Approval Specific Fields -----------//
        updatepunchdate.setAttribute("HIERARCHY_ID", EITLERPGLOBAL.getComboCode(cmbHierarchy));
        updatepunchdate.setAttribute("FROM", EITLERPGLOBAL.gNewUserID);
        updatepunchdate.setAttribute("TO", EITLERPGLOBAL.getComboCode(cmbSendTo));
        updatepunchdate.setAttribute("FROM_REMARKS", txtToRemarks.getText());

        if (OpgApprove.isSelected()) {
            updatepunchdate.setAttribute("APPROVAL_STATUS", "A");
        }

        if (OpgFinal.isSelected()) {
            updatepunchdate.setAttribute("APPROVAL_STATUS", "F");
        }

        if (OpgReject.isSelected()) {
            updatepunchdate.setAttribute("APPROVAL_STATUS", "R");
            updatepunchdate.setAttribute("SEND_DOC_TO", EITLERPGLOBAL.getComboCode(cmbSendTo));
        }

        if (OpgHold.isSelected()) {
            updatepunchdate.setAttribute("APPROVAL_STATUS", "H");
        }
        //-------------------------------------------------//

        if (EditMode == EITLERPGLOBAL.ADD) {
            updatepunchdate.setAttribute("CREATED_BY", EITLERPGLOBAL.gNewUserID);
            updatepunchdate.setAttribute("CREATED_DATE", EITLERPGLOBAL.getCurrentDateDB());
        } else {
            updatepunchdate.setAttribute("MODIFIED_BY", EITLERPGLOBAL.gNewUserID);
            updatepunchdate.setAttribute("MODIFIED_DATE", EITLERPGLOBAL.getCurrentDateDB());
            updatepunchdate.setAttribute("UPDATED_BY", EITLERPGLOBAL.gNewUserID);
            updatepunchdate.setAttribute("UPDATED_DATE", EITLERPGLOBAL.getCurrentDateDB());
        }
        int i = 0;
        String empcode = "", attendancedate = "", lctime = "", punch = "", updatedpunchdate = "", gatepasstype = "";
        updatepunchdate.hmTimeCorrectionDetails.clear();
        //Check the entered details in Table.
        for (i = 0; i <= Table.getRowCount() - 1; i++) {
            empcode = ((String) Table.getValueAt(i, 1)).trim().toUpperCase();
            attendancedate = ((String) Table.getValueAt(i, 4)).trim().toUpperCase();
            punch = ((String) Table.getValueAt(i, 5)).trim().toUpperCase();
            updatedpunchdate = ((String) Table.getValueAt(i, 6)).trim();
            System.out.println("1 "+punch);
            System.out.println("2 "+updatedpunchdate);
//            if(correctionType.equals("MIS-PUNCH")){
...
//            }
            ...
            correctiondetails.setAttribute("PUNCH", punch);
            correctiondetails.setAttribute("UPDATED_PUNCH_DATE", updatedpunchdate);            
            
            
            //correctiondetails.setAttribute("GATEPASS_TYPE", gatepasstype);

            updatepunchdate.hmTimeCorrectionDetails.put(Integer.toString(updatepunchdate.hmTimeCorrectionDetails.size() + 1), correctiondetails);
