...
public class clsUpdatePunchDateEntry {
    ...
    public static int ModuleID = 822; //
    ...
    public clsUpdatePunchDateEntry() {
        LastError = "";
        props=new HashMap();
        props.put("UPD_DOC_NO", new Variant(""));
        props.put("UPD_DOC_DATE", new Variant(""));
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            //resultSet=statement.executeQuery("SELECT DISTINCT UPD_DOC_NO  FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY WHERE PROD_DEPT = 'NEEDLING' AND PROD_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND PROD_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY PROD_DATE");
            resultSet=statement.executeQuery("SELECT DISTINCT UPD_DOC_NO FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY WHERE UPD_DOC_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND UPD_DOC_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY UPD_DOC_DATE");
            HistoryView=false;
            MoveLast();
            return true;
        }catch(Exception e) {
    ...
    public boolean Insert() {
        ...
        try {
            ...
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY WHERE UPD_DOC_NO=''");
            
            // Production data history connection
            statementHistory=connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            resultSetHistory=statementHistory.executeQuery("SELECT * FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY_H WHERE UPD_DOC_NO=''");
            ...
            for(int i=1;i<=hmTimeCorrectionDetails.size();i++) {
                ...
                resultSetTemp.updateString("UPD_DOC_NO", getAttribute("UPD_DOC_NO").getString());
                resultSetTemp.updateString("UPD_DOC_DATE", EITLERPGLOBAL.formatDateDB(getAttribute("UPD_DOC_DATE").getString()));                
                ...
                resultSetTemp.updateString("PUNCH",EITLERPGLOBAL.formatDateTimeDB(objTimeCorrectionDetails.getAttribute("PUNCH").getString()));
                resultSetTemp.updateString("UPDATED_PUNCH_DATE",EITLERPGLOBAL.formatDateDB(objTimeCorrectionDetails.getAttribute("UPDATED_PUNCH_DATE").getString()));
                ...
                resultSetHistory.updateString("UPD_DOC_DATE", EITLERPGLOBAL.formatDateDB(getAttribute("UPD_DOC_DATE").getString()));
                resultSetHistory.updateString("UPD_DOC_NO", getAttribute("UPD_DOC_NO").getString());
                ...
                resultSetHistory.updateString("PUNCH",EITLERPGLOBAL.formatDateTimeDB(objTimeCorrectionDetails.getAttribute("PUNCH").getString()));
                resultSetHistory.updateString("UPDATED_PUNCH_DATE",EITLERPGLOBAL.formatDateDB(objTimeCorrectionDetails.getAttribute("UPDATED_PUNCH_DATE").getString()));
                ...
                try {
                    if ("F".equals(getAttribute("APPROVAL_STATUS").getString())) {
                        if (getAttribute("CORRECTION_TYPE").getString().contains("UPDATE PUNCH DATE")) {
                          String strInsert = "INSERT INTO SDMLATTPAY.ATT_UPDATE_PUNCH_DATE "
                                    + " VALUES ('" + objTimeCorrectionDetails.getAttribute("EMP_CODE").getString() + "',"                                    
                                    + "'" + EITLERPGLOBAL.formatDateTimeDB(objTimeCorrectionDetails.getAttribute("PUNCH").getString()) + "',"                                                                        
                                    + "'" + EITLERPGLOBAL.formatDateDB(objTimeCorrectionDetails.getAttribute("UPDATED_PUNCH_DATE").getString()) + "')";                                    
            ...
            objFlow.ModuleID=822; //Update punch date entry
            objFlow.DocNo=getAttribute("UPD_DOC_NO").getString();
            objFlow.DocDate=EITLERPGLOBAL.getCurrentDateDB();
            objFlow.From=(int)getAttribute("FROM").getVal();
            objFlow.To=(int)getAttribute("TO").getVal();
            objFlow.Status=(String)getAttribute("APPROVAL_STATUS").getObj();
            objFlow.TableName="ATT_UPDATE_PUNCH_DATE_ENTRY";
            objFlow.IsCreator=true;
            objFlow.HierarchyID=(int)getAttribute("HIERARCHY_ID").getVal();
            objFlow.Remarks=getAttribute("FROM_REMARKS").getString();
            objFlow.FieldName="UPD_DOC_NO";
    ...
    public boolean Update() {
        ...
        try {
            ...
            resultSetHistory=statementHistory.executeQuery("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY_H WHERE UPD_DOC_NO='"+getAttribute("UPD_DOC_NO").getString()+"'");
            ...
            statementHistory.execute("DELETE FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY WHERE UPD_DOC_NO='"+getAttribute("UPD_DOC_NO").getString()+"'");
            ...
            objFlow.TableName="SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY";
            ...
            if(IsRejected) {
                ...
                data.Execute("UPDATE SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY SET REJECTED=0,CHANGED=1 WHERE UPD_DOC_NO='"+getAttribute("UPD_DOC_NO").getString()+"'");
                //Remove Old Records from DOCUMENT APPROVAL TABLE
                //data.Execute("DELETE FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=822 AND DOC_NO='"+getAttribute("UPD_DOC_NO").getString()+"'");
    ...
    public boolean CanDelete(String documentNo,String stringDocDate,int userID) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY WHERE UPD_DOC_NO='"+documentNo +"' AND APPROVED="+1;
            ...
            if(rsTmp.getInt("COUNT")>0){  //Item is Approved
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=822 AND USER_ID="+userID+" AND DOC_NO='"+ documentNo +"' AND STATUS='W'";
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
                
                if(rsTmp.getInt("COUNT")>0) {
                    strSQL = "DELETE FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY WHERE "
                    + " AND UPD_DOC_DATE= '"+ EITLERPGLOBAL.formatDateDB(stringDocDate) +"'"
                    + " AND UPD_DOC_NO='" + documentNo + "'";
    ...
    public boolean IsEditable(String DocumentNo,int userID) {
        ...
        try {
            ...
            if(rsTmp.getInt("COUNT")>0){  //Item is Approved
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=822 AND USER_ID="+Integer.toString(userID)+" AND DOC_NO='"+ DocumentNo +"' AND STATUS='W'";
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean Filter(String stringFindQuery) {
        Ready=false;
        try {
            String strSql = "SELECT DISTINCT UPD_DOC_NO FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY WHERE " + stringFindQuery + "ORDER BY UPD_DOC_DATE";
            connection=data.getConn();
            statement=connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean setData() {
        ...
        try {
            ...
            if (HistoryView) {
                ...
                setAttribute("UPD_DOC_NO",resultSet.getString("UPD_DOC_NO"));
            } else {
                setAttribute("REVISION_NO", 0);
                setAttribute("UPD_DOC_NO","");
            }
            */
            ...
            //if(resultSet!=null){
            //System.out.println(resultSet.getString("UPD_DOC_NO"));
            //setAttribute("UPD_DOC_NO",resultSet.getString("UPD_DOC_NO"));
            //Now Populate the collection, first clear the collection
            hmTimeCorrectionDetails.clear();
            
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);            
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY WHERE UPD_DOC_NO='"+resultSet.getString("UPD_DOC_NO")+"' ORDER BY SR_NO");
            //resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY ORDER BY UPD_DOC_NO,SR_NO");
            //if(resultSetTemp.next()){
            //    if(resultSetTemp.getRow()>0){
            while(resultSetTemp.next()) {
                serialNoCounter++;
                setAttribute("UPD_DOC_NO",resultSetTemp.getString("UPD_DOC_NO"));
                setAttribute("UPD_DOC_DATE",resultSetTemp.getString("UPD_DOC_DATE"));
                ...
                objTimeCorrectionDetails.setAttribute("PUNCH",resultSetTemp.getString("PUNCH"));
                objTimeCorrectionDetails.setAttribute("UPDATED_PUNCH_DATE",resultSetTemp.getString("UPDATED_PUNCH_DATE"));
    ...
    public boolean setHistoryData(String pDate,String pDocNo) {
        ...
        try {
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.TIME_CORRECTION_H WHERE UPD_DOC_DATE='"+ pDate+"' AND UPD_DOC_NO='"+pDocNo+"' AND REVISION_NO='"+ resultSet.getInt("REVISION_NO") +"' ORDER BY REVISION_NO");
    ...
    public static HashMap getHistoryList(int CompanyID, String DocumentNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT * FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY_H WHERE UPD_DOC_NO='"+DocumentNo+"' GROUP BY REVISION_NO");
            
            while(rsTmp.next()) {
                clsUpdatePunchDateEntry ObjTimeCorrection=new clsUpdatePunchDateEntry();
    ...
    public boolean ShowHistory(String pDate,String pDocNo) {
        ...
        try {
            ...
            resultSet=statement.executeQuery("SELECT DISTINCT REVISION_NO FROM SDMLATTPAY.TIME_CORRECTION_H WHERE UPD_DOC_DATE='"+ pDate+"' AND UPD_DOC_DOC_NO='"+pDocNo+"'");
            Ready=true;
    ...
    public static HashMap getPendingApprovals(int pUserID,int pOrder) {
        ...
        try {
            ...
            if(pOrder==EITLERPGLOBAL.OnRecivedDate) {
                strSQL="SELECT DISTINCT UPD_DOC_NO,UPD_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY,SDMLATTPAY.D_COM_DOC_DATA WHERE UPD_DOC_NO=DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=822 AND CANCELED=0 ORDER BY RECEIVED_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocDate) {
                strSQL="SELECT DISTINCT UPD_DOC_NO,UPD_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY,SDMLATTPAY.D_COM_DOC_DATA WHERE UPD_DOC_NO=DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=822 AND CANCELED=0 ORDER BY PROD_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocNo) {
                strSQL="SELECT DISTINCT UPD_DOC_NO,UPD_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_UPDATE_PUNCH_DATE_ENTRY,SDMLATTPAY.D_COM_DOC_DATA WHERE UPD_DOC_NO=DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=822 AND CANCELED=0 ORDER BY PROD_DOC_NO";
            }
            ...
            while(rsTmp.next()) {
                ...
                Counter=Counter+1;
                clsUpdatePunchDateEntry ObjDoc=new clsUpdatePunchDateEntry();
                
                //------------- Header Fields --------------------//
                ObjDoc.setAttribute("DOC_NO",rsTmp.getString("UPD_DOC_NO"));
