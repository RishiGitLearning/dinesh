...
public class clsSpecialDeductionEntry {
    ...
    public clsSpecialDeductionEntry() {
        LastError = "";
        props=new HashMap();
        props.put("SPECIAL_DEDUCTION_DOC_NO", new Variant(""));
        props.put("SPECIAL_DEDUCTION_DOC_DATE", new Variant(""));
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            //resultSet=statement.executeQuery("SELECT DISTINCT SPECIAL_DEDUCTION_DOC_NO  FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION WHERE PROD_DEPT = 'NEEDLING' AND PROD_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND PROD_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY PROD_DATE");
            resultSet=statement.executeQuery("SELECT DISTINCT SPECIAL_DEDUCTION_DOC_NO FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION WHERE SPECIAL_DEDUCTION_DOC_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND SPECIAL_DEDUCTION_DOC_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY SPECIAL_DEDUCTION_DOC_DATE");
            HistoryView=false;
            MoveLast();
            return true;
        }catch(Exception e) {
    ...
    public boolean Insert() {
        ...
        try {
            ...
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.ATT_SPECIAL_DEDUCTION WHERE SPECIAL_DEDUCTION_DOC_NO=''");
            
            // Production data history connection
            statementHistory=connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            resultSetHistory=statementHistory.executeQuery("SELECT * FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION_H WHERE SPECIAL_DEDUCTION_DOC_NO=''");
            ...
            for(int i=1;i<=hmTimeCorrectionDetails.size();i++) {
                ...
                resultSetTemp.updateString("SPECIAL_DEDUCTION_DOC_NO", getAttribute("SPECIAL_DEDUCTION_DOC_NO").getString());
                resultSetTemp.updateString("SPECIAL_DEDUCTION_DOC_DATE", EITLERPGLOBAL.formatDateDB(getAttribute("SPECIAL_DEDUCTION_DOC_DATE").getString()));                
                ...
                resultSetHistory.updateString("SPECIAL_DEDUCTION_DOC_DATE", EITLERPGLOBAL.formatDateDB(getAttribute("SPECIAL_DEDUCTION_DOC_DATE").getString()));
                resultSetHistory.updateString("SPECIAL_DEDUCTION_DOC_NO", getAttribute("SPECIAL_DEDUCTION_DOC_NO").getString());
            ...
            objFlow.ModuleID=823; //Time correction entry
            objFlow.DocNo=getAttribute("SPECIAL_DEDUCTION_DOC_NO").getString();
            objFlow.DocDate=EITLERPGLOBAL.getCurrentDateDB();
            objFlow.From=(int)getAttribute("FROM").getVal();
            objFlow.To=(int)getAttribute("TO").getVal();
            objFlow.Status=(String)getAttribute("APPROVAL_STATUS").getObj();
            objFlow.TableName="SDMLATTPAY.ATT_SPECIAL_DEDUCTION";
            objFlow.IsCreator=true;
            objFlow.HierarchyID=(int)getAttribute("HIERARCHY_ID").getVal();
            objFlow.Remarks=getAttribute("FROM_REMARKS").getString();
            objFlow.FieldName="SPECIAL_DEDUCTION_DOC_NO";
    ...
    public boolean Update() {
        ...
        try {
            ...
            resultSetHistory=statementHistory.executeQuery("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION_H WHERE SPECIAL_DEDUCTION_DOC_NO='"+getAttribute("SPECIAL_DEDUCTION_DOC_NO").getString()+"'");
            ...
            statementHistory.execute("DELETE FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION WHERE SPECIAL_DEDUCTION_DOC_NO='"+getAttribute("SPECIAL_DEDUCTION_DOC_NO").getString()+"'");
...
//                        if (getAttribute("CORRECTION_TYPE").getString().contains("SPECIAL DEDUCTION")) {
...
//                }                
            ...
            objFlow.ModuleID=823; //Special Sanction Entry
            objFlow.DocNo=getAttribute("SPECIAL_DEDUCTION_DOC_NO").getString();
            objFlow.DocDate=EITLERPGLOBAL.getCurrentDateDB();
            objFlow.From=(int)getAttribute("FROM").getVal();
            objFlow.To=(int)getAttribute("TO").getVal();
            ...
            if(IsRejected) {
                ...
                data.Execute("UPDATE SDMLATTPAY.ATT_SPECIAL_DEDUCTION SET REJECTED=0,CHANGED=1 WHERE SPECIAL_DEDUCTION_DOC_NO='"+getAttribute("SPECIAL_DEDUCTION_DOC_NO").getString()+"'");
                //Remove Old Records from DOCUMENT APPROVAL TABLE
                //data.Execute("DELETE FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=823 AND DOC_NO='"+getAttribute("SPECIAL_DEDUCTION_DOC_NO").getString()+"'");
    ...
    public boolean CanDelete(String documentNo,String stringDocDate,int userID) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION WHERE SPECIAL_DEDUCTION_DOC_NO='"+documentNo +"' AND APPROVED="+1;
            ...
            if(rsTmp.getInt("COUNT")>0){  //Item is Approved
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=823 AND USER_ID="+userID+" AND DOC_NO='"+ documentNo +"' AND STATUS='W'";
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
                
                if(rsTmp.getInt("COUNT")>0) {
                    strSQL = "DELETE FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION WHERE "
                    + " AND SPECIAL_DEDUCTION_DOC_DATE= '"+ EITLERPGLOBAL.formatDateDB(stringDocDate) +"'"
                    + " AND SPECIAL_DEDUCTION_DOC_NO='" + documentNo + "'";
    ...
    public boolean IsEditable(String DocumentNo,int userID) {
        ...
        try {
            ...
            if(rsTmp.getInt("COUNT")>0){  //Item is Approved
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=823 AND USER_ID="+Integer.toString(userID)+" AND DOC_NO='"+ DocumentNo +"' AND STATUS='W'";
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean Filter(String stringFindQuery) {
        Ready=false;
        try {
            String strSql = "SELECT DISTINCT SPECIAL_DEDUCTION_DOC_NO FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION WHERE " + stringFindQuery + "ORDER BY SPECIAL_DEDUCTION_DOC_DATE";
            connection=data.getConn();
            statement=connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean setData() {
        ...
        try {
            ...
            if (HistoryView) {
                ...
                setAttribute("SPECIAL_DEDUCTION_DOC_NO",resultSet.getString("SPECIAL_DEDUCTION_DOC_NO"));
            } else {
                setAttribute("REVISION_NO", 0);
                setAttribute("SPECIAL_DEDUCTION_DOC_NO","");
            }
            */
            ...
            //if(resultSet!=null){
            //System.out.println(resultSet.getString("SPECIAL_DEDUCTION_DOC_NO"));
            //setAttribute("SPECIAL_DEDUCTION_DOC_NO",resultSet.getString("SPECIAL_DEDUCTION_DOC_NO"));
            //Now Populate the collection, first clear the collection
            hmTimeCorrectionDetails.clear();
            
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);            
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.ATT_SPECIAL_DEDUCTION WHERE SPECIAL_DEDUCTION_DOC_NO='"+resultSet.getString("SPECIAL_DEDUCTION_DOC_NO")+"' ORDER BY SR_NO");
            //resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.ATT_SPECIAL_DEDUCTION ORDER BY SPECIAL_DEDUCTION_DOC_NO,SR_NO");
            //if(resultSetTemp.next()){
            //    if(resultSetTemp.getRow()>0){
            while(resultSetTemp.next()) {
                serialNoCounter++;
                setAttribute("SPECIAL_DEDUCTION_DOC_NO",resultSetTemp.getString("SPECIAL_DEDUCTION_DOC_NO"));
                setAttribute("SPECIAL_DEDUCTION_DOC_DATE",resultSetTemp.getString("SPECIAL_DEDUCTION_DOC_DATE"));
    ...
    public boolean setHistoryData(String pDate,String pDocNo) {
        ...
        try {
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.TIME_CORRECTION_H WHERE SPECIAL_DEDUCTION_DOC_DATE='"+ pDate+"' AND SPECIAL_DEDUCTION_DOC_NO='"+pDocNo+"' AND REVISION_NO='"+ resultSet.getInt("REVISION_NO") +"' ORDER BY REVISION_NO");
    ...
    public static HashMap getHistoryList(int CompanyID, String DocumentNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT * FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION_H WHERE SPECIAL_DEDUCTION_DOC_NO='"+DocumentNo+"' GROUP BY REVISION_NO");
            
            while(rsTmp.next()) {
                clsSpecialDeductionEntry ObjTimeCorrection=new clsSpecialDeductionEntry();
    ...
    public boolean ShowHistory(String pDate,String pDocNo) {
        ...
        try {
            ...
            resultSet=statement.executeQuery("SELECT DISTINCT REVISION_NO FROM SDMLATTPAY.TIME_CORRECTION_H WHERE SPECIAL_DEDUCTION_DOC_DATE='"+ pDate+"' AND SPECIAL_DEDUCTION_DOC_DOC_NO='"+pDocNo+"'");
            Ready=true;
    ...
    public static HashMap getPendingApprovals(int pUserID,int pOrder) {
        ...
        try {
            ...
            if(pOrder==EITLERPGLOBAL.OnRecivedDate) {
                strSQL="SELECT DISTINCT SPECIAL_DEDUCTION_DOC_NO,SPECIAL_DEDUCTION_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION,SDMLATTPAY.D_COM_DOC_DATA WHERE SPECIAL_DEDUCTION_DOC_NO=DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=823 AND CANCELED=0 ORDER BY RECEIVED_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocDate) {
                strSQL="SELECT DISTINCT SPECIAL_DEDUCTION_DOC_NO,SPECIAL_DEDUCTION_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION,SDMLATTPAY.D_COM_DOC_DATA WHERE SPECIAL_DEDUCTION_DOC_NO=DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=823 AND CANCELED=0 ORDER BY SPECIAL_DEDUCTION_DOC_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocNo) {
                strSQL="SELECT DISTINCT SPECIAL_DEDUCTION_DOC_NO,SPECIAL_DEDUCTION_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_SPECIAL_DEDUCTION,SDMLATTPAY.D_COM_DOC_DATA WHERE SPECIAL_DEDUCTION_DOC_NO=DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=823 AND CANCELED=0 ORDER BY SPECIAL_DEDUCTION_DOC_NO";
            }
            ...
            while(rsTmp.next()) {
                ...
                Counter=Counter+1;
                clsSpecialDeductionEntry ObjDoc=new clsSpecialDeductionEntry();
                
                //------------- Header Fields --------------------//
                ObjDoc.setAttribute("DOC_NO",rsTmp.getString("SPECIAL_DEDUCTION_DOC_NO"));
                ObjDoc.setAttribute("DOC_DATE",rsTmp.getString("SPECIAL_DEDUCTION_DOC_DATE"));
