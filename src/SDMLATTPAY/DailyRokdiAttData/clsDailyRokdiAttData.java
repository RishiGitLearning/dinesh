 ...
 * @author Vivek Kumar
 */
public class clsDailyRokdiAttData {
     ...
     */
    public clsDailyRokdiAttData() {
        LastError = "";
        props = new HashMap();
        props.put("DOC_NO", new Variant(""));
        props.put("DOC_DATE", new Variant(""));
        
        props.put("ROKDI_ATT_DATE", new Variant(""));
        props.put("ROKDI_ATT_DEPT", new Variant(""));
        props.put("ROKDI_ATT_SHIFT", new Variant(""));
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT * FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA GROUP BY DOC_NO ORDER BY CREATED_DATE,DOC_NO");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
...
//            else setData();
    ...
    public boolean Insert() {
        ...
        try {
            ...
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  SDMLATTPAY.DAILY_ROKDI_ATT_DATA WHERE DOC_NO='1'");

            // Production data history connection
            statementHistory = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetHistory = statementHistory.executeQuery("SELECT * FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA_H WHERE DOC_NO='1'");

            setAttribute("DOC_NO", getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 868, 383, true));

            //Now Insert records into production tables
            for (int i = 1; i <= hmDailyAttDataDetails.size(); i++) {
                clsDailyRokdiAttDataDetails ObjDailyAttDataDetails = (clsDailyRokdiAttDataDetails) hmDailyAttDataDetails.get(Integer.toString(i));
                ...
                resultSetTemp.updateString("ROKDI_ATT_DATE", EITLERPGLOBAL.formatDateDB(getAttribute("ROKDI_ATT_DATE").getString()));
                resultSetTemp.updateString("ROKDI_ATT_DEPT", getAttribute("ROKDI_ATT_DEPT").getString());
                resultSetTemp.updateString("ROKDI_ATT_SHIFT", getAttribute("ROKDI_ATT_SHIFT").getString());
                ...
                resultSetTemp.updateString("ROKDI_PUNCH_TIME", ObjDailyAttDataDetails.getAttribute("ROKDI_PUNCH_TIME").getString());
                resultSetTemp.updateString("EMP_REMARK",ObjDailyAttDataDetails.getAttribute("EMP_REMARK").getString());
                
                resultSetTemp.updateString("ROKDI_CONSIDER",ObjDailyAttDataDetails.getAttribute("ROKDI_CONSIDER").getString());
...
//                resultSetTemp.updateString("EMP_EXCESS",ObjDailyAttDataDetails.getAttribute("EMP_EXCESS").getString());
                resultSetTemp.updateString("DEPT_ROKDI_HOURS",ObjDailyAttDataDetails.getAttribute("DEPT_ROKDI_HOURS").getString());
                ...
                resultSetHistory.updateString("ROKDI_ATT_DATE", EITLERPGLOBAL.formatDateDB(getAttribute("ROKDI_ATT_DATE").getString()));
                resultSetHistory.updateString("ROKDI_ATT_DEPT", getAttribute("ROKDI_ATT_DEPT").getString());
                resultSetHistory.updateString("ROKDI_ATT_SHIFT", getAttribute("ROKDI_ATT_SHIFT").getString());
                ...
                resultSetHistory.updateString("ROKDI_PUNCH_TIME", ObjDailyAttDataDetails.getAttribute("ROKDI_PUNCH_TIME").getString());
                resultSetHistory.updateString("EMP_REMARK",ObjDailyAttDataDetails.getAttribute("EMP_REMARK").getString());
                
                resultSetHistory.updateString("ROKDI_CONSIDER",ObjDailyAttDataDetails.getAttribute("ROKDI_CONSIDER").getString());
...
//                resultSetHistory.updateString("EMP_EXCESS",ObjDailyAttDataDetails.getAttribute("EMP_EXCESS").getString());
                resultSetHistory.updateString("DEPT_ROKDI_HOURS",ObjDailyAttDataDetails.getAttribute("DEPT_ROKDI_HOURS").getString());
            ...
            ObjFlow.TableName = "SDMLATTPAY.DAILY_ROKDI_ATT_DATA";
    ...
    public boolean Update() {
        ...
        try {
            ...
            resultSetHistory = statementHistory.executeQuery("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA_H WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
            ...
            statementHistory.execute("DELETE FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
...
//                resultSetHistory.updateString("EMP_EXCESS",ObjDailyAttDataDetails.getAttribute("EMP_EXCESS").getString());
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE SDMLATTPAY.DAILY_ROKDI_ATT_DATA SET REJECTED=0,CHANGED=1 WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
    ...
    public boolean CanDelete(String documentNo, String stringProductionDate, int userID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA WHERE DOC_NO='" + documentNo + "' AND APPROVED=" + 1;
            ...
            if (rsTmp.getInt("COUNT") > 0) {  //Item is Approved
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=868 AND USER_ID=" + userID + " AND DOC_NO='" + documentNo + "' AND STATUS='W'";
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();

                if (rsTmp.getInt("COUNT") > 0) {
                    strSQL = "DELETE FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA WHERE DOC_NO='" + documentNo + "'";
    ...
    public boolean Filter(String stringFindQuery) {
        Ready = false;
        try {
            String strSql = "SELECT * FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA WHERE 1=1 " + stringFindQuery + " GROUP BY DOC_NO ORDER BY DOC_NO ";
            connection = data.getConn();
            statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean setData() {
        ...
        try {
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            String str = "SELECT * FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA WHERE DOC_NO = '" + docNo + "' AND DOC_DATE = '" + docDate + "' ORDER BY SR_NO";
            ...
            while (!resultSetTemp.isAfterLast()) {
                serialNoCounter++;
                clsDailyRokdiAttDataDetails ObjDailyAttDataDetails = new clsDailyRokdiAttDataDetails();
                setAttribute("REVISION_NO", 0);
                setAttribute("DOC_NO", resultSetTemp.getString("DOC_NO"));
                setAttribute("DOC_DATE", resultSetTemp.getString("DOC_DATE"));
                
                setAttribute("ROKDI_ATT_DATE", resultSetTemp.getString("ROKDI_ATT_DATE"));
                setAttribute("ROKDI_ATT_DEPT", resultSetTemp.getString("ROKDI_ATT_DEPT"));
                setAttribute("ROKDI_ATT_SHIFT", resultSetTemp.getString("ROKDI_ATT_SHIFT"));
                ...
                ObjDailyAttDataDetails.setAttribute("ROKDI_PUNCH_TIME", resultSetTemp.getString("ROKDI_PUNCH_TIME"));
                ObjDailyAttDataDetails.setAttribute("EMP_REMARK", resultSetTemp.getString("EMP_REMARK"));
                
                ObjDailyAttDataDetails.setAttribute("ROKDI_CONSIDER", resultSetTemp.getString("ROKDI_CONSIDER"));
...
//                ObjDailyAttDataDetails.setAttribute("EMP_EXCESS", resultSetTemp.getString("EMP_EXCESS"));
                ObjDailyAttDataDetails.setAttribute("DEPT_ROKDI_HOURS", resultSetTemp.getString("DEPT_ROKDI_HOURS"));
    ...
    public static HashMap getHistoryList(String DocumentNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT * FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA_H WHERE DOC_NO='" + DocumentNo + "' GROUP BY REVISION_NO");

            while (rsTmp.next()) {
                clsDailyRokdiAttData ObjDailyAttData = new clsDailyRokdiAttData();
    ...
    public boolean ShowHistory(String pProductionDate, String pDocNo) {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT * FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA_H WHERE DOC_NO='" + pDocNo + "'");
            Ready = true;
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            ...
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT DISTINCT MA.DOC_NO,MA.DOC_DATE,SUBSTRING(RECEIVED_DATE,1,10) AS RECEIVED_DATE FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA MA,SDMLATTPAY.D_COM_DOC_DATA D WHERE MA.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=868 AND MA.CANCELED=0 ORDER BY RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT DISTINCT MA.DOC_NO,MA.DOC_DATE,SUBSTRING(RECEIVED_DATE,1,10) AS RECEIVED_DATE FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA MA,SDMLATTPAY.D_COM_DOC_DATA D WHERE MA.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=868 AND MA.CANCELED=0 ORDER BY MA.DOC_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT DISTINCT MA.DOC_NO,MA.DOC_DATE,SUBSTRING(RECEIVED_DATE,1,10) AS RECEIVED_DATE FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA MA,SDMLATTPAY.D_COM_DOC_DATA D WHERE MA.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=868 AND MA.CANCELED=0 ORDER BY MA.DOC_NO";
            }
            ...
            while (rsTmp.next()) {
                ...
                Counter = Counter + 1;
                clsDailyRokdiAttData ObjDoc = new clsDailyRokdiAttData();
    ...
    public static void CancelDoc(String pDocNo) {
        ...
        if (CanCancel(pDocNo)) {
            ...
            try {
                rsTmp = data.getResult("SELECT APPROVED FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA WHERE DOC_NO='" + pDocNo + "' ");
                rsTmp.first();
                ...
                if (!Approved) {
                    data.Execute("DELETE FROM SDMLATTPAY.D_COM_DOC_DATA WHERE DOC_NO='" + pDocNo + "' AND MODULE_ID=868");
                }
                data.Execute("UPDATE SDMLATTPAY.DAILY_ROKDI_ATT_DATA SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE DOC_NO='" + pDocNo + "'");
    ...
    public static boolean CanCancel(String pDocNo) {
        ...
        try {
            ...
            System.out.println("SELECT DOC_NO FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA WHERE DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
            rsTmp = stTmp.executeQuery("SELECT DOC_NO FROM SDMLATTPAY.DAILY_ROKDI_ATT_DATA WHERE DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
