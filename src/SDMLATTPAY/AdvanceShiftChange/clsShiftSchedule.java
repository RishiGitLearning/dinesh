...
public class clsShiftSchedule {
    ...
    public static int ModuleID = 863; //72
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM SDMLATTPAY.ATT_SHIFTSCHEDULE   GROUP BY SS_DOC_NO");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHistory = stHistory.executeQuery("SELECT * FROM SDMLATTPAY.ATT_SHIFTSCHEDULE_H WHERE SS_DOC_NO='1'"); // '1' for restricting all data retrieval
            rsHistory.first();
            rsHDetail = stHDetail.executeQuery("SELECT * FROM SDMLATTPAY.ATT_SHIFTSCHEDULE_H WHERE SS_DOC_NO='1'");
            rsHDetail.first();
            //------------------------------------//
            ...
            ObjFlow.ModuleID = clsShiftSchedule.ModuleID;
            ...
            ObjFlow.TableName = "SDMLATTPAY.ATT_SHIFTSCHEDULE";
    ...
    public boolean Update() {
        ...
        try {
            ...
            int RevNo = EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.ATT_SHIFTSCHEDULE_H WHERE SS_DOC_NO='" + (String) getAttribute("SS_DOC_NO").getObj() + "'");
            RevNo++;
            ...
            if (ApprovalFlow.IsCreator(ModuleID, (String) getAttribute("SS_DOC_NO").getObj())) {
                ...
                data.Execute("DELETE FROM SDMLATTPAY.ATT_SHIFTSCHEDULE WHERE SS_DOC_NO='" + mDocNo + "'");
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE SDMLATTPAY.ATT_SHIFTSCHEDULE SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE SS_DOC_NO='" + ObjFlow.DocNo + "' ");

                //Remove Old Records from FELT_DOC_DATA
                data.Execute("DELETE FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=" + clsShiftSchedule.ModuleID + " AND DOC_NO='" + ObjFlow.DocNo + "'");
    ...
    public boolean setData() {
        ...
        try {
            ...
            if (HistoryView) {
                rsTmp = tmpStmt.executeQuery("SELECT * FROM SDMLATTPAY.ATT_SHIFTSCHEDULE_H "
                ...
                rsTmp = tmpStmt.executeQuery("SELECT * FROM SDMLATTPAY.ATT_SHIFTSCHEDULE S "
                        + "LEFT JOIN (SELECT PAY_EMP_NO,EMP_NAME,EMP_DESIGNATION FROM SDMLATTPAY.ATTPAY_EMPMST) AS M ON SS_EMPID=PAY_EMP_NO  "
                        + "LEFT JOIN (SELECT DSGID,NAME,DESIGNATION_PRIORITY FROM SDMLATTPAY.ATT_DESIGNATION_MASTER) AS D ON DSGID=EMP_DESIGNATION  "
    ...
    public boolean IsEditable(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.ATT_SHIFTSCHEDULE WHERE SS_DOC_NO='" + pDocNo + "' AND (APPROVED=1)";
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=863 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getHistoryList(int CompanyID, String DocNo) {
        ...
        try {
            String strSQL = "SELECT SS_DOC_NO,SS_DOC_DATE,REVISION_NO,UPDATED_BY,APPROVAL_STATUS,DATE_FORMAT(ENTRY_DATE,'%d/%m/%Y %H:%i:%S') AS ENTRY_DATE,REJECTED_REMARKS AS APPROVER_REMARKS,FROM_IP FROM SDMLATTPAY.ATT_SHIFTSCHEDULE_H WHERE SS_DOC_NO='" + DocNo + "' GROUP BY SS_DOC_NO,REVISION_NO";
            rsTmp = data.getResult(strSQL);
            rsTmp.first();

            if (rsTmp.getRow() > 0) {
                while (!rsTmp.isAfterLast()) {
                    clsShiftSchedule objParty = new clsShiftSchedule();
    ...
    public boolean Filter(String Condition) {
        ...
        try {
            String strSQL = "";
            strSQL += "SELECT * FROM SDMLATTPAY.ATT_SHIFTSCHEDULE WHERE " + Condition + "  GROUP BY SS_DOC_NO";
            ...
            if (!rsResultSet.first()) {
                strSQL = "SELECT * FROM SDMLATTPAY.ATT_SHIFTSCHEDULE WHERE  GROUP BY SS_DOC_NO";
                rsResultSet = Stmt.executeQuery(strSQL);
                Ready = true;
                MoveLast();
                return false;
    ...
    public boolean CanDelete(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=863 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND TYPE='C' AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO,SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_SHIFTSCHEDULE,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=863 GROUP BY SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO ORDER BY SDMLATTPAY.D_COM_DOC_DATA.RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO,SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_SHIFTSCHEDULE,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=863 GROUP BY SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO  ORDER BY SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO,SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.ATT_SHIFTSCHEDULE,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=863 GROUP BY SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO  ORDER BY SDMLATTPAY.ATT_SHIFTSCHEDULE.SS_DOC_NO";
            }
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                ...
                Counter = Counter + 1;
                clsShiftSchedule ObjItem = new clsShiftSchedule();
    ...
    public boolean ShowHistory(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM SDMLATTPAY.ATT_SHIFTSCHEDULE_H WHERE SS_DOC_NO='" + pDocNo + "' GROUP BY SS_DOC_NO ORDER BY REVISION_NO ");
            Ready = true;
            HistoryView = true;
            MoveFirst();
            return true;
    ...
    private void UpdateShift() {
        try {
            ...
            for (int i = st; i <= end; i++) {
                sql = "UPDATE  SDMLATTPAY.ATT_SHIFT_SCHEDULE S, SDMLATTPAY.ATT_SHIFTSCHEDULE C "
