...
public class clsRokdiCProcess {
    ...
    public static int ModuleID = 829; //72
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL  GROUP BY DOC_NO,DOC_DATE  ORDER BY DOC_DATE,DOC_NO ");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHDetail = stHDetail.executeQuery("SELECT * FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL_H WHERE DOC_NO='1'");
            rsHDetail.first();
            //------------------------------------//
            ...
            rsTmp = tmpStmt.executeQuery("SELECT * FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL WHERE DOC_NO='1'");

            sql = "INSERT INTO SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL "
            ...
            sqlh = "INSERT INTO SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL_H "
            ...
            for (int i = 1; i <= colMRItems.size(); i++) {
                clsRokdiCProcessItem ObjItems = (clsRokdiCProcessItem) colMRItems.get(Integer.toString(i));
                ...
                pstm.setString(4, ObjItems.getAttribute("REPLACEMENT_HOURS").getString());
                pstm.setString(5, ObjItems.getAttribute("EXTRA_HOURS").getString());
                ...
                pstmh.setString(9, ObjItems.getAttribute("REPLACEMENT_HOURS").getString());
                pstmh.setString(10, ObjItems.getAttribute("EXTRA_HOURS").getString());
            ...
            ObjFlow.ModuleID = clsRokdiCProcess.ModuleID;
            ...
            ObjFlow.TableName = "SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL";
    ...
    public boolean Update() {
        ...
        PreparedStatement pstm = null, pstmh = null;
        try {
            data.Execute("DELETE FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL WHERE DOC_NO='" + (String) getAttribute("DOC_NO").getObj() + "'");
            ...
            for (int i = 1; i <= colMRItems.size(); i++) {
                clsRokdiCProcessItem ObjItems = (clsRokdiCProcessItem) colMRItems.get(Integer.toString(i));
                if (i == 1) {
                    RevNo = EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL_H WHERE DOC_NO='" + (String) getAttribute("DOC_NO").getObj() + "'");
                    RevNo++;
                }
                pstm.setString(1, (String) getAttribute("DOC_NO").getObj());
                pstm.setString(2, (String) getAttribute("DOC_DATE").getObj());
            ...
            Conn.setAutoCommit(true);
            String s;
            s = "UPDATE SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL D,SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL_H H "
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL SET REJECTED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE DOC_NO='" + ObjFlow.DocNo + "' ");

                //Remove Old Records from FELT_DOC_DATA
                data.Execute("DELETE FROM SDMLATTPAY.D_COM_DOC_DATA WHERE MODULE_ID=" + clsRokdiCProcess.ModuleID + " AND DOC_NO='" + ObjFlow.DocNo + "'");
    ...
    public boolean setData() {
        ...
        try {
            ...
            if (HistoryView) {
                rsTmp = tmpStmt.executeQuery("SELECT * FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL_H WHERE DOC_NO='" + mDocNo + "' AND REVISION_NO=" + RevNo + "   ORDER BY DOC_NO");
            } else {
                rsTmp = tmpStmt.executeQuery("SELECT * FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL WHERE DOC_NO='" + mDocNo + "'   ORDER BY DOC_NO");
            }
            rsTmp.first();
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                Counter = Counter + 1;
                clsRokdiCProcessItem ObjItems = new clsRokdiCProcessItem();
    ...
    public boolean IsEditable(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL WHERE DOC_NO='" + pDocNo + "' AND (APPROVED=1)";
    ...
    public static HashMap getHistoryList(int CompanyID, String DocNo) {
        ...
        try {
            String strSQL = "SELECT * FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL_H WHERE DOC_NO='" + DocNo + "' GROUP BY REVISION_NO,DOC_NO ";
            rsTmp = data.getResult(strSQL);
            rsTmp.first();

            if (rsTmp.getRow() > 0) {
                while (!rsTmp.isAfterLast()) {
                    clsRokdiCProcess objParty = new clsRokdiCProcess();
    ...
    public boolean Filter(String Condition) {
        ...
        try {
            String strSQL = "";
            strSQL += "SELECT * FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL WHERE " + Condition + " GROUP BY DOC_NO";
            Conn = data.getConn();
            Stmt = Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rsResultSet = Stmt.executeQuery(strSQL);

            if (!rsResultSet.first()) {
                strSQL = "SELECT * FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL WHERE DOC_DATE>='" + EITLERPGLOBAL.FinFromDateDB + "' AND DOC_DATE<='" + EITLERPGLOBAL.FinToDateDB + "'   GROUP BY DOC_NO  ORDER BY DOC_NO";
                rsResultSet = Stmt.executeQuery(strSQL);
                Ready = true;
                MoveLast();
                return false;
    ...
    public boolean Delete(int pUserID) {
        try {
            ...
            if (CanDelete(lCompanyID, lDocNo, pUserID)) {
                String strQry = "DELETE FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL WHERE DOC_NO='" + lDocNo + "'";
                data.Execute(strQry);
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT DISTINCT SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_NO,SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=" + ModuleID + " ORDER BY SDMLATTPAY.D_COM_DOC_DATA.RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT DISTINCT SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_NO,SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=" + ModuleID + " ORDER BY SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT DISTINCT SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_NO,SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_DATE,RECEIVED_DATE FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL,SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.D_COM_DOC_DATA.DOC_NO=SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_NO AND SDMLATTPAY.D_COM_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND SDMLATTPAY.D_COM_DOC_DATA.STATUS='W' AND MODULE_ID=" + ModuleID + " ORDER BY SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_NO";
            }
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                ...
                Counter = Counter + 1;
                clsRokdiCProcess ObjItem = new clsRokdiCProcess();
    ...
    public boolean ShowHistory(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL_H WHERE DOC_NO='" + pDocNo + "'  GROUP BY DOC_NO ORDER BY REVISION_NO");
            Ready = true;
            HistoryView = true;
            MoveFirst();
            return true;
