 ...
 *
 */
public class FrmRokdiCProcess extends javax.swing.JApplet {
    ...
    private int FFNo = 0;
    private int ModuleId = 829;
    private String DOC_NO = "";
    private clsRokdiCProcess RokdiProcess;
    ...
    public void init() {
        ...
        RokdiProcess = new clsRokdiCProcess();
    ...
    private void DisplayData() {
        ...
        try {
            ...
            for (int i = 1; i <= RokdiProcess.colMRItems.size(); i++) {
                ...
                clsRokdiCProcessItem ObjItem = (clsRokdiCProcessItem) RokdiProcess.colMRItems.get(Integer.toString(i));
        ...
        try {
            ...
            HashMap History = clsRokdiCProcess.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsRokdiCProcess ObjHistory = (clsRokdiCProcess) History.get(Integer.toString(i));
    ...
    private void SetMenuForRights() {
        // --- Add Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10065, 100651)) { //7008,70081
        ...
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10065, 100652)) { //7008,70082
            cmdEdit.setEnabled(true);
        } else {
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10065, 100653)) {
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 10065, 100654)) {
    ...
    private void FormatGrid() {
        try {
            ...
            String sql = "SELECT '' AS SR_NO,'' AS EMPLOYEE_NO,'' AS NAME,'' AS DEPT,'' AS \"HRS-E\",'' AS \"HRS-R\",'' AS Rate,"
                    + "'' AS \"PAY-E\",'' AS \"PAY-R\",'' AS TOTAL_PAY,"
                    + "'' AS \"REV STAMP\",'' AS NET_PAY,'' AS SIGNATURE "
                    + " FROM DUAL";
...
//            Table.getColumnModel().getColumn(1).setCellRenderer(CellAlign1);
    ...
    public void FindWaiting() {
        RokdiProcess.Filter(" DOC_NO IN (SELECT DISTINCT SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_NO FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL, SDMLATTPAY.D_COM_DOC_DATA WHERE SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL.DOC_NO=SDMLATTPAY.D_COM_DOC_DATA.DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=" + ModuleId + " AND CANCELED=0) ");
        SetMenuForRights();
        DisplayData();
    }
...
//      
        ...
        for (int m = 0; m < Table.getRowCount(); m++) {
            clsRokdiCProcessItem ObjItem = new clsRokdiCProcessItem();

            ObjItem.setAttribute("EMPLOYEE_CODE", Table.getValueAt(m, 1).toString());
            ObjItem.setAttribute("BASIC_RATE", Double.parseDouble(Table.getValueAt(m, 6).toString()));
            ObjItem.setAttribute("BASIC_EARN", Double.parseDouble(Table.getValueAt(m, 9).toString()));
            ObjItem.setAttribute("TOTAL_EARN", Double.parseDouble(Table.getValueAt(m, 9).toString()));
            ObjItem.setAttribute("TOTAL_PAY", Double.parseDouble(Table.getValueAt(m, 9).toString()));
            ObjItem.setAttribute("REPLACEMENT_HOURS", (Table.getValueAt(m, 5).toString()));
            ObjItem.setAttribute("REPLACEMENT_EARN", Double.parseDouble(Table.getValueAt(m, 8).toString()));
            ObjItem.setAttribute("EXTRA_HOURS", (Table.getValueAt(m, 4).toString()));
            ObjItem.setAttribute("EXTRA_EARN", Double.parseDouble(Table.getValueAt(m, 7).toString()));
            ObjItem.setAttribute("REV_STAMP", Double.parseDouble(Table.getValueAt(m, 10).toString()));
            ObjItem.setAttribute("NET_PAY", Double.parseDouble(Table.getValueAt(m, 11).toString()));
    ...
    private boolean CheckRokdi() {
        boolean chk = false;
        String c;
        c = data.getStringValueFromDB("SELECT COALESCE(GROUP_CONCAT(DISTINCT COFF_DOC_NO),'') FROM SDMLATTPAY.ATT_COFF_ROKDI_ENTRY WHERE COFF_DOC_NO LIKE 'HC%' AND COFF_DATE>='" + EITLERPGLOBAL.formatDateDB(txtfromdt.getText()) + "' AND COFF_DATE<='" + EITLERPGLOBAL.formatDateDB(txttodt.getText()) + "' AND APPROVED=0 AND CANCELED=0");
        ...
        c = data.getStringValueFromDB("SELECT COALESCE(GROUP_CONCAT(DISTINCT DOC_NO),'') FROM SDMLATTPAY.ROKDI_SELECTION_DATA WHERE EMP_ID LIKE 'BRD00%' AND  ROKDI_FROM_DATE>='" + EITLERPGLOBAL.formatDateDB(txtfromdt.getText()) + "' AND ROKDI_FROM_DATE<='" + EITLERPGLOBAL.formatDateDB(txttodt.getText()) + "' AND APPROVED=0 AND CANCELED=0");
        if (c.equalsIgnoreCase("")) {
            chk = true;
        } else {
            JOptionPane.showMessageDialog(this, "Rokdi Document No. " + c + " is Pending.. \n First Approve then Process", "Error", JOptionPane.ERROR_MESSAGE);
        ...
        c = data.getStringValueFromDB("SELECT DISTINCT COALESCE(DOC_NO,'') FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL WHERE EMPLOYEE_CODE LIKE 'BRD0%' AND DOC_NO='" + txtfromdt.getText() + " - " + txttodt.getText() + "'");
        if (c.equalsIgnoreCase("")) {
            chk = true;
        } else {
        ...
        c = data.getStringValueFromDB("SELECT DISTINCT COALESCE(DOC_NO,'') FROM SDMLATTPAY.CONTRACT_ROKDI_PAYMENT_DETAIL WHERE EMPLOYEE_CODE LIKE 'BRD0%' AND  APPROVED=0 AND COALESCE(CANCELED,0)=0");
        if (c.equalsIgnoreCase("")) {
            chk = true;
        } else {
    ...
    private void CalculateRokdi() {
        try {
            ...
            String sql = "";
            sql = "SELECT COFF_EMPID AS EMPLOYEE_CODE,"
                    + "(SUM(TIME_TO_SEC(COFF_HRS_REPLACEMENT)))/3600 AS RHRS,SUM(TIME_TO_SEC(COFF_HRS_REPLACEMENT))/3600 AS REPLACEMENT_HOURS,"
                    + "(SUM(TIME_TO_SEC(COFF_HRS_EXTRA)))/3600 AS EHRS,SUM(TIME_TO_SEC(COFF_HRS_EXTRA))/3600 AS EXTRA_HOURS,"
                    + "(SUM(TIME_TO_SEC(COFF_HRS_EXTRA)+TIME_TO_SEC(COFF_HRS_EXTRA)))/3600 AS HRS,"
                    + "EMP_BASIC_RATE "
                    + " FROM SDMLATTPAY.ATT_COFF_ROKDI_ENTRY A "
                    + "LEFT JOIN SDMLATTPAY.ATTPAY_EMPMST "
                    + "ON PAY_EMP_NO=COFF_EMPID "
                    + "WHERE COFF_DOC_NO LIKE 'HC%' "
                    ...
                    + " GROUP BY COFF_EMPID ORDER BY EMP_DEPARTMENT,COFF_EMPID";
            ...
            if (t.getRow() > 0) {
                ...
                while (!t.isAfterLast()) {
                    ...
                    p++;
                     rowData[p] = t.getString("EHRS");
                    p++;
                     rowData[p] = t.getString("RHRS");
                    p++;
                    rowData[p] = mbasic;
                    p++;
                    rowData[p] = EITLERPGLOBAL.round((mbasic / 8) * (mehrs), 2);
                    p++;
                    rowData[p] = EITLERPGLOBAL.round((mbasic / 8) * (mrhrs), 2);
                    p++;
                    rowData[p] = EITLERPGLOBAL.round((mbasic / 8) * (mhrs), 2);
                    p++;
                    mnetpay = EITLERPGLOBAL.round((mbasic / 8) * (mhrs), 0);
                    if (mnetpay > 5000) {
                        mrev = 0;
                        mnetpay = mnetpay;
                    } else {
