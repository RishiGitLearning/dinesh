...
public class clsModule {
    ...
    public boolean LoadData(long pCompanyID) {
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_COM_MODULES WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" ORDER BY MODULE_ID");
            MoveFirst();
            return true;
        }catch(Exception e) {
    ...
    public boolean Filter(String pCondition) {
        try {
            String strSql = "SELECT * FROM D_COM_MODULES WHERE COMPANY_ID=2 AND " + pCondition ;
            Conn=data.getConn();
            Stmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
