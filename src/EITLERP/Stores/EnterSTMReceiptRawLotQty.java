...
public class EnterSTMReceiptRawLotQty extends javax.swing.JApplet {
    ...
    public EnterSTMReceiptRawLotQty() {
        System.gc();
        initComponents();
    }
    ...
    private void GenerateGrid() {
        ...
        try {
            for(int i=1;i<=colLot.size();i++) {
                clsSTMReceiptRawLot ObjLot=(clsSTMReceiptRawLot)colLot.get(Integer.toString(i));
                Object[] rowData=new Object[6];
                
                rowData[0]=Integer.toString(ObjLot.getAttribute("SR_NO").getInt());
                rowData[1]=ObjLot.getAttribute("ITEM_LOT_NO").getString();
                //generate new lot no
                //rowData[2] = getNextLOTNo(EITLERPGLOBAL.gCompanyID);
    ...
    private void SetLot() {
        ...
        for(int i=0;i<Table.getRowCount();i++) {
            ...
            ObjLot.setAttribute("LOT_RECEIVED_QTY",EITLERPGLOBAL.round(Double.parseDouble(DataModel.getValueByVariable("LOT_RECEIVED_QTY",i)),3));
            ObjLot.setAttribute("LOT_REJECTED_QTY",EITLERPGLOBAL.round(Double.parseDouble(DataModel.getValueByVariable("LOT_REJECTED_QTY",i)),3));
            ObjLot.setAttribute("LOT_ACCEPTED_QTY",EITLERPGLOBAL.round(Double.parseDouble(DataModel.getValueByVariable("LOT_ACCEPTED_QTY",i)),3));
            ObjLot.setAttribute("BALANCE_QTY",EITLERPGLOBAL.round(Double.parseDouble(DataModel.getValueByVariable("LOT_ACCEPTED_QTY",i)),3));
            ObjLot.setAttribute("LOT_CLOSE",false);
    ...
    public static String getNextLOTNo(int CompanyID) {
        ...
        try {
            ...
            Prefix = "S"+Integer.toString(EITLERPGLOBAL.getCurrentFinYear()).substring(2,4);
            String strSQL="SELECT MAX(SUBSTRING(AUTO_LOT_NO,LENGTH('"+Prefix+"')+1)) AS MAX_NO FROM D_INV_GRN_LOT WHERE AUTO_LOT_NO LIKE '"+Prefix+"%' AND CANCELLED=0 ";
