...
public class clsIssueRequisition {
    ...
    public clsIssueRequisition() {
        LastError = "";
        props=new HashMap();
        
        props.put("COMPANY_ID",new Variant(EITLERPGLOBAL.gCompanyID));
        props.put("REQ_NO", new Variant(""));
        props.put("REQ_TYPE",new Variant(1));
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_INV_ISSUE_REQ_HEADER WHERE COMPANY_ID="+pCompanyID+" AND REQ_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND REQ_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' AND REQ_TYPE=1 ORDER BY REQ_NO");
            HistoryView=false;
            Ready=true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            //setAttribute("REQ_NO",clsFirstFree.getNextFreeNo((int)EITLERPGLOBAL.gCompanyID,clsIssueRequisition.ModuleID, (int)getAttribute("FFNO").getVal(),true));
            setAttribute("REQ_NO",clsFirstFree.getNextFreeNo((int)EITLERPGLOBAL.gCompanyID,clsIssueRequisition.ModuleID, (int)getAttribute("FFNO").getVal(),false));            
            //-------------------------------------------------
            ...
            for(int i=1;i<=colMRItems.size();i++) {
                ...
                rsTmp.updateString("ISSUE_CAT",(String)ObjMRItems.getAttribute("ISSUE_CAT").getObj());
                rsTmp.updateString("ISSUE_CAT_DESC",(String)ObjMRItems.getAttribute("ISSUE_CAT_DESC").getObj());
                rsTmp.insertRow();
                ...
                rsHDetail.updateString("ISSUE_CAT",(String)ObjMRItems.getAttribute("ISSUE_CAT").getObj());
                rsHDetail.updateString("ISSUE_CAT_DESC",(String)ObjMRItems.getAttribute("ISSUE_CAT_DESC").getObj());
                rsHDetail.insertRow();
            }
            
            //======== Update the Approval Flow =========
            ObjFlow.CompanyID=(int)getAttribute("COMPANY_ID").getVal();
            ObjFlow.ModuleID=clsIssueRequisition.ModuleID;
    ...
    public boolean Update() {
        ...
        try {
            ...
            data.Execute("DELETE FROM D_INV_ISSUE_REQ_DETAIL WHERE COMPANY_ID="+mCompanyID+" AND REQ_NO='"+mReqNo+"' AND REQ_TYPE=1");
            ...
            if(IsRejected) {
                //Remove the Rejected Flag First
                data.Execute("UPDATE D_INV_REQ_HEADER SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND REQ_NO='"+ObjFlow.DocNo+"' AND REQ_TYPE=1");
                //Remove Old Records from D_COM_DOC_DATA
                data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID="+clsIssueRequisition.ModuleID+" AND DOC_NO='"+ObjFlow.DocNo+"'");
    ...
    public boolean CanDelete(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM D_INV_ISSUE_REQ_HEADER WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND REQ_NO='"+pDocNo+"' AND REQ_TYPE=1 AND (APPROVED=1)";
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM D_COM_DOC_DATA WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND MODULE_ID="+clsIssueRequisition.ModuleID+" AND DOC_NO='"+pDocNo+"' AND USER_ID="+Integer.toString(pUserID)+" AND TYPE='C' AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean IsEditable(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM D_COM_DOC_DATA WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND MODULE_ID="+clsIssueRequisition.ModuleID+" AND DOC_NO='"+pDocNo+"' AND USER_ID="+Integer.toString(pUserID)+" AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean Delete(int pUserID) {
        try {
            ...
            if(CanDelete(lCompanyID,lDocNo,pUserID)) {
                String strQry = "DELETE FROM D_INV_ISSUE_REQ_HEADER WHERE COMPANY_ID=" + lCompanyID +" AND REQ_NO='"+lDocNo+"' AND REQ_TYPE=1";
                data.Execute(strQry);
                strQry = "DELETE FROM D_INV_ISSUE_REQ_DETAIL WHERE COMPANY_ID=" + lCompanyID +" AND REQ_NO='"+lDocNo+"' AND REQ_TYPE=1";
                data.Execute(strQry);
    ...
    public Object getObject(int pCompanyID, String pReqNo) {
        ...
        clsIssueRequisition ObjMaterialRequisition = new clsIssueRequisition();
    ...
    public static HashMap getList(String pCondition) {
        ...
        try {
            ...
            while(rsTmp.next()) {
                ...
                rsTmp2=tmpStmt2.executeQuery("SELECT * FROM D_INV_ISSUE_REQ_DETAIL WHERE COMPANY_ID="+mCompanyID+" AND REQ_NO='" + mReqNo + "' AND REQ_TYPE=1");
                ...
                while(rsTmp2.next()) {
                    ...
                    rsLot=stLot.executeQuery("SELECT * FROM D_INV_ISSUE_REQ_LOT WHERE COMPANY_ID="+rsTmp2.getString("COMPANY_ID")+" AND REQ_NO='"+rsTmp2.getString("REQ_NO")+"' AND REQ_SR_NO="+rsTmp2.getLong("SR_NO"));
                    rsLot.first();
    ...
    public boolean Filter(String pCondition,int pCompanyID) {
        ...
        try {
            //MUFFY
            //String strSql = "SELECT * FROM D_INV_ISSUE_REQ_HEADER " + pCondition ;
            String strSql = "SELECT * FROM D_INV_ISSUE_REQ_HEADER " + pCondition + "  AND REQ_TYPE = 1 ORDER BY REQ_NO";
            Conn=data.getConn();
            Stmt=Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rsResultSet=Stmt.executeQuery(strSql);
            
            if(!rsResultSet.first()) {
                strSql = "SELECT * FROM D_INV_ISSUE_REQ_HEADER WHERE COMPANY_ID="+Long.toString(pCompanyID)+" AND REQ_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND REQ_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' AND REQ_TYPE=1 ORDER BY REQ_NO";
                rsResultSet=Stmt.executeQuery(strSql);
                Ready=true;
                MoveLast();
                return false;
    ...
    public boolean setData() {
        ...
        try {
            ...
            tmpStmt=tmpConn.createStatement();
            if(HistoryView) {
                rsTmp=tmpStmt.executeQuery("SELECT * FROM D_INV_ISSUE_REQ_DETAIL_H WHERE COMPANY_ID=" + mCompanyID + " AND REQ_NO='" + mReqNo + "' AND REQ_TYPE=1 AND REVISION_NO="+RevNo+" ORDER BY SR_NO");
            }
            else {
                rsTmp=tmpStmt.executeQuery("SELECT * FROM D_INV_ISSUE_REQ_DETAIL WHERE COMPANY_ID=" + mCompanyID + " AND REQ_NO='" + mReqNo + "' AND REQ_TYPE=1 ORDER BY SR_NO");
            }
            rsTmp.first();
            ...
            while(!rsTmp.isAfterLast() && rsTmp.getRow()>0) {
                ...
                ObjMRItems.setAttribute("ISSUE_CAT",rsTmp.getString("ISSUE_CAT"));
                ObjMRItems.setAttribute("ISSUE_CAT_DESC",rsTmp.getString("ISSUE_CAT_DESC"));
    ...
    public static HashMap getPendingApprovals(int pCompanyID,int pUserID,int pOrder,int FinYearFrom) {
        ...
        try {
            ...
            if(pOrder==EITLERPGLOBAL.OnRecivedDate) {
                strSQL="SELECT D_INV_ISSUE_REQ_HEADER.REQ_NO,D_INV_ISSUE_REQ_HEADER.REQ_DATE,RECEIVED_DATE,D_INV_ISSUE_REQ_HEADER.DEST_DEPT_ID AS DEPT_ID FROM D_INV_ISSUE_REQ_HEADER,D_COM_DOC_DATA WHERE D_INV_ISSUE_REQ_HEADER.REQ_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_ISSUE_REQ_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_ISSUE_REQ_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_INV_ISSUE_REQ_HEADER.REQ_TYPE=1 AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND MODULE_ID="+clsIssueRequisition.ModuleID+" ORDER BY D_COM_DOC_DATA.RECEIVED_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocDate) {
                strSQL="SELECT D_INV_ISSUE_REQ_HEADER.REQ_NO,D_INV_ISSUE_REQ_HEADER.REQ_DATE,RECEIVED_DATE,D_INV_ISSUE_REQ_HEADER.DEST_DEPT_ID AS DEPT_ID FROM D_INV_ISSUE_REQ_HEADER,D_COM_DOC_DATA WHERE D_INV_ISSUE_REQ_HEADER.REQ_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_ISSUE_REQ_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_ISSUE_REQ_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_INV_ISSUE_REQ_HEADER.REQ_TYPE=1 AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND MODULE_ID="+clsIssueRequisition.ModuleID+" ORDER BY D_INV_ISSUE_REQ_HEADER.REQ_DATE";
            }
            ...
            while(rsTmp3.next()) {
                if(EITLERPGLOBAL.isWithinDate(rsTmp3.getString("REQ_DATE"),FinYearFrom)) {
                    Counter=Counter+1;
                    clsIssueRequisition ObjDoc=new clsIssueRequisition();
    ...
    public static HashMap getMRItemList(int pCompanyID,String pMRNo,boolean pAllData,int pType) {
        ...
        try {
            ...
            if(pAllData) {
                strSQL = "SELECT * FROM D_INV_ISSUE_REQ_DETAIL WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND REQ_NO='"+pMRNo.trim()+"' AND REQ_TYPE="+pType+" AND REQ_NO IN (SELECT REQ_NO FROM D_INV_ISSUE_REQ_HEADER WHERE REQ_NO='"+pMRNo+"' AND APPROVED=1) ORDER BY ITEM_CODE";
            }
            else{
            ...
            while(! rsTmp2.isAfterLast()) {
                ...
                ObjMRItems.setAttribute("COST_CENTER_CODE",rsTmp2.getInt("COST_CENTER_CODE"));
                ObjMRItems.setAttribute("DEPT_ID",rsTmp2.getInt("DEPT_ID"));
                
                ObjMRItems.setAttribute("REQ_QTY",rsTmp2.getFloat("REQ_QTY"));
                double IssuedQty=data.getDoubleValueFromDB("SELECT SUM(QTY) AS TOTAL_QTY FROM D_INV_ISSUE_HEADER A,D_INV_ISSUE_DETAIL B WHERE A.ISSUE_NO=B.ISSUE_NO AND A.APPROVED=1 AND A.CANCELED=0 AND B.REQ_NO='"+pMRNo+"' AND B.REQ_SRNO="+rsTmp2.getLong("SR_NO"));
                ...
                ObjMRItems.setAttribute("ISSUE_CAT",rsTmp2.getString("ISSUE_CAT"));
                ObjMRItems.setAttribute("ISSUE_CAT_DESC",rsTmp2.getString("ISSUE_CAT_DESC"));
                ObjMRItems.setAttribute("REQUIRED_DATE",rsTmp2.getString("REQUIRED_DATE"));
                ...
                //Now Fetch Lots.
                stLot=tmpConn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
                rsLot=stLot.executeQuery("SELECT * FROM D_INV_ISSUE_REQ_LOT WHERE COMPANY_ID="+pCompanyID+" AND REQ_NO='"+pMRNo+"' AND REQ_SR_NO="+rsTmp2.getLong("SR_NO"));
                rsLot.first();
    ...
    public boolean ShowHistory(int pCompanyID,String pDocNo) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_INV_ISSUED_REQ_HEADER_H WHERE COMPANY_ID="+pCompanyID+" AND REQ_NO='"+pDocNo+"' AND REQ_TYPE=1");
            Ready=true;
            HistoryView=true;
            MoveFirst();
            return true;
    ...
    public static HashMap getHistoryList(int pCompanyID,String pDocNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT * FROM D_INV_ISSUE_REQ_HEADER_H WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND REQ_NO='"+pDocNo+"' AND REQ_TYPE=1");
            
            while(rsTmp.next()) {
                clsIssueRequisition ObjMR=new clsIssueRequisition();
    ...
    public static boolean CanCancel(int pCompanyID,String pDocNo) {
        ...
        try {
            rsTmp=data.getResult("SELECT REQ_NO FROM D_INV_ISSUE_REQ_HEADER WHERE COMPANY_ID="+pCompanyID+" AND REQ_NO='"+pDocNo+"' AND CANCELED=0 AND REQ_TYPE=1 ");
            rsTmp.first();
    ...
    public static boolean CancelDoc(int pCompanyID,String pDocNo) {
        ...
        try {
            if(CanCancel(pCompanyID,pDocNo)) {
                ...
                rsTmp=data.getResult("SELECT APPROVED FROM D_INV_ISSUE_REQ_HEADER WHERE COMPANY_ID="+pCompanyID+" AND REQ_NO='"+pDocNo+"' AND REQ_TYPE=1");
                rsTmp.first();
                ...
                if(ApprovedDoc) {
                    ...
                    data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID="+pCompanyID+" AND DOC_NO='"+pDocNo+"' AND MODULE_ID="+(clsIssueRequisition.ModuleID));
                }
                
                //Now Update the header with cancel falg to true
                data.Execute("UPDATE D_INV_ISSUE_REQ_HEADER SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+pCompanyID+" AND REQ_NO='"+pDocNo+"' AND REQ_TYPE=1");
