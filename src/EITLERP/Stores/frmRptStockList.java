...
public class frmRptStockList extends javax.swing.JApplet {
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        lblItem.setBounds(15, 412, 383, 15);
        ...
        chkIncludeNonMoving.setBounds(266, 378, 230, 23);
    ...
    private void cmdPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdPrintActionPerformed
        // TODO add your handling code here:
        int type = EITLERPGLOBAL.getComboCode(cmbItemType);
        if(type == 1) {
            GenerateReportWAvg();
        }
        else if(type == 2) {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    if(ItemType==2) //Raw
                    {
                        strSQL="SELECT * FROM D_INV_ITEM_CATEGORY WHERE CATEGORY_TYPE = 2 AND CATEGORY_ID <>13";
                    }
                    else if(ItemType==3) //raw STM 90
                    {
                        strSQL="SELECT * FROM D_INV_ITEM_CATEGORY WHERE CATEGORY_TYPE = 2 AND CATEGORY_ID = 13";
                    ...
                    if(rsItemGroup.getRow()>0) {
                        while(!rsItemGroup.isAfterLast()) {
                            ...
                            writeLine(aFile,"Category Id: " + rsItemGroup.getString("CATEGORY_NAME"));
                            //========= Create a text file ==============//
                            strSQL="SELECT COUNT(*) THECOUNT FROM D_INV_ITEM_MASTER A WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MAINTAIN_STOCK=1 AND APPROVED=1 AND CANCELLED=0 "+strCondition+" AND CATEGORY_ID = '"+groupID+"' ";
                            ...
                            strSQL="SELECT ITEM_ID,ITEM_DESCRIPTION,UNIT,LOCATION_ID FROM D_INV_ITEM_MASTER A WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MAINTAIN_STOCK=1 AND APPROVED=1 AND CANCELLED=0 "+strCondition+" AND CATEGORY_ID = '"+groupID+"' GROUP BY A.ITEM_ID "+strOrder;
                    ...
                    writeLine(aFile,LeftMargin+"Grand Total :"+dFormat.format(grandtotal));
                    aFile.close();
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    strCondition=strCondition+" SELECT DISTINCT(ITEM_CODE) FROM D_INV_ISSUE_HEADER A,D_INV_ISSUE_DETAIL B WHERE A.ISSUE_NO=B.ISSUE_NO AND A.ISSUE_DATE>='"+EITLERPGLOBAL.formatDateDB(strFromDate)+"' AND ISSUE_DATE <='"+EITLERPGLOBAL.formatDateDB(txtToDate.getText())+"'";
                    strCondition=strCondition+" UNION ";
                    strCondition=strCondition+" SELECT DISTINCT(ITEM_ID) FROM D_INV_STM_HEADER A,D_INV_STM_DETAIL B WHERE A.STM_NO=B.STM_NO AND A.STM_TYPE=B.STM_TYPE AND A.STM_DATE>='"+EITLERPGLOBAL.formatDateDB(strFromDate)+"' AND STM_DATE <='"+EITLERPGLOBAL.formatDateDB(txtToDate.getText())+"'";
                    strCondition=strCondition+" UNION ";
                    strCondition=strCondition+" SELECT DISTINCT(ITEM_ID) FROM D_COM_OPENING_STOCK_DETAIL WHERE ENTRY_NO=" + StockEntryNo + " ";
                    //                    //***********25/03/2009
                    data.Execute(strCondition);
                    ...
                    if((!txtFromItemID.getText().equals(""))||(!txtToItemID.getText().equals(""))) {
                        ...
                        if(!txtFromItemID.getText().equals("")) {
                            strCondition=strCondition+" AND SUBSTRING(A.ITEM_ID,1,8)>='"+txtFromItemID.getText()+"' ";
                        }
                        
                        if(!txtToItemID.getText().equals("")) {
                            strCondition=strCondition+" AND SUBSTRING(A.ITEM_ID,1,8)<='"+txtToItemID.getText()+"' ";
                        }
                    }
                    ...
                    if(rsItemGroup.getRow()>0) {
                        while(!rsItemGroup.isAfterLast()) {
                            ...
                            while((!rsItem.isAfterLast())&&rsItem.getRow()>0) {
                                ...
                                strSQL="SELECT * FROM D_COM_OPENING_STOCK_DETAIL WHERE ITEM_ID LIKE '"+ItemID.trim()+"%' AND ENTRY_NO="+StockEntryNo;
                                rsTmp=data.getResult(strSQL);
                                rsTmp.first();
                                ...
                                strSQL="SELECT IF(SUM(QTY) IS NULL,0,SUM(QTY)) AS SUM_QTY,IF(SUM(ZERO_VAL_QTY) IS NULL,0,SUM(ZERO_VAL_QTY)) AS SUM_ZERO_QTY, IF(SUM(ISSUE_VALUE) IS NULL,0,SUM(ISSUE_VALUE)) AS SUM_VALUE FROM D_INV_ISSUE_HEADER A,D_INV_ISSUE_DETAIL B WHERE A.COMPANY_ID=B.COMPANY_ID AND A.ISSUE_NO=B.ISSUE_NO AND A.APPROVED=1 AND A.CANCELED=0 AND A.ISSUE_DATE>'"+StockEntryDate+"' AND ISSUE_DATE<'"+EITLERPGLOBAL.formatDateDB(strFromDate)+"' AND B.ITEM_CODE='"+ItemID.trim()+"'";
                                rsTmp=data.getResult(strSQL);
                                rsTmp.first();
                                ...
                                strSQL="SELECT B.SR_NO,A.GRN_NO AS DOC_NO,A.GRN_DATE AS DOC_DATE,'+' AS OPERATION,'1' AS TORDER,ROUND(B.QTY,3) AS QTY,B.LANDED_RATE*B.QTY AS VALUE,0 AS ZERO_VAL_QTY FROM D_INV_GRN_HEADER A,D_INV_GRN_DETAIL B WHERE A.COMPANY_ID=B.COMPANY_ID AND A.GRN_NO=B.GRN_NO AND A.GRN_TYPE=B.GRN_TYPE AND A.APPROVED=1 AND A.CANCELLED=0 AND B.ITEM_ID='"+ItemID.trim()+"' AND A.GRN_DATE>='"+EITLERPGLOBAL.formatDateDB(strFromDate)+"' AND GRN_DATE<='"+EITLERPGLOBAL.formatDateDB(txtToDate.getText())+"'";
                                strSQL=strSQL+" UNION ";
                                strSQL=strSQL+"  SELECT B.SR_NO,A.ISSUE_NO AS DOC_NO,A.ISSUE_DATE AS DOC_DATE,'-' AS OPERATION,'2' AS TORDER,ROUND(B.QTY,3) AS QTY,B.ISSUE_VALUE AS VALUE,B.ZERO_VAL_QTY FROM D_INV_ISSUE_HEADER A,D_INV_ISSUE_DETAIL B WHERE A.COMPANY_ID=B.COMPANY_ID AND A.ISSUE_NO=B.ISSUE_NO AND A.APPROVED=1 AND A.CANCELED=0 AND B.ITEM_CODE='"+ItemID.trim()+"' AND A.ISSUE_DATE>='"+EITLERPGLOBAL.formatDateDB(strFromDate)+"' AND ISSUE_DATE<='"+EITLERPGLOBAL.formatDateDB(txtToDate.getText())+"'";
                                strSQL=strSQL+" UNION ";
                                strSQL=strSQL+"  SELECT B.SR_NO,A.STM_NO AS DOC_NO,A.STM_DATE AS DOC_DATE,'-' AS OPERATION,'3' AS TORDER,ROUND(B.QTY,3) AS QTY,B.NET_AMOUNT AS VALUE,B.ZERO_VAL_QTY FROM D_INV_STM_HEADER A,D_INV_STM_DETAIL B WHERE A.COMPANY_ID=B.COMPANY_ID AND A.STM_NO=B.STM_NO AND A.STM_TYPE=B.STM_TYPE AND A.APPROVED=1 AND A.CANCELLED=0 AND B.ITEM_ID='"+ItemID.trim()+"' AND A.STM_DATE>='"+EITLERPGLOBAL.formatDateDB(strFromDate)+"' AND STM_DATE<='"+EITLERPGLOBAL.formatDateDB(txtToDate.getText())+"'";
                                strSQL=strSQL+"  ORDER BY DOC_DATE,TORDER,DOC_NO";
                                ...
                                if(rsTmp.getRow()>0) {
                                    ...
                                    while(!rsTmp.isAfterLast()) {
                                        ...
                                        if(Operation.equals("-")) {
                                            ...
                                            data.Execute("UPDATE D_INV_ITEM_MASTER SET CLOSING_VALUE="+EITLERPGLOBAL.round(OpeningValue,3)+" WHERE ITEM_ID='"+ItemID.trim()+"'");
    ...
    private void GenerateReportRawSTM() {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                cumTotal=0;
                finalTotal=0;
                strFromDate = txtToDate.getText().trim();
                clsItemStock objItemStock=new clsItemStock();
                ...
                try {
                    ...
                    if(rsItemGroup.getRow()>0) {
                        while(!rsItemGroup.isAfterLast()) {
                            ...
                            writeLine(aFile,"Category Id: " + rsItemGroup.getString("CATEGORY_NAME"));
                            //========= Create a text file ==============//
                            strSQL="SELECT COUNT(*) THECOUNT FROM D_INV_ITEM_MASTER A WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND APPROVED=1 AND CANCELLED=0 "+strCondition+" AND CATEGORY_ID = '"+groupID+"' ";
                            ResultSet rsCount=data.getResult(strSQL);
                            rsCount.first();
                            ...
                            strSQL="SELECT ITEM_ID,ITEM_DESCRIPTION,UNIT,LOCATION_ID FROM D_INV_ITEM_MASTER A WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND APPROVED=1 AND CANCELLED=0 "+strCondition+" AND CATEGORY_ID = '"+groupID+"' GROUP BY A.ITEM_ID "+strOrder;
                            rsItem=data.getResult(strSQL);
                            rsItem.first();
                            ...
                            while((!rsItem.isAfterLast())&&rsItem.getRow()>0) {
                                ...
                                strSQL="SELECT IF(SUM(QTY) IS NULL,0,SUM(QTY)) AS SUM_QTY,IF(SUM(ZERO_VAL_QTY) IS NULL,0,SUM(ZERO_VAL_QTY)) AS SUM_ZERO_QTY, IF(SUM(NET_AMOUNT) IS NULL,0,SUM(NET_AMOUNT)) AS SUM_VALUE FROM D_INV_STM_HEADER A,D_INV_STM_DETAIL B WHERE A.COMPANY_ID=B.COMPANY_ID AND A.STM_NO=B.STM_NO AND A.STM_TYPE=B.STM_TYPE AND AND A.APPROVED=1 AND A.CANCELLED=0 AND A.STM_DATE>'"+StockEntryDate+"' AND STM_DATE<'"+EITLERPGLOBAL.formatDateDB(strFromDate)+"' AND B.ITEM_ID='"+ItemID+"'";
                                rsTmp=data.getResult(strSQL);
                                rsTmp.first();
