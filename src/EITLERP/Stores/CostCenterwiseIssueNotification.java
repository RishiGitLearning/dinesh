 ...
 * @author Dharmendra
 */
public class CostCenterwiseIssueNotification {
    ...
    public static void main(String[] args) {
        ...
        //if (EITLERPGLOBAL.getCurrentDay() == 25) {
            //String sql = "SELECT D.DEPT_ID,UPPER(D.DEPT_DESC) AS DEPT_NAME, B.COST_CENTER_ID,UPPER(COST_CENTER_NAME) AS CCNAME,ISSUE_CAT,SUM(QTY),AVG(RATE),SUM(B.ISSUE_VALUE) FROM D_INV_ISSUE_HEADER A,D_INV_ISSUE_DETAIL B,D_INV_ITEM_MASTER I ,D_COM_COST_CENTER C,D_COM_DEPT_MASTER D  WHERE \n"
            String sql = "SELECT D.DEPT_ID ,UPPER(D.DEPT_DESC) AS DEPT_NAME, B.COST_CENTER_ID ,UPPER(COST_CENTER_NAME) AS CC_NAME,ISSUE_CAT ,SUM(QTY) AS QTY,SUM(B.ISSUE_VALUE) AS VALUE FROM D_INV_ISSUE_HEADER A,D_INV_ISSUE_DETAIL B,D_INV_ITEM_MASTER I ,D_COM_COST_CENTER C,D_COM_DEPT_MASTER D  WHERE \n"
                    ...
                    + "AND B.ITEM_CODE NOT LIKE ('RM%') AND D.DEPT_ID=20\n"
                    ...
                    + " MONTH(A.ISSUE_DATE)=MONTH(CURDATE()- INTERVAL 1 MONTH) AND\n"
                    + " YEAR(A.ISSUE_DATE)=YEAR(CURDATE()- INTERVAL 1 MONTH) AND B.ITEM_CODE NOT LIKE ('RM%') AND D.DEPT_ID=20\n"
            ...
            String recievers = "amitkanti@dineshmills.com,yrpatel@dineshmills.com,rakeshdalal@dineshmills.com,brdfltweave1@dineshmills.com,raghavendra@dineshmills.com,hemantprajapati@dineshmills.com";
            ...
            String sql1 = "SELECT D.DEPT_ID,UPPER(D.DEPT_DESC) AS DEPT_NAME, B.COST_CENTER_ID,UPPER(COST_CENTER_NAME) AS CCNAME,ISSUE_CAT,UPPER(B.ITEM_CODE) AS ITEM,I.ITEM_DESCRIPTION,A.ISSUE_NO, A.ISSUE_DATE,QTY,RATE,B.ISSUE_VALUE\n"
                ...
                + "MONTH(A.ISSUE_DATE)=MONTH(CURDATE()- INTERVAL 1 MONTH) AND\n"
                + "YEAR(A.ISSUE_DATE)=YEAR(CURDATE()- INTERVAL 1 MONTH)\n"
                + "AND B.COST_CENTER_ID NOT LIKE '3%' AND B.ITEM_CODE NOT LIKE ('RM%') AND D.DEPT_ID+0=20\n"
                + "UNION ALL\n"
            ...
            //String sql2 = "SELECT D.DEPT_ID,UPPER(D.DEPT_DESC) AS DEPT_NAME, B.COST_CENTER_ID,UPPER(COST_CENTER_NAME) AS CCNAME,ISSUE_CAT,SUM(QTY),AVG(RATE),SUM(B.ISSUE_VALUE)\n"
            String sql2 = "SELECT D.DEPT_ID ,UPPER(D.DEPT_DESC) AS DEPT_NAME, B.COST_CENTER_ID ,UPPER(COST_CENTER_NAME) AS CC_NAME,ISSUE_CAT ,SUM(QTY) AS QTY,SUM(B.ISSUE_VALUE) AS VALUE \n"                    
                    + "FROM\n"
                    + "D_INV_ISSUE_HEADER A,\n"
                    + "D_INV_ISSUE_DETAIL B,\n"
                    + "D_INV_ITEM_MASTER I ,\n"
        ...
        //String recievers = "rishineekhra@dineshmills.com";
            ...
            exprt.fillData(sql3+"#"+sql2+"#"+sql1+"#"+sql, new File("/Email_Attachment/Cost_Centerwise_Issue_Weaving.xls"), "DetailUnapproved#SummaryUnApproved#DetailApproved#SummaryApproved");            
            String responce = sendNotificationMailWithAttachement("Cost Centerwise Issue Weaving", "Cost Centerwise Issue Weaving", recievers, pcc, "/Email_Attachment/Cost_Centerwise_Issue_Weaving.xls", "Cost_Centerwise_Issue_Weaving.xls");
            System.out.println("Mail Response:" + responce);
        //}
