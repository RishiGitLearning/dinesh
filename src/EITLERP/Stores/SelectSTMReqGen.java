...
public class SelectSTMReqGen extends javax.swing.JApplet {
    ...
    public SelectSTMReqGen(String pSQL, int pReturnCol, boolean pShowReturnCol, int pDefaultSearchOn) {
        initComponents();
    ...
    private void cmdShowActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdShowActionPerformed
        ...
        String docStatus=clsSTMReqGen.getDocStatus(CompanyId, SelSTMNo,dburl);
    ...
    private void txtSTMReqNoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtSTMReqNoKeyPressed
        ...
        if(evt.getKeyCode()==112) //F1 Key pressed
        {
            ...
            String test = EITLERPGLOBAL.FinFromDateDB;
            test = EITLERPGLOBAL.FinToDateDB;
            
            String StartDate = EITLERPGLOBAL.getFinYearStartDate(EITLERPGLOBAL.getCurrentDateDB());
            String EndDate = EITLERPGLOBAL.getFinYearEndDate(EITLERPGLOBAL.getCurrentDateDB());
            ...
            if(EITLERPGLOBAL.gCompanyID == 2) {
                Company1="DINESHMILLS";
                Company2="DINESHMILLS";
            }
            else {
                Company1="DINESHMILLSA";
                Company2="DINESHMILLSA";
            }
            ...
            if(chkDept.isSelected()) {
                ...
                "FROM "+Company1+".D_INV_STM_REQ_HEADER A, "+
                ""+Company1+".D_INV_STM_REQ_DETAIL B  "+
                "LEFT JOIN "+Company2+".D_INV_STM_DETAIL I ON (I.STM_REQ_NO=B.STM_REQ_NO AND I.STM_REQ_SR_NO=B.STM_REQ_SR_NO   "+
                "AND I.STM_NO IN (SELECT STM_NO FROM "+Company2+".D_INV_STM_HEADER WHERE STM_NO=I.STM_NO AND APPROVED=1 AND CANCELLED=0 )), "+
                ""+Company2+".D_COM_DEPT_MASTER D "+
                "WHERE A.STM_REQ_NO = B.STM_REQ_NO AND A.STM_REQ_TYPE = 1 "+
                "AND A.COMPANY_ID = B.COMPANY_ID "+
                "AND A.APPROVED =1 AND A.CANCELLED = 0 "+
                //"AND A.FOR_DEPT_ID=D.DEPT_ID  "+
                "GROUP BY B.STM_REQ_NO,B.STM_REQ_TYPE,B.STM_REQ_SR_NO "+
                "HAVING ROUND(IF(SUM(I.QTY) IS NULL,0,SUM(I.QTY)),3)<ROUND(B.STM_REQ_QTY,3) ";
                ...
                strSQL+="A.STM_REQ_DATE >='"+EITLERPGLOBAL.FinFromDateDB+"' AND A.STM_REQ_DATE <='"+EITLERPGLOBAL.FinToDateDB+"' AND ";
                strSQL+="A.STM_REQ_TYPE=B.STM_REQ_TYPE AND ";
                strSQL+="A.COMPANY_ID="+CompanyId+" AND ";
                strSQL+="A.APPROVED=1 AND A.CANCELLED=0 AND ";
                strSQL+="B.COMPANY_ID=D.COMPANY_ID AND A.FOR_DEPT_ID=D.DEPT_ID AND ";
                strSQL+="A.STM_REQ_TYPE="+ReqType+" AND ";
                ...
                strSQL+="A.APPROVED=1 AND A.CANCELLED=0 AND ";
                strSQL+="B.COMPANY_ID=D.COMPANY_ID AND A.FOR_DEPT_ID=D.DEPT_ID AND ";
                strSQL+="A.STM_REQ_TYPE=1 ";
                strSQL+="GROUP BY B.STM_REQ_NO,B.STM_REQ_TYPE,B.STM_REQ_SR_NO ";
                strSQL+="HAVING ROUND(IF(SUM(I.QTY) IS NULL,0,SUM(I.QTY)),3)<ROUND(B.STM_REQ_QTY,3) ";
                
                
                /*strSQL="SELECT DISTINCT(A.STM_REQ_NO),DATE_FORMAT(A.STM_REQ_DATE,'%d/%m/%Y') AS STM_REQ_DATE,B.STM_REQ_QTY "+
                "FROM D_INV_STM_REQ_HEADER A, D_INV_STM_REQ_DETAIL B,D_INV_STM_DETAIL  C, "+
                "D_INV_STM_HEADER  D "+
                "WHERE A.STM_REQ_NO = B.STM_REQ_NO AND A.STM_REQ_TYPE = 1 AND A.COMPANY_ID = B.COMPANY_ID AND A.APPROVED =1  "+
                "AND A.CANCELLED = 0 AND A.STM_REQ_NO = C.STM_REQ_NO AND C.STM_NO = D.STM_NO AND D.CANCELLED=0 "+
                "GROUP BY B.STM_REQ_NO,B.STM_REQ_TYPE,B.STM_REQ_SR_NO "+
                "HAVING ROUND(IF(SUM(C.QTY) IS NULL,0,SUM(C.QTY)),3)<ROUND(B.STM_REQ_QTY,3) ";
                 */
                /*strSQL="SELECT DISTINCT(A.STM_REQ_NO),DATE_FORMAT(A.STM_REQ_DATE,'%d/%m/%Y') AS STM_REQ_DATE,B.STM_REQ_QTY  "+
                "FROM "+Company1+".D_INV_STM_REQ_HEADER A, "+
                ""+Company1+".D_INV_STM_REQ_DETAIL B  "+
                "LEFT JOIN "+Company2+".D_INV_STM_DETAIL I ON (I.STM_REQ_NO=B.STM_REQ_NO AND I.STM_REQ_SR_NO=B.STM_REQ_SR_NO   "+
                "AND I.STM_NO IN (SELECT STM_NO FROM "+Company2+".D_INV_STM_HEADER WHERE STM_NO=I.STM_NO AND CANCELLED=0 )), "+
                ""+Company2+".D_COM_DEPT_MASTER D "+
                "WHERE A.STM_REQ_NO = B.STM_REQ_NO AND A.STM_REQ_TYPE = 1 "+
                "AND A.COMPANY_ID = B.COMPANY_ID "+
                "AND A.APPROVED =1 AND A.CANCELLED = 0 "+
            ...
            //jdbc:mysql://200.0.0.227:3306/DINESHMILLS"?user=root
            if(aList.ShowLOV()) {
    ...
    private void GenerateGrid() {
        ...
        if(!STMNo.equals("")) {
            List=clsSTMReqGen.getSTMReqItemList(CompanyId, STMNo,false,ReqType,dburl);
            ...
            for(int i=1;i<=List.size();i++) {
                ...
                rowData[4]=Double.toString(ObjItem.getAttribute("STM_REQ_QTY").getDouble());
                ...
                rowData[7]=Double.toString(ObjItem.getAttribute("BAL_QTY").getDouble());
                rowData[8]=(String) ObjItem.getAttribute("STM_REQ_NO").getObj();
                rowData[9]=Double.toString(ObjItem.getAttribute("ISSUED_QTY").getDouble());
    ...
    private void SetList() {
        ...
        //========= Get the MR Object =============//
        clsSTMReqGen tmpObj=new clsSTMReqGen();
        ObjSTMReq=(clsSTMReqGen)tmpObj.getObject(EITLERPGLOBAL.gCompanyID, STMNo);
