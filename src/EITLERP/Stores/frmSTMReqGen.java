...
public class frmSTMReqGen extends javax.swing.JApplet {
    
    private int EditMode=0;
    private clsSTMReqGen ObjSTMReqGen; //old clsSTMRequisitionRaw
    ...
    public void init() {
        ...
        if(getName().equals("Link")) {
            ...
            if(ObjSTMReqGen.LoadData(EITLERPGLOBAL.gCompanyID)) {
                ObjSTMReqGen.MoveLast();
                DisplayData();
            }
            else {
                JOptionPane.showMessageDialog(this,"Error occured while loading data. Error is "+ObjSTMReqGen.LastError);
            }
        }
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        lblTitle.setText("STM REQUISITION (GEN)");
    ...
    private void cmdShortCloseActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdShortCloseActionPerformed
        ...
        if(EditMode==0) {
            ...
            if(ObjSTMReqGen.getAttribute("APPROVED").getInt()==0) {
    ...
    private void cmdNormalViewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdNormalViewActionPerformed
        // TODO add your handling code here:
        ObjSTMReqGen.LoadData(EITLERPGLOBAL.gCompanyID);
        MoveLast();
    ...
    private void cmdViewHistoryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdViewHistoryActionPerformed
        ...
        ObjSTMReqGen.ShowHistory(EITLERPGLOBAL.gCompanyID, DocNo);
        MoveLast();
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if(EditMode==EITLERPGLOBAL.EDIT) {
            GenerateRejectedUserCombo();
            if(ApprovalFlow.IsOnceRejectedDoc(EITLERPGLOBAL.gCompanyID,clsSTMReqGen.ModuleID,txtDocNo.getText())) {
                cmbSendTo.setEnabled(true);
            }
            else {
                cmbSendTo.setEnabled(false);
            ...
            if(EditMode!=0) //Only Process keys when in editing mode
            {
                if(evt.getKeyCode()==112) //F1 Key pressed
                {
                    if(Table.getSelectedColumn()==DataModel.getColFromVariable("ITEM_ID")) {
                        ...
                        //showStatus("Press F1 for Enter Item ID.");
                        
                        LOV aList=new LOV();
                        
                        aList.SQL="SELECT ITEM_ID,ITEM_DESCRIPTION FROM D_INV_ITEM_MASTER WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND APPROVED=1 AND CANCELLED=0 AND CATEGORY_ID = 1 ORDER BY ITEM_ID";
                        aList.ReturnCol=1;
    ...
    private void DisplayData() {
        ...
        try {
            if(EditMode==0) {
                ...
                if(ObjSTMReqGen.getAttribute("APPROVED").getInt()==1) {
                ...
                if(ObjSTMReqGen.getAttribute("CANCELLED").getInt()==1) {
        ...
        if(EITLERPGLOBAL.gAuthorityUserID!=EITLERPGLOBAL.gUserID) {
            int ModuleID=clsSTMReqGen.ModuleID;
        ...
        ClearFields();
        txtDocNo.setText(ObjSTMReqGen.getAttribute("STM_REQ_NO").getString());
        lblTitle.setText("STM REQUISITION (GENERAL) - "+txtDocNo.getText());
        lblRevNo.setText(Integer.toString(ObjSTMReqGen.getAttribute("REVISION_NO").getInt()));
        txtDocDate.setText(EITLERPGLOBAL.formatDate(ObjSTMReqGen.getAttribute("STM_REQ_DATE").getString()));
        EITLERPGLOBAL.setComboIndex(cmbTransferTo,ObjSTMReqGen.getAttribute("TRANSFER_TO_UNIT").getInt());
        EITLERPGLOBAL.setComboIndex(cmbDept,ObjSTMReqGen.getAttribute("FOR_DEPT_ID").getInt());
        
        txtPurpose.setText(ObjSTMReqGen.getAttribute("PURPOSE").getString());
        txtRemarks.setText(ObjSTMReqGen.getAttribute("REMARKS").getString());
        
        EITLERPGLOBAL.setComboIndex(cmbHierarchy,ObjSTMReqGen.getAttribute("HIERARCHY_ID").getInt());
        
        DoNotEvaluate=true;
        
        FormatGrid();
        //Now Generate Table
        for(int i=1;i<=ObjSTMReqGen.colSTMReqRawItems.size();i++) {
            clsSTMReqItem ObjItem=(clsSTMReqItem)ObjSTMReqGen.colSTMReqRawItems.get(Integer.toString(i));
        ...
        String DocNo=ObjSTMReqGen.getAttribute("STM_REQ_NO").getString();
        List=ApprovalFlow.getDocumentFlow(EITLERPGLOBAL.gCompanyID, clsSTMReqGen.ModuleID , DocNo);
        ...
        FormatGridHS();
        HashMap History=clsSTMReqGen.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
        for(int i=1;i<=History.size();i++) {
            clsSTMReqGen ObjHistory=(clsSTMReqGen)History.get(Integer.toString(i));
    ...
    private void SetData() {
        ...
        ObjSTMReqGen.setAttribute("PREFIX",SelPrefix);
        ObjSTMReqGen.setAttribute("SUFFIX",SelSuffix);
        ObjSTMReqGen.setAttribute("FFNO",FFNo);
        ObjSTMReqGen.setAttribute("COMPANY_ID",EITLERPGLOBAL.gCompanyID);
        ObjSTMReqGen.setAttribute("STM_REQ_TYPE",1); // 1- GEN 2- RAW
        ObjSTMReqGen.setAttribute("STM_REQ_DATE",EITLERPGLOBAL.formatDateDB(txtDocDate.getText()));
        ObjSTMReqGen.setAttribute("FOR_DEPT_ID",EITLERPGLOBAL.getComboCode(cmbDept));
        ObjSTMReqGen.setAttribute("TRANSFER_TO_UNIT",EITLERPGLOBAL.getComboCode(cmbTransferTo));
        ObjSTMReqGen.setAttribute("PURPOSE",txtPurpose.getText());
        ObjSTMReqGen.setAttribute("REMARKS",txtRemarks.getText());
        
        //----- Update Approval Specific Fields -----------//
        ObjSTMReqGen.setAttribute("HIERARCHY_ID",EITLERPGLOBAL.getComboCode(cmbHierarchy));
        ObjSTMReqGen.setAttribute("FROM",EITLERPGLOBAL.gNewUserID);
        ObjSTMReqGen.setAttribute("TO",EITLERPGLOBAL.getComboCode(cmbSendTo));
        ObjSTMReqGen.setAttribute("FROM_REMARKS",txtToRemarks.getText());
        
        if(OpgApprove.isSelected()) {
            ObjSTMReqGen.setAttribute("APPROVAL_STATUS","A");
        }
        
        if(OpgFinal.isSelected()) {
            ObjSTMReqGen.setAttribute("APPROVAL_STATUS","F");
        }
        
        if(OpgReject.isSelected()) {
            ObjSTMReqGen.setAttribute("APPROVAL_STATUS","R");
            ObjSTMReqGen.setAttribute("SEND_DOC_TO",EITLERPGLOBAL.getComboCode(cmbSendTo));
        }
        
        if(OpgHold.isSelected()) {
            ObjSTMReqGen.setAttribute("APPROVAL_STATUS","H");
        }
        //-------------------------------------------------//
        
        if(EditMode==EITLERPGLOBAL.ADD) {
            ObjSTMReqGen.setAttribute("CREATED_BY",EITLERPGLOBAL.gNewUserID);
            ObjSTMReqGen.setAttribute("CREATED_DATE",EITLERPGLOBAL.getCurrentDateDB());
        }
        else {
            ObjSTMReqGen.setAttribute("MODIFIED_BY",EITLERPGLOBAL.gNewUserID);
            ObjSTMReqGen.setAttribute("MODIFIED_DATE",EITLERPGLOBAL.getCurrentDateDB());
        }
        
        
        //======= Set Line part ============
        ObjSTMReqGen.colSTMReqRawItems.clear();
        ...
        for(int i=0;i<Table.getRowCount();i++) {
            ...
            if(clsItem.IsValidItemID(EITLERPGLOBAL.gCompanyID, lItemID)) {
                ...
                ObjSTMReqGen.colSTMReqRawItems.put(Integer.toString(ObjSTMReqGen.colSTMReqRawItems.size()+1), ObjItem);
            }
        }
    }
    ...
    private void GenerateCombos() {
        ...
        List=clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID= "+clsSTMReqGen.ModuleID);
        
        if(EditMode==EITLERPGLOBAL.EDIT) {
            List=clsHierarchy.getList(" WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID="+clsSTMReqGen.ModuleID);
        }
        for(int i=1;i<=List.size();i++) {
    ...
    private void GenerateFromCombo() {
        ...
        try {
            if(EditMode==EITLERPGLOBAL.ADD) {
                ...
                List=ApprovalFlow.getRemainingUsers((int)EITLERPGLOBAL.gCompanyID, clsSTMReqGen.ModuleID ,(String)ObjSTMReqGen.getAttribute("STM_REQ_NO").getObj());
                for(int i=1;i<=List.size();i++) {
                    clsUser ObjUser=(clsUser) List.get(Integer.toString(i));
                    ComboData aData=new ComboData();
                    aData.Code=(int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void SetupApproval() {
        ...
        if(EditMode==EITLERPGLOBAL.ADD) {
            ...
            int FromUserID=ApprovalFlow.getFromID((int)EITLERPGLOBAL.gCompanyID, clsSTMReqGen.ModuleID ,(String)ObjSTMReqGen.getAttribute("STM_REQ_NO").getObj());
            lnFromID=FromUserID;
            String strFromUser=clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks=ApprovalFlow.getFromRemarks( (int)EITLERPGLOBAL.gCompanyID,clsSTMReqGen.ModuleID ,FromUserID,(String)ObjSTMReqGen.getAttribute("STM_REQ_NO").getObj());
    ...
    private void Add() {
        ...
        aList.ModuleID=clsSTMReqGen.ModuleID ;
        ...
        if(aList.ShowList()) {
            ...
            txtDocNo.setText(clsFirstFree.getNextFreeNo(EITLERPGLOBAL.gCompanyID, clsSTMReqGen.ModuleID , FFNo,  false));
            txtDocDate.requestFocus();
            
            lblTitle.setText("STM REQUISITION (GEN) - "+txtDocNo.getText());
            lblTitle.setBackground(Color.BLUE);
        }
        else {
    ...
    private void Edit() {
        ...
        String lDocNo=(String)ObjSTMReqGen.getAttribute("STM_REQ_NO").getObj();
        if(ObjSTMReqGen.IsEditable(EITLERPGLOBAL.gCompanyID, lDocNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if(ApprovalFlow.IsCreator(clsSTMReqGen.ModuleID ,lDocNo)||clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,11802)) {
                SetFields(true);
            }
            else {
                EnableApproval();
            ...
            /*if(!ApprovalFlow.IsCreator(clsSTMReqGen.ModuleID ,lDocNo)) {
                DataModel.SetReadOnly(1);
                DataModel.SetReadOnly(4);
            }*/
    ...
    private void Delete() {
        ...
        if(ObjSTMReqGen.CanDelete(EITLERPGLOBAL.gCompanyID, lDocNo, EITLERPGLOBAL.gNewUserID)) {
            if(ObjSTMReqGen.Delete(EITLERPGLOBAL.gNewUserID)) {
                MoveLast();
            }
            else {
                JOptionPane.showMessageDialog(this,"Error occured while deleting. Error is "+ObjSTMReqGen.LastError);
            }
        }
        else {
    ...
    private void Save() {
        ...
        if(EditMode==EITLERPGLOBAL.ADD) {
            if(ObjSTMReqGen.Insert()) {
                MoveLast();
                DisplayData();
            }
            else {
                JOptionPane.showMessageDialog(this,"Error occured while saving. Error is "+ObjSTMReqGen.LastError);
                return;
            }
        }
        
        if(EditMode==EITLERPGLOBAL.EDIT) {
            if(ObjSTMReqGen.Update()) {
                DisplayData();
            }
            else {
                JOptionPane.showMessageDialog(this,"Error occured while saving. Error is "+ObjSTMReqGen.LastError);
    ...
    private void Find() {
        ...
        if(ObjReturn.Cancelled==false) {
            if(!ObjSTMReqGen.Filter(ObjReturn.strQuery,EITLERPGLOBAL.gCompanyID)) {
    ...
    private void MoveFirst() {
        ObjSTMReqGen.MoveFirst();
        DisplayData();
    }
    
    private void MovePrevious() {
        ObjSTMReqGen.MovePrevious();
        DisplayData();
    }
    
    private void MoveNext() {
        ObjSTMReqGen.MoveNext();
        DisplayData();
    }
    ...
    public void FindEx(int pCompanyID,String pDocNo) {
        ObjSTMReqGen.Filter(" WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND STM_REQ_NO='"+pDocNo+"'",pCompanyID);
        ObjSTMReqGen.MoveLast();
        DisplayData();
    }
    
    public void FindWaiting() {
        ObjSTMReqGen.Filter(" WHERE STM_REQ_NO IN (SELECT D_INV_STM_REQ_HEADER.STM_REQ_NO FROM D_INV_STM_REQ_HEADER,D_COM_DOC_DATA WHERE D_INV_STM_REQ_HEADER.STM_REQ_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_STM_REQ_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_STM_REQ_HEADER.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND D_COM_DOC_DATA.USER_ID="+EITLERPGLOBAL.gNewUserID+" AND D_COM_DOC_DATA.STATUS='W' AND MODULE_ID="+clsSTMReqGen.ModuleID+")",EITLERPGLOBAL.gCompanyID);
        ObjSTMReqGen.MoveLast();
        DisplayData();
    }
    ...
    private void GenerateRejectedUserCombo() {
        ...
        for(int i=1;i<=List.size();i++) {
            ...
            if(EditMode==EITLERPGLOBAL.EDIT) {
                if(OpgApprove.isSelected()) {
                    IncludeUser=ApprovalFlow.IncludeUserInApproval(EITLERPGLOBAL.gCompanyID,clsSTMReqGen.ModuleID, txtDocNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
                
                if(OpgReject.isSelected()) {
                    IncludeUser=ApprovalFlow.IncludeUserInRejection(EITLERPGLOBAL.gCompanyID,clsSTMReqGen.ModuleID , txtDocNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if(EditMode==EITLERPGLOBAL.EDIT) {
            int Creator=ApprovalFlow.getCreator(EITLERPGLOBAL.gCompanyID,clsSTMReqGen.ModuleID,txtDocNo.getText());
    ...
    private void CopySTMReq() {
        ...
        aListLOV.SQL="SELECT STM_REQ_NO,STM_REQ_DATE FROM D_INV_STM_REQ_HEADER WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND STM_REQ_TYPE = '" + clsSTMReqGen.STMReqType + "' AND APPROVED=1 ORDER BY STM_REQ_NO";
        ...
        if(aListLOV.ShowLOV()) {
            ...
            clsSTMReqGen ObjSTMRequisitionRaw=(clsSTMReqGen)clsSTMReqGen.getObjectEx(EITLERPGLOBAL.gCompanyID, ReqNo);
    ...
    private void DisplaySTMReqData(clsSTMReqGen ObjSTMRequisitionGen) {
        ClearFields();
        txtDocNo.setText((String)ObjSTMRequisitionGen.getAttribute("STM_REQ_NO").getObj());
        txtDocDate.setText(EITLERPGLOBAL.formatDate((String)ObjSTMRequisitionGen.getAttribute("STM_REQ_DATE").getObj()));
        EITLERPGLOBAL.setComboIndex(cmbTransferTo,(int)ObjSTMRequisitionGen.getAttribute("TRANSFER_TO_UNIT").getVal());
        EITLERPGLOBAL.setComboIndex(cmbDept,(int)ObjSTMRequisitionGen.getAttribute("FOR_DEPT_ID").getVal());
        
        txtPurpose.setText((String)ObjSTMRequisitionGen.getAttribute("PURPOSE").getObj());
        txtRemarks.setText((String)ObjSTMRequisitionGen.getAttribute("REMARKS").getObj());
        
        DoNotEvaluate=true;
        
        FormatGrid();
        //Now Generate Table
        for(int i=1;i<=ObjSTMRequisitionGen.colSTMReqRawItems.size();i++) {
            clsSTMReqItem ObjItem=(clsSTMReqItem)ObjSTMRequisitionGen.colSTMReqRawItems.get(Integer.toString(i));
            Object[] rowData=new Object[11];
    ...
    private boolean Validate() {
        ...
        //--------------------- check remarks validation----------------//
        for(int i=0;i<=Table.getRowCount()-1;i++) {
            String strRemarks=Table.getValueAt(i,8).toString();
            if(strRemarks.trim().equals("")) {
                JOptionPane.showMessageDialog(this,"Please Enter Remarks At Sr No:"+i+1);
                return false;
            }
        }
        //---------------------END check remarks validation----------------//
        //Check negative Qty.
        for(int i=0;i<=Table.getRowCount()-1;i++) {
            ...
            double ExQty=data.getDoubleValueFromDB("SELECT SUM(QTY) AS SUMQTY FROM D_INV_STM_HEADER A,D_INV_STM_DETAIL B WHERE A.STM_NO=B.STM_NO AND A.STM_TYPE=B.STM_TYPE AND A.CANCELLED=0 AND STM_REQ_NO='"+txtDocNo.getText().trim()+"' AND STM_REQ_SR_NO="+SrNo,dburl);
