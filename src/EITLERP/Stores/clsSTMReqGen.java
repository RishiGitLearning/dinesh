...
public class clsSTMReqGen {
    ...
    public static String STMReqType = "1"; //1-GEN   2- RAW
    //History Related properties
    private boolean HistoryView=false;
    
    public static int ModuleID=186;
    ...
    public clsSTMReqGen() {
        ...
        props.put("STM_REQ_TYPE",new Variant(1)); //1 - gen 2 - - raw
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_INV_STM_REQ_HEADER WHERE COMPANY_ID="+pCompanyID+" AND STM_REQ_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND STM_REQ_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' AND STM_REQ_TYPE=" + STMReqType + " ORDER BY STM_REQ_NO");
            HistoryView=false;
            Ready=true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            */
            ...
            setAttribute("STM_REQ_NO",clsFirstFree.getNextFreeNo((int)EITLERPGLOBAL.gCompanyID,clsSTMReqGen.ModuleID, (int)getAttribute("FFNO").getVal(),true));
            //-------------------------------------------------
...
//                rsHDetail.updateString("MODIFIED_DATE",getAttribute("MODIFIED_DATE").getString());
            ...
            ObjFlow.ModuleID=clsSTMReqGen.ModuleID;
...
//            rsHistory.updateString("CREATED_DATE",getAttribute("CREATED_DATE").getString());
            ...
            data.Execute("DELETE FROM D_INV_STM_REQ_DETAIL WHERE COMPANY_ID="+mCompanyID+" AND STM_REQ_NO='"+mReqNo+"' AND STM_REQ_TYPE=" + STMReqType + " ");
...
//                rsHDetail.updateString("CREATED_DATE",getAttribute("CREATED_DATE").getString());
            ...
            if(IsRejected) {
                //Remove the Rejected Flag First
                data.Execute("UPDATE D_INV_STM_REQ_HEADER SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND STM_REQ_NO='"+ObjFlow.DocNo+"' AND STM_REQ_TYPE=" + STMReqType + " ");
                //Remove Old Records from D_COM_DOC_DATA
                data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID="+clsSTMReqGen.ModuleID+" AND DOC_NO='"+ObjFlow.DocNo+"'");
    ...
    public boolean CanDelete(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM D_INV_STM_REQ_HEADER WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND STM_REQ_NO='"+pDocNo+"' AND STM_REQ_TYPE=" + STMReqType + " AND (APPROVED=1)";
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM D_COM_DOC_DATA WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND MODULE_ID="+clsSTMReqGen.ModuleID+" AND DOC_NO='"+pDocNo+"' AND USER_ID="+Integer.toString(pUserID)+" AND TYPE='C' AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean IsEditable(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM D_COM_DOC_DATA WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND MODULE_ID="+clsSTMReqGen.ModuleID+" AND DOC_NO='"+pDocNo+"' AND USER_ID="+Integer.toString(pUserID)+" AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean Delete(int pUserID) {
        try {
            ...
            if(CanDelete(lCompanyID,lDocNo,pUserID)) {
                String strQry = "DELETE FROM D_INV_STM_REQ_HEADER WHERE COMPANY_ID=" + lCompanyID +" AND STM_REQ_NO='"+lDocNo+"' AND STM_REQ_TYPE=" + STMReqType + " ";
                data.Execute(strQry);
                strQry = "DELETE FROM D_INV_STM_REQ_DETAIL WHERE COMPANY_ID=" + lCompanyID +" AND STM_REQ_NO='"+lDocNo+"' AND STM_REQ_TYPE=" + STMReqType + " ";
                data.Execute(strQry);
    ...
    public Object getObject(int pCompanyID, String pReqNo) {
        ...
        clsSTMReqGen ObjSTMRequisition = new clsSTMReqGen();
    ...
    public static HashMap getList(String pCondition) {
        ...
        try {
            ...
            while(rsTmp.next()) {
                ...
                rsTmp2=tmpStmt2.executeQuery("SELECT * FROM D_INV_STM_REQ_DETAIL WHERE COMPANY_ID="+mCompanyID+" AND STM_REQ_NO='" + mReqNo + "' AND STM_REQ_TYPE=" + STMReqType + " " );
    ...
    public boolean Filter(String pCondition,int pCompanyID) {
        Ready=false;
        try {
            pCondition += "AND STM_REQ_TYPE= " + STMReqType + " ";
            ...
            if(!rsResultSet.first()) {
                strSql = "SELECT * FROM D_INV_STM_REQ_HEADER WHERE COMPANY_ID="+Long.toString(pCompanyID)+" AND STM_REQ_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND STM_REQ_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' AND STM_REQ_TYPE=" + STMReqType + " ORDER BY STM_REQ_NO";
                rsResultSet=Stmt.executeQuery(strSql);
                Ready=true;
                MoveLast();
                return false;
    ...
    public boolean setData() {
        ...
        try {
            ...
            tmpStmt=tmpConn.createStatement();
            if(HistoryView) {
                rsTmp=tmpStmt.executeQuery("SELECT * FROM D_INV_STM_REQ_DETAIL_H WHERE COMPANY_ID=" + mCompanyID + " AND STM_REQ_NO='" + mReqNo + "' AND STM_REQ_TYPE=" + STMReqType + " AND REVISION_NO="+RevNo+" ORDER BY STM_REQ_SR_NO");
            }
            else {
                rsTmp=tmpStmt.executeQuery("SELECT * FROM D_INV_STM_REQ_DETAIL WHERE COMPANY_ID=" + mCompanyID + " AND STM_REQ_NO='" + mReqNo + "' AND STM_REQ_TYPE=" + STMReqType + " ORDER BY STM_REQ_SR_NO");
            }
            rsTmp.first();
    ...
    public static HashMap getPendingApprovals(int pCompanyID,int pUserID,int pOrder,int FinYearFrom) {
        ...
        try {
            ...
            if(pOrder==EITLERPGLOBAL.OnRecivedDate) {
                strSQL="SELECT D_INV_STM_REQ_HEADER.STM_REQ_NO,D_INV_STM_REQ_HEADER.STM_REQ_DATE,RECEIVED_DATE,D_INV_STM_REQ_HEADER.FOR_DEPT_ID AS DEPT_ID FROM D_INV_STM_REQ_HEADER,D_COM_DOC_DATA WHERE D_INV_STM_REQ_HEADER.STM_REQ_TYPE=" + STMReqType + " AND D_INV_STM_REQ_HEADER.STM_REQ_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_STM_REQ_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_STM_REQ_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_INV_STM_REQ_HEADER.STM_REQ_TYPE=" + STMReqType + " AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND MODULE_ID="+clsSTMReqGen.ModuleID+" ORDER BY D_COM_DOC_DATA.RECEIVED_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocDate) {
                
                System.out.println (STMReqType) ;
                System.out.println (Integer.toString(pCompanyID)) ;
                System.out.println (Integer.toString(pUserID));
                System.out.println (clsSTMReqGen.ModuleID);
               
                
                strSQL="SELECT D_INV_STM_REQ_HEADER.STM_REQ_NO,D_INV_STM_REQ_HEADER.STM_REQ_DATE,RECEIVED_DATE,D_INV_STM_REQ_HEADER.FOR_DEPT_ID AS DEPT_ID FROM D_INV_STM_REQ_HEADER,D_COM_DOC_DATA WHERE D_INV_STM_REQ_HEADER.STM_REQ_TYPE=" + STMReqType + " AND D_INV_STM_REQ_HEADER.STM_REQ_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_STM_REQ_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_STM_REQ_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_INV_STM_REQ_HEADER.STM_REQ_TYPE=" + STMReqType + " AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND MODULE_ID="+clsSTMReqGen.ModuleID+" ORDER BY D_INV_STM_REQ_HEADER.STM_REQ_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocNo) {
                strSQL="SELECT D_INV_STM_REQ_HEADER.STM_REQ_NO,D_INV_STM_REQ_HEADER.STM_REQ_DATE,RECEIVED_DATE,D_INV_STM_REQ_HEADER.FOR_DEPT_ID AS DEPT_ID FROM D_INV_STM_REQ_HEADER,D_COM_DOC_DATA WHERE D_INV_STM_REQ_HEADER.STM_REQ_TYPE=" + STMReqType + " AND D_INV_STM_REQ_HEADER.STM_REQ_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_STM_REQ_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_STM_REQ_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_INV_STM_REQ_HEADER.STM_REQ_TYPE=" + STMReqType + " AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND MODULE_ID="+clsSTMReqGen.ModuleID+" ORDER BY D_INV_STM_REQ_HEADER.STM_REQ_NO";
            }
    ...
    public static HashMap getSTMReqItemList(int pCompanyID,String pSTMNo,boolean pAllData,int pType) {
        ...
        try {
            ...
            if(pAllData) {
                strSQL = "SELECT * FROM D_INV_STM_REQ_DETAIL WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND STM_REQ_NO='"+pSTMNo.trim()+"' AND STM_REQ_TYPE=" + STMReqType + " AND STM_REQ_NO IN (SELECT STM_REQ_NO FROM D_INV_STM_REQ_HEADER WHERE STM_REQ_NO='"+pSTMNo+"' AND APPROVED=1) ORDER BY ITEM_ID";
            }
            else{
    ...
    public static HashMap getSTMReqItemList(int pCompanyID,String pSTMNo,boolean pAllData,int pType,String pdburl) {
        ResultSet rsTmp=null;
        Connection tmpConn=null;
        Statement tmpStmt=null;
    ...
    public boolean ShowHistory(int pCompanyID,String pDocNo) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_INV_STM_REQ_HEADER_H WHERE COMPANY_ID="+pCompanyID+" AND STM_REQ_NO='"+pDocNo+"' AND STM_REQ_TYPE=" + STMReqType + "");
            Ready=true;
            HistoryView=true;
            MoveFirst();
            return true;
    ...
    public static HashMap getHistoryList(int pCompanyID,String pDocNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT * FROM D_INV_STM_REQ_HEADER_H WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND STM_REQ_NO='"+pDocNo+"' AND STM_REQ_TYPE=" + STMReqType + "");
            
            while(rsTmp.next()) {
                clsSTMReqGen ObjMR=new clsSTMReqGen();
    ...
    public static String getDocStatus(int pCompanyID,String pDocNo,String pdburl) {
        ResultSet rsTmp;
        String strMessage="";
        
        try {
            //First check that Document exist
            rsTmp=data.getResult("SELECT STM_REQ_NO,APPROVED,CANCELLED FROM D_INV_STM_REQ_HEADER WHERE COMPANY_ID="+pCompanyID+" AND STM_REQ_NO='"+pDocNo+"'",pdburl);
            rsTmp.first();
    ...
    public static boolean CanCancel(int pCompanyID,String pDocNo) {
        ...
        try {
            rsTmp=data.getResult("SELECT STM_REQ_NO FROM D_INV_STM_REQ_HEADER WHERE COMPANY_ID="+pCompanyID+" AND STM_REQ_NO='"+pDocNo+"' AND CANCELLED=0 AND STM_REQ_TYPE=" + STMReqType + " ");
            rsTmp.first();
    ...
    public static boolean CancelDoc(int pCompanyID,String pDocNo) {
        ...
        try {
            if(CanCancel(pCompanyID,pDocNo)) {
                ...
                rsTmp=data.getResult("SELECT APPROVED FROM D_INV_STM_REQ_HEADER WHERE COMPANY_ID="+pCompanyID+" AND STM_REQ_NO='"+pDocNo+"' AND STM_REQ_TYPE=" + STMReqType + " ");
                rsTmp.first();
                ...
                if(ApprovedDoc) {
                    ...
                    data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID="+pCompanyID+" AND DOC_NO='"+pDocNo+"' AND MODULE_ID="+(clsSTMReqGen.ModuleID));
                }
                
                //Now Update the header with cancel falg to true
                data.Execute("UPDATE D_INV_STM_REQ_HEADER SET CANCELLED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+pCompanyID+" AND STM_REQ_NO='"+pDocNo+"' AND STM_REQ_TYPE=" + STMReqType + " ");
