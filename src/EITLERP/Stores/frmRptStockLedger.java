                                ...
                                while(!rsTmp.isAfterLast()) {
                                    ...
                                    if(Operation.equals("-")) {
                                        ...
                                        if(OpeningValue<0) {
                                            ...
                                            if(OpeningQty>0) {
                                                OpeningRate=EITLERPGLOBAL.round(OpeningValue/OpeningQty,3);
                                                OpeningValue=EITLERPGLOBAL.round(EITLERPGLOBAL.round(OpeningValue,3)- EITLERPGLOBAL.round(rsTmp.getDouble("VALUE"),3),3);
            ...
            public void run() {
               ...
               String strDocDate="";*/
                ...
                try {
                    ...
                    if(ItemType==2) {
                        strCondition=" AND A.CATEGORY_ID IN (2,10,11,12,14) ";
                    }
                    //else {
                    //                        strCondition=" AND A.ITEM_ID LIKE 'RM%' ";
                    //                    }
                    ...
                    while((!rsItem.isAfterLast())&&rsItem.getRow()>0) {
                        ...
                        lblItem.repaint();
                        if(ItemID.trim().equals("RM12125003")) {
                            boolean t = true;
                        ...
                        while((!rsLot.isAfterLast())&&rsLot.getRow()>0) {
                            ...
                            LotQty = EITLERPGLOBAL.round(rsLot.getDouble("QTY"),3);
                            LotRate=rsLot.getDouble("RATE");
                            LotValue=rsLot.getDouble("VALUE");
                            
                            strSQL = "SELECT SUM(TOTAL_ISSUE.QTY) AS QTY FROM ( " +
                            "SELECT  C.ISSUED_LOT_QTY AS QTY,A.ISSUE_NO,A.ISSUE_DATE,B.SR_NO "+
                            ...
                            "UNION "+
                            "SELECT C.ISSUED_LOT_QTY AS QTY,A.STM_NO,A.STM_DATE,B.SR_NO "+
                            "FROM D_INV_STM_HEADER A,D_INV_STM_DETAIL B,D_INV_STM_LOT C "+
                            "WHERE A.COMPANY_ID = "+EITLERPGLOBAL.gCompanyID+" AND A.STM_NO = B.STM_NO AND A.STM_NO = C.STM_NO AND A.STM_TYPE=B.STM_TYPE "+
                            ...
                            double Total_Qty = data.getDoubleValueFromDB(strSQL);
                            
                            if(LotNo.equals("100142")) {
                                boolean t = true;
                            }
                            ...
                            else if(LotQty>Total_Qty) {
                                LotQty-=EITLERPGLOBAL.round(Total_Qty,3);
                            }
                            
                            writeLine(aFile,"Item ID:" + ItemID +" Lot No:"+LotNo);
                            
                            
                            OpeningQty=EITLERPGLOBAL.round(LotQty,3);
                            OpeningValue=EITLERPGLOBAL.round(LotQty * LotRate,3);
                            OpeningRate=LotRate;
                            
                            String d1 = EITLERPGLOBAL.formatDate(rsLot.getString("GRN_DATE"));
                            String d2 = txtFromDate.getText().trim();
                            //if(rsLot.getString("TYPE").equals("R") && EITLERPGLOBAL.formatDate(rsLot.getString("GRN_DATE")).equals(txtFromDate.getText().trim())) {
                            //if(rsLot.getString("TYPE").equals("R") && d1.equals(d2)) {
                            //    OpeningQty=0.000;
                            //    OpeningValue=0.000;
                            //    OpeningRate=0.000;
                            //}
                            if(rsLot.getString("TYPE").equals("R") && (EITLERPGLOBAL.compareDate(EITLERPGLOBAL.formatDate(rsLot.getString("GRN_DATE")),txtFromDate.getText().trim()) == 1 || EITLERPGLOBAL.compareDate(EITLERPGLOBAL.formatDate(rsLot.getString("GRN_DATE")),txtFromDate.getText().trim()) == 0 ) ) {
                                OpeningQty=0.000;
                                OpeningValue=0.000;
                                OpeningRate=0.000;
                            }
                            ...
                            if(rsLot.getString("TYPE").equals("O")) {
                                ...
                                if(EITLERPGLOBAL.compareDate(EITLERPGLOBAL.formatDate(rsLot.getString("GRN_DATE")),txtFromDate.getText().trim()) == -1) {
                                    //if grn date < from date
                                }
                                else {
                                    ...
                                    nRecValue= EITLERPGLOBAL.round(nRecValue + (ClosingQty * IssueRate),3);//rsLot.getDouble("RATE");
                                }
                    ...
                    writeLine(aFile, "Total Closing Qty: " + dFormat.format((nOpnQty+nRecQty)-(nIssueQty+nSTMQty)));
                    writeLine(aFile, "Total Closing Value: " + dFormat.format((nOpnValue+nRecValue)-(nIssueValue+nSTMValue)));
            ...
            public void run() {
               ...
               String strDocDate="";*/
                ...
                try {
                    ...
                    if(ItemType==3) {
                        strCondition=" AND A.CATEGORY_ID IN (13) ";
                    }
                    //else {
                    //                        strCondition=" AND A.ITEM_ID LIKE 'RM%' ";
                    //                    }
                    ...
                    while((!rsItem.isAfterLast())&&rsItem.getRow()>0) {
                        ...
                        while((!rsLot.isAfterLast())&&rsLot.getRow()>0) {
                            ...
                            /*strSQL = "SELECT SUM(TOTAL_ISSUE.QTY) AS QTY FROM ( " +
                            "SELECT  SUM(C.ISSUED_LOT_QTY) AS QTY "+
                            "FROM D_INV_ISSUE_HEADER A,D_INV_ISSUE_DETAIL B,D_INV_ISSUE_LOT C "+
                            "WHERE  A.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND A.ISSUE_NO = B.ISSUE_NO AND A.ISSUE_NO = C.ISSUE_NO "+
                            "AND B.ITEM_CODE = C.ITEM_ID AND A.APPROVED = 1 AND A.CANCELED = 0 "+
                            "AND B.SR_NO = C.ISSUE_SR_NO AND C.AUTO_LOT_NO = '" + LotNo + "' "+
                            ...
                            /*strSQL="SELECT  SUM(C.ISSUED_LOT_QTY) AS QTY "+
                            "FROM D_INV_ISSUE_HEADER A,D_INV_ISSUE_DETAIL B,D_INV_ISSUE_LOT C "+
                            "WHERE  A.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND A.ISSUE_NO = B.ISSUE_NO AND A.ISSUE_NO = C.ISSUE_NO "+
                            "AND B.ITEM_CODE = C.ITEM_ID AND A.APPROVED = 1 AND A.CANCELED = 0 "+
                            "AND B.SR_NO = C.ISSUE_SR_NO AND C.AUTO_LOT_NO = '" + LotNo + "' "+
                            ...
                            Total_Qty += data.getDoubleValueFromDB(strSQL);
                             */
                            if(LotNo.equals("090043")) {
                                boolean t = true;
                            }
                            ...
                            if(ItemID.equals("RM11124001")) {
                                boolean t=false;
                            }
                            OpeningQty=EITLERPGLOBAL.round(LotQty,3);
                            OpeningValue=EITLERPGLOBAL.round(LotQty * LotRate,3);
                            OpeningRate=EITLERPGLOBAL.round(LotRate,3);
                            
                            if(rsLot.getString("TYPE").equals("R") && EITLERPGLOBAL.compareDate(EITLERPGLOBAL.formatDate(rsLot.getString("GRN_DATE")),txtFromDate.getText().trim()) == 1) {
                                OpeningQty=0.000;
                                OpeningValue=0.000;
                                OpeningRate=0.000;
                            }
/*                            if(rsLot.getString("TYPE").equals("O")) {
                            ...
                            writeLine(aFile, "           Opening:" + EITLERPGLOBAL.padLeft(13,String.valueOf(EITLERPGLOBAL.round(OpeningQty,3))," ") + EITLERPGLOBAL.padLeft(14,String.valueOf(OpeningRate)," ") + EITLERPGLOBAL.padLeft(14,String.valueOf(OpeningValue)," ") );
                            nOpnQty = EITLERPGLOBAL.round(nOpnQty + OpeningQty,3);
                            nOpnValue = EITLERPGLOBAL.round(nOpnValue + OpeningValue,3);
                            ...
                            //NOW FIND THE ISSUE OF THE LOT
                            strSQL = "SELECT  'I' AS TYPE,A.ISSUE_NO,A.ISSUE_DATE,B.RATE,B.ISSUE_VALUE AS ISSUE_VALUE,C.ISSUED_LOT_QTY AS ISSUED_LOT_QTY,ROUND((B.RATE* C.ISSUED_LOT_QTY),3) AS VALUE,B.SR_NO "+
                            "FROM D_INV_ISSUE_HEADER A,D_INV_ISSUE_DETAIL B,D_INV_ISSUE_LOT C "+
                            "WHERE  A.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND A.ISSUE_NO = B.ISSUE_NO AND A.ISSUE_NO = C.ISSUE_NO "+
                            "AND B.ITEM_CODE = C.ITEM_ID AND A.APPROVED = 1 AND A.CANCELED = 0 "+
                            "AND B.SR_NO = C.ISSUE_SR_NO AND C.AUTO_LOT_NO = '" + LotNo + "' "+
                            ...
                            "AND ISSUE_DATE <='"+EITLERPGLOBAL.formatDateDB(txtToDate.getText())+"' AND ISSUE_DATE>='" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "' "+
                            "UNION "+
                            "SELECT 'S' AS TYPE,A.STM_NO AS ISSUE_NO ,A.STM_DATE AS ISSUE_DATE,B.RATE,B.NET_AMOUNT AS ISSUE_VALUE,C.ISSUED_LOT_QTY AS ISSUED_LOT_QTY,ROUND((B.RATE* C.ISSUED_LOT_QTY),3) AS VALUE,B.SR_NO "+
                            "FROM D_INV_STM_HEADER A,D_INV_STM_DETAIL B,D_INV_STM_LOT C "+
                            "WHERE A.COMPANY_ID = "+EITLERPGLOBAL.gCompanyID+" AND A.STM_NO = B.STM_NO AND A.STM_NO = C.STM_NO AND A.STM_TYPE=B.STM_TYPE "+
                            "AND B.ITEM_ID = C.ITEM_ID AND A.APPROVED =1 AND A.CANCELLED=0 AND B.SR_NO = C.STM_SR_NO "+
                            "AND C.AUTO_LOT_NO = '" + LotNo + "' "+
                            ...
                            while((!rsIssue.isAfterLast())&&rsIssue.getRow()>0) {
                                ClosingQty = EITLERPGLOBAL.round(ClosingQty - rsIssue.getDouble("ISSUED_LOT_QTY"),3);
                                if(ClosingQty>0) {
                                    ClosingRate = EITLERPGLOBAL.round(IssueRate,3);
                                    ClosingValue= EITLERPGLOBAL.round(ClosingRate * ClosingQty,3);
                                } else{
                                    ClosingRate=0.0;
                                    ClosingValue=0.0;
                    ...
                    writeLine(aFile, "Total Closing Qty: " + dFormat.format(EITLERPGLOBAL.round((nOpnQty+nRecQty),3)-EITLERPGLOBAL.round((nIssueQty+nSTMQty),3)));
                    writeLine(aFile, "Total Closing Value: " + dFormat.format(EITLERPGLOBAL.round((nOpnValue+nRecValue),3)-EITLERPGLOBAL.round((nIssueValue+nSTMValue),3)));
