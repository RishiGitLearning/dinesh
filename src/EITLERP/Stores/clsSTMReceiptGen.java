...
public class clsSTMReceiptGen {
    
    public static int ModuleID=185;
    ...
    public static int STMRecType =4; //1 GEN,2 RAW,3 STM RAW,4 STM GEN
    ...
    public clsSTMReceiptGen() {
        props=new HashMap();
        props.put("COMPANY_ID",new Variant(0));
        props.put("GRN_NO",new Variant(""));
        props.put("GRN_DATE",new Variant(""));
        props.put("GRN_TYPE",new Variant(4)); // 4 - STM GENERAL
    ...
    public boolean LoadData(int pCompanyID) {
        ...
        try {
            ...
            System.out.println("SELECT * FROM D_INV_GRN_HEADER WHERE COMPANY_ID="+pCompanyID+" AND GRN_TYPE='"+STMRecType+"' AND GRN_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND GRN_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY GRN_DATE");
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_INV_GRN_HEADER WHERE COMPANY_ID="+pCompanyID+" AND GRN_TYPE='"+STMRecType+"' AND GRN_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND GRN_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY GRN_DATE");
            HistoryView=false;
            Ready=true;
            MoveLast();
            return true;
    ...
    public static boolean IsGRNExist(int pCompanyID,String pDocNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT GRN_NO FROM D_INV_GRN_HEADER WHERE COMPANY_ID="+pCompanyID+" AND GRN_NO='"+pDocNo+"' AND GRN_TYPE='"+STMRecType+"'");
            rsTmp.first();
    ...
    public static boolean IsGRNExist(String pDocNo) {
        ...
        try {
            tmpConn=data.getConn();
            stTmp=tmpConn.createStatement();
            rsTmp=stTmp.executeQuery("SELECT GRN_NO FROM D_INV_GRN_HEADER WHERE GRN_NO='"+pDocNo+"' AND GRN_TYPE='"+STMRecType+"' AND CANCELLED=0");
            rsTmp.first();
    ...
    public static boolean IsGRNExist(String pDocNo,String dbURL) {
        ...
        try {
            ...
            stTmp=tmpConn.createStatement();
            rsTmp=stTmp.executeQuery("SELECT GRN_NO FROM D_INV_GRN_HEADER WHERE GRN_NO='"+pDocNo+"' AND GRN_TYPE='"+STMRecType+"'1 AND CANCELLED=0");
            rsTmp.first();
    ...
    public boolean Insert() {
        ...
        try {
            ...
            for(int i=1;i<=colGRNItems.size();i++) {
                clsSTMReceiptsGenItems ObjItem=(clsSTMReceiptsGenItems)colGRNItems.get(Integer.toString(i));
                ...
                if((!MIRNo.trim().equals(""))&&(MIRSrNo>0)) //MIR No. entered
                {
                    ...
                    PrevQty=0;
                    stTmp=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
                    strSQL="SELECT SUM(QTY) AS SUMQTY FROM D_INV_GRN_DETAIL WHERE COMPANY_ID="+CompanyID+" AND MIR_NO='"+MIRNo+"' AND MIR_SR_NO="+MIRSrNo+" AND GRN_TYPE='"+STMRecType+"' AND GRN_NO NOT IN (SELECT GRN_NO FROM D_INV_GRN_HEADER WHERE CANCELLED=1)";
                    rsTmp=stTmp.executeQuery(strSQL);
                    rsTmp.first();
...
//                            rsItemMaster.updateString("MODIFIED_DATE",(String)getAttribute("MODIFIED_DATE").getObj());
            ...
            if(UserDocNo) {
                ...
                setAttribute("GRN_NO",clsFirstFree.getNextFreeNo((int)EITLERPGLOBAL.gCompanyID,clsSTMReceiptGen.ModuleID , (int)getAttribute("FFNO").getVal(),true));
            }
            //-------------------------------------------------
...
//            rsHistory.updateString("MODIFIED_DATE",(String)getAttribute("MODIFIED_DATE").getObj());
            ...
            for(int i=1;i<=colGRNItems.size();i++) {
                ...
                rsItem.updateInt("GRN_TYPE",STMRecType); //4 - STM General Material
...
//                rsItem.updateString("MODIFIED_DATE",(String)getAttribute("MODIFIED_DATE").getObj());
                ...
                rsHDetail.updateInt("GRN_TYPE",STMRecType); //1 - General Material
...
//                rsHDetail.updateString("MODIFIED_DATE",(String)getAttribute("MODIFIED_DATE").getObj());
            ...
            ObjFlow.ModuleID=clsSTMReceiptGen.ModuleID; //GRN General Materials
    ...
    private void RevertStockEffect() {
        ...
        try {
            ...
            strSQL="SELECT COMPANY_ID,GRN_NO,SR_NO,WAREHOUSE_ID,LOCATION_ID,QTY,ITEM_ID,BOE_NO FROM D_INV_GRN_DETAIL WHERE COMPANY_ID="+CompanyID+" AND GRN_NO='"+GRNNo+"' AND GRN_TYPE='"+STMRecType+"' ";
            ...
            strSQL="SELECT * FROM D_INV_GRN_DETAIL WHERE COMPANY_ID="+CompanyID+" AND GRN_NO='"+GRNNo+"' AND GRN_TYPE='"+STMRecType+"' ";
            ...
            while(!rsTmp.isAfterLast()) {
                ...
                // Update GRN Received Qty
                data.Execute("UPDATE D_INV_MIR_DETAIL SET GRN_RECD_QTY=GRN_RECD_QTY-"+Qty+",BAL_QTY=QTY-GRN_RECD_QTY,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+CompanyID+" AND MIR_NO='"+MIRNo+"' AND SR_NO="+MIRSrNo+" AND GRN_TYPE='"+STMRecType+"' ");
                rsTmp.next();
            }
        }
        catch(Exception e) {
            ...
            if(Validate) {
                ...
                for(int i=1;i<=colGRNItems.size();i++) {
                    ...
                    if((!MIRNo.trim().equals(""))&&(MIRSrNo>0)) //MIR No. entered
                    {
                        ...
                        PrevQty=0;
                        stTmp=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
                        strSQL="SELECT SUM(QTY) AS SUMQTY FROM D_INV_GRN_DETAIL WHERE COMPANY_ID="+CompanyID+" AND MIR_NO='"+MIRNo+"' AND MIR_SR_NO="+MIRSrNo+" AND MIR_TYPE=1 AND NOT(GRN_NO='"+GRNNo+"' AND GRN_TYPE='"+STMRecType+"' ) AND GRN_NO NOT IN(SELECT GRN_NO FROM D_INV_GRN_HEADER WHERE CANCELLED=1)";
                        rsTmp=stTmp.executeQuery(strSQL);
                        rsTmp.first();
                ...
                for(int i=1;i<=colGRNItems.size();i++) {
                    ...
                    if(MaxQty>0) {
                        ...
                        stTmp=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
                        rsTmp=stTmp.executeQuery("SELECT QTY FROM D_INV_GRN_DETAIL WHERE COMPANY_ID="+CompanyID+" AND GRN_NO='"+GRNNo+"' AND SR_NO="+i+" AND GRN_TYPE='"+STMRecType+"' ");
                        rsTmp.first();
...
//            rsResultSet.updateString("CREATED_DATE",(String)getAttribute("CREATED_DATE").getObj());
            ...
            //========= Inserting Into History =================//
            //Get the Maximum Revision No.
            int RevNo=EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM D_INV_GRN_HEADER_H WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND GRN_NO='"+(String)getAttribute("GRN_NO").getObj()+"' AND GRN_TYPE='"+STMRecType+"' ");
            RevNo++;
...
//            rsHistory.updateString("CREATED_DATE",(String)getAttribute("CREATED_DATE").getObj());
            ...
            data.Execute("DELETE FROM D_INV_GRN_DETAIL WHERE COMPANY_ID="+CompanyID+" AND GRN_NO='"+GRNNo+"' AND GRN_TYPE='"+STMRecType+"' ");
...
//                rsItem.updateString("CREATED_DATE",(String)getAttribute("CREATED_DATE").getObj());
                ...
                rsHDetail.updateString("GRN_NO",(String)getAttribute("GRN_NO").getObj());
                rsHDetail.updateInt("SR_NO",i);
                rsHDetail.updateInt("GRN_TYPE",STMRecType); //4 - STM General Material
                rsHDetail.updateString("WAREHOUSE_ID",(String)ObjItem.getAttribute("WAREHOUSE_ID").getObj());
                rsHDetail.updateString("LOCATION_ID",(String)ObjItem.getAttribute("LOCATION_ID").getObj());
                rsHDetail.updateString("MIR_NO",(String)ObjItem.getAttribute("MIR_NO").getObj());
                rsHDetail.updateInt("MIR_SR_NO",(int)ObjItem.getAttribute("MIR_SR_NO").getVal());
...
//                rsHDetail.updateString("CREATED_DATE",(String)getAttribute("CREATED_DATE").getObj());
            ...
            //======== Update the Approval Flow =========
            ObjFlow.CompanyID=(int)getAttribute("COMPANY_ID").getVal();
            ObjFlow.ModuleID=clsSTMReceiptGen.ModuleID; //GRN General Material
            ObjFlow.DocNo=(String)getAttribute("GRN_NO").getObj();
            ObjFlow.From=(int)getAttribute("FROM").getVal();
            ObjFlow.To=(int)getAttribute("TO").getVal();
            ObjFlow.Status=(String)getAttribute("APPROVAL_STATUS").getObj();
            ...
            if(IsRejected) {
                //Remove the Rejected Flag First
                data.Execute("UPDATE D_INV_GRN_HEADER SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND GRN_NO='"+ObjFlow.DocNo+"' AND GRN_TYPE='"+STMRecType+"' ");
                //Remove Old Records from D_COM_DOC_DATA
                data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID="+clsSTMReceiptGen.ModuleID+" AND DOC_NO='"+ObjFlow.DocNo+"'");
    ...
    public boolean Delete(int pUserID) {
        try {
            ...
            if(CanDelete(lCompanyID,lDocNo,pUserID)) {
                ...
                data.Execute("DELETE FROM D_INV_GRN_HEADER WHERE COMPANY_ID="+lCompanyID+" AND GRN_NO='"+lDocNo.trim()+"' AND GRN_TYPE='"+STMRecType+"' ");
                data.Execute("DELETE FROM D_INV_GRN_DETAIL WHERE COMPANY_ID="+lCompanyID+" AND GRN_NO='"+lDocNo.trim()+"' AND GRN_TYPE='"+STMRecType+"' ");
    ...
    public Object getObject(int pCompanyID,String pDocNo) {
        String strCondition = " WHERE COMPANY_ID=" + Integer.toString(pCompanyID)+" AND GRN_NO='"+pDocNo+"' AND GRN_TYPE='"+STMRecType+"' ";
        clsSTMReceiptGen ObjGRNGen = new clsSTMReceiptGen();
    ...
    public Object getObject(int pCompanyID,String pDocNo,int pType) {
        ...
        clsSTMReceiptGen ObjGRN = new clsSTMReceiptGen();
    ...
    public boolean Filter(String pCondition,int pCompanyID) {
        ...
        try {
            ...
            if(!rsResultSet.first()) {
                strSql = "SELECT * FROM D_INV_GRN_HEADER WHERE COMPANY_ID="+Long.toString(pCompanyID)+" AND GRN_TYPE='"+STMRecType+"' AND GRN_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND GRN_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY GRN_NO";
                rsResultSet=Stmt.executeQuery(strSql);
                Ready=true;
                MoveLast();
                return false;
    ...
    public boolean setData() {
        ...
        try {
            ...
            stItem=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            if(HistoryView) {
                rsItem=stItem.executeQuery("SELECT * FROM D_INV_GRN_DETAIL_H WHERE COMPANY_ID="+CompanyID+" AND GRN_NO='"+GRNNo+"' AND GRN_TYPE='"+STMRecType+"' AND REVISION_NO="+RevNo+" ORDER BY SR_NO");
            }
            else {
                rsItem=stItem.executeQuery("SELECT * FROM D_INV_GRN_DETAIL WHERE COMPANY_ID="+CompanyID+" AND GRN_NO='"+GRNNo+"' AND GRN_TYPE='"+STMRecType+"' ORDER BY SR_NO");
            }
            rsItem.first();
    ...
    public boolean IsEditable(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM D_INV_GRN_HEADER WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND GRN_NO='"+pDocNo+"' AND (APPROVED=1) AND GRN_TYPE='"+STMRecType+"' ";
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM D_COM_DOC_DATA WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND MODULE_ID="+clsSTMReceiptGen.ModuleID+"  AND DOC_NO='"+pDocNo+"' AND USER_ID="+Integer.toString(pUserID)+" AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean CanDelete(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM D_COM_DOC_DATA WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND MODULE_ID="+clsSTMReceiptGen.ModuleID+"  AND DOC_NO='"+pDocNo+"' AND USER_ID="+Integer.toString(pUserID)+" AND TYPE='C' AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean ShowHistory(int pCompanyID,String pDocNo) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_INV_GRN_HEADER_H WHERE COMPANY_ID="+pCompanyID+" AND GRN_NO='"+pDocNo+"' AND GRN_TYPE='"+STMRecType+"' ");
            Ready=true;
            HistoryView=true;
            MoveFirst();
            return true;
    ...
    public static HashMap getHistoryList(int pCompanyID,String pDocNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT * FROM D_INV_GRN_HEADER_H WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND GRN_NO='"+pDocNo+"' AND GRN_TYPE='"+STMRecType+"' ");
    ...
    private boolean PostRJN() {
        ...
        for(int i=1;i<=colGRNItems.size();i++) {
            ...
            clsSTMReceiptsGenItems ObjGRNItem=(clsSTMReceiptsGenItems)colGRNItems.get(Integer.toString(i));
    ...
    public static boolean CanCancel(int pCompanyID,String pDocNo) {
        ...
        try {
            rsTmp=data.getResult("SELECT GRN_NO FROM D_INV_GRN_HEADER WHERE COMPANY_ID="+pCompanyID+" AND GRN_NO='"+pDocNo+"' AND CANCELLED=0 AND GRN_TYPE='"+STMRecType+"' ");
            rsTmp.first();
    ...
    public static boolean CancelGRN(int pCompanyID,String pDocNo) {
        ...
        try {
            ...
            if(CanCancel(pCompanyID,pDocNo)) {
                ...
                rsTmp=data.getResult("SELECT APPROVED FROM D_INV_GRN_HEADER WHERE COMPANY_ID="+pCompanyID+" AND GRN_NO='"+pDocNo+"' AND GRN_TYPE='"+STMRecType+"' ");
                rsTmp.first();
                ...
                if(ApprovedDoc) {
                    rsTmp=data.getResult("SELECT * FROM D_INV_GRN_DETAIL WHERE COMPANY_ID="+pCompanyID+" AND GRN_NO='"+pDocNo+"' AND GRN_TYPE='"+STMRecType+"' ");
                    rsTmp.first();
                    ...
                    data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID="+pCompanyID+" AND DOC_NO='"+pDocNo+"' AND MODULE_ID="+clsSTMReceiptGen.ModuleID);
                }
                
                //Now Update the header with cancel falg to true
                data.Execute("UPDATE D_INV_GRN_HEADER SET CANCELLED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+pCompanyID+" AND GRN_NO='"+pDocNo+"' AND GRN_TYPE='"+STMRecType+"' ");
    ...
    public static HashMap getPendingApprovals(int pCompanyID,int pUserID,int pType,int pOrder,int FinYearFrom) {
        ...
        try {
            ...
            if(pOrder==EITLERPGLOBAL.OnRecivedDate) {
                strSQL="SELECT D_INV_GRN_HEADER.GRN_NO,D_INV_GRN_HEADER.GRN_DATE,RECEIVED_DATE,0 AS DEPT_ID FROM D_INV_GRN_HEADER,D_COM_DOC_DATA WHERE D_INV_GRN_HEADER.GRN_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_GRN_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_GRN_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND D_INV_GRN_HEADER.GRN_TYPE="+pType+" AND MODULE_ID="+ModuleID+" ORDER BY D_COM_DOC_DATA.RECEIVED_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocDate) {
                strSQL="SELECT D_INV_GRN_HEADER.GRN_NO,D_INV_GRN_HEADER.GRN_DATE,RECEIVED_DATE,0 AS DEPT_ID FROM D_INV_GRN_HEADER,D_COM_DOC_DATA WHERE D_INV_GRN_HEADER.GRN_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_GRN_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_GRN_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND D_INV_GRN_HEADER.GRN_TYPE="+pType+" AND MODULE_ID="+ModuleID+" ORDER BY D_INV_GRN_HEADER.GRN_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocNo) {
                strSQL="SELECT D_INV_GRN_HEADER.GRN_NO,D_INV_GRN_HEADER.GRN_DATE,RECEIVED_DATE,0 AS DEPT_ID FROM D_INV_GRN_HEADER,D_COM_DOC_DATA WHERE D_INV_GRN_HEADER.GRN_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_GRN_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_GRN_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND D_INV_GRN_HEADER.GRN_TYPE="+pType+" AND MODULE_ID="+ModuleID+" ORDER BY D_INV_GRN_HEADER.GRN_NO";
            }
