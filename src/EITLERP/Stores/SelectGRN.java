...
public class SelectGRN extends javax.swing.JApplet {
    ...
    private String SelGRNNo="";
    public int GRNType=1;
    
    public boolean CopyHeader=true;
    public clsGRN ObjGRN;
    public clsGRNGen ObjGRNGen;
   ...
   public SelectGRN()
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        jLabel1.setText("Select GRN Items from the List");
    ...
    private void txtGRNNoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtGRNNoKeyPressed
        ...
        if(evt.getKeyCode()==112) //F1 Key pressed
        {
            ...
            if(chkDept.isSelected())
            {
               aList.SQL="SELECT DISTINCT(D_INV_GRN_HEADER.GRN_NO) AS GRN_NO,DATE_FORMAT(GRN_DATE,'%d/%m/%Y') AS DATE,DEPT_DESC FROM D_INV_GRN_HEADER,D_INV_GRN_DETAIL,D_COM_USER_MASTER,D_COM_DEPT_MASTER WHERE D_INV_GRN_HEADER.COMPANY_ID=D_INV_GRN_DETAIL.COMPANY_ID AND D_INV_GRN_HEADER.GRN_NO=D_INV_GRN_DETAIL.GRN_NO AND D_INV_GRN_HEADER.GRN_TYPE=D_INV_GRN_DETAIL.GRN_TYPE  AND D_INV_GRN_HEADER.GRN_TYPE="+GRNType+" AND D_INV_GRN_HEADER.CREATED_BY=D_COM_USER_MASTER.USER_ID AND D_INV_GRN_HEADER.COMPANY_ID=D_COM_USER_MASTER.COMPANY_ID AND D_INV_GRN_HEADER.COMPANY_ID=D_COM_DEPT_MASTER.COMPANY_ID AND D_COM_USER_MASTER.DEPT_ID=D_COM_DEPT_MASTER.DEPT_ID AND D_COM_USER_MASTER.DEPT_ID="+SelDeptID+" AND D_INV_GRN_HEADER.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND CANCELLED=0 AND APPROVED=1"; 
            }
            else
            {
               aList.SQL="SELECT DISTINCT(D_INV_GRN_HEADER.GRN_NO) AS GRN_NO,DATE_FORMAT(GRN_DATE,'%d/%m/%Y') AS DATE,DEPT_DESC FROM D_INV_GRN_HEADER,D_INV_GRN_DETAIL,D_COM_USER_MASTER,D_COM_DEPT_MASTER WHERE D_INV_GRN_HEADER.COMPANY_ID=D_INV_GRN_DETAIL.COMPANY_ID AND D_INV_GRN_HEADER.GRN_NO=D_INV_GRN_DETAIL.GRN_NO AND D_INV_GRN_HEADER.GRN_TYPE=D_INV_GRN_DETAIL.GRN_TYPE  AND D_INV_GRN_HEADER.GRN_TYPE="+GRNType+" AND D_INV_GRN_HEADER.CREATED_BY=D_COM_USER_MASTER.USER_ID AND D_INV_GRN_HEADER.COMPANY_ID=D_COM_USER_MASTER.COMPANY_ID AND D_INV_GRN_HEADER.COMPANY_ID=D_COM_DEPT_MASTER.COMPANY_ID AND D_COM_USER_MASTER.DEPT_ID=D_COM_DEPT_MASTER.DEPT_ID AND D_INV_GRN_HEADER.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND CANCELLED=0 AND APPROVED=1";  
            }
    ...
    private void cmdShowActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdShowActionPerformed
        // TODO add your handling code here:
        SelGRNNo=txtGRNNo.getText();
        
        String docStatus=clsGRN.getDocStatus(EITLERPGLOBAL.gCompanyID, SelGRNNo);
...
private void FormatGrid()    
{
        ...
        DataModel.addColumn("Rejected Qty.");
        DataModel.addColumn("Unit ID");//5
        DataModel.addColumn("Unit");//6
        DataModel.addColumn("BOE No.");//7
        DataModel.addColumn("BOE Sr.");//8
        DataModel.addColumn("BOE Date");//9
        DataModel.addColumn("Remarks");//10
        DataModel.addColumn("P.O. No.");//11
        DataModel.addColumn("P.O. Sr.");//12
        DataModel.addColumn("MIR No.");//13
        DataModel.addColumn("MIR Sr.");//14
        DataModel.addColumn("P.O. Type");//15
...
private void SetList()
{
    ...
    String GRNNo=txtGRNNo.getText();
        
    if(GRNType==1) //General
    {
       clsGRNGen tmpObj=new clsGRNGen();
       ObjGRNGen=(clsGRNGen)tmpObj.getObject(EITLERPGLOBAL.gCompanyID, GRNNo);
    }
    else //Raw Material
    {
       clsGRN tmpObj=new clsGRN();
       ObjGRN=(clsGRN)tmpObj.getObject(EITLERPGLOBAL.gCompanyID, GRNNo);
    }
    
    CopyHeader=chkCopyHeader.isSelected();
    
    List=clsGRN.getGRNItemList(EITLERPGLOBAL.gCompanyID, SelGRNNo,false,GRNType);    
    //Scan the Table to get selected items
    for(int i=1;i<=List.size();i++)
    {
       clsGRNItem ObjItem=(clsGRNItem)List.get(Integer.toString(i));
...
private void GenerateGrid()
{
    ...
    if(!GRNNo.equals(""))
    {
       List=clsGRN.getGRNItemList(EITLERPGLOBAL.gCompanyID, GRNNo,false,GRNType);
       ...
       for(int i=1;i<=List.size();i++)
       {
           ...
           rowData[5]=Double.toString(ObjItem.getAttribute("REJECTED_QTY").getVal());
           ...
           rowData[11]=(String)ObjItem.getAttribute("REMARKS").getObj();
           rowData[12]=(String)ObjItem.getAttribute("PO_NO").getObj();
           rowData[13]=Integer.toString((int)ObjItem.getAttribute("PO_SR_NO").getVal());
           rowData[14]=(String)ObjItem.getAttribute("MIR_NO").getObj();
           rowData[15]=Integer.toString((int)ObjItem.getAttribute("MIR_SR_NO").getVal());
           rowData[16]=Integer.toString((int)ObjItem.getAttribute("PO_TYPE").getVal());
