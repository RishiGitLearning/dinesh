    ...
    public boolean Insert(String pPrefix,String pDocno) {
        ...
        try {
            ...
            if(UserDocNo) {
                ...
                setAttribute("ISSUE_NO",clsFirstFree.getNextFreeNo((int)EITLERPGLOBAL.gCompanyID,14, (int)getAttribute("FFNO").getVal(),true));
            }
            //-------------------------------------------------
            ...
            for(int i=1;i<=colLineItems.size();i++) {
                ...
                rsTmp.updateString("ISSUE_CAT",(String)ObjItem.getAttribute("ISSUE_CAT").getObj());
                rsTmp.updateString("ISSUE_CAT_DESC",(String)ObjItem.getAttribute("ISSUE_CAT_DESC").getObj());
                rsTmp.insertRow();
                ...
                rsHDetail.updateString("ISSUE_CAT",(String)ObjItem.getAttribute("ISSUE_CAT").getObj());
                rsHDetail.updateString("ISSUE_CAT_DESC",(String)ObjItem.getAttribute("ISSUE_CAT_DESC").getObj());
                rsHDetail.insertRow();
            }
            
            ObjFlow.CompanyID=(int)getAttribute("COMPANY_ID").getVal();
            ObjFlow.ModuleID=14; //QUOTATION MODULE ID
    ...
    public boolean CanDelete(int pCompanyID,String pIssueno,int pUserID) {
        ...
        try {
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM D_COM_DOC_DATA WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND MODULE_ID=14 AND DOC_NO='"+pIssueno+"' AND USER_ID="+Integer.toString(pUserID)+" AND TYPE='C' AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean IsEditable(int pCompanyID,String pIssueno,int pUserID) {
        ...
        try {
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM D_COM_DOC_DATA WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND MODULE_ID=14 AND DOC_NO='"+pIssueno+"' AND USER_ID="+Integer.toString(pUserID)+" AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getPendingApprovals(int pCompanyID,int pUserID,int pOrder,int FinYearFrom) {
        ...
        try {
            if(pOrder==EITLERPGLOBAL.OnRecivedDate) {
                strSQL="SELECT D_INV_ISSUE_HEADER.ISSUE_NO,D_INV_ISSUE_HEADER.ISSUE_DATE,RECEIVED_DATE,D_INV_ISSUE_HEADER.FOR_DEPT_ID AS DEPT_ID FROM D_INV_ISSUE_HEADER,D_COM_DOC_DATA WHERE D_INV_ISSUE_HEADER.ISSUE_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_ISSUE_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_ISSUE_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND D_COM_DOC_DATA.MODULE_ID=14 ORDER BY D_COM_DOC_DATA.RECEIVED_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocDate) {
                strSQL="SELECT D_INV_ISSUE_HEADER.ISSUE_NO,D_INV_ISSUE_HEADER.ISSUE_DATE,RECEIVED_DATE,D_INV_ISSUE_HEADER.FOR_DEPT_ID AS DEPT_ID FROM D_INV_ISSUE_HEADER,D_COM_DOC_DATA WHERE D_INV_ISSUE_HEADER.ISSUE_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_ISSUE_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_ISSUE_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND D_COM_DOC_DATA.MODULE_ID=14 ORDER BY D_INV_ISSUE_HEADER.ISSUE_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocNo) {
                strSQL="SELECT D_INV_ISSUE_HEADER.ISSUE_NO,D_INV_ISSUE_HEADER.ISSUE_DATE,RECEIVED_DATE,D_INV_ISSUE_HEADER.FOR_DEPT_ID AS DEPT_ID FROM D_INV_ISSUE_HEADER,D_COM_DOC_DATA WHERE D_INV_ISSUE_HEADER.ISSUE_NO=D_COM_DOC_DATA.DOC_NO AND D_INV_ISSUE_HEADER.COMPANY_ID=D_COM_DOC_DATA.COMPANY_ID AND D_INV_ISSUE_HEADER.COMPANY_ID="+Integer.toString(pCompanyID)+" AND D_COM_DOC_DATA.USER_ID="+Integer.toString(pUserID)+" AND D_COM_DOC_DATA.STATUS='W' AND D_COM_DOC_DATA.MODULE_ID=14 ORDER BY D_INV_ISSUE_HEADER.ISSUE_NO";
            }
    ...
    private boolean setData() {
        ...
        try {
            ...
            while(rsTmp.next()) {
                ...
                ObjItem.setAttribute("ISSUE_CAT",rsTmp.getString("ISSUE_CAT"));
                ObjItem.setAttribute("ISSUE_CAT_DESC",rsTmp.getString("ISSUE_CAT_DESC"));
        ...
        try {
            ...
            if(CanCancel(pCompanyID,pDocNo)) {
                ...
                if(ApprovedDoc) {
                    ...
                    data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID="+pCompanyID+" AND DOC_NO='"+pDocNo+"' AND MODULE_ID=14");
                }
