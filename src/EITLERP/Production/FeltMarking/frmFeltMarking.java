...
public class frmFeltMarking extends javax.swing.JApplet {
    ...
    private clsFeltMarking ObjFeltMarking;
    ...
    public void init() {
        ...
        ObjFeltMarking = new clsFeltMarking();

        SetMenuForRights();
        GenerateHierarchyCombo();
        GenerateSendToCombo();
        GenerateUserNameCombo();
        if (ObjFeltMarking.LoadData()) {
            DisplayData();
        } else {
            JOptionPane.showMessageDialog(this, "Error occured while Loading Data. Error is " + ObjFeltMarking.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void initComponents() {
        ...
        lblTitle.setText(" FELT PRODUCTION MARKING DETAILS - ");
        ...
        Tab.addTab("Marking Details", Tab1);
...
//                }
    ...
    private void cmdPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdPrintActionPerformed
        new TReportWriter.TReportEngine().PreviewReport("http://" + EITLERPGLOBAL.HostIP + "/EITLERP/Reports/Production/rptFeltNeedling.rpt", new HashMap(), ObjFeltMarking.getReportData(EITLERPGLOBAL.formatDateDB(txtFeltProductionDate.getText().trim())));
    ...
    private void cmdPreviewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdPreviewActionPerformed
        ...
        try {
            ...
            String strSQL = "SELECT PROD_DATE,PROD_FORM_NO,PROD_DOC_NO,PROD_PIECE_NO,STYLE,GRP,SYN_PER,\n"
                    ...
                    + "DAY_WEIGHT,TOTAL_WEIGHT,SHIFT_ID,DRYER,MARKING_IN_DATE\n"
                    + ",MARKING_OUT_DATE FROM \n"
                    + "(SELECT PROD_DATE,PROD_FORM_NO,PROD_DOC_NO,REPLACE(PROD_PIECE_NO,' ','') \n"
                    + "PROD_PIECE_NO,PROD_PARTY_CODE,WEIGHT,A.GROUP GRP,SHIFT_ID,DRYER,MARKING_IN_DATE\n"
                    + ",MARKING_OUT_DATE FROM PRODUCTION.FELT_PROD_DATA A \n"
                    + "WHERE PROD_DOC_NO = '"+txtProductionDocumentNo.getText().trim()+"' AND PROD_DEPT='MARKING') D \n"
    ...
    private void cmdBackToNormalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdBackToNormalActionPerformed
        ObjFeltMarking.HistoryView = false;
        ObjFeltMarking.LoadData();
        MoveLast();
    ...
    private void cmdViewRevisionsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdViewRevisionsActionPerformed
        ObjFeltMarking.ShowHistory(EITLERPGLOBAL.formatDateDB(txtFeltProductionDate.getText()), txtProductionDocumentNo.getText());
        MoveLast();
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            GenerateRejectedSendToCombo();
            if (clsFeltProductionApprovalFlow.IsOnceRejectedDoc(706, txtProductionDocumentNo.getText())) {
                cmbSendTo.setEnabled(true);
            } else {
                cmbSendTo.setEnabled(false);
            }
            ...
            if ((EditMode == 1) || (EditMode == 2 && clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID))) {
                if (Table.getSelectedColumn() == 1) {
                    ...
                    aList.SQL = "SELECT WIP_EXT_PIECE_NO AS PIECE_NO,WIP_PIECE_NO AS PIECE,WIP_PARTY_CODE AS PARTY_CODE,PARTY_NAME,"
                            ...
                            + " AND WIP_PIECE_STAGE='MARKING' \n"
                            + " AND WIP_STATUS='WOVEN'";
                        ...
                        if (!data.getStringValueFromDB("SELECT PD.PIECE_NO FROM PRODUCTION.FELT_SALES_PIECE_AMENDMENT_HEADER_WIP PH, "
                            ...
                            try {
                                ...
                                String sql = "SELECT WIP_PARTY_CODE,COALESCE(WIP_WEAVING_WEIGHT_A,0)+COALESCE(WIP_WEAVING_WEIGHT_B,0) AS WIP_WEAVING_WEIGHT,"
                                        + " WIP_PRODUCT_CODE,WIP_GROUP,WIP_GSM,WIP_STYLE,WIP_WVG_DATE FROM PRODUCTION.FELT_WIP_PIECE_REGISTER "
                                ...
                                if (trs.getRow() > 0) {
                                    ...
                                    Table.setValueAt(EITLERPGLOBAL.formatDate(trs.getString("WIP_WVG_DATE")), Table.getSelectedRow(), 4);
                                    Table.setValueAt(trs.getString("WIP_PRODUCT_CODE"), Table.getSelectedRow(), 9); //7 //6
                                    Table.setValueAt(trs.getString("WIP_GROUP"), Table.getSelectedRow(), 10); //8 //7
                                    //Table.setValueAt(trs.getString("WIP_GSM"), Table.getSelectedRow(), 11);  //9//8
                                    Table.setValueAt(trs.getString("WIP_STYLE"), Table.getSelectedRow(), 11);  //10//9
...
try {
    ...
    private void DisplayData() {
        ...
        if (EITLERPGLOBAL.gAuthorityUserID != EITLERPGLOBAL.gUserID) {
            if (clsAuthority.IsAuthorityGiven(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gUserID, EITLERPGLOBAL.gAuthorityUserID, 706)) {
        ...
        try {
            if (EditMode == 0) {
                if (ObjFeltMarking.getAttribute("APPROVED").getInt() == 1) {
                    lblTitle.setBackground(Color.BLUE);
                } else {
                    lblTitle.setBackground(Color.GRAY);
                }

                /*if(ObjFeltMarking.getAttribute("CANCELED").getInt()==1) {
                 lblTitle.setBackground(Color.RED);
                 }*/
            }
            //============================================//

            String productionDate = EITLERPGLOBAL.formatDate(ObjFeltMarking.getAttribute("PRODUCTION_DATE").getString());
            String documentNo = ObjFeltMarking.getAttribute("PRODUCTION_DOCUMENT_NO").getString();
            lblTitle.setText(" FELT PRODUCTION MARKING DETAILS - " + productionDate);
            lblRevNo.setText(Integer.toString((int) ObjFeltMarking.getAttribute("REVISION_NO").getVal()));
            txtMonthWeight.setText(Double.toString(EITLERPGLOBAL.round(ObjFeltMarking.getAttribute("TOTAL_WEIGHT").getVal(), 2)));
            txtPreviousWeight.setText(Double.toString(EITLERPGLOBAL.round(ObjFeltMarking.getAttribute("PREVIOUS_WEIGHT").getVal(), 2)));
            EITLERPGLOBAL.setComboIndex(cmbHierarchy, (int) ObjFeltMarking.getAttribute("HIERARCHY_ID").getVal());
            DoNotEvaluate = true;

            txtFeltProductionDate.setText(productionDate);
            txtProductionDocumentNo.setText(documentNo);
            txtFormNo.setText(ObjFeltMarking.getAttribute("PRODUCTION_FORM_NO").getString());

            FormatGrid();
            //Now Generate Table
            for (int i = 1; i <= ObjFeltMarking.hmFeltNeedlingDetails.size(); i++) {
                clsFeltMarkingDetails ObjFeltMarkingDetails = (clsFeltMarkingDetails) ObjFeltMarking.hmFeltNeedlingDetails.get(Integer.toString(i));

                Object[] rowData = new Object[15];
                rowData[0] = Integer.toString(i);
                String PieceNo = ObjFeltMarkingDetails.getAttribute("PRODUCTION_PIECE_NO").getString();
                rowData[1] = ObjFeltMarkingDetails.getAttribute("PRODUCTION_PIECE_NO").getString();
                rowData[2] = ObjFeltMarkingDetails.getAttribute("PRODUCTION_PARTY_CODE").getString();
                rowData[3] = Double.toString(EITLERPGLOBAL.round(ObjFeltMarkingDetails.getAttribute("WEIGHT").getVal(), 2));
                if (ObjFeltMarkingDetails.getAttribute("MARKING_IN_DATE").getString().equals("")) {
                    rowData[4] = "";  //11
                } else {
                    rowData[4] = EITLERPGLOBAL.formatDate(ObjFeltMarkingDetails.getAttribute("MARKING_IN_DATE").getString());  //11
                }
                if (ObjFeltMarkingDetails.getAttribute("MARKING_OUT_DATE").getString().equals("")) {
                    rowData[5] = "";  //10
                } else {
                    rowData[5] = EITLERPGLOBAL.formatDate(ObjFeltMarkingDetails.getAttribute("MARKING_OUT_DATE").getString());  //10
                }
                rowData[6] = ObjFeltMarkingDetails.getAttribute("SHIFT_ID").getString();                 
                rowData[7] = ObjFeltMarkingDetails.getAttribute("REMARKS").getString(); 
                
                //String ocYearMonth=data.getStringValueFromDB("SELECT PR_OC_MONTHYEAR FROM PRODUCTION.FELT_SALES_PIECE_REGISTER WHERE PR_PIECE_NO='"+PieceNo+"'");
                String ocYearMonth=data.getStringValueFromDB("SELECT WIP_OC_MONTHYEAR FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_EXT_PIECE_NO='"+PieceNo+"'");
                rowData[8] = ocYearMonth;
                
                //rowData[9] = ObjFeltMarkingDetails.getAttribute("DRYER").getString(); 
                
                rowData[9] = data.getStringValueFromDB("SELECT WIP_PRODUCT_CODE FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_EXT_PIECE_NO='" + ObjFeltMarkingDetails.getAttribute("PRODUCTION_PIECE_NO").getString() + "'"); //6
                rowData[10] = data.getStringValueFromDB("SELECT WIP_GROUP FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_EXT_PIECE_NO='" + ObjFeltMarkingDetails.getAttribute("PRODUCTION_PIECE_NO").getString() + "'"); //7                
                rowData[11] = data.getStringValueFromDB("SELECT WIP_STYLE FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_EXT_PIECE_NO='" + ObjFeltMarkingDetails.getAttribute("PRODUCTION_PIECE_NO").getString() + "'");  //9
            ...
            hmList = clsFeltProductionApprovalFlow.getDocumentFlow(706, documentNo);
            ...
            FormatGridUpdateHistory();
            HashMap hmApprovalHistory = clsFeltMarking.getHistoryList(ObjFeltMarking.getAttribute("PRODUCTION_DATE").getString(), txtProductionDocumentNo.getText());
            for (int i = 1; i <= hmApprovalHistory.size(); i++) {
                clsFeltMarking ObjFeltMarking = (clsFeltMarking) hmApprovalHistory.get(Integer.toString(i));
                Object[] rowData = new Object[6];

                rowData[0] = Integer.toString((int) ObjFeltMarking.getAttribute("REVISION_NO").getVal());
                rowData[1] = FeltUser.getUserName((int) ObjFeltMarking.getAttribute("UPDATED_BY").getVal());
                rowData[2] = EITLERPGLOBAL.formatDate(ObjFeltMarking.getAttribute("ENTRY_DATE").getString()) + ObjFeltMarking.getAttribute("ENTRY_DATE").getString().substring(10, 19);

                String ApprovalStatus = "";

                if ((ObjFeltMarking.getAttribute("APPROVAL_STATUS").getString()).equals("H")) {
                    ApprovalStatus = "Hold";
                }

                if ((ObjFeltMarking.getAttribute("APPROVAL_STATUS").getString()).equals("A")) {
                    ApprovalStatus = "Approved";
                }

                if ((ObjFeltMarking.getAttribute("APPROVAL_STATUS").getString()).equals("F")) {
                    ApprovalStatus = "Final Approved";
                }

                if ((ObjFeltMarking.getAttribute("APPROVAL_STATUS").getString()).equals("W")) {
                    ApprovalStatus = "Waiting";
                }

                if ((ObjFeltMarking.getAttribute("APPROVAL_STATUS").getString()).equals("R")) {
                    ApprovalStatus = "Rejected";
                }

                if ((ObjFeltMarking.getAttribute("APPROVAL_STATUS").getString()).equals("P")) {
                    ApprovalStatus = "Pending";
                }

                if ((ObjFeltMarking.getAttribute("APPROVAL_STATUS").getString()).equals("C")) {
                    ApprovalStatus = "Skiped";
                }

                rowData[3] = ApprovalStatus;
                rowData[4] = ObjFeltMarking.getAttribute("APPROVER_REMARKS").getString();
                rowData[5] = ObjFeltMarking.getAttribute("FROM_IP").getString();
    ...
    private void FormatGrid() {
        try {
            ...
            DataModel.addColumn("Marking In Date");  //4
            DataModel.addColumn("Marking Out Date");   //5
            ...
            DataModel.addColumn("Product Code"); //9
            DataModel.addColumn("Group");   //10
            
            DataModel.addColumn("Style");  //11
    ...
    private void GenerateHierarchyCombo() {
        ...
        hmHierarchyList = clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=706 ");

        if (EditMode == EITLERPGLOBAL.EDIT) {
            hmHierarchyList = clsHierarchy.getList(" WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=706 ");
        }
    ...
    private void GenerateSendToCombo() {
        ...
        try {
            ...
            if (EditMode == EITLERPGLOBAL.ADD) {
                ...
                hmSendToList = clsFeltProductionApprovalFlow.getRemainingUsers(706, ObjFeltMarking.getAttribute("PRODUCTION_DOCUMENT_NO").getString());
                for (int i = 1; i <= hmSendToList.size(); i++) {
                    clsUser ObjUser = (clsUser) hmSendToList.get(Integer.toString(i));
                    ComboData aData = new ComboData();
                    aData.Code = (int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void GenerateRejectedSendToCombo() {
        ...
        for (int i = 1; i <= hmRejectedSendToList.size(); i++) {
            ...
            if (EditMode == EITLERPGLOBAL.EDIT) {
                if (OpgApprove.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInApproval(706, txtProductionDocumentNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }

                if (OpgReject.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInRejection(706, txtProductionDocumentNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            int Creator = clsFeltProductionApprovalFlow.getCreator(706, txtProductionDocumentNo.getText());
    ...
    private void GenerateUserNameCombo() {
        ...
        hmUserNameList = ObjFeltMarking.getUserNameList(EITLERPGLOBAL.getComboCode(cmbHierarchy), EITLERPGLOBAL.gNewUserID, "MARKING");
    ...
    private void SetupApproval() {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = clsFeltProductionApprovalFlow.getFromID(706, ObjFeltMarking.getAttribute("PRODUCTION_DOCUMENT_NO").getString());
            lnFromUserId = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = clsFeltProductionApprovalFlow.getFromRemarks(706, FromUserID, ObjFeltMarking.getAttribute("PRODUCTION_DOCUMENT_NO").getString());
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            cmbHierarchy.setEnabled(false);
            if (clsFeltProductionApprovalFlow.IsCreator(706, txtProductionDocumentNo.getText())) {
                OpgReject.setEnabled(false);
            }
        }
    ...
    private void Edit() {
        String productionDocumentNo = (String) ObjFeltMarking.getAttribute("PRODUCTION_DOCUMENT_NO").getObj();
        if (ObjFeltMarking.IsEditable(productionDocumentNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(706, productionDocumentNo)) {
                SetFields(true);
                txtFeltProductionDate.setEnabled(false);
                txtFormNo.setEnabled(false);
            } else {
    ...
    private void Delete() {
        if (ObjFeltMarking.CanDelete(txtProductionDocumentNo.getText(), txtFeltProductionDate.getText(), EITLERPGLOBAL.gNewUserID)) {
            DisplayData();
        } else {
            JOptionPane.showMessageDialog(this, ObjFeltMarking.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void Save() {
        ...
        String markingOutDt = "", markingInDt = "", shiftId = "" , startTime = "", endTime = "" , dryer = "";
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            productionDocumentNo = "MR" + productionDate.substring(6, 10) + productionDate.substring(3, 5) + productionDate.substring(0, 2);
        }else{
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            if (ObjFeltMarking.checkProductionDateInDB(EITLERPGLOBAL.formatDateDB(productionDate))) {
                JOptionPane.showMessageDialog(frmFeltMarking.this, "Production Date " + productionDate + " already Exists.", "Production Date Already Exists", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
        ...
        try {
            ...
            ObjFeltMarking.hmFeltNeedlingDetails.clear();
...
//                } 
                ...
                markingOutDt = Table.getValueAt(i, 5).toString().trim();  //10
                ...
                markingInDt = Table.getValueAt(i, 4).toString().trim();   //11
                if (!markingInDt.equals("") && !EITLERPGLOBAL.isDate(markingInDt)) {
                ...
                if (EditMode == EITLERPGLOBAL.ADD) {
                    if (ObjFeltMarking.checkPieceNoInDB(pieceNo)) {
                        JOptionPane.showMessageDialog(frmFeltMarking.this, "Piece No." + pieceNo + " already Needled", " Piece No Already Exists", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
                ...
                // check piece no is already needled?
                if (EditMode == EITLERPGLOBAL.EDIT) {
                    if (ObjFeltMarking.checkPieceNoInDB(pieceNo, productionDate)) {
                        JOptionPane.showMessageDialog(frmFeltMarking.this, "Piece No." + pieceNo + " already Needled.", " Piece No Already Exists", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
                ...
                ObjFeltMarkingDetails.setAttribute("PRODUCTION_PIECE_NO", pieceNo);
                ObjFeltMarkingDetails.setAttribute("PRODUCTION_PARTY_CODE", partyCode);
                ObjFeltMarkingDetails.setAttribute("WEIGHT", weight);
                ObjFeltMarkingDetails.setAttribute("REMARKS", remarks);
//                ObjFeltMarkingDetails.setAttribute("DRYER", dryer);
                ObjFeltMarkingDetails.setAttribute("MARKING_OUT_DATE", markingOutDt);
                ObjFeltMarkingDetails.setAttribute("MARKING_IN_DATE", markingInDt);
                ObjFeltMarkingDetails.setAttribute("SHIFT_ID", shiftId);
                

                ObjFeltMarking.hmFeltNeedlingDetails.put(Integer.toString(ObjFeltMarking.hmFeltNeedlingDetails.size() + 1), ObjFeltMarkingDetails);
            }
        ...
        ObjFeltMarking.setAttribute("PRODUCTION_DATE", productionDate);
        ObjFeltMarking.setAttribute("PRODUCTION_DOCUMENT_NO", productionDocumentNo);
        ObjFeltMarking.setAttribute("PRODUCTION_FORM_NO", formNo);
        SetData();

        if (EditMode == EITLERPGLOBAL.ADD) {
            if (ObjFeltMarking.Insert()) {
                DisplayData();
                ...
                if (OpgFinal.isSelected()) {
                    try {
                        ...
                        String responce = JavaMail.sendNotificationMailOfDetail(706, DOC_NO, DOC_DATE, Party_Code, EITLERPGLOBAL.gNewUserID, EITLERPGLOBAL.getComboCode(cmbHierarchy), false);
                ...
                JOptionPane.showMessageDialog(this, "Error occured while saving. Error is " + ObjFeltMarking.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }

        if (EditMode == EITLERPGLOBAL.EDIT) {
            if (ObjFeltMarking.Update()) {
                DisplayData();
                ...
                JOptionPane.showMessageDialog(this, "Error occured while saving editing. Error is " + ObjFeltMarking.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
    ...
    private void SetData() {
        ...
        ObjFeltMarking.setAttribute("HIERARCHY_ID", EITLERPGLOBAL.getComboCode(cmbHierarchy));
        ObjFeltMarking.setAttribute("FROM", EITLERPGLOBAL.gNewUserID);
        ObjFeltMarking.setAttribute("TO", EITLERPGLOBAL.getComboCode(cmbSendTo));
        ObjFeltMarking.setAttribute("FROM_REMARKS", txtToRemarks.getText().trim());
        ObjFeltMarking.setAttribute("UPDATED_BY", EITLERPGLOBAL.getComboCode(cmbUserName));

        if (OpgApprove.isSelected()) {
            ObjFeltMarking.setAttribute("APPROVAL_STATUS", "A");
        }

        if (OpgFinal.isSelected()) {
            ObjFeltMarking.setAttribute("APPROVAL_STATUS", "F");
        }

        if (OpgReject.isSelected()) {
            ObjFeltMarking.setAttribute("APPROVAL_STATUS", "R");
            ObjFeltMarking.setAttribute("SEND_DOC_TO", EITLERPGLOBAL.getComboCode(cmbSendTo));
        }

        if (OpgHold.isSelected()) {
            ObjFeltMarking.setAttribute("APPROVAL_STATUS", "H");
        }
        //-------------------------------------------------//

        if (EditMode == EITLERPGLOBAL.ADD) {
            ObjFeltMarking.setAttribute("CREATED_BY", EITLERPGLOBAL.gNewUserID);
        } else {
            ObjFeltMarking.setAttribute("CREATED_BY", (int) ObjFeltMarking.getAttribute("CREATED_BY").getVal());
            ObjFeltMarking.setAttribute("CREATED_DATE", ObjFeltMarking.getAttribute("CREATED_DATE").getString());
            ObjFeltMarking.setAttribute("MODIFIED_BY", EITLERPGLOBAL.gNewUserID);
        }
    }
    ...
    private void Find() {
        ...
        frmFindFeltMarking ObjFindFeltNeedling = (frmFindFeltMarking) ObjLoader.getObj();

        if (ObjFindFeltNeedling.Cancelled == false) {
            if (!ObjFeltMarking.Filter(ObjFindFeltNeedling.stringFindQuery)) {
                JOptionPane.showMessageDialog(frmFeltMarking.this, " No records found.", "Find Felt Needling Details", JOptionPane.YES_OPTION);
            }
            MoveLast();
        }
    }
    ...
    public void Find(String prodDate) {
        ObjFeltMarking.Filter("AND PROD_DATE='" + prodDate + "'");
        SetMenuForRights();
        DisplayData();
    }
    ...
    public void Find(String pieceNo, String prodDate) {
        ObjFeltMarking.Filter("AND PROD_PIECE_NO='" + pieceNo + "'");
        SetMenuForRights();
        DisplayData();
    }
    ...
    public void FindWaiting() {
        ObjFeltMarking.Filter(" AND PROD_DOC_NO IN (SELECT DISTINCT PROD_DOC_NO FROM PRODUCTION.FELT_PROD_DATA, PRODUCTION.FELT_PROD_DOC_DATA WHERE PROD_DOC_NO=DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=706 AND CANCELED=0) ");
        SetMenuForRights();
        DisplayData();
    }

    private void MoveFirst() {
        ObjFeltMarking.MoveFirst();
        DisplayData();
    }

    private void MovePrevious() {
        ObjFeltMarking.MovePrevious();
        DisplayData();
    }

    private void MoveNext() {
        ObjFeltMarking.MoveNext();
        DisplayData();
    }

    private void MoveLast() {
        ObjFeltMarking.MoveLast();
        DisplayData();
    }
