...
public class frmFeltHeatSetting extends javax.swing.JApplet {
    ...
    private clsFeltHeatSetting ObjFeltHeatSetting;
    ...
    /**
     * Creates new form frmFeltHeatSetting
     */
    public void init() {
        System.gc();
        setSize(830, 590);
        ...
        ObjFeltHeatSetting = new clsFeltHeatSetting();
        ...
        if (ObjFeltHeatSetting.LoadData()) {
            DisplayData();
        } else {
            JOptionPane.showMessageDialog(this, "Error occured while Loading Data. Error is " + ObjFeltHeatSetting.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void initComponents() {
        ...
        lblTitle.setText(" FELT PRODUCTION HEAT SETTING DETAILS - ");
        ...
        Tab1.add(txtFormNo);
        txtFormNo.setBounds(480, 10, 50, 21);
        ...
        Tab1.add(jLabel13);
        jLabel13.setBounds(550, 10, 40, 20);
        ...
        Tab1.add(cmbShiftNo);
        cmbShiftNo.setBounds(600, 10, 130, 24);
        ...
        Tab1.add(txtShiftNo);
        txtShiftNo.setBounds(740, 10, 40, 20);

        Tab.addTab("Heat Setting Details", Tab1);
...
//                }
    ...
    private void cmdPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdPrintActionPerformed
        new TReportWriter.TReportEngine().PreviewReport("http://" + EITLERPGLOBAL.HostIP + "/EITLERP/Reports/Production/rptFeltHeatSetting.rpt", new HashMap(), ObjFeltHeatSetting.getReportData(EITLERPGLOBAL.formatDateDB(txtFeltProductionDate.getText().trim())));
    ...
    private void cmdPreviewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdPreviewActionPerformed
        /*
         try {
         URL reportFile = new URL("http://" + EITLERPGLOBAL.HostIP + "/EITLERP/Reports/Production/rptFeltHeatSetting.jsp?dbURL=" + EITLERPGLOBAL.DatabaseURL + "&PROD_DATE=" + EITLERPGLOBAL.formatDateDB(txtFeltProductionDate.getText()));
        ...
        try {
            ...
            String strSQL = "SELECT PROD_DATE,PROD_FORM_NO,PROD_DOC_NO,PROD_PIECE_NO,STYLE,GRP,SYN_PER,\n"
                    ...
                    + "DAY_WEIGHT,TOTAL_WEIGHT,SHIFT_ID,DRYER,HEAT_SETTING_IN_DATE\n"
                    + ",HEAT_SETTING_OUT_DATE FROM \n"
                    + "(SELECT PROD_DATE,PROD_FORM_NO,PROD_DOC_NO,REPLACE(PROD_PIECE_NO,' ','') \n"
                    + "PROD_PIECE_NO,PROD_PARTY_CODE,WEIGHT,A.GROUP GRP,SHIFT_ID,DRYER,HEAT_SETTING_IN_DATE\n"
                    + ",HEAT_SETTING_OUT_DATE FROM PRODUCTION.FELT_PROD_DATA A \n"
                    + "WHERE PROD_DOC_NO = '"+txtProductionDocumentNo.getText().trim()+"' AND PROD_DEPT='HEAT_SETTING') D \n"
    ...
    private void cmdBackToNormalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdBackToNormalActionPerformed
        ObjFeltHeatSetting.HistoryView = false;
        ObjFeltHeatSetting.LoadData();
        MoveLast();
    ...
    private void cmdViewRevisionsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdViewRevisionsActionPerformed
        ObjFeltHeatSetting.ShowHistory(EITLERPGLOBAL.formatDateDB(txtFeltProductionDate.getText()), txtProductionDocumentNo.getText());
        MoveLast();
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            GenerateRejectedSendToCombo();
            if (clsFeltProductionApprovalFlow.IsOnceRejectedDoc(704, txtProductionDocumentNo.getText())) {
                cmbSendTo.setEnabled(true);
            } else {
                cmbSendTo.setEnabled(false);
            }
    ...
    private void cmdAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdAddActionPerformed
        if(txtShiftNo.getText().equals("0")){
            JOptionPane.showMessageDialog(this, "Please Select Shift before entering data", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }
            ...
            if ((EditMode == 1) || (EditMode == 2 && clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID))) {
                if (Table.getSelectedColumn() == 1) {
                    ...
                    aList.SQL = "SELECT WIP_EXT_PIECE_NO AS PIECE_NO,WIP_PIECE_NO AS PIECE,WIP_PARTY_CODE AS PARTY_CODE,PARTY_NAME,"
                            ...
                            + " AND WIP_PIECE_STAGE='HEAT_SETTING' \n"
                            + " AND WIP_STATUS='MENDED'";
                    ...
                    if (aList.ShowLOV()) {
                        if (!data.getStringValueFromDB("SELECT PD.PIECE_NO FROM PRODUCTION.FELT_SALES_PIECE_AMENDMENT_HEADER_WIP PH, "
                            ...
                            try {
                                ...
                                Table.setValueAt(ocYearMonth, Table.getSelectedRow(), 8); //5                               
                                String sql = "SELECT WIP_PARTY_CODE,COALESCE(WIP_MENDING_WEIGHT_A,0)+COALESCE(WIP_MENDING_WEIGHT_B,0) AS WIP_MENDING_WEIGHT,"
                                        + " WIP_PRODUCT_CODE,WIP_GROUP,WIP_GSM,WIP_STYLE FROM PRODUCTION.FELT_WIP_PIECE_REGISTER "
                                ...
                                if (trs.getRow() > 0) {
                                    ...
                                    Table.setValueAt(trs.getString("WIP_MENDING_WEIGHT"), Table.getSelectedRow(), 3);
...
try {
    ...
    private void DisplayData() {
        ...
        if (EITLERPGLOBAL.gAuthorityUserID != EITLERPGLOBAL.gUserID) {
            if (clsAuthority.IsAuthorityGiven(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gUserID, EITLERPGLOBAL.gAuthorityUserID, 704)) {
        ...
        try {
            if (EditMode == 0) {
                if (ObjFeltHeatSetting.getAttribute("APPROVED").getInt() == 1) {
                    lblTitle.setBackground(Color.BLUE);
                } else {
                    lblTitle.setBackground(Color.GRAY);
                }

                /*if(ObjFeltHeatSetting.getAttribute("CANCELED").getInt()==1) {
                 lblTitle.setBackground(Color.RED);
                 }*/
            }
            //============================================//

            String productionDate = EITLERPGLOBAL.formatDate(ObjFeltHeatSetting.getAttribute("PRODUCTION_DATE").getString());
            String documentNo = ObjFeltHeatSetting.getAttribute("PRODUCTION_DOCUMENT_NO").getString();
            lblTitle.setText(" FELT PRODUCTION HEAT SETTING DETAILS - " + productionDate);
            lblRevNo.setText(Integer.toString((int) ObjFeltHeatSetting.getAttribute("REVISION_NO").getVal()));
            txtMonthWeight.setText(Double.toString(EITLERPGLOBAL.round(ObjFeltHeatSetting.getAttribute("TOTAL_WEIGHT").getVal(), 2)));
            txtPreviousWeight.setText(Double.toString(EITLERPGLOBAL.round(ObjFeltHeatSetting.getAttribute("PREVIOUS_WEIGHT").getVal(), 2)));
            EITLERPGLOBAL.setComboIndex(cmbHierarchy, (int) ObjFeltHeatSetting.getAttribute("HIERARCHY_ID").getVal());
            DoNotEvaluate = true;

            txtFeltProductionDate.setText(productionDate);
            txtProductionDocumentNo.setText(documentNo);
            txtFormNo.setText(ObjFeltHeatSetting.getAttribute("PRODUCTION_FORM_NO").getString());
            txtShiftNo.setText(ObjFeltHeatSetting.getAttribute("PRODUCTION_SHIFT_NO").getString());
            
            
            FormatGrid();
            //Now Generate Table
            for (int i = 1; i <= ObjFeltHeatSetting.hmFeltHeatSettingDetails.size(); i++) {
                clsFeltHeatSettingDetails ObjFeltHeatSettingDetails = (clsFeltHeatSettingDetails) ObjFeltHeatSetting.hmFeltHeatSettingDetails.get(Integer.toString(i));

                Object[] rowData = new Object[15];
                rowData[0] = Integer.toString(i);
                String PieceNo = ObjFeltHeatSettingDetails.getAttribute("PRODUCTION_PIECE_NO").getString();
                rowData[1] = ObjFeltHeatSettingDetails.getAttribute("PRODUCTION_PIECE_NO").getString();
                rowData[2] = ObjFeltHeatSettingDetails.getAttribute("PRODUCTION_PARTY_CODE").getString();
                rowData[3] = Double.toString(EITLERPGLOBAL.round(ObjFeltHeatSettingDetails.getAttribute("WEIGHT").getVal(), 2));
                if (ObjFeltHeatSettingDetails.getAttribute("HEAT_SETTING_IN_DATE").getString().equals("")) {
                    rowData[4] = "";  //11
                } else {
                    rowData[4] = EITLERPGLOBAL.formatDate(ObjFeltHeatSettingDetails.getAttribute("HEAT_SETTING_IN_DATE").getString());  //11
                }
                if (ObjFeltHeatSettingDetails.getAttribute("HEAT_SETTING_OUT_DATE").getString().equals("")) {
                    rowData[5] = "";  //10
                } else {
                    rowData[5] = EITLERPGLOBAL.formatDate(ObjFeltHeatSettingDetails.getAttribute("HEAT_SETTING_OUT_DATE").getString());  //10
                }
                rowData[6] = ObjFeltHeatSettingDetails.getAttribute("SHIFT_ID").getString();                 
                rowData[7] = ObjFeltHeatSettingDetails.getAttribute("REMARKS").getString(); 
                
                //String ocYearMonth=data.getStringValueFromDB("SELECT PR_OC_MONTHYEAR FROM PRODUCTION.FELT_SALES_PIECE_REGISTER WHERE PR_PIECE_NO='"+PieceNo+"'");
                String ocYearMonth=data.getStringValueFromDB("SELECT WIP_OC_MONTHYEAR FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_EXT_PIECE_NO='"+PieceNo+"'");
                rowData[8] = ocYearMonth;
                
                rowData[9] = ObjFeltHeatSettingDetails.getAttribute("DRYER").getString(); 
                
                rowData[10] = data.getStringValueFromDB("SELECT WIP_PRODUCT_CODE FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_EXT_PIECE_NO='" + ObjFeltHeatSettingDetails.getAttribute("PRODUCTION_PIECE_NO").getString() + "'"); //6
                rowData[11] = data.getStringValueFromDB("SELECT WIP_GROUP FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_EXT_PIECE_NO='" + ObjFeltHeatSettingDetails.getAttribute("PRODUCTION_PIECE_NO").getString() + "'"); //7                
                rowData[12] = data.getStringValueFromDB("SELECT WIP_STYLE FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_EXT_PIECE_NO='" + ObjFeltHeatSettingDetails.getAttribute("PRODUCTION_PIECE_NO").getString() + "'");  //9
            ...
            hmList = clsFeltProductionApprovalFlow.getDocumentFlow(704, documentNo);
            ...
            FormatGridUpdateHistory();
            HashMap hmApprovalHistory = clsFeltHeatSetting.getHistoryList(ObjFeltHeatSetting.getAttribute("PRODUCTION_DATE").getString(), txtProductionDocumentNo.getText());
            for (int i = 1; i <= hmApprovalHistory.size(); i++) {
                clsFeltHeatSetting ObjFeltHeatSetting = (clsFeltHeatSetting) hmApprovalHistory.get(Integer.toString(i));
                Object[] rowData = new Object[6];

                rowData[0] = Integer.toString((int) ObjFeltHeatSetting.getAttribute("REVISION_NO").getVal());
                rowData[1] = FeltUser.getUserName((int) ObjFeltHeatSetting.getAttribute("UPDATED_BY").getVal());
                rowData[2] = EITLERPGLOBAL.formatDate(ObjFeltHeatSetting.getAttribute("ENTRY_DATE").getString()) + ObjFeltHeatSetting.getAttribute("ENTRY_DATE").getString().substring(10, 19);

                String ApprovalStatus = "";

                if ((ObjFeltHeatSetting.getAttribute("APPROVAL_STATUS").getString()).equals("H")) {
                    ApprovalStatus = "Hold";
                }

                if ((ObjFeltHeatSetting.getAttribute("APPROVAL_STATUS").getString()).equals("A")) {
                    ApprovalStatus = "Approved";
                }

                if ((ObjFeltHeatSetting.getAttribute("APPROVAL_STATUS").getString()).equals("F")) {
                    ApprovalStatus = "Final Approved";
                }

                if ((ObjFeltHeatSetting.getAttribute("APPROVAL_STATUS").getString()).equals("W")) {
                    ApprovalStatus = "Waiting";
                }

                if ((ObjFeltHeatSetting.getAttribute("APPROVAL_STATUS").getString()).equals("R")) {
                    ApprovalStatus = "Rejected";
                }

                if ((ObjFeltHeatSetting.getAttribute("APPROVAL_STATUS").getString()).equals("P")) {
                    ApprovalStatus = "Pending";
                }

                if ((ObjFeltHeatSetting.getAttribute("APPROVAL_STATUS").getString()).equals("C")) {
                    ApprovalStatus = "Skiped";
                }

                rowData[3] = ApprovalStatus;
                rowData[4] = ObjFeltHeatSetting.getAttribute("APPROVER_REMARKS").getString();
                rowData[5] = ObjFeltHeatSetting.getAttribute("FROM_IP").getString();
    ...
    private void FormatGrid() {
        try {
            ...
            DataModel.addColumn("Heat Setting In Date");  //4
            DataModel.addColumn("Heat Setting Out Date");   //5
            DataModel.addColumn("Shift");  //6            
            DataModel.addColumn("Remarks");  //7
            
            DataModel.addColumn("OC MnthYr");  //8
            
            DataModel.addColumn("Dryer");  //9
            ...
            DataModel.SetReadOnly(10); //7
            DataModel.SetReadOnly(11); //8
            DataModel.SetReadOnly(12); //9
            ...
            Renderer.setCustomComponent(5, "ComboBox");
            Renderer.setCustomComponent(5, cmbCopyType);

            Table.getColumnModel().getColumn(9).setCellEditor(new DefaultCellEditor(cmbCopyType));  //5
    ...
    private void GenerateHierarchyCombo() {
        ...
        hmHierarchyList = clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=704 ");

        if (EditMode == EITLERPGLOBAL.EDIT) {
            hmHierarchyList = clsHierarchy.getList(" WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=704 ");
        }
    ...
    private void GenerateSendToCombo() {
        ...
        try {
            ...
            if (EditMode == EITLERPGLOBAL.ADD) {
                ...
                hmSendToList = clsFeltProductionApprovalFlow.getRemainingUsers(704, ObjFeltHeatSetting.getAttribute("PRODUCTION_DOCUMENT_NO").getString());
                for (int i = 1; i <= hmSendToList.size(); i++) {
                    clsUser ObjUser = (clsUser) hmSendToList.get(Integer.toString(i));
                    ComboData aData = new ComboData();
                    aData.Code = (int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void GenerateRejectedSendToCombo() {
        ...
        for (int i = 1; i <= hmRejectedSendToList.size(); i++) {
            ...
            if (EditMode == EITLERPGLOBAL.EDIT) {
                if (OpgApprove.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInApproval(704, txtProductionDocumentNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }

                if (OpgReject.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInRejection(704, txtProductionDocumentNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            int Creator = clsFeltProductionApprovalFlow.getCreator(704, txtProductionDocumentNo.getText());
    ...
    private void GenerateUserNameCombo() {
        ...
        hmUserNameList = ObjFeltHeatSetting.getUserNameList(EITLERPGLOBAL.getComboCode(cmbHierarchy), EITLERPGLOBAL.gNewUserID, "HEATSETTING");
    ...
    private void GenerateShiftNoCombo() {
        ...
        try {
            ...
            while (!rs.isAfterLast()) {
                ...
                combodata.Text = rs.getString("PARA_CODE");
    ...
    private void SetupApproval() {
        /*// --- Hierarchy Change Rights Check --------
         if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6082,75)) {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = clsFeltProductionApprovalFlow.getFromID(704, ObjFeltHeatSetting.getAttribute("PRODUCTION_DOCUMENT_NO").getString());
            lnFromUserId = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = clsFeltProductionApprovalFlow.getFromRemarks(704, FromUserID, ObjFeltHeatSetting.getAttribute("PRODUCTION_DOCUMENT_NO").getString());
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            cmbHierarchy.setEnabled(false);
            if (clsFeltProductionApprovalFlow.IsCreator(704, txtProductionDocumentNo.getText())) {
                OpgReject.setEnabled(false);
            }
        }
    ...
    private void SetMenuForRights() {
        // --- Add Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6082, 60821)) {
            cmdNew.setEnabled(true);
        } else {
            cmdNew.setEnabled(false);
        }
        ...
        // --- Edit Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6082, 60822)) {
            cmdEdit.setEnabled(true);
        } else {
            cmdEdit.setEnabled(false);
        }
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6082, 60823)) {
            cmdDelete.setEnabled(true);
        } else {
            cmdDelete.setEnabled(false);
        }
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6082, 60825)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        } else {
            cmdPreview.setEnabled(false);
    ...
    private void Edit() {
        String productionDocumentNo = (String) ObjFeltHeatSetting.getAttribute("PRODUCTION_DOCUMENT_NO").getObj();
        if (ObjFeltHeatSetting.IsEditable(productionDocumentNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(704, productionDocumentNo)) {
                SetFields(true);
                txtFeltProductionDate.setEnabled(false);
                cmbShiftNo.setEnabled(false);
                txtFormNo.setEnabled(false);
    ...
    private void Delete() {
        if (ObjFeltHeatSetting.CanDelete(txtProductionDocumentNo.getText(), txtFeltProductionDate.getText(), EITLERPGLOBAL.gNewUserID)) {
            DisplayData();
        } else {
            JOptionPane.showMessageDialog(this, ObjFeltHeatSetting.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void Save() {
        ...
        String heatOutDt = "", heatInDt = "", shiftId = "" , startTime = "", endTime = "" , dryer = "";
        ...
        // check duplicate piece no in table
            ...
            int k=0;
                for (int l = k; l <= Table.getRowCount() - 1; l++) {
                    if (l != k && !((String) Table.getValueAt(k, 6)).trim().equals(((String) Table.getValueAt(l, 6)).trim())) {
                        JOptionPane.showMessageDialog(this, "Different Shift at Row " + (k + 1) + " and " + (l + 1), "ERROR", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
            //}
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            productionDocumentNo = "HS"+txtShiftNo.getText()+ productionDate.substring(6, 10) + productionDate.substring(3, 5) + productionDate.substring(0, 2);
        }else{
            productionDocumentNo = txtProductionDocumentNo.getText();            
        }     
        

        // CHECK Production HeatSetting Date already exist in database?
        if (EditMode == EITLERPGLOBAL.ADD) {
            //if (ObjFeltHeatSetting.checkProductionDateInDB(EITLERPGLOBAL.formatDateDB(productionDate))) {
            if (ObjFeltHeatSetting.checkProductionDocumentNoInDB(productionDocumentNo)) {
                JOptionPane.showMessageDialog(frmFeltHeatSetting.this, "Production Document " + productionDocumentNo + " already Exists.", "Document Already Exists", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
        ...
        if (Table.getRowCount() <= 0) {
            JOptionPane.showMessageDialog(this, "Enter HeatSetting Details Before Saving.", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }

        try {
            // check duplicate piece no in table
            for (int m = 0; m <= Table.getRowCount() - 1; m++) {
                for (int l = m; l <= Table.getRowCount() - 1; l++) {
                    if (l != m && ((String) Table.getValueAt(m, 1)).trim().equals(((String) Table.getValueAt(l, 1)).trim())) {
                        JOptionPane.showMessageDialog(this, "Same Piece No at Row " + (m + 1) + " and " + (l + 1), "ERROR", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
            }

            ObjFeltHeatSetting.hmFeltHeatSettingDetails.clear();
            ...
            for (i = 0; i <= Table.getRowCount() - 1; i++) {
                ...
                heatOutDt = Table.getValueAt(i, 5).toString().trim();  //10
                if (!heatOutDt.equals("") && !EITLERPGLOBAL.isDate(heatOutDt)) {
                    JOptionPane.showMessageDialog(this, "Enter Valid Heat Out Date.", "ERROR", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                
                heatInDt = Table.getValueAt(i, 4).toString().trim();   //11
                if (!heatInDt.equals("") && !EITLERPGLOBAL.isDate(heatInDt)) {
                    JOptionPane.showMessageDialog(this, "Enter Valid Heat In Date.", "ERROR", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                
                shiftId = ((String) Table.getValueAt(i, 6)).trim();                
                remarks = ((String) Table.getValueAt(i, 7)).trim();

                // check piece no is already heatset?
                if (EditMode == EITLERPGLOBAL.ADD) {
                    if (ObjFeltHeatSetting.checkPieceNoInDB(pieceNo)) {
                        JOptionPane.showMessageDialog(frmFeltHeatSetting.this, "Piece No." + pieceNo + " already HeatSet", " Piece No Already Exists", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
                ...
                if (EditMode == EITLERPGLOBAL.EDIT) {
                    //if (ObjFeltHeatSetting.checkPieceNoInDB(pieceNo, productionDate)) {
                    if (ObjFeltHeatSetting.checkPieceNoInDB(pieceNo, productionDocumentNo)) {
                        JOptionPane.showMessageDialog(frmFeltHeatSetting.this, "Piece No." + pieceNo + " already HeatSet.", " Piece No Already Exists", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
                ...
                ObjFeltHeatSettingDetails.setAttribute("PRODUCTION_PIECE_NO", pieceNo);
                ObjFeltHeatSettingDetails.setAttribute("PRODUCTION_PARTY_CODE", partyCode);
                ObjFeltHeatSettingDetails.setAttribute("WEIGHT", weight);
                ObjFeltHeatSettingDetails.setAttribute("REMARKS", remarks);
                ObjFeltHeatSettingDetails.setAttribute("DRYER", dryer);
                ObjFeltHeatSettingDetails.setAttribute("HEAT_SETTING_OUT_DATE", heatOutDt);
                ObjFeltHeatSettingDetails.setAttribute("HEAT_SETTING_IN_DATE", heatInDt);
                ObjFeltHeatSettingDetails.setAttribute("SHIFT_ID", shiftId);
                

                ObjFeltHeatSetting.hmFeltHeatSettingDetails.put(Integer.toString(ObjFeltHeatSetting.hmFeltHeatSettingDetails.size() + 1), ObjFeltHeatSettingDetails);
            }
        ...
        ObjFeltHeatSetting.setAttribute("PRODUCTION_DATE", productionDate);
        ObjFeltHeatSetting.setAttribute("PRODUCTION_DOCUMENT_NO", productionDocumentNo);
        ObjFeltHeatSetting.setAttribute("PRODUCTION_FORM_NO", formNo);
        SetData();

        if (EditMode == EITLERPGLOBAL.ADD) {
            if (ObjFeltHeatSetting.Insert()) {
                DisplayData();
                ...
                if (OpgFinal.isSelected()) {
                    try {
                        ...
                        String responce = JavaMail.sendNotificationMailOfDetail(704, DOC_NO, DOC_DATE, Party_Code, EITLERPGLOBAL.gNewUserID, EITLERPGLOBAL.getComboCode(cmbHierarchy), false);
                ...
                JOptionPane.showMessageDialog(this, "Error occured while saving. Error is " + ObjFeltHeatSetting.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }

        if (EditMode == EITLERPGLOBAL.EDIT) {
            if (ObjFeltHeatSetting.Update()) {
                DisplayData();
                ...
                JOptionPane.showMessageDialog(this, "Error occured while saving editing. Error is " + ObjFeltHeatSetting.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
    ...
    private void SetData() {
        ...
        ObjFeltHeatSetting.setAttribute("HIERARCHY_ID", EITLERPGLOBAL.getComboCode(cmbHierarchy));
        ObjFeltHeatSetting.setAttribute("FROM", EITLERPGLOBAL.gNewUserID);
        ObjFeltHeatSetting.setAttribute("TO", EITLERPGLOBAL.getComboCode(cmbSendTo));
        ObjFeltHeatSetting.setAttribute("FROM_REMARKS", txtToRemarks.getText().trim());
        ObjFeltHeatSetting.setAttribute("UPDATED_BY", EITLERPGLOBAL.getComboCode(cmbUserName));

        if (OpgApprove.isSelected()) {
            ObjFeltHeatSetting.setAttribute("APPROVAL_STATUS", "A");
        }

        if (OpgFinal.isSelected()) {
            ObjFeltHeatSetting.setAttribute("APPROVAL_STATUS", "F");
        }

        if (OpgReject.isSelected()) {
            ObjFeltHeatSetting.setAttribute("APPROVAL_STATUS", "R");
            ObjFeltHeatSetting.setAttribute("SEND_DOC_TO", EITLERPGLOBAL.getComboCode(cmbSendTo));
        }

        if (OpgHold.isSelected()) {
            ObjFeltHeatSetting.setAttribute("APPROVAL_STATUS", "H");
        }
        //-------------------------------------------------//

        if (EditMode == EITLERPGLOBAL.ADD) {
            ObjFeltHeatSetting.setAttribute("CREATED_BY", EITLERPGLOBAL.gNewUserID);
        } else {
            ObjFeltHeatSetting.setAttribute("CREATED_BY", (int) ObjFeltHeatSetting.getAttribute("CREATED_BY").getVal());
            ObjFeltHeatSetting.setAttribute("CREATED_DATE", ObjFeltHeatSetting.getAttribute("CREATED_DATE").getString());
            ObjFeltHeatSetting.setAttribute("MODIFIED_BY", EITLERPGLOBAL.gNewUserID);
        }
    }
    ...
    private void Find() {
        Loader ObjLoader = new Loader(this, "EITLERP.Production.FeltHeatSetting.frmFindFeltHeatSetting", true);
        frmFindFeltHeatSetting ObjFindFeltHeatSetting = (frmFindFeltHeatSetting) ObjLoader.getObj();

        if (ObjFindFeltHeatSetting.Cancelled == false) {
            if (!ObjFeltHeatSetting.Filter(ObjFindFeltHeatSetting.stringFindQuery)) {
                JOptionPane.showMessageDialog(frmFeltHeatSetting.this, " No records found.", "Find Felt HeatSetting Details", JOptionPane.YES_OPTION);
            }
            MoveLast();
        }
    }
    ...
    public void Find(String prodDate) {
        ObjFeltHeatSetting.Filter("AND PROD_DOC_NO='" + prodDate + "'");
        SetMenuForRights();
        DisplayData();
    }
    ...
    public void Find(String pieceNo, String prodDate) {
        ObjFeltHeatSetting.Filter("AND PROD_PIECE_NO='" + pieceNo + "'");
        SetMenuForRights();
        DisplayData();
    }
    ...
    public void FindWaiting() {
        ObjFeltHeatSetting.Filter(" AND PROD_DOC_NO IN (SELECT DISTINCT PROD_DOC_NO FROM PRODUCTION.FELT_PROD_DATA, PRODUCTION.FELT_PROD_DOC_DATA WHERE PROD_DOC_NO=DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=704 AND CANCELED=0) ");
        SetMenuForRights();
        DisplayData();
    }

    private void MoveFirst() {
        ObjFeltHeatSetting.MoveFirst();
        DisplayData();
    }

    private void MovePrevious() {
        ObjFeltHeatSetting.MovePrevious();
        DisplayData();
    }

    private void MoveNext() {
        ObjFeltHeatSetting.MoveNext();
        DisplayData();
    }

    private void MoveLast() {
        ObjFeltHeatSetting.MoveLast();
        DisplayData();
    }
