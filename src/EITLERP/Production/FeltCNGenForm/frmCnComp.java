...
public class frmCnComp extends javax.swing.JApplet {
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        lblTitle.setText("CREDIT NOTE for Compensation");
        ...
        jLabel2.setText("Compensation Data Process");
        ...
        jButton3.setText("2. CN COMP DETAIL");
        ...
        jButton5.setText("3. COMP CREDIT NOTE ");
        ...
        jButton4.setText("1. CN COMP SUMMARY");
    ...
    private void jButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton6ActionPerformed
        ...
        try {
            ...
            if(data.IsRecordExist("SELECT VOUCHER_NO FROM D_FIN_VOUCHER_HEADER WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+"  AND LEFT(VOUCHER_NO,2) = 'CN'",FinanceGlobal.FinURL)) {
                ...
                ResultSet rsSchemeHeader =Stmt.executeQuery("SELECT CNH_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_YEAR_MON_ID ='"+t+"' AND CNH_TYPE ='COMP' ORDER BY CNH_SUB_ACCOUNT_CODE");
                ...
                if(rsSchemeHeader.getRow()>0){
                    while(!rsSchemeHeader.isAfterLast()){
                        ...
                        String SQL1 ="UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_DRAFT_CR_NOTE_NO='"+draftNo+(draftSrNo+counter)+"',CNH_DRAFT_CR_NOTE_DATE='"+draftDate+"' WHERE CNH_TYPE ='COMP' AND CNH_YEAR_MON_ID ='"+t+"' AND  CNH_SUB_ACCOUNT_CODE='"+rsSchemeHeader.getString("CNH_SUB_ACCOUNT_CODE")+"'";//AND CNH_PERCENT='"+rsSchemeHeader.getString("CNH_PERCENT")+"'
...
//                        String SQL2 ="UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_FIN_VOUCHER_NO = CONCAT('CN',171700000 +ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))),CNH_LEGACY_NO = ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))  WHERE CNH_TYPE ='COMP' AND CNH_YEAR_MON_ID ='"+t+"' AND CNH_SUB_ACCOUNT_CODE='"+rsSchemeHeader.getString("CNH_SUB_ACCOUNT_CODE")+"' ";//AND CNH_PERCENT='"+rsSchemeHeader.getString("CNH_PERCENT")+"'
                        String SQL2 ="UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_FIN_VOUCHER_NO = CONCAT('CN','"+frmYr+"',1700000 +ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))),CNH_LEGACY_NO = ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))  WHERE CNH_TYPE ='COMP' AND CNH_YEAR_MON_ID ='"+t+"' AND CNH_SUB_ACCOUNT_CODE='"+rsSchemeHeader.getString("CNH_SUB_ACCOUNT_CODE")+"' ";//AND CNH_PERCENT='"+rsSchemeHeader.getString("CNH_PERCENT")+"'
    ...
    */
    ...
    private void GenerateCreditNoteDetail() {
        ...
        try {            
            ...
            String strSQL="SELECT * FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_TYPE='COMP' AND CND_YEAR_MON_ID='"+str+"' ORDER BY CND_SUB_ACCOUNT_CODE";
            ...
            objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/Production/rptCompCNDetail.rpt",Parameters,objData);                       
        }
        catch(Exception e) {
            e.printStackTrace();
        }
    ...
    private void GenerateCreditNote() {
       String str=IntervalComboModel.getText(cmbIntervalCombo.getSelectedIndex()).substring(0,8);
        ...
        try {            
            ...
            String strSQL="SELECT CNH_ID,CNH_NO,CNH_TYPE,CNH_MAIN_ACCOUNT_CODE,CNH_MAIN_CODE,CNH_SUB_ACCOUNT_CODE,CNH_CITY,CNH_PARTY_NAME,CNH_REASON_CODE,CNH_BOOK_CODE,CNH_PERCENT,ROUND(CNH_CREDIT_AMOUNT,0) AS CNH_CREDIT_AMOUNT,CNH_LINK_NO,CNH_YEAR_MON_ID,CNH_INTERVAL_CAPTION,CNH_INVOICE_AMOUNT,CNH_DRAFT_CR_NOTE_NO,CNH_DRAFT_CR_NOTE_DATE,CNH_EXT11,CNH_FIN_VOUCHER_NO,CNH_REMARKS FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_YEAR_MON_ID='"+str+"' AND CNH_TYPE='COMP' ORDER BY CNH_SUB_ACCOUNT_CODE";
            ResultSet  rsTemp =data.getResult(strSQL);                    
            rsTemp.first();            
            if(rsTemp.getRow()>0){
                while(!rsTemp.isAfterLast()){
            ...
            objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/Production/rptCompCNDraft.rpt",Parameters,objData);                       
        }
        catch(Exception e) {
            e.printStackTrace();
        }
    ...
    private void GenerateCreditNoteSummary() {
       ...
       String strto=IntervalComboModel.getText(cmbIntervalCombo.getSelectedIndex()).substring(30,40);
        ...
        try {            
            ...
            String strSQL="SELECT CNH_ID,CNH_NO,CNH_TYPE,CNH_MAIN_ACCOUNT_CODE,CNH_MAIN_CODE,CNH_SUB_ACCOUNT_CODE,CNH_CITY,CNH_PARTY_NAME,CNH_REASON_CODE,CNH_BOOK_CODE,CNH_PERCENT,ROUND(CNH_CREDIT_AMOUNT,0) AS CNH_CREDIT_AMOUNT,CNH_LINK_NO,CNH_YEAR_MON_ID,CNH_INTERVAL_CAPTION,CNH_INVOICE_AMOUNT FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_YEAR_MON_ID='"+str+"' AND CNH_TYPE='COMP' ORDER BY CNH_SUB_ACCOUNT_CODE";
            ResultSet  rsTemp =data.getResult(strSQL);                    
            rsTemp.first();            
            if(rsTemp.getRow()>0){
                while(!rsTemp.isAfterLast()){
            ...
            Parameters.put("FROM_DATE",strfrom);
            Parameters.put("TO_DATE",strto);
            objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/Production/rptCompCNSummary.rpt",Parameters,objData);                       
        }
        catch(Exception e) {
            e.printStackTrace();
        }
    ...
    private void generateIntervalCombo(){
        ...
        try{
            ...
            ResultSet rs=data.getResult("SELECT DISTINCT CND_YEAR_MON_ID,CND_INTERVAL_CAPTION FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_TYPE ='COMP' ORDER BY CND_YEAR_MON_ID DESC ");
            while(!rs.isAfterLast()){
    ...
    */
    ...
    private void GenerateCompCN(){
        ...
        try{
            ...
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTETEST SET LCN_YEAR_MON_ID=CONCAT('CM','"+fyr+"','"+tyr+"','"+lstno+"')"); //WHERE LCN_INVOICE_DATE>='" + frmdt + "' AND LCN_INVOICE_DATE<='" + todt + "'");
            ...
            stmt.execute("DELETE FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID = CONCAT('CM','"+fyr+"','"+tyr+"','"+lstno+"')");
            ...
            stmt.execute("UPDATE DINESHMILLS.D_SAL_PARTY_MASTER,PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_PARTY_NAME = PARTY_NAME,CNH_CITY = CITY_ID WHERE CNH_SUB_ACCOUNT_CODE = PARTY_CODE AND CNH_YEAR_MON_ID = CONCAT('CM','"+fyr+"','"+tyr+"','"+lstno+"') ");
    ...
    */
     ...
     public String getNextNo(String toyr,String frmyr) {
        ...
        try {
            ...
            strSQL="SELECT MAX(CND_YEAR_MON_ID) AS L_NO FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_TYPE='COMP' AND CND_LINK_YEAR=CONCAT('"+frmyr+"','/','"+toyr+"')";
