...
public class frmCnYearEnd extends javax.swing.JApplet {
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        lblTitle.setText("CREDIT NOTE for YEAR END DISCOUNT");
        ...
        jLabel2.setText("Year End Discount Data Process");
        ...
        jButton3.setText("2. CN YEAR END DETAIL");
        ...
        jButton5.setText("3.YEAR END CREDIT NOTE ");
        ...
        jButton4.setText("1. CN YEAR END SUMMARY");
        ...
        jLabel6.setText("Link No :");
        getContentPane().add(jLabel6);
        jLabel6.setBounds(210, 380, 85, 15);

        getContentPane().add(txtLinkNo);
        txtLinkNo.setBounds(300, 380, 100, 19);
        ...
        try {
            ...
            //   rsResultSet=Stmt.executeQuery("SELECT MAX(VOUCHER_NO) AS VOUCHER_NO,SUBSTRING(VOUCHER_NO,1,4) AS CNV,RIGHT(VOUCHER_NO,5) AS CNSRNO FROM FINANCE.D_FIN_VOUCHER_HEADER WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID +"  AND LEFT(VOUCHER_NO,2) = 'CN' AND SUBSTRING(VOUCHER_NO,5,2)=14 ");
              ...
              //  txtVoucherNo.setText(Lastcnsrno);
                ...
                //String lnkn=data.getStringValueFromDB("SELECT MAX(CND_YEAR_MON_ID) AS L_NO FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_TYPE='UNADJ' AND CND_LINK_YEAR=CONCAT('"+frmyr+"','/','"+toyr+"')");
    ...
    //        String draftDate="2014-2015";
            ...
            String linkNo=txtLinkNo.getText().trim();//+Integer.toString(EITLERPGLOBAL.FinYearTo)+"-"+Integer.toString(EITLERPGLOBAL.FinYearFrom);
            int lnk=0;
            if(txtLinkNo.getText().trim().equals(""))
                lnk=8501;
            else
                lnk=Integer.parseInt(linkNo);
            
            //linkNo=data.getStringValueFromDB("");
         //   int draftSrNo=521;
      ...
      //     int reasonCode=36;
            ...
            int counter=1;
            String t=txtInterval.getText().substring(0,11);
            ResultSet rsSchemeHeader =Stmt.executeQuery("SELECT CNH_SUB_ACCOUNT_CODE,CNH_PERCENT FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_YEAR_MON_ID ='"+t+"' AND CNH_TYPE ='YEAREND' ORDER BY CNH_SUB_ACCOUNT_CODE");
          ...
          //  ResultSet rsSchemeHeader=stmt.executeQuery("SELECT SCH_PARTY_CODE FROM SALES.D_SAL_SCHEME_HEADER WHERE SCH_SCHEME_ID="+schemeId+" AND SCH_SEASON_ID='"+seasonId+"' ORDER BY SCH_PARTY_CODE");
            ...
            if(rsSchemeHeader.getRow()>0){
                while(!rsSchemeHeader.isAfterLast()){
                    ...
                    //String SQL1 ="UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_DRAFT_CR_NOTE_NO='"+draftNo+(draftSrNo+counter)+"',CNH_DRAFT_CR_NOTE_DATE='"+draftDate+"',CNH_LINK_NO=CONCAT('"+lnk+"',RIGHT(CNH_LINK_NO,10)) WHERE CNH_TYPE ='YEAREND' AND CNH_YEAR_MON_ID ='"+t+"' AND  CNH_SUB_ACCOUNT_CODE='"+rsSchemeHeader.getString("CNH_SUB_ACCOUNT_CODE")+"' AND CNH_PERCENT='"+rsSchemeHeader.getString("CNH_PERCENT")+"'"; 
                    String SQL1 ="UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_DRAFT_CR_NOTE_NO='"+draftNo+(draftSrNo+counter)+"',CNH_DRAFT_CR_NOTE_DATE='"+draftDate+"',CNH_LINK_NO=CONCAT('"+lnk+"',RIGHT(CNH_LINK_NO,10)) WHERE CNH_TYPE ='YEAREND' AND CNH_YEAR_MON_ID ='"+t+"' AND  CNH_SUB_ACCOUNT_CODE='"+rsSchemeHeader.getString("CNH_SUB_ACCOUNT_CODE")+"' "; 
...
//		    String SQL2 ="UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_FIN_VOUCHER_NO = CONCAT('CN',161700000 +ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))),CNH_LEGACY_NO = ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))  WHERE CNH_TYPE ='YEAREND' AND CNH_YEAR_MON_ID ='"+t+"' AND CNH_SUB_ACCOUNT_CODE='"+rsSchemeHeader.getString("CNH_SUB_ACCOUNT_CODE")+"' AND CNH_PERCENT='"+rsSchemeHeader.getString("CNH_PERCENT")+"'";
                    //String SQL2 ="UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_FIN_VOUCHER_NO = CONCAT('CN','"+frmYr+"',1700000 +ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))),CNH_LEGACY_NO = ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))  WHERE CNH_TYPE ='YEAREND' AND CNH_YEAR_MON_ID ='"+t+"' AND CNH_SUB_ACCOUNT_CODE='"+rsSchemeHeader.getString("CNH_SUB_ACCOUNT_CODE")+"' AND CNH_PERCENT='"+rsSchemeHeader.getString("CNH_PERCENT")+"'";
    ...
    */
    ...
    private void GenerateCreditNoteDetail() {
        String str=IntervalComboModel.getText(cmbIntervalCombo.getSelectedIndex()).substring(0,11);
        String strfrom=IntervalComboModel.getText(cmbIntervalCombo.getSelectedIndex()).substring(19,29);
        String strto=IntervalComboModel.getText(cmbIntervalCombo.getSelectedIndex()).substring(33,43);
        try {
            ...
            objData.AddColumn("CND_EXT2");
            objData.AddColumn("CND_EXT3");
            objData.AddColumn("CND_EXT4");
            
            //String strSQL="SELECT CNH_NO AS NO,CNH_TYPE,CNH_SUB_ACCOUNT_CODE,CNH_PARTY_NAME,CNH_CITY,CND_INVOICE_NO,CND_INVOICE_DATE,CND_QUALITY,CND_SHADE,CND_PIECE_NO,CND_GROSS_QTY,CND_RATE,CND_NET_AMOUNT,CND_CATG,CND_PERCENT,CND_CREDIT_AMOUNT,CND_DISC_GIVEN,1 AS A,1 AS B,'' AS C FROM (SELECT CNH_NO,CNH_TYPE,CNH_SUB_ACCOUNT_CODE,CNH_PARTY_NAME,CNH_CITY FROM SALES.D_CREDIT_NOTE_HEADER WHERE CNH_TYPE='YEAREND' AND CNH_YEAR_MON_ID='"+str+"' ) AS A LEFT JOIN (SELECT CND_NO,CND_TYPE,CND_SUB_ACCOUNT_CODE,CND_INVOICE_NO,CND_INVOICE_DATE,CND_QUALITY,CND_SHADE,CND_PIECE_NO,CND_GROSS_QTY,CND_RATE,CND_NET_AMOUNT,CND_CATG,CND_PERCENT,CND_CREDIT_AMOUNT,CND_DISC_GIVEN FROM SALES.D_CREDIT_NOTE_DETAIL WHERE CND_TYPE='YEAREND' AND CND_YEAR_MON_ID='"+str+"') AS B ON A.CNH_NO=B.CND_NO AND A.CNH_SUB_ACCOUNT_CODE=B.CND_SUB_ACCOUNT_CODE  UNION ALL SELECT CNH_NO AS NO,'' AS CNH_TYPE,'' AS CNH_SUB_ACCOUNT_CODE,'' AS CNH_PARTY_NAME,'' AS CNH_CITY,'' AS CND_INVOICE_NO,'' AS CND_INVOICE_DATE,'' AS CND_QUALITY,'' AS CND_SHADE,'' AS CND_PIECE_NO,'' AS CND_GROSS_QTY,'' AS CND_RATE,'' AS CND_NET_AMOUNT,'' AS CND_CATG,0.00 AS CND_PERCENT,'' AS CND_CREDIT_AMOUNT,'' AS CND_DISC_GIVEN,2 AS A,1 AS B,'' AS C FROM SALES.D_CREDIT_NOTE_HEADER WHERE CNH_TYPE='YEAREND' AND CNH_YEAR_MON_ID='"+str+"'  UNION ALL (SELECT CNH_NO AS NO,'' AS CNH_TYPE,'' AS CNH_SUB_ACCOUNT_CODE,'' AS CNH_PARTY_NAME,'' AS CNH_CITY,'' AS CND_INVOICE_NO,'' AS CND_INVOICE_DATE,'' AS CND_QUALITY,'' AS CND_SHADE,'' AS CND_PIECE_NO,'' AS CND_GROSS_QTY,'' AS CND_RATE,'' AS CND_NET_AMOUNT,'' AS CND_CATG,0.00 AS CND_PERCENT,'' AS CND_CREDIT_AMOUNT,'' AS CND_DISC_GIVEN,2 AS A,0 AS B,CONCAT(CNH_SUB_ACCOUNT_CODE,'  ',CNH_PARTY_NAME,'     ',CNH_CITY) AS C FROM SALES.D_CREDIT_NOTE_HEADER WHERE CNH_TYPE='YEAREND' AND CNH_YEAR_MON_ID='"+str+"' ORDER BY CNH_NO LIMIT 1)  ORDER BY B,NO,A,CND_INVOICE_NO";
            String strSQL="SELECT * FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID='"+str+"' AND CND_TYPE='YEAREND' ORDER BY CND_SUB_ACCOUNT_CODE,CND_RC_VOUCHER_NO";
            ...
            if(rsTemp.getRow()>0){
                ...
                objRow.setValue("CND_EXT2", rsTemp.getString("CND_EXT2"));                                               
                objRow.setValue("CND_EXT3", rsTemp.getString("CND_EXT3"));                                               
                objRow.setValue("CND_EXT4", rsTemp.getString("CND_EXT4"));                                               
            ...
            objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/Production/rptYearEndCNDetail.rpt",Parameters,objData);
        }
        catch(Exception e) {
            e.printStackTrace();
        }
    ...
    private void GenerateCreditNote() {
        ...
        try {
            ...
            //String strSQL="SELECT CNH_ID,CNH_NO,CNH_TYPE,CNH_MAIN_ACCOUNT_CODE,CNH_MAIN_CODE,CNH_SUB_ACCOUNT_CODE,CNH_CITY,CNH_PARTY_NAME,CNH_REASON_CODE,CNH_BOOK_CODE,CNH_PERCENT,CNH_CREDIT_AMOUNT,CNH_LINK_NO,CNH_YEAR_MON_ID,CNH_INTERVAL_CAPTION,CNH_INVOICE_AMOUNT FROM SALES.D_CREDIT_NOTE_HEADER WHERE CNH_YEAR_MON_ID='"+str+"' AND CNH_TYPE='YEAREND' ORDER BY CNH_SUB_ACCOUNT_CODE";
            String strSQL="SELECT * FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_YEAR_MON_ID='"+str+"' AND CNH_TYPE='YEAREND' ORDER BY CNH_SUB_ACCOUNT_CODE";
            ResultSet  rsTemp =data.getResult(strSQL);
            rsTemp.first();
            if(rsTemp.getRow()>0){
                while(!rsTemp.isAfterLast()){
            ...
            objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/Production/rptYearEndCNDraft.rpt",Parameters,objData);
        }
        catch(Exception e) {
            e.printStackTrace();
        }
    ...
    private void GenerateCreditNoteSummary() {
        ...
        try {
            ...
            String strSQL="SELECT CNH_ID,CNH_NO,CNH_TYPE,CNH_MAIN_ACCOUNT_CODE,CNH_MAIN_CODE,CNH_SUB_ACCOUNT_CODE,CNH_CITY,CNH_PARTY_NAME,CNH_REASON_CODE,CNH_BOOK_CODE,CNH_PERCENT,CNH_CREDIT_AMOUNT,CNH_LINK_NO,CNH_YEAR_MON_ID,CNH_INTERVAL_CAPTION,CNH_INVOICE_AMOUNT FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_YEAR_MON_ID='"+str+"' AND CNH_TYPE='YEAREND' ORDER BY CNH_SUB_ACCOUNT_CODE";
            ResultSet  rsTemp =data.getResult(strSQL);
            rsTemp.first();
            if(rsTemp.getRow()>0){
                while(!rsTemp.isAfterLast()){
            ...
            Parameters.put("FROM_DATE",strfrom);
            Parameters.put("TO_DATE",strto);
            objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/Production/rptYearEndCNSummary.rpt",Parameters,objData);
        }
        catch(Exception e) {
            e.printStackTrace();
        }
    ...
    private void generateIntervalCombo(){
        ...
        try{
            ...
            ResultSet rs=data.getResult("SELECT DISTINCT CND_YEAR_MON_ID,CND_INTERVAL_CAPTION FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_TYPE ='YEAREND' ORDER BY CND_YEAR_MON_ID DESC ");
            while(!rs.isAfterLast()){
    ...
    private void FormatGrid() {
        try{
            ...
            DataModel.addColumn("Invoice No");    //1               rowData[0]=rs.getString("CN_INVOICE_NO");
            ...
            DataModel.addColumn("Product Name");    //7
            DataModel.addColumn("Sqr Mtr");    //8
            DataModel.addColumn("Gross Value");    //9
            DataModel.addColumn("Disc Bill");    //10
            DataModel.addColumn("Year End Disc");    //11
            DataModel.addColumn("Year End Disc Rs");    //12
            DataModel.addColumn("Unadj Disc");    //13
            DataModel.addColumn("Unadj Rs");    //14
            DataModel.addColumn("Net Basic Value"); //15
            DataModel.addColumn("Oem");    //16
            DataModel.addColumn("Oem Remarks");  //17
            
            //DataModelDesc.TableReadOnly(true);
            //DataModelDesc.SetVariable(0,"SR_NO");  //0
            DataModel.SetVariable(0,"CN_INVOICE_NO");    //1
            DataModel.SetVariable(1,"CN_INVOICE_DATE");    //2
            DataModel.SetVariable(2,"CN_PARTY_CODE");    //3
            DataModel.SetVariable(3,"CN_PARTY_NAME");    //4
            DataModel.SetVariable(4,"CN_INV_PIECE_NO");    //5
            DataModel.SetVariable(5,"CN_INV_PRODUCT_CODE");    //6
            DataModel.SetVariable(6,"CN_PRODUCT_GROUP");    //7
            DataModel.SetVariable(7,"CN_INV_WI_SQMTR");    //8
            DataModel.SetVariable(8,"CN_GROSS_VALUE");    //9
            DataModel.SetVariable(9,"CN_DISC_BILL");    //10
            DataModel.SetVariable(10,"CN_YEAR_END_DISC");    //11
            DataModel.SetVariable(11,"CN_YEAR_END_DISC_RS");    //12
            DataModel.SetVariable(12,"CN_UNADJUSTED_DISC");    //13
            DataModel.SetVariable(13,"CN_UNADJUSTED_DISC_RS");    //14
            DataModel.SetVariable(14,"CN_NET_BASIC");    //15
            DataModel.SetVariable(15,"CN_OEM");    //16
            DataModel.SetVariable(16,"CN_OEM_VALUE");    //17
    ...
    private void GenerateData() {
        try {
            ...
            String strSQL="";
            String fdt = EITLERPGLOBAL.formatDateDB(txtFromDate.getText().trim());
            String tdt = EITLERPGLOBAL.formatDateDB(txtToDate.getText().trim());
            
            strSQL="SELECT H.UNADJ_ID,PARTY_NAME,PARTY_CODE,PIECE_NO,PRODUCT_CODE,INVOICE_NO,INVOICE_DATE,KG,SQR_MTR,WIDTH,LENGTH,RATE,INV_BASIC_AMT,WORK_DISC_PER,SEAM_PER,DISC_AMT,D_REMARK1,D_REMARK2 FROM PRODUCTION.FELT_UNADJUSTED_TRN_H H,PRODUCTION.FELT_UNADJUSTED_TRN_D D WHERE H.UNADJ_ID=D.UNADJ_ID AND UNADJ_FROM_DATE BETWEEN '" + fdt + "' AND '" + tdt + "' AND UNADJ_TO_DATE BETWEEN '" + fdt + "' AND '" + tdt + "'";
            ...
            if(rsTmp.getRow()>0) {
                ...
                while(!rsTmp.isAfterLast()) {
                    ...
                    rowData[3]=UtilFunctions.getString(rsTmp,"PRODUCT_CODE","");
                    ...
                    rowData[6]=UtilFunctions.getString(rsTmp,"KG","");
                    rowData[7]=UtilFunctions.getString(rsTmp,"SQR_MTR","");
                    ...
                    rowData[11]=UtilFunctions.getString(rsTmp,"INV_BASIC_AMT","");
                    rowData[12]=UtilFunctions.getString(rsTmp,"WORK_DISC_PER","");
                    rowData[13]=UtilFunctions.getString(rsTmp,"SEAM_PER","");
                    rowData[14]=UtilFunctions.getString(rsTmp,"DISC_AMT","");
                    rowData[15]=UtilFunctions.getString(rsTmp,"REMARK1","");
                    rowData[16]=UtilFunctions.getString(rsTmp,"REMARK2","");
                    //rowData[17]="";
    ...
    private void GenerateYearEndCN(){
        ...
        try{
            ...
            //stmt.execute("INSERT INTO PRODUCTION.TMP_LC_CREDITNOTETEST (LCN_VOUCHER_NO,LCN_SUB_ACCOUNT_CODE,EXT1,LCN_RC_VOUCHER,LCN_INVOICE_NO,LCN_INVOICE_DATE,LCN_AMOUNT,EXT2,LCN_MAIN_ACCOUNT_CODE) SELECT YEAR_END_ID,PARTY_CODE,PARTY_NAME,QUALITY_NO,INVOICE_NO,INVOICE_DATE,GROSS_AMOUNT,SPIRAL_CHG,210010 FROM PRODUCTION.FELT_INVOICE_DATA I,PRODUCTION.FELT_YEAR_END_DISCOUNT_DETAIL Y WHERE Y.YEAR_END_PARTY_CODE=I.PARTY_CODE AND Y.APPROVED=1 AND Y.CANCELED=0 AND YEAR_END_YES_NO='YES' AND INVOICE_DATE>='2015-04-01' AND INVOICE_DATE<='2016-03-31' AND YEAR_FROM_DATE>='2014-01-01' AND YEAR_TO_DATE<='2016-03-31'");
            //stmt.execute("INSERT INTO PRODUCTION.TMP_LC_CREDITNOTETEST (LCN_VOUCHER_NO,LCN_SUB_ACCOUNT_CODE,EXT1,LCN_RC_VOUCHER,LCN_INVOICE_NO,LCN_INVOICE_DATE,LCN_AMOUNT,EXT2,LCN_MAIN_ACCOUNT_CODE) SELECT Y.FORM2_YEAR_END_ID,PARTY_CODE,PARTY_NAME,QUALITY_NO,INVOICE_NO,INVOICE_DATE,GROSS_AMOUNT,SPIRAL_CHG,210010 FROM PRODUCTION.FELT_INVOICE_DATA I,PRODUCTION.FELT_YEAR_END_DISCOUNT_DETAIL_FORM2 Y, PRODUCTION.FELT_YEAR_END_DISCOUNT_HEADER_FORM2 H WHERE Y.FORM2_YEAR_END_PARTY_CODE=I.PARTY_CODE AND Y.FORM2_YEAR_END_PRODUCT_CODE=I.QUALITY_NO AND Y.FORM2_YEAR_END_ID=H.FORM2_YEAR_END_ID AND H.APPROVED=1 AND H.CANCELED=0 AND UPPER(Y.FORM2_YEAR_END_YES_NO)='YES' AND INVOICE_DATE>='2015-04-01' AND INVOICE_DATE<='2016-03-31' AND Y.FORM2_FROM_DATE>='2015-04-01' AND Y.FORM2_TO_DATE<='2016-03-31'");
            stmt.execute("INSERT INTO PRODUCTION.TMP_LC_CREDITNOTETEST (LCN_INVOICE_NO,LCN_VOUCHER_NO,LCN_SUB_ACCOUNT_CODE,EXT1,LCN_RC_VOUCHER,LCN_INVOICE_DATE,LCN_AMOUNT,EXT2,LCN_MAIN_ACCOUNT_CODE) SELECT DISTINCT INVOICE_NO,Y.FORM2_YEAR_END_ID,PARTY_CODE,PARTY_NAME,QUALITY_NO,INVOICE_DATE,GROSS_AMOUNT,SPIRAL_CHG,210010 FROM PRODUCTION.FELT_INVOICE_DATA I,PRODUCTION.FELT_YEAR_END_DISCOUNT_DETAIL_FORM2 Y, PRODUCTION.FELT_YEAR_END_DISCOUNT_HEADER_FORM2 H WHERE Y.FORM2_YEAR_END_PARTY_CODE=I.PARTY_CODE AND Y.FORM2_YEAR_END_PRODUCT_CODE=I.QUALITY_NO AND Y.FORM2_YEAR_END_ID=H.FORM2_YEAR_END_ID AND H.APPROVED=1 AND H.CANCELED=0 AND UPPER(Y.FORM2_YEAR_END_YES_NO)='YES' AND INVOICE_DATE>='" + frmdt + "' AND INVOICE_DATE<='" + todt + "' AND Y.FORM2_FROM_DATE>='" + frmdt + "' AND Y.FORM2_TO_DATE<='" + todt + "' AND INVOICE_DATE>=Y.FORM2_EFFECTIVE_FROM AND INVOICE_DATE<=Y.FORM2_EFFECTIVE_TO");
            
//            stmt.execute("TRUNCATE TABLE PRODUCTION.TMP_98");
//            
//            //stmt.execute("INSERT INTO PRODUCTION.TMP_98 SELECT FORM2_YEAR_END_PARTY_CODE,FORM2_YEAR_END_PRODUCT_CODE,FORM2_YEAR_PERCENT,FORM2_YEAR_SEAM_PERCENT FROM PRODUCTION.FELT_YEAR_END_DISCOUNT_DETAIL_FORM2 A,PRODUCTION.FELT_YEAR_END_DISCOUNT_DETAIL B WHERE A.FORM2_YEAR_END_PARTY_CODE=B.YEAR_END_PARTY_CODE AND FORM2_YEAR_END_YES_NO='YES' AND YEAR_FROM_DATE>='2014-01-01' AND YEAR_TO_DATE<='2016-03-31' AND YEAR_END_YES_NO='YES' AND A.APPROVED=1 AND A.CANCELED=0");
//            //stmt.execute("INSERT INTO PRODUCTION.TMP_98 SELECT A.FORM2_YEAR_END_PARTY_CODE,A.FORM2_YEAR_END_PRODUCT_CODE,A.FORM2_YEAR_PERCENT,A.FORM2_YEAR_SEAM_PERCENT FROM PRODUCTION.FELT_YEAR_END_DISCOUNT_DETAIL_FORM2 A,PRODUCTION.FELT_YEAR_END_DISCOUNT_HEADER_FORM2 H WHERE A.FORM2_YEAR_END_ID=H.FORM2_YEAR_END_ID AND UPPER(A.FORM2_YEAR_END_YES_NO)='YES' AND A.FORM2_FROM_DATE>='2015-04-01' AND A.FORM2_TO_DATE<='2016-03-31' AND H.APPROVED=1 AND H.CANCELED=0");
//            stmt.execute("INSERT INTO PRODUCTION.TMP_98 SELECT A.FORM2_YEAR_END_PARTY_CODE,A.FORM2_YEAR_END_PRODUCT_CODE,A.FORM2_YEAR_PERCENT,A.FORM2_YEAR_SEAM_PERCENT FROM PRODUCTION.FELT_YEAR_END_DISCOUNT_DETAIL_FORM2 A,PRODUCTION.FELT_YEAR_END_DISCOUNT_HEADER_FORM2 H WHERE A.FORM2_YEAR_END_ID=H.FORM2_YEAR_END_ID AND UPPER(A.FORM2_YEAR_END_YES_NO)='YES' AND A.FORM2_FROM_DATE>='" + frmdt + "' AND A.FORM2_TO_DATE<='" + todt + "' AND H.APPROVED=1 AND H.CANCELED=0");
//            
//            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTETEST,PRODUCTION.TMP_98 SET LCN_PER=COL3,EXT3=COL4 WHERE LCN_SUB_ACCOUNT_CODE=COL1 AND LCN_RC_VOUCHER=COL2");
            ...
            //stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTETEST SET LCN_YEAR_MON_ID='YEAREND1516' WHERE LCN_INVOICE_DATE>='2015-04-01' AND LCN_INVOICE_DATE<='2016-03-31'");
            ...
            //stmt.execute("DELETE FROM PRODUCTION.D_CREDIT_NOTE_HEADER  WHERE CNH_YEAR_MON_ID = 'YEAREND1516'");
            stmt.execute("DELETE FROM PRODUCTION.D_CREDIT_NOTE_HEADER  WHERE CNH_YEAR_MON_ID = CONCAT('YEAREND','"+fyr+"','"+tyr+"')");
            
            //stmt.execute("INSERT INTO PRODUCTION.D_CREDIT_NOTE_HEADER (CNH_ID,CNH_NO,CNH_TYPE,CNH_MAIN_ACCOUNT_CODE,CNH_MAIN_CODE,CNH_SUB_ACCOUNT_CODE,CNH_PARTY_NAME,CNH_EFFECT,CNH_INVOICE_AMOUNT,CNH_PERCENT,CNH_CREDIT_AMOUNT,CNH_YEAR_MON_ID,CNH_INTERVAL_CAPTION,CNH_BOOK_CODE,CNH_EXT4) SELECT CND_ID,CND_NO,CND_TYPE,435132,210010,CND_SUB_ACCOUNT_CODE,CND_MAIN_ACCOUNT_NAME,'D',SUM(CND_INVOICE_AMOUNT),CND_PERCENT,SUM(CND_CREDIT_AMOUNT+CND_EXT4),CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,'TR-17',SUM(CND_EXT4) FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  ='YEAREND1516' GROUP BY CND_ID,CND_NO,CND_TYPE,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION LIMIT 1000000000000");
            ...
            //stmt.execute("UPDATE DINESHMILLS.D_SAL_PARTY_MASTER,PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_PARTY_NAME = PARTY_NAME,CNH_CITY = CITY_ID WHERE CNH_SUB_ACCOUNT_CODE = PARTY_CODE AND CNH_YEAR_MON_ID = 'YEAREND1516' ");
