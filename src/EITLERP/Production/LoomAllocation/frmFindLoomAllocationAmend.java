...
public class frmFindLoomAllocationAmend extends javax.swing.JApplet {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Find :: Loom Allocation Amend");
    ...
    private void cmdFindActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdFindActionPerformed
        ...
        if(!txtPieceNo.getText().trim().equals(""))
        {
            if(flag || flag2)
            {
                stringFindQuery = stringFindQuery+" AND DOC_NO IN (SELECT DOC_NO FROM PRODUCTION.SPILLOVER_RESCHEDULING_NEW_DETAIL WHERE PIECE_NO = '"+txtPieceNo.getText()+"') ";
            }
            else
            {
                stringFindQuery = " DOC_NO IN (SELECT DOC_NO FROM PRODUCTION.SPILLOVER_RESCHEDULING_NEW_DETAIL WHERE PIECE_NO = '"+txtPieceNo.getText()+"') ";
            }
        }
        getParent().getParent().getParent().getParent().show(false);
