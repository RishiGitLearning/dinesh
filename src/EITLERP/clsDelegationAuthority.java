...
public class clsDelegationAuthority {
    ...
    public static clsDelegationAuthority getObject(int pCompanyID,int pEntryNo) {
        ...
        try {
            ...
            if(rsTmp.getRow()>0) {
                ...
                System.out.println("frin date   "+rsTmp.getString("AUTH_FROM_DATE")); 
    ...
    public static HashMap getList(int pCompanyID,int pUserID) {
        ...
        try {
            ...
            rsTmp = data.getResult("SELECT 2 AS COMPANY_ID,A.MODULE_ID AS AUTH_MODULE_ID,MODULE_DESC AS AUTH_MODULE_DESC,USER_ID AS AUTH_USER_ID,'' AUTH_NOMINEE1,''  AUTH_NOMINEE2,'' AUTH_NOMINEE3, '' AUTH_PRIORITY,'' AUTH_NOMINEE1_ID,'' AUTH_NOMINEE2_ID,'' AUTH_NOMINEE3_ID,'' AUTH_FROM_DATE,'' AUTH_TO_DATE,'' AUTH_ACTIVE_MODULE_ID FROM\n"
                    + " (SELECT MODULE_ID,HR.USER_ID FROM DINESHMILLS.D_COM_HIERARCHY_RIGHTS HR,DINESHMILLS.D_COM_HIERARCHY H \n"
                    + " WHERE H.HIERARCHY_ID=HR.HIERARCHY_ID  AND (APPROVER=1 OR FINAL_APPROVER=1) AND CREATOR=0 AND USER_ID="+pUserID+" \n"
                    ...
                    + " UNION \n"
                    + " SELECT 2,AUTH_MODULE_ID, AUTH_MODULE_DESC, AUTH_USER_ID, AUTH_NOMINEE1, AUTH_NOMINEE2, AUTH_NOMINEE3, AUTH_PRIORITY, AUTH_NOMINEE1_ID, AUTH_NOMINEE2_ID, AUTH_NOMINEE3_ID, AUTH_FROM_DATE, AUTH_TO_DATE, AUTH_ACTIVE_MODULE_ID FROM DINESHMILLS.D_COM_DELEGATION_AUTHORITY WHERE AUTH_USER_ID="+pUserID+" AND (AUTH_NOMINEE1!='' OR AUTH_NOMINEE2!='' OR AUTH_NOMINEE3!='') ORDER BY AUTH_MODULE_ID");
    ...
    public static boolean IsAuthorityGiven(int pCompanyID,int pUserID,int pAuthorityUserID,int pModuleID,int pDeptID) {
        ...
        try {
            ...
            if(rsTmp.getRow()>0)
            {
              ...
              strSQL="SELECT * FROM D_COM_AUTHORITY WHERE COMPANY_ID="+pCompanyID+" AND USER_ID="+pUserID+" AND AUTHORITY_USER_ID="+pAuthorityUserID+" AND MODULE_ID="+pModuleID;
            }
            else
            {
              HasAllDeptAuthority=false;   
              strSQL="SELECT * FROM D_COM_AUTHORITY WHERE COMPANY_ID="+pCompanyID+" AND USER_ID="+pUserID+" AND AUTHORITY_USER_ID="+pAuthorityUserID+" AND MODULE_ID="+pModuleID+" AND DEPT_ID="+pDeptID;
            }
