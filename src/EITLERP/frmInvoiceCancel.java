...
public class frmInvoiceCancel extends javax.swing.JApplet {
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        Tab1.add(txtDocNo);
        txtDocNo.setBounds(115, 14, 104, 19);

        jLabel3.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        jLabel3.setText("Date :");
        Tab1.add(jLabel3);
        jLabel3.setBounds(310, 16, 40, 15);
        ...
        Tab1.add(cmbForDept);
        cmbForDept.setBounds(115, 88, 212, 24);
        ...
        Tab1.add(cmbUser);
        cmbUser.setBounds(115, 120, 214, 24);

        lblRevNo.setText("...");
        Tab1.add(lblRevNo);
        lblRevNo.setBounds(240, 15, 44, 15);
        ...
        cmdPurposeBig.setBounds(470, 200, 33, 21);
        ...
        cmdRemarksBig.setBounds(470, 250, 33, 21);
        ...
        txtInvoiceNo.setName("DOC_NO");
        txtInvoiceNo.setEnabled(false);
        Tab1.add(txtInvoiceNo);
        txtInvoiceNo.setBounds(115, 161, 100, 20);
    ...
    private void GenerateCombos() {
        ...
        strCondition=" WHERE COMPANY_ID="+Long.toString(EITLERPGLOBAL.gCompanyID)+" AND MODULE_ID="+clsSalesInvoice.ModuleID+" ORDER BY MODULE_ID";
    ...
    private void SetupApproval() {
        ...
        if(EditMode==EITLERPGLOBAL.ADD) {
            ...
            int FromUserID=ApprovalFlow.getFromID((int)EITLERPGLOBAL.gCompanyID, clsSalesInvoice.ModuleID ,(String)ObjDoc.getAttribute("REQ_NO").getObj());
            lnFromID=FromUserID;
            String strFromUser=clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks=ApprovalFlow.getFromRemarks( (int)EITLERPGLOBAL.gCompanyID,clsSalesInvoice.ModuleID,FromUserID,(String)ObjDoc.getAttribute("REQ_NO").getObj());
    ...
    private void SetMenuForRights() {
        // --- Add Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,11581)) {
            cmdNew.setEnabled(true);
        }
        else {
            cmdNew.setEnabled(false);
        ...
        // --- Delete Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,11583)) {
            cmdDelete.setEnabled(true);
        }
        else {
            cmdDelete.setEnabled(false);
        ...
        // --- Print Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,11584)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        }
        else {
    ...
    private void Edit() {
        ...
        if(ObjDoc.IsEditable(EITLERPGLOBAL.gCompanyID, lDocNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if(ApprovalFlow.IsCreator(44,lDocNo)||clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,11582)) {
                SetFields(true);
            }
            else {
                EnableApproval();
    ...
    private void Save() {
        ...
        if(!clsSalesInvoice.IsDocExist(EITLERPGLOBAL.gCompanyID,txtInvoiceNo.getText().trim(),EITLERPGLOBAL.formatDateDB(txtInvoiceDate.getText().trim()))) {
        ...
        if(EITLERPGLOBAL.getComboCode(cmbModule)==clsSalesInvoice.ModuleID) {
            CanCancel=clsSalesInvoice.CanCancel(EITLERPGLOBAL.gCompanyID, txtInvoiceNo.getText().trim(),EITLERPGLOBAL.formatDateDB(txtInvoiceDate.getText().trim()));
        }
        ...
        if(EditMode==EITLERPGLOBAL.ADD) {
            ...
            try {
                ResultSet rsTmp=data.getResult("SELECT * FROM D_COM_DOC_CANCEL_REQUEST WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID="+ModuleID+" AND DOC_NO='"+DocNo+"' AND DOC_DATE='"+EITLERPGLOBAL.formatDateDB(DocDate)+"' AND APPROVED=0 AND CANCELLED=0");
                rsTmp.first();
    ...
    private void GenerateRejectedUserCombo() {
        ...
        for(int i=1;i<=List.size();i++) {
            ...
            if(EditMode==EITLERPGLOBAL.EDIT) {
                if(OpgApprove.isSelected()) {
                    IncludeUser=ApprovalFlow.IncludeUserInApproval(EITLERPGLOBAL.gCompanyID, 44, DocCode, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
                
                if(OpgReject.isSelected()) {
                    IncludeUser=ApprovalFlow.IncludeUserInRejection(EITLERPGLOBAL.gCompanyID, 44, DocCode, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
