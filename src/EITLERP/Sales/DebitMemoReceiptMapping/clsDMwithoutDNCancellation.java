...
public class clsDMwithoutDNCancellation {
    ...
    public static int ModuleID=100;
    ...
    public clsDMwithoutDNCancellation() {
        LastError = "";
        props=new HashMap();
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER WHERE COMPANY_ID="+pCompanyID+" AND DOC_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND DOC_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY DOC_NO");
            HistoryView=false;
            Ready=true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHistory=stHistory.executeQuery("SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER_H WHERE DOC_NO='1'"); // '1' for restricting all data retrieval
            rsHistory.first();
            rsHDetail=stHDetail.executeQuery("SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_DETAIL_H WHERE DOC_NO='1'");
            rsHDetail.first();
            //------------------------------------//
            ...
            setAttribute("DOC_NO",clsFirstFree.getNextFreeNo(getAttribute("COMPANY_ID").getInt(),clsDMwithoutDNCancellation.ModuleID, getAttribute("FFNO").getInt(),true));
            //-------------------------------------------------
...
//            rsHistory.updateString("MODIFIED_DATE",EITLERPGLOBAL.formatDateDB(getAttribute("MODIFIED_DATE").getString()));
            ...
            rsTmp=tmpStmt.executeQuery("SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_DETAIL WHERE DOC_NO='1'");
            
            int CompanyID=getAttribute("COMPANY_ID").getInt();
            
            //Now Insert records into detail table
            for(int i=1;i<=colItems.size();i++) {
                clsDMwithoutDNCancellationItem objItem=(clsDMwithoutDNCancellationItem) colItems.get(Integer.toString(i));
                
                rsTmp.moveToInsertRow();
                rsTmp.updateInt("COMPANY_ID",CompanyID);
                rsTmp.updateString("DOC_NO",getAttribute("DOC_NO").getString());
                rsTmp.updateInt("SR_NO",i);
                rsTmp.updateString("DEBITMEMO_NO",objItem.getAttribute("DEBITMEMO_NO").getString());
                rsTmp.updateString("DEBITMEMO_DATE",EITLERPGLOBAL.formatDateDB(objItem.getAttribute("DEBITMEMO_DATE").getString()));
...
//                rsTmp.updateString("MODIFIED_DATE",EITLERPGLOBAL.formatDateDB(getAttribute("MODIFIED_DATE").getString()));
                ...
                rsHDetail.updateString("DEBITMEMO_NO",objItem.getAttribute("DEBITMEMO_NO").getString());
                rsHDetail.updateString("DEBITMEMO_DATE",EITLERPGLOBAL.formatDateDB(objItem.getAttribute("DEBITMEMO_DATE").getString()));
...
//                rsHDetail.updateString("MODIFIED_DATE",EITLERPGLOBAL.formatDateDB(getAttribute("MODIFIED_DATE").getString()));
            ...
            ObjFlow.ModuleID=ModuleID; //Debit Memo Cancellation
            ...
            ObjFlow.TableName="FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER";
            ...
            if(AStatus.equals("F")) {//On Final Approval
                ...
                for(int i=1;i<=colItems.size();i++) {
                    ...
                    DMNo = objItem.getAttribute("DEBITMEMO_NO").getString();
                    DMDate = objItem.getAttribute("DEBITMEMO_DATE").getString();
                    InvoiceNo = objItem.getAttribute("INVOICE_NO").getString();
                    InvoiceDate = objItem.getAttribute("INVOICE_DATE").getString();
                    
                    String Qry ="UPDATE FINANCE.D_FIN_DEBITMEMO_RECEIPT_MAPPING SET CANCELLED=1,APPROVED=0 "
                            +"WHERE DEBITMEMO_NO = '" + DMNo + "' AND DEBITMEMO_DATE = '" + EITLERPGLOBAL.formatDateDB(DMDate) + "' " 
//                            +"AND INVOICE_NO = '" + InvoiceNo + "' AND INVOICE_DATE = '" + EITLERPGLOBAL.formatDateDB(InvoiceDate) + "' "
                            + " ";
                    System.out.println("UPDATE CANCELLATION : "+Qry);
                    data.Execute(Qry);
                }
            }
...
//            rsResultSet.updateString("CREATED_DATE",EITLERPGLOBAL.formatDateDB(getAttribute("CREATED_BY").getString()));
            ...
            int RevNo=data.getIntValueFromDB("SELECT MAX(REVISION_NO) FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER_H WHERE COMPANY_ID="+getAttribute("COMPANY_ID").getInt()+" AND DOC_NO='"+getAttribute("DOC_NO").getString()+"'");
            RevNo++;
...
//            rsHistory.updateString("CREATED_DATE",EITLERPGLOBAL.formatDateDB(getAttribute("CREATED_DATE").getString()));
            ...
            data.Execute("DELETE FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_DETAIL WHERE COMPANY_ID="+getAttribute("COMPANY_ID").getInt()+" AND DOC_NO='"+theDocNo+"'");
...
//                rsHDetail.updateString("CREATED_DATE",EITLERPGLOBAL.formatDateDB(getAttribute("CREATED_DATE").getString()));
            ...
            if(IsRejected) {
                ...
                data.Execute("UPDATE FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+getAttribute("COMPANY_ID").getInt()+" AND DOC_NO='"+ObjFlow.DocNo+"'");
...
//                            +"AND INVOICE_NO = '" + InvoiceNo + "' AND INVOICE_DATE = '" + EITLERPGLOBAL.formatDateDB(InvoiceDate) + "' "
    ...
    public boolean CanDelete(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER WHERE COMPANY_ID="+pCompanyID+" AND DOC_NO='"+pDocNo+"' AND (APPROVED=1)";
    ...
    public boolean Delete(int pUserID) {
        try {
            ...
            if(CanDelete(CompanyID,lDocNo,pUserID)) {
                String strQry = "DELETE FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER WHERE COMPANY_ID=" + CompanyID +" AND DOC_NO='"+lDocNo+"'";
                data.Execute(strQry);
                strQry = "DELETE FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_DETAIL WHERE COMPANY_ID=" + CompanyID +" AND DOC_NO='"+lDocNo+"'";
                data.Execute(strQry);
    ...
    public Object getObject(int CompanyID, String DocNo) {
        ...
        clsDMwithoutDNCancellation objDMCancellation =  new clsDMwithoutDNCancellation();
        objDMCancellation.Filter(strCondition,CompanyID);
        return objDMCancellation;
    }
    ...
    public boolean Filter(String Condition,int CompanyID) {
        ...
        try {
            ...
            String strSql = "SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER " + Condition ;
            Conn=data.getConn();
            Stmt=Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rsResultSet=Stmt.executeQuery(strSql);
            
            if(!rsResultSet.first()) {
                //strSql = "SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER WHERE COMPANY_ID="+CompanyID+" AND DOC_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND DOC_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY DOC_NO";
                strSql = "SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER " + Condition + " ORDER BY DOC_NO";
                rsResultSet=Stmt.executeQuery(strSql);
                Ready=true;
                MoveLast();
                return false;
    ...
    public boolean setData() {
        ...
        try {
            ...
            if(HistoryView) {
                rsTmp=tmpStmt.executeQuery("SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_DETAIL_H WHERE COMPANY_ID=" + CompanyID + " AND DOC_NO='" + VoucherNo + "' AND REVISION_NO="+RevNo+" ORDER BY SR_NO");
            }
            else {
                rsTmp=tmpStmt.executeQuery("SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_DETAIL WHERE COMPANY_ID=" + CompanyID + " AND DOC_NO='" + VoucherNo + "' ORDER BY SR_NO");
            }
            ...
            while(!rsTmp.isAfterLast() && rsTmp.getRow()>0) {
                ...
                objItem.setAttribute("DEBITMEMO_NO",UtilFunctions.getString(rsTmp,"DEBITMEMO_NO",""));
                objItem.setAttribute("DEBITMEMO_DATE",UtilFunctions.getString(rsTmp,"DEBITMEMO_DATE","0000-00-00"));
                ...
                objItem.setAttribute("INTEREST_AMOUNT",UtilFunctions.getDouble(rsTmp,"INTEREST_AMOUNT",0));
    ...
    public static HashMap getPendingApprovals(int CompanyID,int UserID,int Order) {
        ...
        try {
            ...
            if(Order==EITLERPGLOBAL.OnRecivedDate) {
                strSQL="SELECT FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_NO,FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_DATE,FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.MAIN_ACCOUNT_CODE,FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.PARTY_CODE,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.RECEIVED_DATE FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA WHERE FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_NO="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.DOC_NO AND FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.COMPANY_ID="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.COMPANY_ID AND FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.COMPANY_ID="+CompanyID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.USER_ID="+UserID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.STATUS='W' AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.MODULE_ID="+ModuleID+" ORDER BY D_COM_DOC_DATA.RECEIVED_DATE";
            }
            
            if(Order==EITLERPGLOBAL.OnDocDate) {
                strSQL="SELECT FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_NO,FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_DATE,FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.MAIN_ACCOUNT_CODE,FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.PARTY_CODE,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.RECEIVED_DATE FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA WHERE FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_NO="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.DOC_NO AND FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.COMPANY_ID="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.COMPANY_ID AND FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.COMPANY_ID="+CompanyID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.USER_ID="+UserID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.STATUS='W' AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.MODULE_ID="+ModuleID+" ORDER BY FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_DATE";
            }
            
            if(Order==EITLERPGLOBAL.OnDocNo) {
                strSQL="SELECT FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_NO,FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_DATE,FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.MAIN_ACCOUNT_CODE,FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.PARTY_CODE,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.RECEIVED_DATE FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA WHERE FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_NO="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.DOC_NO AND FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.COMPANY_ID="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.COMPANY_ID AND FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.COMPANY_ID="+CompanyID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.USER_ID="+UserID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.STATUS='W' AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.MODULE_ID="+ModuleID+" ORDER BY FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER.DOC_NO";
            }
            ...
            rsTmp3.first();
            Counter=0;
            while(!rsTmp3.isAfterLast()&&rsTmp3.getRow()>0) {
                    Counter=Counter+1;
                    clsDMwithoutDNCancellation ObjDoc=new clsDMwithoutDNCancellation();
    ...
    public boolean ShowHistory(int CompanyID,String DocNo) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER_H WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"'");
            Ready=true;
            HistoryView=true;
            MoveFirst();
            return true;
    ...
    public static HashMap getHistoryList(int CompanyID,String DocNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT * FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER_H WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"'");
            rsTmp.first();
            
            if(rsTmp.getRow()>0) {
                
                while(!rsTmp.isAfterLast()) {
                    clsDMwithoutDNCancellation objVoucher=new clsDMwithoutDNCancellation();
    ...
    public static String getDocStatus(int CompanyID,String DocNo) {
        ...
        try {
            rsTmp=data.getResult("SELECT DOC_NO,APPROVED,CANCELLED FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"'");
            rsTmp.first();
    ...
    public static boolean CanCancel(int CompanyID,String DocNo) {
        ...
        try {
            rsTmp=data.getResult("SELECT DOC_NO FROM FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"' AND APPROVED=0 AND CANCELLED=0");
            rsTmp.first();
    ...
    public static boolean CancelDoc(int CompanyID,String DocNo) {
        ...
        try {
            if(CanCancel(CompanyID,DocNo)) {
                data.Execute("DELETE FROM DINESHMILLS.D_COM_DOC_DATA WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"' AND MODULE_ID="+ModuleID);
                data.Execute("UPDATE FINANCE.D_FIN_DEBITMEMO_CANCELLATION_HEADER SET CANCELLED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"'");
                Cancelled=true;
            }
        } catch(Exception e) {
        }
