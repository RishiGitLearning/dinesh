...
public class clsSalesInvoiceDueDate {
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_SAL_INVOICE_DUE_DATE_HEADER WHERE COMPANY_ID="+pCompanyID+" AND DOC_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND DOC_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY DOC_NO");
            HistoryView=false;
            Ready=true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHistory=stHistory.executeQuery("SELECT * FROM D_SAL_INVOICE_DUE_DATE_HEADER_H WHERE DOC_NO='1'"); // '1' for restricting all data retrieval
            rsHistory.first();
            rsHDetail=stHDetail.executeQuery("SELECT * FROM D_SAL_INVOICE_DUE_DATE_DETAIL_H WHERE DOC_NO='1'");
            rsHDetail.first();
            //------------------------------------//
            ...
            setAttribute("DOC_NO",clsFirstFree.getNextFreeNo(getAttribute("COMPANY_ID").getInt(),clsSalesInvoiceDueDate.ModuleID, getAttribute("FFNO").getInt(),true));
            //-------------------------------------------------
            ...
            rsTmp=tmpStmt.executeQuery("SELECT * FROM D_SAL_INVOICE_DUE_DATE_DETAIL WHERE DOC_NO='1'");
            
            int CompanyID=getAttribute("COMPANY_ID").getInt();
            
            //Now Insert records into detail table
            for(int i=1;i<=colItems.size();i++) {
                clsSalesInvoiceDueDateItem objItem=(clsSalesInvoiceDueDateItem) colItems.get(Integer.toString(i));
                ...
                rsTmp.updateDouble("NET_AMOUNT",objItem.getAttribute("NET_AMOUNT").getDouble());
                rsTmp.updateString("DUE_DATE",EITLERPGLOBAL.formatDateDB(objItem.getAttribute("DUE_DATE").getString()));
                rsTmp.updateInt("DAYS",objItem.getAttribute("DAYS").getInt());
                rsTmp.updateString("NEW_DATE",EITLERPGLOBAL.formatDateDB(objItem.getAttribute("NEW_DATE").getString()));
                ...
                rsHDetail.updateString("DUE_DATE",EITLERPGLOBAL.formatDateDB(objItem.getAttribute("DUE_DATE").getString()));
                rsHDetail.updateInt("DAYS",objItem.getAttribute("DAYS").getInt());
                rsHDetail.updateString("NEW_DATE",EITLERPGLOBAL.formatDateDB(objItem.getAttribute("NEW_DATE").getString()));
            ...
            ObjFlow.TableName="D_SAL_INVOICE_DUE_DATE_HEADER";
            ...
            if(AStatus.equals("F")) {//On Final Approval
                ...
                String PartyCode = getAttribute("SUB_ACCOUNT_CODE").getString();
                String InvoiceNo = "";
                String InvoiceDate = "";
                String DueDate="";
                String NewDate="";
                ...
                for(int i=1;i<=colItems.size();i++) {
                    ...
                    DueDate=objItem.getAttribute("DUE_DATE").getString();
                    NewDate=objItem.getAttribute("NEW_DATE").getString();
                    String Qry ="UPDATE D_SAL_INVOICE_HEADER SET DUE_DATE='" + EITLERPGLOBAL.formatDateDB(NewDate) + "' "+
                    "WHERE INVOICE_NO = '" + InvoiceNo + "' AND DUE_DATE='" + EITLERPGLOBAL.formatDateDB(DueDate) + "' " +
                    "AND PARTY_CODE = '" + PartyCode + "' AND INVOICE_DATE = '" + EITLERPGLOBAL.formatDateDB(InvoiceDate) + "' ";
                    data.Execute(Qry);
                }
            }
    ...
    public boolean Update() {
        ...
        try {
            ...
            int RevNo=data.getIntValueFromDB("SELECT MAX(REVISION_NO) FROM D_SAL_INVOICE_DUE_DATE_HEADER_H WHERE COMPANY_ID="+getAttribute("COMPANY_ID").getInt()+" AND DOC_NO='"+getAttribute("DOC_NO").getString()+"'");
            RevNo++;
            ...
            data.Execute("DELETE FROM D_SAL_INVOICE_DUE_DATE_DETAIL WHERE COMPANY_ID="+getAttribute("COMPANY_ID").getInt()+" AND DOC_NO='"+theDocNo+"'");
            ...
            if(IsRejected) {
                ...
                data.Execute("UPDATE D_SAL_INVOICE_DUE_DATE_HEADER SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+getAttribute("COMPANY_ID").getInt()+" AND DOC_NO='"+ObjFlow.DocNo+"'");
            ...
            if(AStatus.equals("F")) //On Final Approval
            {
                ...
                for(int i=1;i<=colItems.size();i++) {
                    ...
                    System.out.println("Invoice no A - "+InvoiceNo);
                    System.out.println("Invoice Date A - "+InvoiceDate);
                    System.out.println("Invoice Due Date A - "+DueDate);
                    System.out.println("Invoice New Date A - "+NewDate);
    ...
    public boolean CanDelete(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM D_SAL_INVOICE_DUE_DATE_HEADER WHERE COMPANY_ID="+pCompanyID+" AND DOC_NO='"+pDocNo+"' AND (APPROVED=1)";
    ...
    public boolean Delete(int pUserID) {
        try {
            ...
            if(CanDelete(CompanyID,lDocNo,pUserID)) {
                String strQry = "DELETE FROM D_SAL_INVOICE_DUE_DATE_HEADER WHERE COMPANY_ID=" + CompanyID +" AND DOC_NO='"+lDocNo+"'";
                data.Execute(strQry);
                strQry = "DELETE FROM D_SAL_INVOICE_DUE_DATE_DETAIL WHERE COMPANY_ID=" + CompanyID +" AND DOC_NO='"+lDocNo+"'";
                data.Execute(strQry);
    ...
    public Object getObject(int CompanyID, String DocNo) {
        ...
        clsSalesInvoiceDueDate objSalesInvoiceDueDate =  new clsSalesInvoiceDueDate();
    ...
    public boolean Filter(String Condition,int CompanyID) {
        ...
        try {
            ...
            String strSql = "SELECT * FROM D_SAL_INVOICE_DUE_DATE_HEADER " + Condition ;
            Conn=data.getConn();
            Stmt=Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rsResultSet=Stmt.executeQuery(strSql);
            
            if(!rsResultSet.first()) {
                //strSql = "SELECT * FROM D_SAL_INVOICE_DUE_DATE_HEADER WHERE COMPANY_ID="+CompanyID+" AND DOC_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND DOC_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY DOC_NO";
                strSql = "SELECT * FROM D_SAL_INVOICE_DUE_DATE_HEADER " + Condition + " ORDER BY DOC_NO";
                rsResultSet=Stmt.executeQuery(strSql);
                Ready=true;
                MoveLast();
                return false;
    ...
    public boolean setData() {
        ...
        try {
            ...
            setAttribute("DOC_DATE",UtilFunctions.getString(rsResultSet,"DOC_DATE",""));
            ...
            setAttribute("FROM_DATE",UtilFunctions.getString(rsResultSet,"FROM_DATE",""));
            setAttribute("TO_DATE",UtilFunctions.getString(rsResultSet,"TO_DATE",""));
            ...
            setAttribute("CREATED_DATE",UtilFunctions.getString(rsResultSet,"CREATED_DATE",""));
            setAttribute("MODIFIED_BY",UtilFunctions.getString(rsResultSet,"MODIFIED_BY",""));
            setAttribute("MODIFIED_DATE",UtilFunctions.getString(rsResultSet,"MODIFIED_DATE",""));
            setAttribute("CHANGED",UtilFunctions.getInt(rsResultSet,"CHANGED",0));
            setAttribute("CHANGED_DATE",UtilFunctions.getString(rsResultSet,"CHANGED_DATE",""));
            setAttribute("APPROVED",UtilFunctions.getInt(rsResultSet,"APPROVED",0));
            setAttribute("APPROVED_DATE",UtilFunctions.getString(rsResultSet,"APPROVED_DATE",""));
            setAttribute("REJECTED",UtilFunctions.getInt(rsResultSet,"REJECTED",0));
            setAttribute("REJECTED_DATE",UtilFunctions.getString(rsResultSet,"REJECTED_DATE",""));
            ...
            if(HistoryView) {
                rsTmp=tmpStmt.executeQuery("SELECT * FROM D_SAL_INVOICE_DUE_DATE_DETAIL_H WHERE COMPANY_ID=" + CompanyID + " AND DOC_NO='" + VoucherNo + "' AND REVISION_NO="+RevNo+" ORDER BY SR_NO");
            }
            else {
                rsTmp=tmpStmt.executeQuery("SELECT * FROM D_SAL_INVOICE_DUE_DATE_DETAIL WHERE COMPANY_ID=" + CompanyID + " AND DOC_NO='" + VoucherNo + "' ORDER BY SR_NO");
            }
            ...
            while(!rsTmp.isAfterLast() && rsTmp.getRow()>0) {
                Counter++;
                clsSalesInvoiceDueDateItem objItem = new clsSalesInvoiceDueDateItem();
                ...
                objItem.setAttribute("INVOICE_DATE",UtilFunctions.getString(rsTmp,"INVOICE_DATE",""));
                ...
                objItem.setAttribute("DUE_DATE",UtilFunctions.getString(rsTmp,"DUE_DATE",""));
                objItem.setAttribute("DAYS",UtilFunctions.getInt(rsTmp,"DAYS",0));
                objItem.setAttribute("NEW_DATE",UtilFunctions.getString(rsTmp,"NEW_DATE",""));
                objItem.setAttribute("CREATED_BY",UtilFunctions.getString(rsTmp,"CREATED_BY",""));
                objItem.setAttribute("CREATED_DATE",UtilFunctions.getString(rsTmp,"CREATED_DATE",""));
                objItem.setAttribute("MODIFIED_BY",UtilFunctions.getString(rsTmp,"MODIFIED_BY",""));
                objItem.setAttribute("MODIFIED_DATE",UtilFunctions.getString(rsTmp,"MODIFIED_DATE",""));
                objItem.setAttribute("CHANGED",true);
                objItem.setAttribute("CHANGED_DATE",UtilFunctions.getString(rsTmp,"CHANGED_DATE",""));
    ...
    public static HashMap getPendingApprovals(int CompanyID,int UserID,int Order,int FinYearFrom) {
        ...
        try {
            ...
            if(Order==EITLERPGLOBAL.OnRecivedDate) {
                strSQL="SELECT D_SAL_INVOICE_DUE_DATE_HEADER.DOC_NO,D_SAL_INVOICE_DUE_DATE_HEADER.DOC_DATE,D_SAL_INVOICE_DUE_DATE_HEADER.MAIN_ACCOUNT_CODE,D_SAL_INVOICE_DUE_DATE_HEADER.PARTY_CODE,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.RECEIVED_DATE FROM D_SAL_INVOICE_DUE_DATE_HEADER,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA WHERE D_SAL_INVOICE_DUE_DATE_HEADER.DOC_NO="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.DOC_NO AND D_SAL_INVOICE_DUE_DATE_HEADER.COMPANY_ID="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.COMPANY_ID AND D_SAL_INVOICE_DUE_DATE_HEADER.COMPANY_ID="+CompanyID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.USER_ID="+UserID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.STATUS='W' AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.MODULE_ID="+ModuleID+" ORDER BY D_COM_DOC_DATA.RECEIVED_DATE";
            }
            
            if(Order==EITLERPGLOBAL.OnDocDate) {
                strSQL="SELECT D_SAL_INVOICE_DUE_DATE_HEADER.DOC_NO,D_SAL_INVOICE_DUE_DATE_HEADER.DOC_DATE,D_SAL_INVOICE_DUE_DATE_HEADER.MAIN_ACCOUNT_CODE,D_SAL_INVOICE_DUE_DATE_HEADER.PARTY_CODE,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.RECEIVED_DATE FROM D_SAL_INVOICE_DUE_DATE_HEADER,"+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA WHERE D_SAL_INVOICE_DUE_DATE_HEADER.DOC_NO="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.DOC_NO AND D_SAL_INVOICE_DUE_DATE_HEADER.COMPANY_ID="+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.COMPANY_ID AND D_SAL_INVOICE_DUE_DATE_HEADER.COMPANY_ID="+CompanyID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.USER_ID="+UserID+" AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.STATUS='W' AND "+EITLERPGLOBAL.DBName+".D_COM_DOC_DATA.MODULE_ID="+ModuleID+" ORDER BY D_SAL_INVOICE_DUE_DATE_HEADER.DOC_DATE";
            }
            ...
            while(rsTmp3.next()) {
                if(EITLERPGLOBAL.isWithinDate(rsTmp3.getString("DOC_DATE"),FinYearFrom)) {
                    Counter=Counter+1;
                    clsSalesInvoiceDueDate ObjDoc=new clsSalesInvoiceDueDate();
    ...
    public boolean ShowHistory(int CompanyID,String DocNo) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_SAL_INVOICE_DUE_DATE_HEADER_H WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"'");
            Ready=true;
            HistoryView=true;
            MoveFirst();
            return true;
    ...
    public static HashMap getHistoryList(int CompanyID,String DocNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT * FROM D_SAL_INVOICE_DUE_DATE_HEADER_H WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"'");
            rsTmp.first();
            
            if(rsTmp.getRow()>0) {
                
                while(!rsTmp.isAfterLast()) {
                    clsSalesInvoiceDueDate objVoucher=new clsSalesInvoiceDueDate();
    ...
    public static String getDocStatus(int CompanyID,String DocNo) {
        ...
        try {
            rsTmp=data.getResult("SELECT DOC_NO,APPROVED,CANCELLED FROM D_SAL_INVOICE_DUE_DATE_HEADER WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"'");
            rsTmp.first();
    ...
    public static boolean CanCancel(int CompanyID,String DocNo) {
        ...
        try {
            rsTmp=data.getResult("SELECT DOC_NO FROM D_SAL_INVOICE_DUE_DATE_HEADER WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"' AND APPROVED=0 AND CANCELLED=0");
            rsTmp.first();
    ...
    public static boolean CancelDoc(int CompanyID,String DocNo) {
        ...
        try {
            if(CanCancel(CompanyID,DocNo)) {
                ...
                data.Execute("UPDATE D_SAL_INVOICE_DUE_DATE_HEADER SET CANCELLED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+CompanyID+" AND DOC_NO='"+DocNo+"'");
                Cancelled=true;
            }
        } catch(Exception e) {
        }
