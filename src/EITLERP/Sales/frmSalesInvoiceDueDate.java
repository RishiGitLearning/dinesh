...
public class frmSalesInvoiceDueDate extends javax.swing.JApplet {
    
    private int EditMode=0;
    private clsSalesInvoiceDueDate objSalesInvoiceDueDate;
    ...
    public frmSalesInvoiceDueDate() {
        ...
        objSalesInvoiceDueDate=new clsSalesInvoiceDueDate();
        ...
        if(getName().equals("Link")) {
            ...
            if(objSalesInvoiceDueDate.LoadData(EITLERPGLOBAL.gCompanyID)) {
                objSalesInvoiceDueDate.MoveLast();
                DisplayData();
                SetMenuForRights();
            } else {
                JOptionPane.showMessageDialog(null,"Error occured while loading data. Error is "+objSalesInvoiceDueDate.LastError);
            }
        }
    }
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        HeaderDetail.add(txtFromDate);
        txtFromDate.setBounds(100, 70, 90, 20);

        jLabel4.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        jLabel4.setText("To Date :");
        HeaderDetail.add(jLabel4);
        jLabel4.setBounds(210, 70, 60, 20);

        txtToDate.setEnabled(false);
        HeaderDetail.add(txtToDate);
        txtToDate.setBounds(280, 70, 90, 20);
        ...
        cmdGenerateInvoices.setText("Generate Invoices");
        ...
        cmdGenerateInvoices.setBounds(380, 70, 160, 20);
        ...
        lblPartyName.setBounds(210, 40, 400, 20);
        ...
        txtRemarks.setBounds(100, 350, 440, 20);

        jLabel6.setText("Remarks :");
        Detail.add(jLabel6);
        jLabel6.setBounds(10, 350, 80, 15);
        ...
        cmdRemarksBig.setBounds(550, 350, 33, 21);

        MainPanel.addTab("Invoice Information", Detail);
    ...
    private void cmdNextPanelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdNextPanelActionPerformed
        ...
        for(int i=0;i<=TableInvoices.getRowCount();i++) {
            ...
            if(DataModelInvoices.getValueAt(i,0).toString().equals("true")) {
                ...
                rowData[1]=DataModelInvoices.getValueAt(i,2);
                rowData[2]=DataModelInvoices.getValueAt(i,3);
                rowData[3]=DataModelInvoices.getValueAt(i,4);
                rowData[4]=DataModelInvoices.getValueAt(i,5).toString();
                rowData[5]=DataModelInvoices.getValueAt(i,6).toString();
                rowData[6]="";
    ...
    private void cmdNormalViewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdNormalViewActionPerformed
        // TODO add your handling code here:
        objSalesInvoiceDueDate.LoadData(EITLERPGLOBAL.gCompanyID);
        MoveFirst();
    }//GEN-LAST:event_cmdNormalViewActionPerformed
    
    private void cmdViewHistoryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdViewHistoryActionPerformed
        // TODO add your handling code here:
        objSalesInvoiceDueDate.ShowHistory(EITLERPGLOBAL.gCompanyID, objSalesInvoiceDueDate.getAttribute("DOC_NO").getString());
        MoveFirst();
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if(EditMode==EITLERPGLOBAL.EDIT) {
            GenerateRejectedUserCombo();
            if(ApprovalFlow.IsOnceRejectedDoc(EITLERPGLOBAL.gCompanyID,clsSalesInvoiceDueDate.ModuleID , objSalesInvoiceDueDate.getAttribute("DOC_NO").getString())) {
                cmbSendTo.setEnabled(true);
            }
            else {
                cmbSendTo.setEnabled(false);
    ...
    private void DisplayData() {
        ...
        try {
            if(EditMode==0) {
                if(objSalesInvoiceDueDate.getAttribute("APPROVED").getInt()==1) {
                ...
                if(objSalesInvoiceDueDate.getAttribute("CANCELLED").getInt()==1) {
        ...
        if(EITLERPGLOBAL.gAuthorityUserID!=EITLERPGLOBAL.gUserID) {
            int ModuleID=clsSalesInvoiceDueDate.ModuleID;
        ...
        txtDocNo.setText(objSalesInvoiceDueDate.getAttribute("DOC_NO").getString());
        txtDocDate.setText(EITLERPGLOBAL.formatDate(objSalesInvoiceDueDate.getAttribute("DOC_DATE").getString()));
        txtPartyCode.setText(objSalesInvoiceDueDate.getAttribute("SUB_ACCOUNT_CODE").getString());
        EITLERPGLOBAL.setComboIndex(cmbMainCodeType,objSalesInvoiceDueDate.getAttribute("MAIN_ACCOUNT_CODE").getString());
        txtFromDate.setText(EITLERPGLOBAL.formatDate(objSalesInvoiceDueDate.getAttribute("FROM_DATE").getString()));
        txtToDate.setText(EITLERPGLOBAL.formatDate(objSalesInvoiceDueDate.getAttribute("TO_DATE").getString()));
        txtRemarks.setText(objSalesInvoiceDueDate.getAttribute("REMARKS").getString());
        ...
        EITLERPGLOBAL.setComboIndex(cmbHierarchy,objSalesInvoiceDueDate.getAttribute("HIERARCHY_ID").getInt());
        
        //======== Generating Grid for Document Approval Flow ========//
        FormatGridA();
        HashMap List=new HashMap();
        String DocNo=objSalesInvoiceDueDate.getAttribute("DOC_NO").getString();
        List=ApprovalFlow.getDocumentFlow(EITLERPGLOBAL.gCompanyID, clsSalesInvoiceDueDate.ModuleID , DocNo);
        ...
        HashMap History=clsSalesInvoiceDueDate.getHistoryList(EITLERPGLOBAL.gCompanyID, objSalesInvoiceDueDate.getAttribute("DOC_NO").getString());
        for(int i=1;i<=History.size();i++) {
            clsSalesInvoiceDueDate ObjHistory=(clsSalesInvoiceDueDate)History.get(Integer.toString(i));
        ...
        FormatGrid();
        FormatGridInvoices();
        for(int i=1;i<=objSalesInvoiceDueDate.colItems.size();i++) {
            clsSalesInvoiceDueDateItem objItem=(clsSalesInvoiceDueDateItem) objSalesInvoiceDueDate.colItems.get(Integer.toString(i));
            //-------------------------------------
            ...
            DataModel.setValueByVariable("NET_AMOUNT",Double.toString(objItem.getAttribute("NET_AMOUNT").getDouble()),NewRow);
            DataModel.setValueByVariable("DUE_DATE",EITLERPGLOBAL.formatDate(objItem.getAttribute("DUE_DATE").getString()),NewRow);
            DataModel.setValueByVariable("DAYS",Integer.toString(objItem.getAttribute("DAYS").getInt()),NewRow);
            DataModel.setValueByVariable("NEW_DATE",EITLERPGLOBAL.formatDate(objItem.getAttribute("NEW_DATE").getString()),NewRow);
            //-------------------------------------
        }
        //********************************************//
    }
    ...
    private void SetData() {
        objSalesInvoiceDueDate.setAttribute("PREFIX",SelPrefix);
        objSalesInvoiceDueDate.setAttribute("SUFFIX",SelSuffix);
        objSalesInvoiceDueDate.setAttribute("FFNO",FFNo);
        
        objSalesInvoiceDueDate.setAttribute("COMPANY_ID",EITLERPGLOBAL.gCompanyID);
        objSalesInvoiceDueDate.setAttribute("DOC_NO",txtDocNo.getText());
        objSalesInvoiceDueDate.setAttribute("DOC_DATE",txtDocDate.getText());
        objSalesInvoiceDueDate.setAttribute("REMARKS",txtRemarks.getText().trim());
        
        //----- Update Approval Specific Fields -----------//
        objSalesInvoiceDueDate.setAttribute("HIERARCHY_ID",EITLERPGLOBAL.getComboCode(cmbHierarchy));
        objSalesInvoiceDueDate.setAttribute("FROM",EITLERPGLOBAL.gNewUserID);
        objSalesInvoiceDueDate.setAttribute("TO",EITLERPGLOBAL.getComboCode(cmbSendTo));
        objSalesInvoiceDueDate.setAttribute("FROM_REMARKS",txtToRemarks.getText());
        objSalesInvoiceDueDate.setAttribute("MAIN_ACCOUNT_CODE",EITLERPGLOBAL.getCombostrCode(cmbMainCodeType));
        objSalesInvoiceDueDate.setAttribute("SUB_ACCOUNT_CODE",txtPartyCode.getText().trim());
        objSalesInvoiceDueDate.setAttribute("FROM_DATE",txtFromDate.getText().trim());
        objSalesInvoiceDueDate.setAttribute("TO_DATE",txtToDate.getText().trim());
        objSalesInvoiceDueDate.setAttribute("REJECTED_REASON","");
        
        if(OpgApprove.isSelected()) {
            objSalesInvoiceDueDate.setAttribute("APPROVAL_STATUS","A");
        }
        
        if(OpgFinal.isSelected()) {
            objSalesInvoiceDueDate.setAttribute("APPROVAL_STATUS","F");
        }
        
        if(OpgReject.isSelected()) {
            objSalesInvoiceDueDate.setAttribute("APPROVAL_STATUS","R");
            objSalesInvoiceDueDate.setAttribute("SEND_DOC_TO",EITLERPGLOBAL.getComboCode(cmbSendTo));
            objSalesInvoiceDueDate.setAttribute("REJECTED_REASON",txtToRemarks.getText().trim());
        }
        
        if(OpgHold.isSelected()) {
            objSalesInvoiceDueDate.setAttribute("APPROVAL_STATUS","H");
        }
        
        if(EditMode==EITLERPGLOBAL.ADD) {
            objSalesInvoiceDueDate.setAttribute("CREATED_BY",EITLERPGLOBAL.gUserID);
            objSalesInvoiceDueDate.setAttribute("CREATED_DATE",EITLERPGLOBAL.getCurrentDate());
            objSalesInvoiceDueDate.setAttribute("MODIFIED_BY","");
            objSalesInvoiceDueDate.setAttribute("MODIFIED_DATE","");
        } else {
            objSalesInvoiceDueDate.setAttribute("CREATED_BY","");
            objSalesInvoiceDueDate.setAttribute("CREATED_DATE","");
            objSalesInvoiceDueDate.setAttribute("MODIFIED_BY",EITLERPGLOBAL.gUserID);
            objSalesInvoiceDueDate.setAttribute("MODIFIED_DATE",EITLERPGLOBAL.getCurrentDate());
        }
        
        objSalesInvoiceDueDate.colItems.clear();
        ...
        for(int i=0;i<Table.getRowCount();i++) {
            ...
            objItem.setAttribute("NET_AMOUNT",Double.parseDouble(DataModel.getValueByVariable("NET_AMOUNT", i).toString()));
            objItem.setAttribute("DUE_DATE",DataModel.getValueByVariable("DUE_DATE", i));
            objItem.setAttribute("DAYS",Integer.parseInt(DataModel.getValueByVariable("DAYS", i)));
            objItem.setAttribute("NEW_DATE",DataModel.getValueByVariable("NEW_DATE", i));
            
            objSalesInvoiceDueDate.colItems.put(Integer.toString(objSalesInvoiceDueDate.colItems.size()+1),objItem);
        }
        //--------------------------------------------------------------------------------------------------//
    ...
    private void SetMenuForRights() {
        ...
        // --- Add Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,40211)) {
            cmdNew.setEnabled(true);
        }
        else {
            cmdNew.setEnabled(false);
        ...
        // --- Edit Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,40212)) {
            cmdEdit.setEnabled(true);
        }
        else {
            cmdEdit.setEnabled(false);
        ...
        // --- Delete Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,40213)) {
            cmdDelete.setEnabled(true);
        }
        else {
            cmdDelete.setEnabled(false);
        ...
        // --- Print Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,40214)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        }
        else {
    ...
    private void Add() {
        ...
        aList.ModuleID=clsSalesInvoiceDueDate.ModuleID;
        ...
        if(aList.ShowList()) {
            ...
            txtDocNo.setText(clsFirstFree.getNextFreeNo(EITLERPGLOBAL.gCompanyID, clsSalesInvoiceDueDate.ModuleID , FFNo,  false));
    ...
    private void Edit() {
        ...
        String lDocNo=objSalesInvoiceDueDate.getAttribute("DOC_NO").getString();
        
        if(objSalesInvoiceDueDate.IsEditable(EITLERPGLOBAL.gCompanyID, lDocNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if(ApprovalFlow.IsCreator(clsSalesInvoiceDueDate.ModuleID ,lDocNo)||clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 0,10532)) {
                SetFields(true);
            }
            else {
                EnableApproval();
    ...
    private void Delete() {
        ...
        if(objSalesInvoiceDueDate.Delete(EITLERPGLOBAL.gNewUserID)) {
            MoveLast();
        }
    }
    ...
    private void Save() {
        ...
        if(EditMode==EITLERPGLOBAL.ADD) {
            if(objSalesInvoiceDueDate.Insert()) {
                MoveLast();
                DisplayData();
            }
            else {
                JOptionPane.showMessageDialog(null," "+objSalesInvoiceDueDate.LastError);
                EITLERPGLOBAL.ChangeCursorToDefault(this);
                return;
            }
        }
        ...
        if(EditMode==EITLERPGLOBAL.EDIT) {
            if(objSalesInvoiceDueDate.Update()) {
                //Nothing to do
                DisplayData();
            }
            else {
    ...
    private void Find() {
        Loader ObjLoader=new Loader(this,"EITLERP.Sales.frmSalesInvoiceDueDateFind",true);
        frmSalesInvoiceDueDateFind ObjReturn= (frmSalesInvoiceDueDateFind) ObjLoader.getObj();
        
        if(ObjReturn.Cancelled==false) {
            if(!objSalesInvoiceDueDate.Filter(ObjReturn.strQuery,EITLERPGLOBAL.gCompanyID)) {
    ...
    //    objSalesInvoiceDueDate.Filter(" AND PROD_DOC_NO IN (SELECT DISTINCT PROD_DOC_NO FROM PRODUCTION.FELT_PROD_DATA, PRODUCTION.FELT_PROD_DOC_DATA WHERE PROD_DOC_NO=DOC_NO AND USER_ID="+EITLERPGLOBAL.gNewUserID+" AND STATUS='W' AND MODULE_ID=707 AND CANCELED=0) ");
        SetMenuForRights();
        DisplayData();
    }
    ...
    private void MoveFirst() {
        EITLERPGLOBAL.ChangeCursorToWait(this);
        objSalesInvoiceDueDate.MoveFirst();
        DisplayData();
        EITLERPGLOBAL.ChangeCursorToDefault(this);
    }
    ...
    private void MovePrevious() {
        EITLERPGLOBAL.ChangeCursorToWait(this);
        objSalesInvoiceDueDate.MovePrevious();
        DisplayData();
        EITLERPGLOBAL.ChangeCursorToDefault(this);
    }
    ...
    private void MoveNext() {
        EITLERPGLOBAL.ChangeCursorToWait(this);
        objSalesInvoiceDueDate.MoveNext();
        DisplayData();
        EITLERPGLOBAL.ChangeCursorToDefault(this);
    }
    ...
    public void FindEx(int pCompanyID,String pDocNo) {
        objSalesInvoiceDueDate.FilterEx(" WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND DOC_NO='"+pDocNo+"' ",pCompanyID);
        objSalesInvoiceDueDate.MoveFirst();
        DisplayData();
    }
    ...
    private void SetupApproval() {
        ...
        if(EditMode==EITLERPGLOBAL.ADD) {
            ...
            int FromUserID=ApprovalFlow.getFromID((int)EITLERPGLOBAL.gCompanyID, clsSalesInvoiceDueDate.ModuleID , objSalesInvoiceDueDate.getAttribute("DOC_NO").getString());
            lnFromID=FromUserID;
            String strFromUser=clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks=ApprovalFlow.getFromRemarks( (int)EITLERPGLOBAL.gCompanyID,clsSalesInvoiceDueDate.ModuleID ,FromUserID,objSalesInvoiceDueDate.getAttribute("DOC_NO").getString());
    ...
    private void GenerateFromCombo() {
        ...
        try {
            if(EditMode==EITLERPGLOBAL.ADD) {
                ...
                List=ApprovalFlow.getRemainingUsers((int)EITLERPGLOBAL.gCompanyID, clsSalesInvoiceDueDate.ModuleID ,objSalesInvoiceDueDate.getAttribute("DOC_NO").getString());
                for(int i=1;i<=List.size();i++) {
                    clsUser ObjUser=(clsUser) List.get(Integer.toString(i));
                    ComboData aData=new ComboData();
                    aData.Code=(int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void GenerateRejectedUserCombo() {
        ...
        for(int i=1;i<=List.size();i++) {
            ...
            if(EditMode==EITLERPGLOBAL.EDIT) {
                if(OpgApprove.isSelected()) {
                    IncludeUser=ApprovalFlow.IncludeUserInApproval(EITLERPGLOBAL.gCompanyID,clsSalesInvoiceDueDate.ModuleID , objSalesInvoiceDueDate.getAttribute("DOC_NO").getString(), ObjUser.getAttribute("USER_ID").getInt(), EITLERPGLOBAL.gNewUserID);
                }
                
                if(OpgReject.isSelected()) {
                    IncludeUser=ApprovalFlow.IncludeUserInRejection(EITLERPGLOBAL.gCompanyID, clsSalesInvoiceDueDate.ModuleID, objSalesInvoiceDueDate.getAttribute("DOC_NO").getString(), ObjUser.getAttribute("USER_ID").getInt() , EITLERPGLOBAL.gNewUserID);
                }
        ...
        if(EditMode==EITLERPGLOBAL.EDIT) {
            int Creator=ApprovalFlow.getCreator(EITLERPGLOBAL.gCompanyID, clsSalesInvoiceDueDate.ModuleID , objSalesInvoiceDueDate.getAttribute("DOC_NO").getString());
    ...
    private void GenerateCombos() {
        ...
        try {
            ...
            List=clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID="+clsSalesInvoiceDueDate.ModuleID);
            
            if(EditMode==EITLERPGLOBAL.EDIT) {
                List=clsHierarchy.getList(" WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID="+clsSalesInvoiceDueDate.ModuleID );
            }
            for(int i=1;i<=List.size();i++) {
    ...
    private void FormatGrid() {
        ...
        try {
            ...
            DataModel.addColumn("Net Amount"); //4
            DataModel.addColumn("Due Date."); //5
            DataModel.addColumn("Days"); //6
            DataModel.addColumn("New Due Date"); //7
            
            DataModel.SetVariable(0,"SR_NO"); //0 - Read Only
            DataModel.SetVariable(1,"INVOICE_NO"); //1 - Read Only
            DataModel.SetVariable(2,"INVOICE_DATE"); //2 - Read Only
            DataModel.SetVariable(3,"LINK_NO"); //3 - Read Only
            DataModel.SetVariable(4,"NET_AMOUNT"); //4
            DataModel.SetVariable(5,"DUE_DATE"); //5
            DataModel.SetVariable(6,"DAYS"); //6
            DataModel.SetVariable(7,"NEW_DATE"); //7
    ...
    private void FormatGridInvoices() {
        ...
        DataModelInvoices.addColumn("Link No"); //3
        DataModelInvoices.addColumn("Net Amount"); //4
        DataModelInvoices.addColumn("Due Date."); //5
        ...
        DataModelInvoices.SetVariable(2,"INVOICE_NO"); //1 - Read Only
        DataModelInvoices.SetVariable(3,"INVOICE_DATE"); //2 - Read Only
        DataModelInvoices.SetVariable(4,"LINK_NO"); //3 - Read Only
        DataModelInvoices.SetVariable(5,"NET_AMOUNT"); //4 -- Read Only
        DataModelInvoices.SetVariable(6,"DUE_DATE"); //5 - Read Only
    ...
    private boolean Validate() {
        ...
        for(int i=0;i<Table.getRowCount();i++) {
            if(DataModel.getValueByVariable("DAYS", i).toString().equals("") || DataModel.getValueByVariable("DAYS", i).toString().equals("0") || Integer.parseInt(DataModel.getValueByVariable("DAYS", i).toString())<0) {
                JOptionPane.showMessageDialog(null,"Enter proper days at sr no " + (i+1));
                return false;
            }
            
            if(!data.IsRecordExist("SELECT * FROM DINESHMILLS.D_SAL_INVOICE_HEADER WHERE PARTY_CODE='"+txtPartyCode.getText().trim()+"' AND INVOICE_NO='"+DataModel.getValueByVariable("INVOICE_NO", i).toString()+"' AND INVOICE_DATE='"+EITLERPGLOBAL.formatDateDB(DataModel.getValueByVariable("INVOICE_DATE", i).toString())+"' AND APPROVED=1 AND CANCELLED=0 ",FinanceGlobal.FinURL)) {
                JOptionPane.showMessageDialog(null,"Invoice with party code does not exists");
                return false;
            }
    ...
    private void InvoiceList() {
        try {
            ...
            if(String.valueOf(EITLERPGLOBAL.getCombostrCode(cmbMainCodeType)).equals("210027")) {
                InvoiceType = "1";
                Condition = " AND (PAYMENT_TERM_CODE LIKE '2%' OR PAYMENT_TERM_CODE LIKE '4%' OR PAYMENT_TERM_CODE LIKE '8%' OR PAYMENT_TERM_CODE LIKE '7%') ";
            }
            else if(String.valueOf(EITLERPGLOBAL.getCombostrCode(cmbMainCodeType)).equals("210010")) {
                InvoiceType = "2";
                Condition = " AND (PAYMENT_TERM_CODE LIKE '2%' OR PAYMENT_TERM_CODE LIKE '8%') ";
            }
            
            String Sql="SELECT INVOICE_NO,INVOICE_DATE,NET_AMOUNT,DUE_DATE "+
            "FROM D_SAL_INVOICE_HEADER "+
            "WHERE INVOICE_DATE>='" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText().trim()) + "' AND INVOICE_DATE<='" + EITLERPGLOBAL.formatDateDB(txtToDate.getText().trim()) + "' "+
            "AND PARTY_CODE ='" + txtPartyCode.getText().trim() +"' "+
            "AND INVOICE_TYPE =" + InvoiceType + " AND APPROVED = 1 AND CANCELLED = 0 " + Condition +
            "ORDER BY INVOICE_NO ";
            
            
            ResultSet rsInvoices = data.getResult(Sql);
            int SrNo=0;
            if(rsInvoices.getRow()>0) {
                while(!rsInvoices.isAfterLast()) {
                    ...
                    rowData[2]=rsInvoices.getString("INVOICE_NO");
                    rowData[3]=EITLERPGLOBAL.formatDate(rsInvoices.getString("INVOICE_DATE"));
                    rowData[4]=clsSalesInvoice.getAgentAlphaSrNo(rsInvoices.getString("INVOICE_NO"),rsInvoices.getString("INVOICE_DATE"),txtPartyCode.getText().trim());
                    rowData[5]=rsInvoices.getString("NET_AMOUNT");
                    rowData[6]=EITLERPGLOBAL.formatDate(rsInvoices.getString("DUE_DATE"));
