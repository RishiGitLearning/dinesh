...
public class clsSalesInvoiceDetailImport {
    
    /** Creates a new instance of clsSalesInvoiceDetailImport */
    public clsSalesInvoiceDetailImport() {
    ...
    public static void main(String[] args) {
        ...
        clsSalesInvoiceDetailImport objImport=new clsSalesInvoiceDetailImport();
        ...
        if(Type.equals("3")) {
            objImport.ImportInvoicesFilter(FileName);
        }
        
        
           if(Type.equals("4")) {
            objImport.ImportInvoicesBlanket(FileName);
        }
                ...
                try {
                    ...
                    if(!data.IsRecordExist(str,dbURL)) {
                        ...
                        if(!data.IsRecordExist("SELECT INVOICE_NO FROM D_SAL_INVOICE_DETAIL WHERE INVOICE_NO='"+InvoiceNo+"' AND SR_NO=" + SrNo ,dbURL)) {
                            ...
                            rsInvoice.updateInt("INVOICE_TYPE",1); Pointer+=1; //QIIN-ACC-TYPE
                            ...
                            rsInvoice.updateString("RECEIPT_DATE", "20"+ReceiptDate.substring(0,2)+"-"+ReceiptDate.substring(2,4)+"-"+ReceiptDate.substring(4));
                            ...
                            rsInvoice.updateString("HSN_CODE",FileRecord.substring(Pointer,Pointer+4)); Pointer+=4;//HSN_CODE
                            rsInvoice.updateString("QUALITY_FLAG",FileRecord.substring(Pointer,Pointer+1)); Pointer+=1;//QUALITY_FLAG
    ...
    public void ImportInvoicesFilter(String invoiceFile) {
                   ...
                   // String InvoiceNo="B"+FileRecord.substring(1,7);
                    ...
                    if(!data.IsRecordExist(str,dbURL)) {
                        ...
                        if(!data.IsRecordExist("SELECT INVOICE_NO FROM D_SAL_INVOICE_DETAIL WHERE INVOICE_NO='"+InvoiceNo+"' AND SR_NO=" + SrNo ,dbURL)) {
                            ...
                            rsInvoice.moveToInsertRow();
                            rsInvoice.updateInt("COMPANY_ID", EITLERPGLOBAL.gCompanyID);
                            rsInvoice.updateInt("INVOICE_TYPE",3); Pointer+=1; //QIIN-ACC-TYPE
                            rsInvoice.updateString("INVOICE_NO",InvoiceNo);  Pointer+=6; //QIIN-ACC-SRNO
                            rsInvoice.updateInt("SR_NO",SrNo);
                            rsInvoice.updateString("QUALITY_NO",FileRecord.substring(Pointer,Pointer+7));  Pointer+=7; //QIIN-QLTY-NO
                            rsInvoice.updateString("PATTERN_CODE",FileRecord.substring(Pointer,Pointer+2)); Pointer+=2; //QIIN-PATT-CD
                            ...
                            //                            rsInvoice.updateString("GATEPASS_NO",FileRecord.substring(Pointer,Pointer+6)); Pointer+=6;//QIIN-ETP-NO
                            //
                            rsInvoice.updateString("RECEIPT_DATE", "0000-00-00");
                            rsInvoice.updateDouble("BASIC_EXC",0);
                            rsInvoice.updateDouble("ADDITIONAL_DUTY",0);
                            rsInvoice.updateInt("AGENT_SR_NO", 0);
                            rsInvoice.updateString("AGENT_ALPHA","");
                            rsInvoice.updateString("GATEPASS_NO","");
    ...
    public void ImportInvoicesFelt(String invoiceFile) {
      ...
      public void ImportInvoicesBlanket(String invoiceFile) {
