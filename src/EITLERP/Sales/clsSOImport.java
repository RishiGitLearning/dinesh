...
public class clsSOImport {
    ...
    public void postDetailOrder(String OrdNO, String PartyCD) {
        try {
            ...
            String oDate = data.getStringValueFromDB("SELECT CONCAT('20',SUBSTR(ORDER_DATE,5,2) ,'-',SUBSTR(ORDER_DATE,3,2),'-',SUBSTR(ORDER_DATE,1,2)) AS ODATE FROM D_SAL_OF41 WHERE ORDER_NO='"+OrdNO+"'");
            ...
            sqlqry =sqlqry +" A.SH1, A.UNITS1, A.SH2, A.UNITS2, A.SH3, A.UNITS3, A.SH4, A.UNITS4, A.SH5, A.UNITS5, A.SH6, A.UNITS6, A.SH7, A.UNITS7, A.SH8, A.UNITS8, A.SH9, A.UNITS9, A.SH10, A.UNITS10,A.TOTL_UNITS ";
            sqlqry =sqlqry + " FROM D_SAL_SEASON_MASTER C, D_SAL_OF41 A ";
            ...
            sqlqry =sqlqry + "AND C.SEASON_ID = SUBSTR(PRICE_LIST_NO,3,3)  AND '"+oDate+"' >= DATE_FROM AND '"+oDate+"'<= DATE_TO ";
            ...
            //    String ShadeListNew = FindUniqueOrder(OrdNO, PartyCD,rsTmp2.getString("QLTY"),ShadeList);
                ...
                rsDetail.updateString("UNIT_ID", UtilFunctions.getString(rsTmp2,"UNIT_ID",""));
