...
public class clsCOExport {
    ...
    public static void main(String[] args) {
        try {
            ...
            clsCOExport objExport=new clsCOExport();
    ...
    public boolean ExportSalesOrders(String InitialDir) {
        try {
            ...
            String FileName="co"+Integer.toString(EITLERPGLOBAL.getCurrentYear())+Integer.toString(EITLERPGLOBAL.getCurrentMonth())+Integer.toString(EITLERPGLOBAL.getCurrentDay());
            ...
            ResultSet rsOrder=data.getResult("SELECT DISTINCT A.ORDER_NO,A.PARTY_CODE,ORDER_DATE,CITY_ID,TRANSPORTER_ID FROM D_SAL_ORDER_HEADER A,D_SAL_ORDER_DETAIL B WHERE A.ORDER_NO = B.ORDER_NO AND APPROVED=1 AND  A.CANCELLED=0 AND QUALITY_ID LIKE ('72%') AND (STATUS='' OR STATUS IS NULL) ");
            ...
            if(rsOrder.getRow()>0) {
                while(!rsOrder.isAfterLast()) {
                   ...
                   // OrderNo=rsOrder.getString("ORDER_NO").substring(2,8);
                    ...
                    OrderNo='X'+ rsOrder.getString("ORDER_NO");
                    }
                    ...
                    ResultSet rsOrderDetail=data.getResult("SELECT * FROM D_SAL_ORDER_DETAIL WHERE QUALITY_ID LIKE ('72%') AND ORDER_NO='"+rsOrder.getString("ORDER_NO")+"'");
                    rsOrderDetail.first();
