 ...
 * @author  
 */
public class frmMiscellaneousSales extends javax.swing.JApplet {
    ...
    private void initComponents() {
        ...
        Export_B2B.setBounds(10, 360, 170, 27);
...
//        }
    ...
    private void FormatGrid() {
        try {
            ...
            DataModelB2B.addColumn("Party code"); //1
            DataModelB2B.addColumn("Party Name"); //2
            DataModelB2B.addColumn("GSTIN"); //3
            DataModelB2B.addColumn("Invoice No"); //4
            ...
            DataModelB2B.addColumn("ECommerce GSTIN"); //10
            DataModelB2B.addColumn("Rate"); //11
            DataModelB2B.addColumn("Taxable Value"); //12
            //ADDED EXTRA FIELDS FOR MORE DETAIL
            DataModelB2B.addColumn("SGST Per"); //13
            DataModelB2B.addColumn("SGST Value"); //13
            DataModelB2B.addColumn("CGST Per"); //13
            DataModelB2B.addColumn("CGST Value"); //13
            DataModelB2B.addColumn("IGST Per"); //13
            DataModelB2B.addColumn("IGST Value"); //13            
            DataModelB2B.addColumn("HSN Code"); //13
            DataModelB2B.addColumn("Total Qtu"); //13
...
//                    + "MAX(COALESCE(CASE WHEN MAIN_ACCOUNT_CODE NOT IN (231757,231756,231758,231759,231761) AND EFFECT ='D'THEN MAIN_ACCOUNT_CODE END,0)) AS MAC_PURCHASE, "
                    ...
                    + "231756,231719,214836,127588,127587,127584,127583,127582,127581,127580,127579,127577,127576,127575,127574,127573,127572,127571,127570,127569,127568,127567,127566,127565,127171,127172) "
...
//                    + "AND EXTRACT(MONTH FROM VOUCHER_DATE) = 2 "
                    ...
                    + "AND SUBSTRING(H.VOUCHER_NO,1,2) IN ('PJ','PA') "
            ...
            while (!rs.isAfterLast()) {
                ...
                rowData[0] = cnt++;
                rowData[1] = rs.getString("SAB");
                rowData[2] = data.getStringValueFromDB("SELECT DISTINCT PARTY_NAME FROM D_FIN_PARTY_MASTER WHERE PARTY_CODE='"+rs.getString("SAB")+"'",FinanceGlobal.FinURL);
                //rowData[3] = df.format(rs.getDate("INVOICE_DATE"));
                rowData[3] = data.getStringValueFromDB("SELECT DISTINCT GSTIN_NO FROM D_FIN_PARTY_MASTER WHERE PARTY_CODE='"+rs.getString("SAB")+"'",FinanceGlobal.FinURL);
                rowData[4] = rs.getString("INVOICE_NO").toString();
                rowData[5] = rs.getString("INVOICE_DATE");
                rowData[6] = rs.getString("INV_AMT");
                //rowData[4] = rs.getString("MAC");
                rowData[7] = data.getStringValueFromDB("SELECT DISTINCT SUBSTRING(GSTIN_NO,1,2) FROM D_FIN_PARTY_MASTER WHERE PARTY_CODE='"+rs.getString("SAB")+"'",FinanceGlobal.FinURL);
                rowData[11] = rs.getString("RATE");                
                rowData[12] = rs.getString("TAXABLE_AMT");
                rowData[13] = rs.getString("SGST_PER");
                rowData[14] = rs.getString("SGST_AMT_C");                               
                rowData[15] = rs.getString("CGST_PER");                
                rowData[16] = rs.getString("CGST_AMT_C");
                rowData[17] = rs.getString("IGST_PER");                
                rowData[18] = rs.getString("IGST_AMT_C");
                rowData[19] = rs.getString("HSN_SAC_CODE");
