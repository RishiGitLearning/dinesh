                ...
                try {
                    ...
                    if (!data.IsRecordExist(str, dbURL)) {
                        ...
                        if (data.IsRecordExist("SELECT * FROM DINESHMILLS.D_SAL_INVOICE_HEADER WHERE INVOICE_NO='" + InvoiceNo + "' AND INVOICE_DATE='" + InvoiceDate + "' ")) {
                            data.Execute("UPDATE FINANCE.D_SAL_GSTR_INVOICE G,DINESHMILLS.D_SAL_INVOICE_HEADER H SET G.APPROVED=H.APPROVED,G.CANCELLED=H.CANCELLED WHERE INVOICE_NO='" + InvoiceNo + "' AND INVOICE_DATE='" + InvoiceDate + "' ");
                        }
                    }
                } catch (Exception c) {
                    c.printStackTrace();
