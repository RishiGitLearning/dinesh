...
public class frmDepositRefundFind extends javax.swing.JApplet {
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        lblDocNo.setBounds(10, 16, 80, 15);
    ...
    private void txtReceiptNoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtReceiptNoKeyPressed
        ...
        if(evt.getKeyCode()==112) //F1 Key pressed
        {
            ...
            aList.SQL="SELECT A.DOC_NO,A.DOC_DATE,B.RECEIPT_NO FROM D_FD_DEPOSIT_RENEW A,D_FD_DEPOSIT_MASTER B WHERE A.COMPANY_ID=B.COMPANY_ID AND A.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND A.RECEIPT_NO=B.OLD_RECEIPT_NO AND B.DEPOSIT_STATUS=0 ORDER BY A.DOC_NO";
            aList.ReturnCol=1;
            aList.SecondCol=3;
            ...
            if(aList.ShowLOV()) {
                if(!aList.ReturnVal.trim().equals("")) {
                    ...
                    lblDateFrom.setText("Doc Date :");
                    txtDateTo.setEnabled(false);
                    txtDateFrom.setText(EITLERPGLOBAL.formatDate(data.getStringValueFromDB("SELECT DOC_DATE FROM D_FD_DEPOSIT_RENEW WHERE DOC_NO='"+aList.ReturnVal+"' ",FinanceGlobal.FinURL)));
                    txtReceiptNo.setText(aList.SecondVal);
                }
            }
        }
    ...
    private void txtReceiptNoFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtReceiptNoFocusLost
        ...
        try {
            if(!txtReceiptNo.getText().equals("")) {
                if(data.IsRecordExist("SELECT COUNT(*) FROM D_FD_DEPOSIT_MASTER WHERE RECEIPT_NO='"+txtReceiptNo.getText()+"' AND DEPOSIT_STATUS=0 ",FinanceGlobal.FinURL)) {
                    String SQL = "SELECT A.DOC_NO,A.DOC_DATE,B.RECEIPT_NO FROM D_FD_DEPOSIT_RENEW A, D_FD_DEPOSIT_MASTER B WHERE A.COMPANY_ID=B.COMPANY_ID AND A.COMPANY_ID=2 AND B.OLD_RECEIPT_NO=A.RECEIPT_NO AND B.RECEIPT_NO='"+txtReceiptNo.getText()+"' AND B.DEPOSIT_STATUS=0 ORDER BY A.DOC_NO";
                    ...
                    rsTmp.first();
                    txtDocNo.setText(rsTmp.getString("DOC_NO"));
                    lblDateFrom.setText("Doc Date :");
                    txtDateTo.setEnabled(false);
                    txtDateFrom.setText(EITLERPGLOBAL.formatDate(rsTmp.getString("DOC_DATE")));
    ...
    private void txtDateToFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtDateToFocusLost
        // TODO add your handling code here:
        Validate();
    }//GEN-LAST:event_txtDateToFocusLost
    
    private void txtDocNoFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtDocNoFocusLost
        // TODO add your handling code here:
        try {
            if(!txtDocNo.getText().equals("")) {
                if(data.IsRecordExist("SELECT COUNT(*) FROM D_FD_DEPOSIT_RENEW WHERE DOC_NO='"+txtDocNo.getText()+"' ",FinanceGlobal.FinURL)) {
                    
                    String SQL="SELECT A.DOC_NO,A.DOC_DATE,B.RECEIPT_NO FROM D_FD_DEPOSIT_RENEW A,D_FD_DEPOSIT_MASTER B WHERE A.COMPANY_ID=B.COMPANY_ID AND A.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND A.RECEIPT_NO=B.OLD_RECEIPT_NO AND B.DEPOSIT_STATUS=0 ORDER BY A.DOC_NO";
                    Statement stTmp = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
                    ResultSet rsTmp = stTmp.executeQuery(SQL);
                    rsTmp.first();
                    ...
                    lblShowMessage.setText("  Wrong Doc No ...");
                }
            }
        } catch(Exception e) {
            e.printStackTrace();
    ...
    private void txtDateFromFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtDateFromFocusLost
        // TODO add your handling code here:
        Validate();
    }//GEN-LAST:event_txtDateFromFocusLost
    ...
    private void cmdFindActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdFindActionPerformed
        ...
        strQuery=" WHERE A.COMPANY_ID="+Long.toString(EITLERPGLOBAL.gCompanyID)+" AND A.COMPANY_ID=B.COMPANY_ID AND B.DEPOSIT_STATUS=0 AND A.RECEIPT_NO=B.OLD_RECEIPT_NO ";
        ...
        if(!txtReceiptNo.getText().equals("")) {
            strQuery=strQuery+" AND B.RECEIPT_NO='"+txtReceiptNo.getText()+"' ";
        }
        
        if(lblDateFrom.getText().equals("Doc Date :")) {
            strQuery=strQuery+" AND A.DOC_DATE='"+EITLERPGLOBAL.formatDateDB(txtDateFrom.getText())+"' ";
        } else {
            strQuery=strQuery+" AND A.DOC_DATE>='"+EITLERPGLOBAL.formatDateDB(txtDateFrom.getText())+"' AND A.DOC_DATE<='"+ EITLERPGLOBAL.formatDateDB(txtDateTo.getText()) +"' ";
        }
    ...
    private void Validate() {
        ...
        if(txtDateTo.isEnabled()){
            if(!txtDateFrom.equals("")){
                if(!EITLERPGLOBAL.isDate(txtDateFrom.getText())) {
        ...
        if(txtDateTo.isEnabled()) {
            if(!txtDateTo.equals("")){
                if(!EITLERPGLOBAL.isDate(txtDateTo.getText())) {
                    lblShowMessage.setText("Wrong To Date...");
                    txtDateTo.requestFocus();
                    return;
                }
                lblShowMessage.setText("  ...");
                return;
