...
public class clsPolicyTurnoverSlabs{
    ...
    public clsPolicyTurnoverSlabs() {
        ...
        props.put("FROM_VALUE",new Variant(0.0));
        props.put("TO_VALUE",new Variant(0.0));
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM D_SAL_POLICY_TURNOVER_SLABS ORDER BY SR_NO");
            Ready=true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHistory=stHistory.executeQuery("SELECT * FROM D_SAL_POLICY_TURNOVER_SLABS_H WHERE POLICY_ID='1'"); // '1' for restricting all data retrieval
            rsHistory.first();
            //------------------------------------//
            
            
            long SrNo=data.getMaxID(EITLERPGLOBAL.gCompanyID,"D_SAL_POLICY_TURNOVER_SLABS","SR_NO");
            ...
            rsResultSet.updateDouble("FROM_VALUE",getAttribute("FROM_VALUE").getDouble());
            rsResultSet.updateDouble("TO_VALUE",getAttribute("TO_VALUE").getDouble());
            ...
            rsHistory.updateDouble("FROM_VALUE",getAttribute("FROM_VALUE").getDouble());
            rsHistory.updateDouble("TO_VALUE",getAttribute("TO_VALUE").getDouble());
    ...
    public boolean Update() {
        ...
        try {
            ...
            //========= Inserting Into History =================//
            int RevNo=data.getIntValueFromDB("SELECT MAX(REVISION_NO) FROM D_SAL_POLICY_TURNOVER_SLABS_H WHERE POLICY_ID='"+getAttribute("POLICY_ID").getString()+"'");
    ...
    public boolean CanDelete(int CompanyID,String PolicyID,int UserID,int SrNo) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM D_SAL_POLICY_TURNOVER_SLABS WHERE POLICY_ID='"+PolicyID+"' AND SR_NO="+ SrNo +" ";
    ...
    public boolean Delete(int UserID) {
        try {
            ...
            if(CanDelete(EITLERPGLOBAL.gCompanyID,PolicyID,UserID,SrNo)) {
                String strSQL = "DELETE FROM D_SAL_POLICY_TURNOVER_SLABS WHERE POLICY_ID='"+PolicyID+"' AND SR_NO ="+SrNo;
                data.Execute(strSQL);
    ...
    public boolean Filter(String Condition,int CompanyID) {
        ...
        try {
            String strSQL = "SELECT * FROM D_SAL_POLICY_TURNOVER_SLABS " + Condition ;
            Conn=data.getConn();
            Stmt=Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rsResultSet=Stmt.executeQuery(strSQL);
            
            if(!rsResultSet.first()) {
                strSQL = "SELECT * FROM D_SAL_POLICY_TURNOVER_SLABS ORDER BY SR_NO ";
                rsResultSet=Stmt.executeQuery(strSQL);
                Ready=true;
                MoveLast();
                return false;
    ...
    public boolean setData() {
        ...
        try {
            ...
            setAttribute("FROM_VALUE",UtilFunctions.getDouble(rsResultSet,"FROM_VALUE", 0));
            setAttribute("TO_VALUE",UtilFunctions.getDouble(rsResultSet,"TO_VALUE", 0));
