/*
 * frmGRNFind.java
 *
 * Created on May 15, 2004, 1:35 PM
 */

package EITLERP.Finance;

/**
 *
 * @author  nrpithva
 */

/*<APPLET CODE=frmGRNFind.class HEIGHT=405 WIDTH=565></APPLET>*/

import javax.swing.*;
import java.awt.*;
import java.util.*;
import EITLERP.*;
import java.sql.*;


public class frmFASCardwithoutGRNFind extends javax.swing.JApplet {
    
    private EITLComboModel cmbStatusModel;
    
    public boolean Cancelled=false;
    public String strQuery;
    
    
    /** Initializes the applet frmGRNFind */
    public void init() {
        System.gc();
        setSize(550,250);
        initComponents();
    }
    
    /** This method is called from within the init() method to
     * initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is
     * always regenerated by the Form Editor.
     */
    private void initComponents() {//GEN-BEGIN:initComponents
        jLabel1 = new javax.swing.JLabel();
        cmdFind = new javax.swing.JButton();
        cmdCancel = new javax.swing.JButton();
        cmdClear = new javax.swing.JButton();
        jPanel1 = new javax.swing.JPanel();
        jLabel2 = new javax.swing.JLabel();
        txtAssetNo = new javax.swing.JTextField();
        jLabel13 = new javax.swing.JLabel();
        txtFromDate = new javax.swing.JTextField();
        jLabel14 = new javax.swing.JLabel();
        txtToDate = new javax.swing.JTextField();
        txtItemID = new javax.swing.JTextField();
        jLabel3 = new javax.swing.JLabel();
        txtSupplierID = new javax.swing.JTextField();
        jLabel4 = new javax.swing.JLabel();
        lblItemName = new javax.swing.JLabel();
        lblSupplierName = new javax.swing.JLabel();
        jLabel5 = new javax.swing.JLabel();
        txtItemName = new javax.swing.JTextField();
        lblTitle = new javax.swing.JLabel();

        getContentPane().setLayout(null);

        addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                formMouseClicked(evt);
            }
        });

        jLabel1.setFont(new java.awt.Font("Arial", 1, 12));
        jLabel1.setText("Find Records");
        getContentPane().add(jLabel1);
        jLabel1.setBounds(0, 40, 170, 15);

        cmdFind.setText("Find");
        cmdFind.setBorder(new javax.swing.border.SoftBevelBorder(javax.swing.border.BevelBorder.RAISED));
        cmdFind.setNextFocusableComponent(cmdCancel);
        cmdFind.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cmdFindActionPerformed(evt);
            }
        });

        getContentPane().add(cmdFind);
        cmdFind.setBounds(470, 70, 70, 25);

        cmdCancel.setText("Cancel");
        cmdCancel.setBorder(new javax.swing.border.SoftBevelBorder(javax.swing.border.BevelBorder.RAISED));
        cmdCancel.setNextFocusableComponent(cmdClear);
        cmdCancel.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cmdCancelActionPerformed(evt);
            }
        });

        getContentPane().add(cmdCancel);
        cmdCancel.setBounds(470, 110, 70, 25);

        cmdClear.setText("Clear");
        cmdClear.setBorder(new javax.swing.border.SoftBevelBorder(javax.swing.border.BevelBorder.RAISED));
        cmdClear.setNextFocusableComponent(txtAssetNo);
        cmdClear.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cmdClearActionPerformed(evt);
            }
        });

        getContentPane().add(cmdClear);
        cmdClear.setBounds(470, 140, 70, 25);

        jPanel1.setLayout(null);

        jPanel1.setBorder(new javax.swing.border.EtchedBorder(javax.swing.border.EtchedBorder.RAISED));
        jLabel2.setDisplayedMnemonic('G');
        jLabel2.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        jLabel2.setLabelFor(txtAssetNo);
        jLabel2.setText("Asset No :");
        jPanel1.add(jLabel2);
        jLabel2.setBounds(5, 16, 80, 15);

        jPanel1.add(txtAssetNo);
        txtAssetNo.setBounds(90, 16, 90, 19);

        jLabel13.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        jLabel13.setText("From Date :");
        jPanel1.add(jLabel13);
        jLabel13.setBounds(5, 48, 80, 15);

        txtFromDate.addFocusListener(new java.awt.event.FocusAdapter() {
            public void focusLost(java.awt.event.FocusEvent evt) {
                txtFromDateFocusLost(evt);
            }
        });
        txtFromDate.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                txtFromDateKeyPressed(evt);
            }
        });

        jPanel1.add(txtFromDate);
        txtFromDate.setBounds(90, 50, 90, 19);

        jLabel14.setText("To Date :");
        jPanel1.add(jLabel14);
        jLabel14.setBounds(190, 50, 80, 15);

        jPanel1.add(txtToDate);
        txtToDate.setBounds(270, 50, 90, 19);

        txtItemID.addFocusListener(new java.awt.event.FocusAdapter() {
            public void focusLost(java.awt.event.FocusEvent evt) {
                txtItemIDFocusLost(evt);
            }
        });
        txtItemID.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                txtItemIDKeyPressed(evt);
            }
        });

        jPanel1.add(txtItemID);
        txtItemID.setBounds(90, 80, 90, 19);

        jLabel3.setDisplayedMnemonic('G');
        jLabel3.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        jLabel3.setLabelFor(txtAssetNo);
        jLabel3.setText("Item ID :");
        jPanel1.add(jLabel3);
        jLabel3.setBounds(5, 80, 80, 15);

        txtSupplierID.addFocusListener(new java.awt.event.FocusAdapter() {
            public void focusLost(java.awt.event.FocusEvent evt) {
                txtSupplierIDFocusLost(evt);
            }
        });
        txtSupplierID.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                txtSupplierIDKeyPressed(evt);
            }
        });

        jPanel1.add(txtSupplierID);
        txtSupplierID.setBounds(90, 130, 90, 19);

        jLabel4.setDisplayedMnemonic('G');
        jLabel4.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        jLabel4.setLabelFor(txtAssetNo);
        jLabel4.setText("Supplier ID :");
        jPanel1.add(jLabel4);
        jLabel4.setBounds(10, 130, 80, 15);

        lblItemName.setText("....");
        jPanel1.add(lblItemName);
        lblItemName.setBounds(200, 80, 250, 15);

        lblSupplierName.setText("....");
        jPanel1.add(lblSupplierName);
        lblSupplierName.setBounds(200, 130, 250, 15);

        jLabel5.setDisplayedMnemonic('G');
        jLabel5.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        jLabel5.setLabelFor(txtAssetNo);
        jLabel5.setText("Item Name :");
        jPanel1.add(jLabel5);
        jLabel5.setBounds(5, 105, 80, 15);

        jPanel1.add(txtItemName);
        txtItemName.setBounds(90, 105, 310, 19);

        getContentPane().add(jPanel1);
        jPanel1.setBounds(0, 70, 458, 190);

        lblTitle.setBackground(new java.awt.Color(0, 102, 153));
        lblTitle.setForeground(java.awt.Color.white);
        lblTitle.setText("Fixed Asset Card (Without GRN) Find");
        lblTitle.setBorder(new javax.swing.border.BevelBorder(javax.swing.border.BevelBorder.RAISED));
        lblTitle.setOpaque(true);
        getContentPane().add(lblTitle);
        lblTitle.setBounds(0, 10, 550, 25);

    }//GEN-END:initComponents
    
    private void txtSupplierIDFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtSupplierIDFocusLost
        // TODO add your handling code here:
        try {
            if(!txtSupplierID.getText().trim().equals("")) //F1 Key pressed
            {
                LOV aList=new LOV();
                
                String SQL="SELECT SUPPLIER_CODE,SUPP_NAME FROM D_COM_SUPP_MASTER WHERE  COMPANY_ID = '" + EITLERPGLOBAL.gCompanyID + "' AND SUPPLIER_CODE = '" + txtSupplierID.getText().trim() + "' AND APPROVED=1 AND CANCELLED=0";
                ResultSet rsSupp = data.getResult(SQL);
                if(rsSupp.getRow()>0) {
                    lblSupplierName.setText(rsSupp.getString("SUPP_NAME"));
                }
                else {
                    JOptionPane.showMessageDialog(null,"Invalid Supplier ID");
                }
            }
            else {
                lblSupplierName.setText("");
            }
        }
        catch(Exception e) {
            
        }
    }//GEN-LAST:event_txtSupplierIDFocusLost
    
    private void txtSupplierIDKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtSupplierIDKeyPressed
        // TODO add your handling code here:
        if(evt.getKeyCode()==112) //F1 Key pressed
        {
            LOV aList=new LOV();
            
            aList.SQL="SELECT SUPPLIER_CODE,SUPP_NAME FROM D_COM_SUPP_MASTER WHERE  COMPANY_ID = '" + EITLERPGLOBAL.gCompanyID + "' AND APPROVED=1 AND CANCELLED=0";
            aList.ReturnCol=1;
            aList.SecondCol=2;
            aList.ShowReturnCol=true;
            aList.DefaultSearchOn=2;
            
            if(aList.ShowLOV()) {
                txtSupplierID.setText(aList.ReturnVal);
                lblSupplierName.setText(aList.SecondVal);
            }
        }
        
    }//GEN-LAST:event_txtSupplierIDKeyPressed
    
    private void txtItemIDKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtItemIDKeyPressed
        // TODO add your handling code here:
        if(evt.getKeyCode()==112) //F1 Key pressed
        {
            //String CompanyDatabase="";
            LOV aList=new LOV();
           /* if(EITLERPGLOBAL.gCompanyID ==2) {
                CompanyDatabase="DINESHMILLS";
            }
            else if(EITLERPGLOBAL.gCompanyID ==3) {
                CompanyDatabase="DINESHMILLSA";
            
            }*/
            aList.SQL="SELECT A.ITEM_ID,A.ITEM_DESCRIPTION FROM "+ EITLERPGLOBAL.DBName +".D_INV_ITEM_MASTER A,FINANCE.D_FAS_ITEM_MASTER_HEADER B "+
            "WHERE B.COMPANY_ID = '" + EITLERPGLOBAL.gCompanyID + "' AND A.ITEM_ID = B.ITEM_ID  "+
            "AND A.APPROVED=1 AND A.CANCELLED = 0   "+
            "AND B.APPROVED=1 AND B.CANCELLED = 0   "+
            "AND A.CATEGORY_ID=1 "+
            "ORDER BY A.ITEM_ID";
            
            aList.ReturnCol=1;
            aList.SecondCol=2;
            aList.ShowReturnCol=true;
            aList.DefaultSearchOn=2;
            
            if(aList.ShowLOV()) {
                txtItemID.setText(aList.ReturnVal);
                lblItemName.setText(aList.SecondVal);
            }
            
            
        }
    }//GEN-LAST:event_txtItemIDKeyPressed
    
    private void txtItemIDFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtItemIDFocusLost
        // TODO add your handling code here:
        if(!txtItemID.getText().trim().equals("")) {
            lblItemName.setText(clsItem.getItemName(EITLERPGLOBAL.gCompanyID,txtItemID.getText().trim()));
        }
    }//GEN-LAST:event_txtItemIDFocusLost
    
    private void txtFromDateFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtFromDateFocusLost
        // TODO add your handling code here:
        
    }//GEN-LAST:event_txtFromDateFocusLost
    
    private void txtFromDateKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtFromDateKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_txtFromDateKeyPressed
    
    private void formMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_formMouseClicked
        // TODO add your handling code here:
        
    }//GEN-LAST:event_formMouseClicked
    
    private void cmdClearActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdClearActionPerformed
        // TODO add your handling code here:
        strQuery=" WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" ";
        Cancelled=false;
        getParent().getParent().getParent().getParent().show(false);
    }//GEN-LAST:event_cmdClearActionPerformed
    
    private void cmdCancelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdCancelActionPerformed
        // TODO add your handling code here:
        strQuery="";
        Cancelled=true;
        getParent().getParent().getParent().getParent().show(false);
    }//GEN-LAST:event_cmdCancelActionPerformed
    
    private void cmdFindActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdFindActionPerformed
        // TODO add your handling code here:
        Cancelled=false;
        String subQuery="";
        
        strQuery=" WHERE COMPANY_ID="+Long.toString(EITLERPGLOBAL.gCompanyID)+" AND ASSET_TYPE = '" + clsFASCardwithoutGRN.AssetType + "' ";
        
        if(!txtAssetNo.getText().equals("")) {
            strQuery=strQuery+" AND ASSET_NO='"+txtAssetNo.getText()+"' ";
        }
        
        if(!txtFromDate.getText().equals("")) {
            strQuery=strQuery+" AND ASSET_DATE >= '"+EITLERPGLOBAL.formatDateDB(txtFromDate.getText())+"' ";
        }
        
        if(!txtToDate.getText().equals("")) {
            strQuery=strQuery+" AND ASSET_DATE <= '"+EITLERPGLOBAL.formatDateDB(txtToDate.getText())+"' ";
        }
        
        
        if(!txtItemID.getText().equals("")) {
            strQuery=strQuery+" AND ITEM_ID = '"+txtItemID.getText()+"' ";
        }
        
        if(!txtItemName.getText().equals("")) {
            strQuery=strQuery+" AND ITEM_DESC LIKE '%"+txtItemName.getText()+"%' ";
            
        }
        
        if(!txtSupplierID.getText().equals("")) {
            strQuery=strQuery+" AND SUPPLIER_CODE= '"+txtSupplierID.getText()+"' ";
        }
        
        strQuery=strQuery;
        //========== End Sub Query ============//
        
        getParent().getParent().getParent().getParent().show(false);
    }//GEN-LAST:event_cmdFindActionPerformed
    
    
    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton cmdCancel;
    private javax.swing.JButton cmdClear;
    private javax.swing.JButton cmdFind;
    private javax.swing.JLabel jLabel1;
    private javax.swing.JLabel jLabel13;
    private javax.swing.JLabel jLabel14;
    private javax.swing.JLabel jLabel2;
    private javax.swing.JLabel jLabel3;
    private javax.swing.JLabel jLabel4;
    private javax.swing.JLabel jLabel5;
    private javax.swing.JPanel jPanel1;
    private javax.swing.JLabel lblItemName;
    private javax.swing.JLabel lblSupplierName;
    private javax.swing.JLabel lblTitle;
    private javax.swing.JTextField txtAssetNo;
    private javax.swing.JTextField txtFromDate;
    private javax.swing.JTextField txtItemID;
    private javax.swing.JTextField txtItemName;
    private javax.swing.JTextField txtSupplierID;
    private javax.swing.JTextField txtToDate;
    // End of variables declaration//GEN-END:variables
    
    
    
}
