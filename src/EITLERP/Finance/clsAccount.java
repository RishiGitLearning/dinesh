        ...
        try {
            if(!SubCode.trim().equals("")) {
                ...
                if(data.IsRecordExist(strSQL,FinanceGlobal.FinURL)) {
                    ...
                    OpeningDrTotal=data.getDoubleValueFromDB(strSQL, FinanceGlobal.FinURL);
                    System.out.println("Main Code : "+ MainCode +"Opening Dr total : "+ OpeningDrTotal);
                    Opening=OpeningDrTotal+OpeningCrTotal;
                ...
                if(!OpeningDate.equals("")) {
                    ...
                    DrTotal=data.getDoubleValueFromDB(strSQL,FinanceGlobal.FinURL);
                    System.out.println("1 CrTotal :"+CrTotal+"DrTotal :"+DrTotal);
                }
                else {
                    ...
                    DrTotal=data.getDoubleValueFromDB(strSQL,FinanceGlobal.FinURL);
                    System.out.println("2 CrTotal :"+CrTotal+"DrTotal :"+DrTotal);
                }
                ClosingBalance=Opening+DrTotal+CrTotal;
            }
        }
        ...
        try {
            ...
            if(!SubCode.trim().equals("")) {
                ...
                if(data.IsRecordExist(strSQL,FinanceGlobal.FinURL)) {
                    ...
                    strSQL="SELECT ROUND(IF(SUM(AMOUNT) IS NULL,0,SUM(AMOUNT)),2) FROM D_FIN_OPENING_DETAIL WHERE  EFFECT='C' AND MAIN_ACCOUNT_CODE='"+MainCode+"' AND SUB_ACCOUNT_CODE='"+SubCode+"' AND ENTRY_NO="+EntryNo;
                    OpeningCrTotal=-1*data.getDoubleValueFromDB(strSQL, FinanceGlobal.FinURL);
                    System.out.println("Main Code : "+ MainCode +"Opening Cr total : "+ OpeningCrTotal);
                    strSQL="SELECT ROUND(IF(SUM(AMOUNT) IS NULL,0,SUM(AMOUNT)),2) FROM D_FIN_OPENING_DETAIL WHERE  EFFECT='D' AND MAIN_ACCOUNT_CODE='"+MainCode+"' AND SUB_ACCOUNT_CODE='"+SubCode+"' AND ENTRY_NO="+EntryNo;
                    OpeningDrTotal=data.getDoubleValueFromDB(strSQL, FinanceGlobal.FinURL);
                    System.out.println("Main Code : "+ MainCode +"Opening Dr total : "+ OpeningDrTotal);
                    Opening=OpeningDrTotal+OpeningCrTotal;
...
//            
            String strSQL="SELECT A.COMPANY_ID,B.REF_COMPANY_ID,A.APPROVED,A.VOUCHER_NO,A.VOUCHER_DATE," +
            ///"SUM(B.AMOUNT) AMOUNT,'' INVOICE_NO,'' INVOICE_DATE,B.GRN_NO,B.GRN_NO,B.PO_NO,B.PO_DATE,B.REMARKS," +
                    "SUM(B.AMOUNT) AS AMOUNT,'' AS INVOICE_NO,'' AS INVOICE_DATE,GRN_NO,GRN_NO,PO_NO,PO_DATE,B.REMARKS AS REMARKS," +
            "B.MODULE_ID,B.LINK_NO FROM D_FIN_VOUCHER_HEADER A,D_FIN_VOUCHER_DETAIL B WHERE A.VOUCHER_TYPE="+VoucherType+" " +
            "AND A.APPROVED=1 AND A.CANCELLED=0 AND A.VOUCHER_NO=B.VOUCHER_NO AND B.EFFECT='D' AND " +
            "B.MAIN_ACCOUNT_CODE='"+MainCode+"' AND (B.MATCHED=0 OR B.MATCHED IS NULL) AND " +
            "B.SUB_ACCOUNT_CODE='"+PartyCode+"' GROUP BY A.VOUCHER_NO ORDER BY A.VOUCHER_DATE";
                    ...
                    if(Continue) {
                        ...
                        if(BlockNo>0) {
                            ...
                            if(rsOpposite.getRow()>0) {
                                ...
                                //comment start by ashutosh to change account name in gridview
                                /*
                                if(VoucherType==FinanceGlobal.TYPE_JOURNAL) {
                                    AcName = clsAccount.getAccountName(vMainCode, vSubCode);
                                }
                                 */
                                //comment end by ashutosh to change account name in gridview
                                /*if(!SubCode.equals("")) {
    ...
    public static HashMap getLedger(String MainCode,String FromSubCode,String ToSubCode,String FromDate,String ToDate,boolean OnlyApproved,double FromAmount,double ToAmount,String BookCode) {
        ...
        try {
            ...
            if ((!FromSubCode.trim().equals("")) && (ToSubCode.trim().equals(""))) {
                cond = "AND SUB_ACCOUNT_CODE >= '"+FromSubCode+"' ";
            }
            else if ((!FromSubCode.trim().equals("")) && (!ToSubCode.trim().equals(""))) {
                cond = "AND SUB_ACCOUNT_CODE BETWEEN '"+FromSubCode+"' AND '"+ToSubCode+"' ";
            }
            else if ((FromSubCode.trim().equals("")) && (!ToSubCode.trim().equals(""))) {
                cond = "AND SUB_ACCOUNT_CODE <= '"+FromSubCode+"' ";
            }
            ...
            if ((FromSubCode.trim().equals(""))  && (ToSubCode.trim().equals(""))) {
                if(OnlyApproved) {
                    strSQL="SELECT DISTINCT SUB_ACCOUNT_CODE FROM  D_FIN_VOUCHER_HEADER A,D_FIN_VOUCHER_DETAIL_EX  B WHERE A.CANCELLED=0 AND A.VOUCHER_DATE>='"+FromDate+"' AND A.VOUCHER_DATE<='"+ToDate+"' AND B.MAIN_ACCOUNT_CODE='"+MainCode+"' AND A.VOUCHER_NO=B.VOUCHER_NO AND A.APPROVED=1 "+Condition+" ORDER BY A.VOUCHER_DATE,A.VOUCHER_NO, B.MAIN_ACCOUNT_CODE,B.SUB_ACCOUNT_CODE";
                }
                else {
                    strSQL="SELECT DISTINCT SUB_ACCOUNT_CODE FROM  D_FIN_VOUCHER_HEADER A,D_FIN_VOUCHER_DETAIL_EX  B WHERE A.CANCELLED=0 AND A.VOUCHER_DATE>='"+FromDate+"' AND A.VOUCHER_DATE<='"+ToDate+"' AND B.MAIN_ACCOUNT_CODE='"+MainCode+"' AND A.VOUCHER_NO=B.VOUCHER_NO "+Condition+" ORDER BY A.VOUCHER_DATE,A.VOUCHER_NO, B.MAIN_ACCOUNT_CODE,B.SUB_ACCOUNT_CODE";
                }
                ...
                if(OnlyApproved) {
                    ...
                    strSQL = "SELECT DISTINCT SUB_ACCOUNT_CODE FROM  D_FIN_VOUCHER_HEADER A,D_FIN_VOUCHER_DETAIL_EX B WHERE A.CANCELLED=0 AND A.VOUCHER_DATE>='"+FromDate+"' AND A.VOUCHER_DATE<='"+ToDate+"' AND B.MAIN_ACCOUNT_CODE='"+MainCode+"' "+ cond +"  AND A.VOUCHER_NO=B.VOUCHER_NO AND A.APPROVED=1 "+Condition+" "+
                    "UNION "+
            ...
            if(rsParty.getRow()>0) {
                while(!rsParty.isAfterLast()) {
                    ...
                    System.out.println(Party_Code+"    1");
                    double ClosingBalance=0;
                    String PrevDate=EITLERPGLOBAL.DeductDaysFromDate(java.sql.Date.valueOf(FromDate), 1);
                    ...
                    if(rsTmp.getRow()>0) {
                        while(!rsTmp.isAfterLast()) {
                            ...
                            if(Continue) {
                                ...
                                //String Description=UtilFunctions.getString(rsTmp,"REMARKS","");
                                String Description=UtilFunctions.getString(rsTmp,"REMARKS","").trim().replace('\n',' ').replace('\t',' ');
                                ...
                                String vSubCode=data.getStringValueFromDB("SELECT DISTINCT SUB_ACCOUNT_CODE FROM D_FIN_VOUCHER_DETAIL WHERE VOUCHER_NO='"+VoucherNo+"' AND SUB_ACCOUNT_CODE<>'' ",FinanceGlobal.FinURL);
                                ...
                                if(BlockNo>0) {
                                    ...
                                    if(rsOpposite.getRow()>0) {
                                        ...
                                        objItem.setAttribute("REMARKS",Description);
                                        if(vSubCode.equals("")){
                                            String AllocatedAccoutCode=data.getStringValueFromDB("SELECT DISTINCT MAIN_ACCOUNT_CODE FROM D_FIN_VOUCHER_DETAIL WHERE VOUCHER_NO='"+VoucherNo+"' AND EFFECT='"+oEffect+"'",FinanceGlobal.FinURL);
                                            objItem.setAttribute("PARTY_CODE1",AllocatedAccoutCode);
                                        }else{                                                                                  
                                           objItem.setAttribute("PARTY_CODE1",vSubCode);
                                        }
                                        System.out.println(vSubCode+" 2  ");
                                        objItem.setAttribute("BOOK_CODE",UtilFunctions.getString(rsTmp,"BOOK_CODE",""));
                                        //  objItem.setAttribute("CHEQUE_NO",UtilFunctions.getString(rsOpposite,"CHEQUE_NO",""));//COMMENTED BY NISARG
                                        //objItem.setAttribute("CHEQUE_DATE",EITLERPGLOBAL.formatDate(UtilFunctions.getString(rsOpposite,"CHEQUE_DATE","0000-00-00")));//COMMENTED BY NISARG
                                        objItem.setAttribute("BANK_NAME",UtilFunctions.getString(rsOpposite,"BANK_NAME",""));
                                        ...
                                        objItem.setAttribute("MAIN_ACCOUNT_CODE",UtilFunctions.getString(rsOpposite,"MAIN_ACCOUNT_CODE",""));
                                        String PartyCode = data.getStringValueFromDB("SELECT DISTINCT SUB_ACCOUNT_CODE FROM D_FIN_VOUCHER_DETAIL WHERE VOUCHER_NO='"+VoucherNo+"' AND EFFECT='"+Effect+"'",FinanceGlobal.FinURL);
                                        //String PartyCode = data.getStringValueFromDB("SELECT DISTINCT SUB_ACCOUNT_CODE FROM D_FIN_VOUCHER_DETAIL WHERE VOUCHER_NO='"+VoucherNo+"' AND SUB_ACCOUNT_CODE<>''",FinanceGlobal.FinURL);
                                        System.out.println(PartyCode+"    3  ");
                                        objItem.setAttribute("SUB_ACCOUNT_CODE",PartyCode);
                                        //objItem.setAttribute("SUB_ACCOUNT_CODE",UtilFunctions.getString(rsOpposite,"SUB_ACCOUNT_CODE",""));
                                        String AcName=clsAccount.getAccountName(UtilFunctions.getString(rsOpposite,"MAIN_ACCOUNT_CODE",""), UtilFunctions.getString(rsOpposite,"SUB_ACCOUNT_CODE",""));
                                        objItem.setAttribute("ACCOUNT_NAME", AcName );
                                        ...
                                        objItem.setAttribute("INVOICE_NO",UtilFunctions.getString(rsTmp,"INVOICE_NO",""));
                                        objItem.setAttribute("INVOICE_DATE",EITLERPGLOBAL.formatDate(UtilFunctions.getString(rsTmp,"INVOICE_DATE","0000-00-00")));
                                        objItem.setAttribute("LINK_NO",UtilFunctions.getString(rsTmp,"B.LINK_NO",""));
                                        objItem.setAttribute("CHEQUE_NO",UtilFunctions.getString(rsTmp,"CHEQUE_NO",""));
                                        objItem.setAttribute("CHEQUE_DATE",EITLERPGLOBAL.formatDate(UtilFunctions.getString(rsTmp,"CHEQUE_DATE","0000-00-00")));
                    ...
                    objItem = new clsVoucherLedgerItem();
                    objItem.setAttribute("COMPANY_ID","");
                    ...
                    objItem.setAttribute("LEGACY_NO","");
                    objItem.setAttribute("BANK_NAME","");                    
                    objItem.setAttribute("CREDIT_AMOUNT","");
                    objItem.setAttribute("DEBIT_AMOUNT","");
                    objItem.setAttribute("CLOSING_BALANCE","");
                    objItem.setAttribute("LINK_NO","");
                    objItem.setAttribute("INVOICE_NO","");
                    objItem.setAttribute("INVOICE_DATE","0000-00-00");
                    objItem.setAttribute("MIR_NO","");
                    ...
                    objItem.setAttribute("DUE_DATE","0000-00-00");
                    objItem.setAttribute("GROUP","");
                    objItem.setAttribute("EXECUTED_PO","");
                    ...
                    objItem.setAttribute("SUB_ACCOUNT_CODE",Party_Code);
                    objItem.setAttribute("ACCOUNT_NAME","Closing Balance");
                    objItem.setAttribute("REMARKS"," *** LEDGERWISE TOTAL/CLOSING BALANCE *** ");
    ...
    public static HashMap getLedgerNomYearly(String MainCode,String FromSubCode,String ToSubCode,String FromDate,String ToDate,boolean OnlyApproved,double FromAmount,double ToAmount,String BookCode) {
        HashMap List=new HashMap();
        clsVoucherLedgerItem objItem;
        ResultSet rsOpposite;
        //MainCode="";
        ...
        try {
            ...
            int VoucherType=0;
            double LineAmount=0;
          ResultSet rsMaincode= data.getResult("SELECT DISTINCT MAIN_ACCOUNT_CODE FROM FINANCE.D_FIN_GL WHERE IS_SUBSIDAIRY!=1 AND APPROVED=1 AND CANCELLED=0 AND MAIN_ACCOUNT_CODE NOT IN ('999999') AND SUBSTRING(MAIN_ACCOUNT_CODE,1,1)='"+MainCode+"' ORDER BY MAIN_ACCOUNT_CODE",FinanceGlobal.FinURL);  
        //ResultSet rsMaincode= data.getResult("SELECT DISTINCT MAIN_ACCOUNT_CODE FROM FINANCE.D_FIN_GL WHERE IS_SUBSIDAIRY!=1 AND APPROVED=1 AND CANCELLED=0 AND MAIN_ACCOUNT_CODE NOT IN ('999999') AND SUBSTRING(MAIN_ACCOUNT_CODE,1,1)='"+1+"' ORDER BY MAIN_ACCOUNT_CODE",FinanceGlobal.FinURL);
        //ResultSet rsMaincode= data.getResult("SELECT DISTINCT MAIN_ACCOUNT_CODE FROM FINANCE.D_FIN_GL WHERE IS_SUBSIDAIRY!=1 AND APPROVED=1 AND CANCELLED=0 AND MAIN_ACCOUNT_CODE NOT IN ('999999') ORDER BY MAIN_ACCOUNT_CODE",FinanceGlobal.FinURL);
        rsMaincode.first();
        while(!rsMaincode.isAfterLast()) {
            counter++;
            MainCode=rsMaincode.getString("MAIN_ACCOUNT_CODE");
            System.out.println(counter+"     "+MainCode);
            if(!BookCode.trim().equals("")) {
                Condition=" AND A.BOOK_CODE='"+BookCode+"' ";
            }
            String cond = "";
            ...
            if(rsParty.getRow()>0) {
                while(!rsParty.isAfterLast()) {
                    ...
                    objItem.setAttribute("SUB_ACCOUNT_CODE",Party_Code);
                    objItem.setAttribute("ACCOUNT_NAME","Opening Balance");
                    objItem.setAttribute("REMARKS","          *** OPENING BALANCE ***");   //23
                    objItem.setAttribute("CHEQUE_NO","");
                    objItem.setAttribute("CHEQUE_DATE","0000-00-00");
                    objItem.setAttribute("LEGACY_NO","");
                    objItem.setAttribute("BANK_NAME","");
                    ...
                    objItem.setAttribute("MAIN_ACCOUNT_CODE","");
                    //rsMaincode.next();
                    //objItem.setAttribute("MAIN_ACCOUNT_CODE",rsMaincode.getString("MAIN_ACCOUNT_CODE").toString());
                    //rsMaincode.previous();
                    //objItem.setAttribute("MAIN_ACCOUNT_CODE",MainCode);
                    objItem.setAttribute("SUB_ACCOUNT_CODE",Party_Code);
                    objItem.setAttribute("ACCOUNT_NAME","Closing Balance");
                    objItem.setAttribute("REMARKS","          *** LEDGERWISE TOTAL/CLOSING BALANCE ***");  //40
    ...
    public static HashMap getPartyAdvReceivedDetail(int CompanyID,String MainCode,String PartyCode,String AgentAlpha) {
        ...
        try {
            ...
            "AND A.APPROVED=1 AND A.CANCELLED=0 AND A.VOUCHER_TYPE IN (6,7,8,9,12) " +
    ...
    public static HashMap getMyLedger(String MainCode,String FromSubCode,String ToSubCode,String FromDate,String ToDate,boolean OnlyApproved,double FromAmount,double ToAmount,String BookCode) {
        ...
        try {
            ...
            if(OnlyMainCode) {
                ...
                //objItem.setAttribute("SUB_ACCOUNT_CODE",strParty);
            ...
            if(rsParty.getRow()>0) {
                while(!rsParty.isAfterLast()) {
                    ...
                    String SQLPartyTran = "";
                    if(OnlyApproved) {
                        SQLPartyTran="SELECT A.*,B.MAIN_ACCOUNT_CODE,B.SUB_ACCOUNT_CODE,B.AMOUNT,B.EFFECT,B.BLOCK_NO,B.PO_NO,B.PO_DATE,B.INVOICE_NO,B.INVOICE_DATE,B.GRN_NO,B.GRN_DATE,B.MODULE_ID,B.REF_COMPANY_ID,B.LINK_NO " +
                    ...
                    //Add for skiping parties having opening balance but not any trasactions in current fin year-start
                    /*
                    else{
                        if(rsPartyTran.getRow() > 0) {
                            //do nothing
                     ...
                     */
                    //Add for skiping parties having opening balance but not any trasactions in current fin year-end
                    objItem = new clsVoucherLedgerItem();
                    objItem.setAttribute("COMPANY_ID",0);
                    objItem.setAttribute("VOUCHER_DATE","0000-00-00");
                    objItem.setAttribute("VOUCHER_NO","");
                    ...
                    if(rsPartyTran.getRow() > 0 ) {
                        ...
                        while(!rsPartyTran.isAfterLast()) {
                            ...
                            if(Continue) {
                                ...
                                String Description=UtilFunctions.getString(rsPartyTran,"REMARKS","").trim().replace('\n',' ').replace('\t',' ');
                                if(Description.length()>122) {
                                    Description=Description.substring(0,122);
                                }
                                ...
                                if(BlockNo>0) {
                                    ...
                                    if(rsOpposite.getRow()>0) {
                                        ...
                                        if(UtilFunctions.getString(rsOpposite,"SUB_ACCOUNT_CODE","").equals("")){
                                        objItem.setAttribute("PARTY_CODE1",UtilFunctions.getString(rsOpposite,"MAIN_ACCOUNT_CODE",""));    
                                        }else{
                                        objItem.setAttribute("PARTY_CODE1",UtilFunctions.getString(rsOpposite,"SUB_ACCOUNT_CODE",""));    
                                        }
                                        ...
                                        objItem.setAttribute("LINK_NO",UtilFunctions.getString(rsPartyTran,"B.LINK_NO",""));
                    ...
                    objItem.setAttribute("SUB_ACCOUNT_CODE",PartyCode);
                    objItem.setAttribute("ACCOUNT_NAME",clsAccount.getAccountName(MainCode,PartyCode));
                    objItem.setAttribute("REMARKS","CR/DR TOTAL AND CLOSING BALANCE");
    ...
    public static HashMap getMyLedgerSummary(String MainCode, String FromDate, String ToDate, boolean OnlyApproved) {
        ...
        try {
            ...
            String strRecords = "SELECT A.VOUCHER_NO,A.VOUCHER_DATE,A.LEGACY_NO,A.BOOK_CODE,SUBSTRING(A.REMARKS,1,80) AS REMARKS,B.AMOUNT,B.EFFECT, " +
    ...
    public static double get09AmountByParty(String MainCode,String SubCode,String InvoiceDate) {
        ...
        try {
            ...
            "WHERE A.VOUCHER_NO = B.VOUCHER_NO AND A.VOUCHER_TYPE IN (6,7,8,9,12) AND B.MAIN_ACCOUNT_CODE='"+MainCode+"' " +
