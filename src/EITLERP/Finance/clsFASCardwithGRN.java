...
public class clsFASCardwithGRN {
    ...
    public static int ModuleID=196;
    public static int AssetType = 1; // 1 - with GRN , 2 - without GRN
   ...
   /* public static int getVoucherModuleID(int VoucherType) {
    ...
    public clsFASCardwithGRN() {
        ...
        props.put("ASSET_DESC",new Variant(""));
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            String SQL = "SELECT * FROM D_FAS_MASTER_HEADER WHERE COMPANY_ID="+pCompanyID+" AND ASSET_TYPE = '" + clsFASCardwithGRN.AssetType + "' ORDER BY ASSET_NO";
    ...
    public boolean Insert() {
        ...
        try {
            ...
            java.sql.Date DocDate=java.sql.Date.valueOf((String)getAttribute("ASSET_DATE").getObj());
             ...
             /////////////////////////VALIDATION FOR DUPLICATE PJV////////////////////////////
            double CurrentAmount=0.0;
            double Amount=0.0;
            
            if(!getAttribute("PJ_VOUCHER_NO").getString().equals("")) {
                String CAmount = String.valueOf(new BigDecimal(getAttribute("PJ_VOUCHER_AMOUNT").getDouble()).setScale(0,BigDecimal.ROUND_HALF_UP));
                CurrentAmount = Double.parseDouble(CAmount);
                String SQL="SELECT A.VOUCHER_NO,A.VOUCHER_DATE,B.AMOUNT "+
                "FROM D_FIN_VOUCHER_HEADER A,D_FIN_VOUCHER_DETAIL B "+
                "WHERE A.COMPANY_ID =  '" + EITLERPGLOBAL.gCompanyID + "' AND A.VOUCHER_NO = B.VOUCHER_NO AND A.VOUCHER_NO = '" + getAttribute("PJ_VOUCHER_NO").getString() + "' "+
                "AND B.EFFECT ='C' AND B.SUB_ACCOUNT_CODE = '" + getAttribute("SUPPLIER_CODE").getString() + "' ";
                
                if(!data.IsRecordExist(SQL,FinanceGlobal.FinURL)) {
                    JOptionPane.showMessageDialog(null,"Invalid PJV Vocuher No.");
                    return false;
                }
                ...
                "WHERE PJ_VOUCHER_NO = '" + getAttribute("PJ_VOUCHER_NO").getString() + "' AND CANCELLED =0 "+
                "AND ASSET_NO <> '" + getAttribute("ASSET_NO").getString() + "' ";
                
                if(data.getDoubleValueFromDB(SQL,FinanceGlobal.FinURL) + CurrentAmount > Amount ) {
                    JOptionPane.showMessageDialog(null,"Amount of PJV Voucher Amount is already used.");
                    return false;
                }
            }
           ///////////////////////////////////////////////// END OF VALIDATION /////////////////////////// 
            ...
            //--------- Generate New ASSET No. ------------
            setAttribute("ASSET_NO",clsFirstFree.getNextFreeNo((int)EITLERPGLOBAL.gCompanyID,clsFASCardwithGRN.ModuleID, (int)getAttribute("FFNO").getVal(),true));
            //-------------------------------------------------
            ...
            rsResultSet.updateString("ASSET_TYPE",String.valueOf(clsFASCardwithGRN.AssetType)); //1 with GRN ,2 without GRN
            rsResultSet.updateString("ITEM_ID",getAttribute("ITEM_ID").getString());
            rsResultSet.updateString("ASSET_DESC",getAttribute("ASSET_DESC").getString());
            rsResultSet.updateString("ITEM_DESC","");
            rsResultSet.updateString("METHOD_ID","");
            rsResultSet.updateString("BOOK_PER","");
            rsResultSet.updateString("IT_PER","");
            ...
            rsResultSet.updateString("GRN_NO",getAttribute("GRN_NO").getString());
            ...
            rsHistory.updateString("ASSET_TYPE",String.valueOf(clsFASCardwithGRN.AssetType)); //1 with GRN ,2 without GRN
            rsHistory.updateString("ITEM_ID",getAttribute("ITEM_ID").getString());
            rsHistory.updateString("ASSET_DESC",getAttribute("ASSET_DESC").getString());
            rsHistory.updateString("ITEM_DESC","");
            rsHistory.updateString("METHOD_ID","");
            rsHistory.updateString("BOOK_PER","");
            rsHistory.updateString("IT_PER","");
            ...
            rsHistory.updateString("GRN_NO",getAttribute("GRN_NO").getString());
            ...
            for(int i=1;i<=colFASItemsDetail.size();i++) {
                clsFASCardwithGRNDetail objDetail=(clsFASCardwithGRNDetail) colFASItemsDetail.get(Integer.toString(i));
                ...
                rsDetail.updateDouble("SALE_VALUE",Double.parseDouble(objDetail.getAttribute("SALE_VALUE").getString()));
               ...
               // rsDetail.updateString("SJ_NUMBER",objDetail.getAttribute("SJ_NUMBER").getString());
                ...
                rsHDetail.updateDouble("SALE_VALUE",Double.parseDouble(objDetail.getAttribute("SALE_VALUE").getString()));
            ...
            if(colFASItemsDetailExBook.size()>0)
            {
            for(int i=1;i<=colFASItemsDetailExBook.size();i++) {
                clsFASCardwithGRNDetailEx objDetailEx = (clsFASCardwithGRNDetailEx) colFASItemsDetailExBook.get(Integer.toString(i));
                
                rsDetailEx.moveToInsertRow();
                rsDetailEx.updateInt("COMPANY_ID",getAttribute("COMPANY_ID").getInt());
                rsDetailEx.updateString("ASSET_NO",getAttribute("ASSET_NO").getString());
                ...
                rsHDetailEx.updateString("ASSET_NO",getAttribute("ASSET_NO").getString());
            ...
            for(int i=1;i<=colFASItemsDetailExBook.size();i++) {
                clsFASCardwithGRNDetailEx objDetailEx = (clsFASCardwithGRNDetailEx) colFASItemsDetailExIT.get(Integer.toString(i));
...
//            }
            ...
            ObjFlow.ModuleID=clsFASCardwithGRN .ModuleID;
            ObjFlow.DocNo=getAttribute("ASSET_NO").getString();
            ObjFlow.From=(int)getAttribute("FROM").getVal();
            ObjFlow.To=(int)getAttribute("TO").getVal();
            ObjFlow.Status=getAttribute("APPROVAL_STATUS").getString();
            ObjFlow.TableName=clsModules.getHeaderTableName(EITLERPGLOBAL.gCompanyID,clsFASCardwithGRN .ModuleID);
    ...
    public boolean Update() {
        ...
        try {
            ...
            if(IsRejected) {
                //Remove the Rejected Flag First
                data.Execute("UPDATE D_FAS_MASTER_HEADER SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND ASSET_NO='"+ObjFlow.DocNo+"' AND ASSET_TYPE = '" + clsFASCardwithGRN .AssetType + "' ");
                //Remove Old Records from D_COM_DOC_DATA
                data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID='" + clsFASCardwithGRN .ModuleID + "' AND DOC_NO='"+ObjFlow.DocNo+"'");
    ...
    public boolean Filter(String Condition,int CompanyID) {
        ...
        try {
            ...
            if(!rsResultSet.first()) {
                strSql = "SELECT * FROM D_FAS_MASTER_HEADER WHERE COMPANY_ID="+CompanyID+" AND ASSET_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND ASSET_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' ORDER BY ASSET_NO";
                rsResultSet=Stmt.executeQuery(strSql);
                Ready=true;
                MoveLast();
                return false;
    ...
    public boolean setData() {
        ...
        try {
            ...
            setAttribute("ASSET_DESC",UtilFunctions.getString(rsResultSet,"ASSET_DESC",""));
            ...
            setAttribute("GRN_NO",UtilFunctions.getString(rsResultSet,"GRN_NO",""));
            ...
            while(!rsTmp.isAfterLast() && rsTmp.getRow()>0) {
                Counter=Counter+1;
                clsFASCardwithGRNDetail objItem = new clsFASCardwithGRNDetail();
                ...
                objItem.setAttribute("SALE_VALUE",UtilFunctions.getDouble(rsTmp,"SALE_VALUE",0));
            ...
            while(!rsTmp.isAfterLast() && rsTmp.getRow()>0) {
                Counter=Counter+1;
                clsFASCardwithGRNDetailEx objItem = new clsFASCardwithGRNDetailEx();
        ...
        try {
            ...
            while(rsTmp3.next()) {
                if(EITLERPGLOBAL.isWithinDate(rsTmp3.getString("ASSET_DATE"),FinYearFrom)) {
                    Counter=Counter+1;
                    clsFASCardwithGRN  ObjDoc=new clsFASCardwithGRN ();
