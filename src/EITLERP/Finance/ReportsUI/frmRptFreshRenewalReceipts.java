...
public class frmRptFreshRenewalReceipts extends javax.swing.JApplet {
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        jLabel6.setText("FRESH OR RENEWAL RECEIPT REPORT");
        jPanel3.add(jLabel6);
        ...
        jLabel4.setBounds(20, 130, 90, 15);
        ...
        cmbReportType.setBounds(120, 130, 150, 24);

        getContentPane().add(cmbReceiptType);
        cmbReceiptType.setBounds(120, 100, 150, 24);
    ...
    private void GenerateReport() {
        try {
            ...
            "DMST.DEPOSIT_PERIOD, DMST.INTEREST_RATE, DMST.OLD_RECEIPT_NO, "+
            ...
            "WHERE DMST.COMPANY_ID =" + EITLERPGLOBAL.gCompanyID + " AND DMST.APPROVED=1  "+
            "AND DMST.DEPOSIT_ENTRY_TYPE = '"+ReportType+"' AND DMST.CANCELLED=0  "+
            "AND DMST.COMPANY_ID = SMST.COMPANY_ID AND DMST.SCHEME_ID = SMST.SCHEME_ID AND SMST.SCHEME_TYPE='"+ReceiptType+"' "+
            "AND DMST.RECEIPT_DATE >= '"+ EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) +"' "+
            "AND DMST.RECEIPT_DATE <= '"+ EITLERPGLOBAL.formatDateDB(txtToDate.getText()) +"' ORDER BY DMST.RECEIPT_DATE";
            ...
            Parameters.put("RECEIPT_TYPE",cmbReceiptType.getSelectedItem().toString());
            Parameters.put("REPORT_TYPE",cmbReportType.getSelectedItem().toString());
            
            objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/finance/rptFreshRenewalReceipt.rpt",Parameters,objReportData);
    ...
    private void GenerateSummary() {
        try {
            ...
            String Condition = "";
            if(ReportType != 2) {
                Condition = "AND DEPOSITMST.OLD_RECEIPT_NO = '' ";
            }
            //FD Fresh Receipt
            ...
            //"WHERE DEPOSITMST.COMPANY_ID =" + EITLERPGLOBAL.gCompanyID + " AND DEPOSITMST.DEPOSIT_STATUS=0 AND DEPOSITMST.DEPOSIT_ENTRY_TYPE = "+ReportType+"  "+
            "WHERE DEPOSITMST.COMPANY_ID =" + EITLERPGLOBAL.gCompanyID + " AND DEPOSITMST.DEPOSIT_ENTRY_TYPE = "+ReportType+"  "+
            //"AND DEPOSITMST.OLD_RECEIPT_NO = '' AND DEPOSITMST.CANCELLED=0 AND DEPOSITMST.APPROVED=1 "+
            Condition +" AND DEPOSITMST.CANCELLED=0 AND DEPOSITMST.APPROVED=1 "+
            "AND DEPOSITMST.RECEIPT_DATE >= '"+ EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) +"' AND DEPOSITMST.RECEIPT_DATE <= '"+ EITLERPGLOBAL.formatDateDB(txtToDate.getText()) +"' "+
           ...
           /*
            ...
            if ((ReceiptType == 1) & (ReportType == 2)) {
                ...
                "IF (SUM(DEPOSITMST.AMOUNT) > 0,SUM(DEPOSITMST.AMOUNT),0) AS SUM_AMOUNT "+
                "FROM D_FD_DEPOSIT_MASTER DEPOSITMST, D_FD_SCHEME_MASTER SCHEMEMST "+
                "WHERE DEPOSITMST.COMPANY_ID =" + EITLERPGLOBAL.gCompanyID + " AND DEPOSITMST.DEPOSIT_STATUS=0 AND DEPOSITMST.DEPOSIT_ENTRY_TYPE = 2 AND DEPOSITMST.APPROVED=1 "+
                "AND DEPOSITMST.OLD_RECEIPT_NO <> '' AND DEPOSITMST.CANCELLED=0  "+
                "AND DEPOSITMST.EFFECTIVE_DATE >= '"+ EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) +"' AND DEPOSITMST.EFFECTIVE_DATE <= '"+ EITLERPGLOBAL.formatDateDB(txtToDate.getText()) +"' "+
                "AND DEPOSITMST.COMPANY_ID = SCHEMEMST.COMPANY_ID "+
                "AND DEPOSITMST.SCHEME_ID = SCHEMEMST.SCHEME_ID AND SCHEMEMST.SCHEME_TYPE=1 "+
            ...
            if ((ReceiptType == 2) & (ReportType == 1)) {
                ...
                "IF (SUM(DEPOSITMST.AMOUNT) > 0,SUM(DEPOSITMST.AMOUNT),0) AS SUM_AMOUNT "+
                "FROM D_FD_DEPOSIT_MASTER DEPOSITMST, D_FD_SCHEME_MASTER SCHEMEMST "+
                "WHERE DEPOSITMST.COMPANY_ID =" + EITLERPGLOBAL.gCompanyID + " AND DEPOSITMST.DEPOSIT_STATUS=0 AND DEPOSITMST.DEPOSIT_ENTRY_TYPE = 1 AND DEPOSITMST.APPROVED=1 "+
                "AND DEPOSITMST.OLD_RECEIPT_NO = '' AND DEPOSITMST.CANCELLED=0  "+
                "AND DEPOSITMST.EFFECTIVE_DATE >= '"+ EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) +"' AND DEPOSITMST.EFFECTIVE_DATE <= '"+ EITLERPGLOBAL.formatDateDB(txtToDate.getText()) +"' "+
                "AND DEPOSITMST.COMPANY_ID = SCHEMEMST.COMPANY_ID "+
                "AND DEPOSITMST.SCHEME_ID = SCHEMEMST.SCHEME_ID AND SCHEMEMST.SCHEME_TYPE=3 " +
                "GROUP BY DEPOSITMST.DEPOSITER_STATUS " ;
            ...
            if ((ReceiptType == 3) & (ReportType == 1)) {
                ...
                "AND DEPOSITMST.EFFECTIVE_DATE >= '"+ EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) +"' AND DEPOSITMST.EFFECTIVE_DATE <= '"+ EITLERPGLOBAL.formatDateDB(txtToDate.getText()) +"' "+
                "AND DEPOSITMST.COMPANY_ID = SCHEMEMST.COMPANY_ID "+
                "AND DEPOSITMST.SCHEME_ID = SCHEMEMST.SCHEME_ID AND SCHEMEMST.SCHEME_TYPE=2 " +
                "GROUP BY DEPOSITMST.DEPOSITER_STATUS " ;
            }
            //LD Renewal Receipt
            ...
            */
            ...
            //FD Fresh Receipt
            if ((ReceiptType == 1) & (ReportType == 1)) {
                objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/finance/rptFDFreshReceiptSummary.rpt",Parameters,objReportData);
            }
            //FD Renewal Receipt
            if ((ReceiptType == 1) & (ReportType == 2)) {
                objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/finance/rptFDRenewalReceiptSummary.rpt",Parameters,objReportData);
            }
            
            //CD Fresh Receipt
            if ((ReceiptType == 2) & (ReportType == 1)) {
                objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/finance/rptCDFreshReceiptSummary.rpt",Parameters,objReportData);
            }
            //CD Renewal Receipt
            if ((ReceiptType == 2) & (ReportType == 2)) {
                objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/finance/rptCDRenewalReceiptSummary.rpt",Parameters,objReportData);
            }
            
            //LD Fresh Receipt
            if ((ReceiptType == 3) & (ReportType == 1)) {
                objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/finance/rptLDFreshReceiptSummary.rpt",Parameters,objReportData);
            }
            //LD Renewal Receipt
            if ((ReceiptType == 3) & (ReportType == 2)) {
                objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/finance/rptLDRenewalReceiptSummary.rpt",Parameters,objReportData);
            }
    ...
    private boolean Validate() {
        ...
        if(cmbReceiptType.getSelectedIndex()==0) {
            JOptionPane.showMessageDialog(null,"Select Receipt Type");
            return false;
        }
