...
public class frmRptGRNPJVLinkReport extends javax.swing.JApplet {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (GRNType == 1) {
                        ...
                        Parameters.put("S_GRN_FREIGHT_AMT", Double.toString(EITLERPGLOBAL.round(sumMIRFreightAmt, 2)));
                        Parameters.put("S_VOU_FREIGHT_AMT", Double.toString(EITLERPGLOBAL.round(sumVouFreightAmt, 2)));
                        objEngine.PreviewReport("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/finance/GRNPJVLink_Raw.rpt",Parameters , objData);
                    } else {
                    ...
                    if (!SupplierCode.trim().equals("888888") && !SupplierCode.trim().equals("999999")) {
                        ...
                        if(GRNValue>0) {
                            ...
                            for(int n=0;n<sMIRList.length;n++) {
                                ...
                                " AND A.VOUCHER_NO=B.VOUCHER_NO AND B.GRN_NO='"+sMIRList[n]+"' AND B.MODULE_ID=6 AND B.EFFECT='C' AND GRN_DATE>='"+FromDate+"' AND GRN_DATE<='"+ToDate+"' AND A.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" ";
                            ...
                            if(rsVoucher.getRow()>0) {
                                ...
                                for(int n=0;n<sMIRList.length;n++) {
                                    ...
                                    strQry = "SELECT SUM(B.AMOUNT) FROM D_FIN_VOUCHER_HEADER A, D_FIN_VOUCHER_DETAIL B "+
                                    " WHERE A.CANCELLED=0 AND A.APPROVED=1  " +
                                    " AND A.VOUCHER_NO=B.VOUCHER_NO AND B.GRN_NO='"+sMIRList[n]+"' AND B.MODULE_ID=6 AND EFFECT='C' AND GRN_DATE>='"+FromDate+"' AND GRN_DATE<='"+ToDate+"' AND A.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" ";
                                    FinanceFreightAmount  = FinanceFreightAmount + data.getDoubleValueFromDB(strQry,FinanceGlobal.FinURL);
                                }
