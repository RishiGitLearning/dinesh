...
public class clsVoucherReports {
    
    /** Creates a new instance of clsVoucherReports */
    public clsVoucherReports() {
    ...
    public static TTable getVoucherReport(String VoucherNo,int ProcessType) {
        switch(ProcessType) {
            ...
            case 13: return getAccountDebitVoucherReportGST(VoucherNo);
            case 14: return getSalesDebitVoucherReportGST(VoucherNo);
            case 15: return getDebitVoucherReportGST(VoucherNo);    
            case 16: return getGeneralVoucherReportGST(VoucherNo);
...
//    }
    ...
    public static TTable getAccountDebitVoucherReportGST(String VoucherNo) {
        TTable objData=new TTable();
        System.out.println("Downloading From GST1 ");
        objData.AddColumn("INVOICE_NO");
        objData.AddColumn("INVOICE_DATE");
        ...
        objData.AddColumn("HSN_CODE");
            ...
            System.out.println("Downloading From GST2 ");
        ...
        try{
            ...
            if(rsVoucher.getRow()>0) {
                while(!rsVoucher.isAfterLast()) {
                    ...
                    objRow.setValue("INVOICE_NO",clsSalesInvoice.getAgentAlphaSrNo(rsVoucher.getString("INVOICE_NO"),rsVoucher.getString("INVOICE_DATE")));
                    objRow.setValue("INVOICE_DUE_DATE",EITLERPGLOBAL.formatDate(rsVoucher.getString("INVOICE_DUE_DATE")));
                    ...
                    String ChargeCode = clsSalesInvoice.getInvoiceChargeCode(rsVoucher.getString("INVOICE_NO"), rsVoucher.getString("INVOICE_DATE"));
                    String PartyCode = data.getStringValueFromDB("SELECT SUB_ACCOUNT_CODE FROM  D_FIN_VOUCHER_DETAIL WHERE VOUCHER_NO = '" + VoucherNo + "' AND EFFECT = 'D'",FinanceGlobal.FinURL);
                    String HSN_CODE =  data.getStringValueFromDB("SELECT HSN_SAC_CODE FROM  DINESHMILLS.D_COM_HSN_SAC_MASTER WHERE CATG_CODE  = 'STG' AND TAX_CODE_TYPE = 'HSN'",FinanceGlobal.FinURL);
                    double InterestPercentage = clsSalesInvoice.getDebitNotePercentage(rsVoucher.getString("INVOICE_DATE"),InvoiceType,ChargeCode,PartyCode);
                        System.out.println("Downloading From GST3 "+PartyCode+" ");
                        
                    objRow.setValue("HSN_CODE",HSN_CODE);
                    objRow.setValue("INTEREST_RATE",InterestPercentage + "%");
                    objRow.setValue("PAYMENT_DATE",EITLERPGLOBAL.formatDate(rsVoucher.getString("VALUE_DATE")));
                    objRow.setValue("PAYMENT_AMOUNT",rsVoucher.getString("DEBIT_NOTE_AMOUNT"));
                    objRow.setValue("IGST_PER",rsVoucher.getString("IGST_PER"));
                    objRow.setValue("IGST_AMT",rsVoucher.getString("IGST_AMT"));
                    objRow.setValue("CGST_PER",rsVoucher.getString("CGST_PER"));
                    objRow.setValue("CGST_AMT",rsVoucher.getString("CGST_AMT"));
                    objRow.setValue("SGST_PER",rsVoucher.getString("SGST_PER"));
                    objRow.setValue("SGST_AMT",rsVoucher.getString("SGST_AMT"));
                    objRow.setValue("INTEREST_PER",rsVoucher.getString("INTEREST_PER"));
                    objRow.setValue("INTEREST_AMT",rsVoucher.getString("INTEREST_AMT"));
                    objRow.setValue("INVOICE_AMOUNT",rsVoucher.getString("INVOICE_AMOUNT"));
                    
                        System.out.println("Downloading From GST4 ");
    ...
    public static TTable getSalesDebitVoucherReport(String VoucherNo) {
     ...
     public static TTable getSalesDebitVoucherReportGST(String VoucherNo) {
        TTable objData=new TTable();
        //Populate Columns
        
        System.out.println("Downloading From sGST1 ");
        ...
        System.out.println("Downloading From sGST2");
        try {
            String strSQL="";
            String FullAccountName="";
            String PartyName="",PartyCode="";
            ...
            double TotalDebitAmount=data.getDoubleValueFromDB("SELECT SUM(AMOUNT) FROM D_FIN_VOUCHER_DETAIL WHERE VOUCHER_NO ='"+VoucherNo+"' AND EFFECT = 'D' ",FinanceGlobal.FinURL);
            String HSN_CODE =  data.getStringValueFromDB("SELECT HSN_SAC_CODE FROM  DINESHMILLS.D_COM_HSN_SAC_MASTER WHERE CATG_CODE  = 'STG' AND TAX_CODE_TYPE = 'HSN'",FinanceGlobal.FinURL);
            System.out.println("Downloading From sGSt31 ");     
            strSQL+=" SELECT HEADER_REMARKS,A.CHEQUE_NO,A.CHEQUE_DATE,CHEQUE_AMOUNT,EFFECT,VOUCHER_NO,VOUCHER_DATE,ACCOUNT_CODE,ACCOUNT_NAME,CASE WHEN ACCOUNT_CODE NOT IN (127565,127566,127567,127568,127569,127570) THEN REMARKS WHEN ACCOUNT_CODE IN (127565,127566,127567,127568,127569,127570) THEN REPLACE(ACCOUNT_NAME,'PAYABLE','') END AS REMARKS,CR_AMOUNT,DR_AMOUNT,BOOK_CODE,INVOICE_NO,INVOICE_DATE,PO_NO,PO_DATE,BOOK_CODE,ST_CATEGORY ,MAIN_ACCOUNT_CODE,SUB_ACCOUNT_CODE,BOOK_NAME,LINK_NO,ACCOUNT_NAME_EX,REF_VOUCHER_NO FROM (SELECT A.REMARKS AS HEADER_REMARKS,A.CHEQUE_NO,A.CHEQUE_DATE,A.CHEQUE_AMOUNT,B.EFFECT, "+
            "IF(A.REMARKS <> '',A.REMARKS, B.REMARKS)  AS REMARKS,A.VOUCHER_NO,A.VOUCHER_DATE, "+
            "IF(B.SUB_ACCOUNT_CODE<>'',B.SUB_ACCOUNT_CODE,B.MAIN_ACCOUNT_CODE) AS ACCOUNT_CODE, "+
            "IF(IF(B.SUB_ACCOUNT_CODE<>'',PARTY.PARTY_NAME,GL.ACCOUNT_NAME) IS NULL,'', "+
            "IF(B.SUB_ACCOUNT_CODE<>'',PARTY.PARTY_NAME,GL.ACCOUNT_NAME)) AS ACCOUNT_NAME, "+
            ...
            "IF(EX.EX_ACCOUNT_NAME IS NULL, IF(IF(B.SUB_ACCOUNT_CODE<>'',PARTY.PARTY_NAME,GL.ACCOUNT_NAME) IS NULL,'', " +
            "IF(B.SUB_ACCOUNT_CODE<>'',PARTY.PARTY_NAME,GL.ACCOUNT_NAME)) ,EX_ACCOUNT_NAME) AS ACCOUNT_NAME_EX, " +
            "B.MAIN_ACCOUNT_CODE,B.SUB_ACCOUNT_CODE,BOOK.BOOK_NAME,B.LINK_NO,B.REF_VOUCHER_NO ";
            strSQL+=" FROM ";
            strSQL+=" D_FIN_VOUCHER_HEADER A ";
            strSQL+=" LEFT JOIN D_FIN_BOOK_MASTER BOOK ON (BOOK.BOOK_CODE=A.BOOK_CODE), ";
            strSQL+=" D_FIN_VOUCHER_DETAIL B ";
            strSQL+=" LEFT JOIN D_FIN_GL AS GL ON (GL.MAIN_ACCOUNT_CODE=B.MAIN_ACCOUNT_CODE) ";
            strSQL+=" LEFT JOIN D_FIN_PARTY_MASTER AS PARTY ON (PARTY.PARTY_CODE=B.SUB_ACCOUNT_CODE) ";
            strSQL+=" LEFT JOIN D_FIN_EX_ACCOUNT_MASTER AS EX ON ( SUBSTRING(B.MAIN_ACCOUNT_CODE,LENGTH(B.MAIN_ACCOUNT_CODE)-1)=EX.EX_ACCOUNT_CODE) WHERE  A.VOUCHER_NO=B.VOUCHER_NO AND A.VOUCHER_NO='"+VoucherNo+"' AND EFFECT ='C' GROUP BY VOUCHER_NO,SR_NO ORDER BY REF_VOUCHER_NO,VOUCHER_NO,SR_NO ) AS A;";
            rsVoucher=data.getResult(strSQL,FinanceGlobal.FinURL);
            
            System.out.println("Downloading From sGST3 ");
            rsVoucher.first();
            if(rsVoucher.getRow()>0) {
                while(!rsVoucher.isAfterLast()) {
                    TRow objRow=new TRow();
                    ...
                    objRow.setValue("PARTY_MAIN_ACCOUNT_CODE",rsVoucher.getString("MAIN_ACCOUNT_CODE"));
                    objRow.setValue("HSN_CODE",HSN_CODE);
                    objRow.setValue("REF_VOUCHER_NO",rsVoucher.getString("REF_VOUCHER_NO"));
                  System.out.println("Downloading From sGST4 ");
                    String LinkNo="";
                    if(rsVoucher.getString("LINK_NO").equals(null) || rsVoucher.getString("LINK_NO").equals("")) {
                        LinkNo = "";//LinkNo.substring(0,LinkNo.indexOf("/")+1)+Year;
                    }
    ...
    public static TTable getCashPaymentAdvice(String VoucherNo) {
        ...
        objData.AddColumn("ACCOUNT_CODE");
        objData.AddColumn("PARTY_MAIN_ACCOUNT_CODE");
      //  objData.AddColumn("ACCOUNT_NAME1");
        ...
        try {
            ...
            if(rsVoucher.getRow()>0) {
                ...
                while(!rsVoucher.isAfterLast()) {
                    ...
                    objRow.setValue("MAIN_ACCOUNT_CODE",rsVoucher.getString("MAIN_ACCOUNT_CODE"));
                    objRow.setValue("SUB_ACCOUNT_CODE",rsVoucher.getString("SUB_ACCOUNT_CODE"));
                    objRow.setValue("ACCOUNT_NAME",clsPartyMaster.getAccountName(rsVoucher.getString("MAIN_ACCOUNT_CODE"),rsVoucher.getString("SUB_ACCOUNT_CODE")) +", " +rsVoucher.getString("EMP_NAME"));
                    objRow.setValue("DR_AMOUNT",Double.toString(rsVoucher.getDouble("AMOUNT")));
                    objRow.setValue("CR_AMOUNT","0");
                  //  objRow.setValue("ACCOUNT_NAME1",rsVoucher.getString("EMP_NAME"));
                    objRow.setValue("INVOICE_NO",rsVoucher.getString("INVOICE_NO"));
                    objRow.setValue("INVOICE_DATE",rsVoucher.getString("INVOICE_DATE"));
                    objRow.setValue("REMARKS",rsVoucher.getString("HEADER_REMARKS").trim());
                    objData.AddRow(objRow);
    ...
    public static TTable getGeneralVoucherReportGST(String VoucherNo) {
    ...
    public static TTable getBriefPJVReport(String VoucherNo) {
        ...
        try {
            ...
            int msme=data.getIntValueFromDB("SELECT MSME FROM DINESHMILLS.D_COM_SUPP_MASTER WHERE SUPPLIER_CODE='"+PartySubCode+"'");
            //int msme=data.getIntValueFromDB("SELECT MSME FROM FINANCE.D_FIN_PARTY_MASTER WHERE PARTY_CODE='"+PartySubCode+"'");
            ...
            if(rsTmp.getRow()>0) {
                while(!rsTmp.isAfterLast()) {
                    ...
                    if(Continue) {
                        ...
                        if(msme==1){
                        objRow.setValue("PARTY_NAME",PartyName+" (MSME Party");
                        }
                        else{
                            objRow.setValue("PARTY_NAME",PartyName);
                        }
    ...
    public static TTable getDebitVoucherReport(String VoucherNo) {
     ...
     public static TTable getDebitVoucherReportGST(String VoucherNo) {
        ...
        try {
          ...
          //  strSQL+="SELECT HEADER_REMARKS,A.CHEQUE_NO,CHEQUE_DATE,CHEQUE_AMOUNT,EFFECT,CASE WHEN ACCOUNT_CODE NOT IN (127565,127566,127567,127568,127569,127570) THEN SUB_REMARKS WHEN ACCOUNT_CODE IN (127565,127566,127567,127568,127569,127570) THEN REPLACE(ACCOUNT_NAME,'PAYABLE','') END AS SUB_REMARKS,VOUCHER_NO,VOUCHER_DATE,CR_AMOUNT,DR_AMOUNT,BOOK_CODE,INVOICE_NO,INVOICE_DATE,PO_NO,PO_DATE,BOOK_CODE,ST_CATEGORY ,MAIN_ACCOUNT_CODE,SUB_ACCOUNT_CODE,BOOK_NAME,ACCOUNT_NAME_EX FROM (";
              strSQL+="SELECT HEADER_REMARKS,A.CHEQUE_NO,CHEQUE_DATE,CHEQUE_AMOUNT,EFFECT,CASE WHEN SUBSTRING(GST_CATG,1,4) NOT IN ('IGST','CGST','SGST') THEN SUB_REMARKS WHEN SUBSTRING(GST_CATG,1,4)  IN ('IGST','CGST','SGST') THEN REPLACE(ACCOUNT_NAME,'PAYABLE','') END AS SUB_REMARKS,VOUCHER_NO,VOUCHER_DATE,CR_AMOUNT,DR_AMOUNT,BOOK_CODE,INVOICE_NO,INVOICE_DATE,PO_NO,PO_DATE,BOOK_CODE,ST_CATEGORY ,MAIN_ACCOUNT_CODE,SUB_ACCOUNT_CODE,BOOK_NAME,ACCOUNT_NAME_EX FROM (";
            strSQL+=" SELECT A.REMARKS AS HEADER_REMARKS,A.CHEQUE_NO,A.CHEQUE_DATE,A.CHEQUE_AMOUNT,B.EFFECT,B.REMARKS AS SUB_REMARKS,A.VOUCHER_NO,A.VOUCHER_DATE, IF(B.SUB_ACCOUNT_CODE<>'',B.SUB_ACCOUNT_CODE,B.MAIN_ACCOUNT_CODE) AS ACCOUNT_CODE, IF(IF(B.SUB_ACCOUNT_CODE<>'',PARTY.PARTY_NAME,GL.ACCOUNT_NAME) IS NULL,'',IF(B.SUB_ACCOUNT_CODE<>'',PARTY.PARTY_NAME,GL.ACCOUNT_NAME)) AS ACCOUNT_NAME,IF(EFFECT='C',AMOUNT,0) AS CR_AMOUNT,IF(EFFECT='D',AMOUNT,0) AS DR_AMOUNT,B.PO_NO,B.PO_DATE,B.INVOICE_NO,B.INVOICE_DATE,A.BOOK_CODE,A.ST_CATEGORY, IF(EX.EX_ACCOUNT_NAME IS NULL, IF(IF(B.SUB_ACCOUNT_CODE<>'',PARTY.PARTY_NAME,GL.ACCOUNT_NAME) IS NULL,'',IF(B.SUB_ACCOUNT_CODE<>'',PARTY.PARTY_NAME,GL.ACCOUNT_NAME)) ,EX_ACCOUNT_NAME) AS ACCOUNT_NAME_EX,B.MAIN_ACCOUNT_CODE,B.SUB_ACCOUNT_CODE,BOOK.BOOK_NAME,GST_CATG ";
            strSQL+=" FROM ";
            strSQL+=" D_FIN_VOUCHER_HEADER A ";
            strSQL+=" LEFT JOIN D_FIN_BOOK_MASTER BOOK ON (BOOK.BOOK_CODE=A.BOOK_CODE), ";
            strSQL+=" D_FIN_VOUCHER_DETAIL B ";
            strSQL+=" LEFT JOIN D_FIN_GL AS GL ON (GL.MAIN_ACCOUNT_CODE=B.MAIN_ACCOUNT_CODE) ";
            strSQL+=" LEFT JOIN D_FIN_PARTY_MASTER AS PARTY ON (PARTY.PARTY_CODE=B.SUB_ACCOUNT_CODE) ";
            strSQL+=" LEFT JOIN D_FIN_EX_ACCOUNT_MASTER AS EX ON ( SUBSTRING(B.MAIN_ACCOUNT_CODE,LENGTH(B.MAIN_ACCOUNT_CODE)-1)=EX.EX_ACCOUNT_CODE) WHERE  A.VOUCHER_NO=B.VOUCHER_NO AND A.VOUCHER_NO='"+VoucherNo+"' AND EFFECT ='C' GROUP BY VOUCHER_NO,SR_NO ORDER BY VOUCHER_NO,SR_NO ) AS A";
