 ...
 *
 */
public class FrmZoneMaster extends javax.swing.JApplet {
    ...
    private int FFNo = 0;
    private int ModuleId = 876;
    private String DOC_NO = "";
    private clsZoneMaster ZoneMaster;
    private EITLComboModel cmbSendToModel;
    private int FinalApprovedBy = 0;
    private int lstzone = 0;
    ...
    private void DisplayData() {
        ...
        try {
            ...
            for (int i = 1; i <= ZoneMaster.colMRItems.size(); i++) {
                ...
                if (ObjItem.getAttribute("ZONE_STATUS").getString().equalsIgnoreCase("MASTER") || ObjItem.getAttribute("ZONE_STATUS").getString().equalsIgnoreCase("ADD")) {
                    s = 0;
                    if (ObjItem.getAttribute("ZONE_STATUS").getString().equalsIgnoreCase("MASTER")) {
                        cntmst++;
                        rowData[s] = Integer.toString(cntmst);
                    }
                    if (ObjItem.getAttribute("ZONE_STATUS").getString().equalsIgnoreCase("ADD")) {
                        cntadd++;
                    ...
                    s++;
                    rowData[s] = ObjItem.getAttribute("ZONE_CATEGORY").getString();
                    s++;
                    rowData[s] = emp.get(ObjItem.getAttribute("INCHARGE_COMMERCIAL").getString()) + "(#" + ObjItem.getAttribute("INCHARGE_COMMERCIAL").getString() + ")";
                    s++;
                    rowData[s] = emp.get(ObjItem.getAttribute("INCHARGE_TECHNICAL").getString()) + "(#" + ObjItem.getAttribute("INCHARGE_TECHNICAL").getString() + ")";;
                    s++;
                    rowData[s] = emp.get(ObjItem.getAttribute("COORDINATOR").getString()) + "(#" + ObjItem.getAttribute("COORDINATOR").getString() + ")";;
                    s++;
                    rowData[s] = emp.get(ObjItem.getAttribute("REGIONAL_HEAD").getString()) + "(#" + ObjItem.getAttribute("REGIONAL_HEAD").getString() + ")";;
                    s++;
                    if (ObjItem.getAttribute("ZONE_STATUS").getString().equalsIgnoreCase("MASTER")) {
                        DataModelZone.addRow(rowData);
                    }
                    if (ObjItem.getAttribute("ZONE_STATUS").getString().equalsIgnoreCase("ADD")) {
                        DataModelZoneAdd.addRow(rowData);
                    }
                }
                if (ObjItem.getAttribute("ZONE_STATUS").getString().equalsIgnoreCase("UPDATE") || ObjItem.getAttribute("ZONE_STATUS").getString().equalsIgnoreCase("DELETE")) {
                    s = 0;
                    cntupdt++;
                    rowData[s] = Integer.toString(cntupdt);
                    s++;
                    rowData[s] = ObjItem.getAttribute("ZONE_STATUS").getString();
                    s++;
                    rowData[s] = ObjItem.getAttribute("ZONE_CD").getString();
                    s++;
                    rowData[s] = ObjItem.getAttribute("ZONE_NAME").getString();
                    ...
                    rowData[s] = emp.get(ObjItem.getAttribute("REGIONAL_HEAD").getString()) + "(#" + ObjItem.getAttribute("REGIONAL_HEAD").getString() + ")";;
                    s++;
                    rowData[s] = ObjItem.getAttribute("OLD_ZONE_CODE").getString();
                    s++;
                    rowData[s] = ObjItem.getAttribute("OLD_ZONE_NAME").getString();
                    s++;
                    rowData[s] = ObjItem.getAttribute("OLD_ZONE_CATEGORY").getString();
                    s++;
                    rowData[s] = emp.get(ObjItem.getAttribute("OLD_INCHARGE_COMMERCIAL").getString()) + "(#" + ObjItem.getAttribute("OLD_INCHARGE_COMMERCIAL").getString() + ")";
                    s++;
                    rowData[s] = emp.get(ObjItem.getAttribute("OLD_INCHARGE_TECHNICAL").getString()) + "(#" + ObjItem.getAttribute("OLD_INCHARGE_TECHNICAL").getString() + ")";;
                    s++;
                    rowData[s] = emp.get(ObjItem.getAttribute("OLD_COORDINATOR").getString()) + "(#" + ObjItem.getAttribute("OLD_COORDINATOR").getString() + ")";;
                    s++;
                    rowData[s] = emp.get(ObjItem.getAttribute("OLD_REGIONAL_HEAD").getString()) + "(#" + ObjItem.getAttribute("OLD_REGIONAL_HEAD").getString() + ")";;
                    s++;
            ...
            tbladdnewKeyReleased(null);
            tblupdateKeyReleased(null);
        } catch (Exception s) {
            s.printStackTrace();
        }
        try {
            ...
            HashMap History = clsZoneMaster.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsZoneMaster ObjHistory = (clsZoneMaster) History.get(Integer.toString(i));
    ...
    private void FormatGrid() {
        try {
            ...
            DataModelZone.addColumn("Category");
            DataModelZone.addColumn("Incharge Commercial");
            DataModelZone.addColumn("Incharge Technical");
            DataModelZone.addColumn("Coordinator");
            DataModelZone.addColumn("Regional Head");
            ...
            DataModelZoneAdd = new EITLTableModel();

            tbladdnew.removeAll();
            tbladdnew.setModel(DataModelZoneAdd);

            //Set the table Readonly
            DataModelZoneAdd.TableReadOnly(false);
            DataModelZoneAdd.addColumn("SrNo"); //0 - Read Only
            DataModelZoneAdd.addColumn("Zone Cd");
            DataModelZoneAdd.addColumn("Zone Name");
            DataModelZoneAdd.addColumn("Category");
            DataModelZoneAdd.addColumn("Incharge Commercial");
            DataModelZoneAdd.addColumn("Incharge Technical");
            DataModelZoneAdd.addColumn("Coordinator");
            DataModelZoneAdd.addColumn("Regional Head");

            tbladdnew.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
            tbladdnew.setRowSelectionAllowed(true);
            //tbladdnew.setColumnSelectionAllowed(true);
            for (int i = 0; i < 8; i++) {
                if (i == 2) {
                    if ((EditMode == 1) || (EditMode == 2 && clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID))) {
                        DataModelZoneAdd.ResetReadOnly(i);
                    } else {
                        DataModelZoneAdd.SetReadOnly(i);
                    }
                } else {
                    DataModelZoneAdd.SetReadOnly(i);
                }
            ...
            tblupdate.removeAll();
            tblupdate.setModel(DataModelZoneUpdt);

            //Set the table Readonly
            DataModelZoneUpdt.TableReadOnly(false);
            DataModelZoneUpdt.addColumn("SrNo"); //0 - Read Only
            DataModelZoneUpdt.addColumn("Status");
            DataModelZoneUpdt.addColumn("Zone Cd");
            DataModelZoneUpdt.addColumn("Zone Name");
            DataModelZoneUpdt.addColumn("Category");
            DataModelZoneUpdt.addColumn("Incharge Commercial");
            DataModelZoneUpdt.addColumn("Incharge Technical");
            DataModelZoneUpdt.addColumn("Coordinator");
            DataModelZoneUpdt.addColumn("Regional Head");
            DataModelZoneUpdt.addColumn("Old Zone Cd");
            DataModelZoneUpdt.addColumn("Old Zone Name");
            DataModelZoneUpdt.addColumn("Old Category");
            DataModelZoneUpdt.addColumn("Old Incharge Commercial");
            DataModelZoneUpdt.addColumn("Old Incharge Technical");
            DataModelZoneUpdt.addColumn("Old Coordinator");
            DataModelZoneUpdt.addColumn("Old Regional Head");
            tblupdate.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
            tblupdate.setRowSelectionAllowed(true);
            //tblupdate.setColumnSelectionAllowed(true);

            for (int i = 0; i < 15; i++) {
                if (i == 3) {
                    if ((EditMode == 1) || (EditMode == 2 && clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID))) {
                        DataModelZoneUpdt.ResetReadOnly(i);
                    } else {
    ...
    private void initComponents() {
        ...
        panel_addnew = new javax.swing.JScrollPane();
        tbladdnew = new javax.swing.JTable();
        panel_update = new javax.swing.JScrollPane();
        ...
        jScrollPane1.setBounds(10, 60, 1160, 90);

        jLabel17.setText("Zone Detail");
        ...
        tbladdnew.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        tbladdnew.setSelectionBackground(new java.awt.Color(208, 220, 234));
        tbladdnew.setSelectionForeground(new java.awt.Color(51, 51, 255));
        tbladdnew.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                tbladdnewKeyPressed(evt);
            }
            public void keyReleased(java.awt.event.KeyEvent evt) {
                tbladdnewKeyReleased(evt);
            }
        });
        panel_addnew.setViewportView(tbladdnew);

        jPanel1.add(panel_addnew);
        panel_addnew.setBounds(10, 180, 1160, 90);
        ...
        tblupdate.setSelectionBackground(new java.awt.Color(208, 220, 234));
        tblupdate.setSelectionForeground(new java.awt.Color(51, 51, 255));
        tblupdate.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                tblupdateKeyPressed(evt);
            }
            public void keyReleased(java.awt.event.KeyEvent evt) {
                tblupdateKeyReleased(evt);
            }
        });
        panel_update.setViewportView(tblupdate);

        jPanel1.add(panel_update);
        panel_update.setBounds(10, 310, 1160, 90);
        ...
        jPanel1.add(cmd_delete);
        cmd_delete.setBounds(100, 270, 80, 30);
        ...
        cmdupdate.setBounds(10, 270, 80, 30);

        Tab.addTab("Zone Master", jPanel1);
    ...
    private void tbladdnewKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbladdnewKeyPressed
        // TODO add your handling code here:
        if (evt.getKeyCode() == 112) //F1 Key pressed
        {
            if ((EditMode == 1) || (EditMode == 2 && clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID))) {
                if (tbladdnew.getSelectedColumn() == 3) {
                    LOV aList = new LOV();
                    ...
                    if (aList.ShowLOV()) {
                        tbladdnew.setValueAt(aList.ReturnVal, tbladdnew.getSelectedRow(), tbladdnew.getSelectedColumn());
                    }
                }
                if (tbladdnew.getSelectedColumn() == 4 || tbladdnew.getSelectedColumn() == 5 || tbladdnew.getSelectedColumn() == 6 || tbladdnew.getSelectedColumn() == 7) {
                    LOV aList = new LOV();
                    ...
                    aList.SecondCol = 2;
                    if (aList.ShowLOV()) {
                        tbladdnew.setValueAt(aList.SecondVal + "(#" + aList.ReturnVal + ")", tbladdnew.getSelectedRow(), tbladdnew.getSelectedColumn());
                    }
                }
                tbladdnewKeyReleased(null);
            }
        }
    }//GEN-LAST:event_tbladdnewKeyPressed

    private void tbladdnewKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbladdnewKeyReleased
        // TODO add your handling code here:
        final TableColumnModel columnModel = tbladdnew.getColumnModel();
        for (int column = 0; column < tbladdnew.getColumnCount(); column++) {
            int width = 60; // Min width
            for (int row = 0; row < tbladdnew.getRowCount(); row++) {
                TableCellRenderer renderer = tbladdnew.getCellRenderer(row, column);
                Component comp = tbladdnew.prepareRenderer(renderer, row, column);
                width = Math.max(comp.getPreferredSize().width + 10, width);
            }
            if (width > 300) {
                width = 300;
    ...
    }//GEN-LAST:event_tbladdnewKeyReleased

    private void tblupdateKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tblupdateKeyPressed
        // TODO add your handling code here:
        if (evt.getKeyCode() == 112) //F1 Key pressed
        {
            if ((EditMode == 1) || (EditMode == 2 && clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID))) {
                if (tblupdate.getValueAt(tblupdate.getSelectedRow(), 1).toString().equalsIgnoreCase("UPDATE")) {
                    if (tblupdate.getSelectedColumn() == 4) {
                        LOV aList = new LOV();
                        aList.SQL = "SELECT 'DOMESTIC' AS CATEGORY,'DOMESTIC' AS CATEGORY1 FROM DUAL "
                                + "UNION ALL SELECT 'INTERNATIONAL' AS CATEGORY,'INTERNATIONAL' AS CATEGORY1 FROM DUAL ";
                        aList.ReturnCol = 1;
                        ...
                        aList.DefaultSearchOn = 1;
                        if (aList.ShowLOV()) {
                            tblupdate.setValueAt(aList.ReturnVal, tblupdate.getSelectedRow(), tblupdate.getSelectedColumn());
                        }
                    }
                    if (tblupdate.getSelectedColumn() == 5 || tblupdate.getSelectedColumn() == 6 || tblupdate.getSelectedColumn() == 7 || tblupdate.getSelectedColumn() == 8) {
                        LOV aList = new LOV();
                        aList.SQL = "SELECT USER_ID,USER_NAME FROM DINESHMILLS.D_COM_USER_MASTER "
                                + "WHERE DEPT_ID=29 AND LEFT_DATE='0000-00-00'";
                        aList.ReturnCol = 1;
                        ...
                        aList.SecondCol = 2;
                        if (aList.ShowLOV()) {
                            tblupdate.setValueAt(aList.SecondVal + "(#" + aList.ReturnVal + ")", tblupdate.getSelectedRow(), tblupdate.getSelectedColumn());
                        }
                    }
                }
                tblupdateKeyReleased(null);
    ...
    }//GEN-LAST:event_tblupdateKeyPressed

    private void tblupdateKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tblupdateKeyReleased
        // TODO add your handling code here:
        final TableColumnModel columnModel = tblupdate.getColumnModel();
        for (int column = 0; column < tblupdate.getColumnCount(); column++) {
            int width = 60; // Min width
            for (int row = 0; row < tblupdate.getRowCount(); row++) {
                TableCellRenderer renderer = tblupdate.getCellRenderer(row, column);
                Component comp = tblupdate.prepareRenderer(renderer, row, column);
                width = Math.max(comp.getPreferredSize().width + 10, width);
            }
            if (width > 300) {
                width = 300;
    ...
    private void cmdAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdAddActionPerformed
        if (JOptionPane.showConfirmDialog(this, "Are you want to Add New Zone ?", "ADD ZONE", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            if (JOptionPane.showConfirmDialog(this, "Are you sure want to Add New Zone ?", "ADD ZONE", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                Object[] rowData = new Object[40];
                rowData[0] = Integer.toString(tbladdnew.getRowCount() + 1);
                lstzone++;
                rowData[1] = lstzone;
                DataModelZoneAdd.addRow(rowData);
                UpdateSrNo(tbladdnew);
                tbladdnew.changeSelection(tbladdnew.getRowCount() - 1, 1, false, false);
                tbladdnew.requestFocus();
            }
        }
    ...
    private void cmd_deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmd_deleteActionPerformed
        if (Table.getSelectedRow() < 0) {
            JOptionPane.showMessageDialog(this, "Please Select Zone to be Delete...");
        } else {
            if (JOptionPane.showConfirmDialog(this, "Are you want to Delete Zone ?", "Delete ZONE", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                if (JOptionPane.showConfirmDialog(this, "Are you sure want to Delete Zone ?", "Delete ZONE", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                    int s = 0;
                    Object[] rowData = new Object[40];
                    rowData[s] = Integer.toString(tblupdate.getRowCount() + 1);
                    s++;
                    rowData[s] = "DELETE";
                    s++;
                    rowData[s] = Table.getValueAt(Table.getSelectedRow(), 1);
                    s++;
                    rowData[s] = Table.getValueAt(Table.getSelectedRow(), 2);
                    s++;
                    rowData[s] = Table.getValueAt(Table.getSelectedRow(), 3);
                    s++;
                    rowData[s] = Table.getValueAt(Table.getSelectedRow(), 4);
                    s++;
                    rowData[s] = Table.getValueAt(Table.getSelectedRow(), 5);
                    s++;
                    rowData[s] = Table.getValueAt(Table.getSelectedRow(), 6);
                    s++;
                    rowData[s] = Table.getValueAt(Table.getSelectedRow(), 7);
                    s++;
                    DataModelZoneUpdt.addRow(rowData);
                    UpdateSrNo(tblupdate);
                    tblupdate.changeSelection(tblupdate.getRowCount() - 1, 1, false, false);
                    tblupdate.requestFocus();
                }
            }
        }
    ...
    private void cmdupdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdupdateActionPerformed
        ...
        if (Table.getSelectedRow() < 0) {
            ...
            if (JOptionPane.showConfirmDialog(this, "Are you want to Update Zone ?", "Update ZONE", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                if (JOptionPane.showConfirmDialog(this, "Are you sure want to Update Zone ?", "Update ZONE", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                    int s = 0;
                    Object[] rowData = new Object[40];
                    rowData[s] = Integer.toString(tblupdate.getRowCount() + 1);
                    s++;
...
// find rate update by doc no
    ...
    private void SetFields(boolean pStat) {
        ...
        cmdupdate.setEnabled(pStat);
        cmd_delete.setEnabled(pStat);
...
//      
        ...
        for (int m = 0; m < Table.getRowCount(); m++) {
            ...
            ObjItem.setAttribute("ZONE_CD", Table.getValueAt(m, 1).toString());
            ObjItem.setAttribute("ZONE_NAME", Table.getValueAt(m, 2).toString());
            ObjItem.setAttribute("ZONE_CATEGORY", Table.getValueAt(m, 3).toString());
            ObjItem.setAttribute("INCHARGE_COMMERCIAL", GetUserID(Table.getValueAt(m, 4).toString()));
            ObjItem.setAttribute("INCHARGE_TECHNICAL", GetUserID(Table.getValueAt(m, 5).toString()));
            ObjItem.setAttribute("COORDINATOR", GetUserID(Table.getValueAt(m, 6).toString()));
            ObjItem.setAttribute("REGIONAL_HEAD", GetUserID(Table.getValueAt(m, 7).toString()));
            ObjItem.setAttribute("OLD_ZONE_CODE", "");
            ObjItem.setAttribute("OLD_ZONE_NAME", "");
            ObjItem.setAttribute("OLD_ZONE_CATEGORY", "");
            ObjItem.setAttribute("OLD_INCHARGE_COMMERCIAL", "");
            ObjItem.setAttribute("OLD_INCHARGE_TECHNICAL", "");
            ObjItem.setAttribute("OLD_COORDINATOR", "");
            ObjItem.setAttribute("OLD_REGIONAL_HEAD", "");
            ObjItem.setAttribute("ZONE_STATUS", "MASTER");
            ZoneMaster.colMRItems.put(Integer.toString(ZoneMaster.colMRItems.size() + 1), ObjItem);
        }
        for (int m = 0; m < tbladdnew.getRowCount(); m++) {
            clsZoneMasterItem ObjItem = new clsZoneMasterItem();
            ObjItem.setAttribute("DOC_NO", DOC_NO1.getText());
            ObjItem.setAttribute("DOC_DATE", EITLERPGLOBAL.getCurrentDateDB());
            ObjItem.setAttribute("ZONE_CD", tbladdnew.getValueAt(m, 1).toString());
            ObjItem.setAttribute("ZONE_NAME", tbladdnew.getValueAt(m, 2).toString());
            ObjItem.setAttribute("ZONE_CATEGORY", tbladdnew.getValueAt(m, 3).toString());
            ObjItem.setAttribute("INCHARGE_COMMERCIAL", GetUserID(tbladdnew.getValueAt(m, 4).toString()));
            ObjItem.setAttribute("INCHARGE_TECHNICAL", GetUserID(tbladdnew.getValueAt(m, 5).toString()));
            ObjItem.setAttribute("COORDINATOR", GetUserID(tbladdnew.getValueAt(m, 6).toString()));
            ObjItem.setAttribute("REGIONAL_HEAD", GetUserID(tbladdnew.getValueAt(m, 7).toString()));
            ObjItem.setAttribute("OLD_ZONE_CODE", "");
            ObjItem.setAttribute("OLD_ZONE_NAME", "");
            ObjItem.setAttribute("OLD_ZONE_CATEGORY", "");
            ObjItem.setAttribute("OLD_INCHARGE_COMMERCIAL", "");
            ...
            ObjItem.setAttribute("OLD_COORDINATOR", "");
            ObjItem.setAttribute("OLD_REGIONAL_HEAD", "");
            ObjItem.setAttribute("ZONE_STATUS", "ADD");
            ZoneMaster.colMRItems.put(Integer.toString(ZoneMaster.colMRItems.size() + 1), ObjItem);
        }
        for (int m = 0; m < tblupdate.getRowCount(); m++) {
            clsZoneMasterItem ObjItem = new clsZoneMasterItem();
            ObjItem.setAttribute("DOC_NO", DOC_NO1.getText());
            ObjItem.setAttribute("DOC_DATE", EITLERPGLOBAL.getCurrentDateDB());
            ObjItem.setAttribute("ZONE_CD", tblupdate.getValueAt(m, 2).toString());
            ObjItem.setAttribute("ZONE_NAME", tblupdate.getValueAt(m, 3).toString());
            ObjItem.setAttribute("ZONE_CATEGORY", tblupdate.getValueAt(m, 4).toString());
            ObjItem.setAttribute("INCHARGE_COMMERCIAL", GetUserID(tblupdate.getValueAt(m, 5).toString()));
            ObjItem.setAttribute("INCHARGE_TECHNICAL", GetUserID(tblupdate.getValueAt(m, 6).toString()));
            ObjItem.setAttribute("COORDINATOR", GetUserID(tblupdate.getValueAt(m, 7).toString()));
            ObjItem.setAttribute("REGIONAL_HEAD", GetUserID(tblupdate.getValueAt(m, 8).toString()));
            ObjItem.setAttribute("OLD_ZONE_CODE", tblupdate.getValueAt(m, 9).toString());
            ObjItem.setAttribute("OLD_ZONE_NAME", tblupdate.getValueAt(m, 10).toString());
            ObjItem.setAttribute("OLD_ZONE_CATEGORY", tblupdate.getValueAt(m, 11).toString());
            ObjItem.setAttribute("OLD_INCHARGE_COMMERCIAL", GetUserID(tblupdate.getValueAt(m, 12).toString()));
            ObjItem.setAttribute("OLD_INCHARGE_TECHNICAL", GetUserID(tblupdate.getValueAt(m, 13).toString()));
            ObjItem.setAttribute("OLD_COORDINATOR", GetUserID(tblupdate.getValueAt(m, 14).toString()));
            ObjItem.setAttribute("OLD_REGIONAL_HEAD", GetUserID(tblupdate.getValueAt(m, 15).toString()));
            ObjItem.setAttribute("ZONE_STATUS", tblupdate.getValueAt(m, 1).toString());
            ZoneMaster.colMRItems.put(Integer.toString(ZoneMaster.colMRItems.size() + 1), ObjItem);
        }
    ...
    private String GetUserID(String str) {
        ...
        if (str.contains("#")) {
            return str_split[1].substring(0, str_split[1].trim().length() - 1);
        } else {
            return "";
        }
    }
    ...
    private javax.swing.JScrollPane panel_addnew;
    private javax.swing.JScrollPane panel_update;
    private javax.swing.JTable tbladdnew;
    ...
    private void Zone_Master_Data() {
        try {
            ...
            emp.put("", "");
            rs = data.getResult("SELECT INCHARGE_CD, INCHARGE_NAME,  COALESCE(INCHARGE_CATEGORY,'') AS INCHARGE_CATEGORY,"
                    + " COALESCE(INCHARGE_COMMERCIAL,'') AS INCHARGE_COMMERCIAL, COALESCE(INCHARGE_TECHNICAL,'') AS INCHARGE_TECHNICAL,"
                    + " COALESCE(INCHARGE_COORDINATOR,'') AS INCHARGE_COORDINATOR,COALESCE(REGIONAL_HEAD,'') AS REGIONAL_HEAD FROM PRODUCTION.FELT_INCHARGE WHERE INCHARGE_CD!=0 AND INCHARGE_ACTIVE IN ('Y','y') ORDER BY INCHARGE_CD");
            rs.first();
            int i = 1, s;
            if (rs.getRow() > 0) {
                while (!rs.isAfterLast()) {
                    ...
                    s++;
                    rowData[s] = rs.getString("INCHARGE_CATEGORY");
                    s++;
                    rowData[s] = emp.get(rs.getString("INCHARGE_COMMERCIAL")) + "(#" + rs.getString("INCHARGE_COMMERCIAL") + ")";
                    s++;
                    rowData[s] = emp.get(rs.getString("INCHARGE_TECHNICAL")) + "(#" + rs.getString("INCHARGE_TECHNICAL") + ")";;
                    s++;
                    rowData[s] = emp.get(rs.getString("INCHARGE_COORDINATOR")) + "(#" + rs.getString("INCHARGE_COORDINATOR") + ")";;
                    s++;
                    rowData[s] = emp.get(rs.getString("REGIONAL_HEAD")) + "(#" + rs.getString("REGIONAL_HEAD") + ")";;
                    s++;
                    i++;

                    DataModelZone.addRow(rowData);
                    lstzone = rs.getInt("INCHARGE_CD");
                    rs.next();
                }
                final TableColumnModel columnModel = Table.getColumnModel();
                for (int column = 0; column < Table.getColumnCount(); column++) {
