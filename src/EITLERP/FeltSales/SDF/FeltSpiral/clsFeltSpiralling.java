 ...
 * @version
 */
public class clsFeltSpiralling {
    ...
    public HashMap hmFeltSpirallingDetails;
    ...
    public clsFeltSpiralling() {
        ...
        hmFeltSpirallingDetails = new HashMap();
    }
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT DISTINCT PROD_DOC_NO FROM PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT = 'SDF' AND PROD_DATE>='" + EITLERPGLOBAL.FinFromDateDB + "' AND PROD_DATE<='" + EITLERPGLOBAL.FinToDateDB + "' ORDER BY PROD_DATE");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            for (int i = 1; i <= hmFeltSpirallingDetails.size(); i++) {
                clsFeltSpirallingDetails ObjFeltMendingDetails = (clsFeltSpirallingDetails) hmFeltSpirallingDetails.get(Integer.toString(i));
                ...
                resultSetTemp.updateString("PROD_DEPT", "SDF");
                ...
                resultSetHistory.updateString("PROD_DEPT", "SDF");
                ...
                try {
                    if ("F".equals(getAttribute("APPROVAL_STATUS").getString())) {
                        String Piece = ObjFeltMendingDetails.getAttribute("PRODUCTION_PIECE_NO").getString();
                        data.Execute("UPDATE PRODUCTION.FELT_WIP_PIECE_REGISTER SET WIP_PIECE_STAGE='FINISHING',WIP_STATUS='ASSEMBLED',WIP_GIDC_STATUS='SDML',WIP_SDF_ASSEMBLED_DATE='" + EITLERPGLOBAL.getCurrentDateDB() + "' WHERE WIP_PIECE_NO='" + Piece + "'");
                        data.Execute("UPDATE PRODUCTION.FELT_SALES_PIECE_REGISTER SET PR_PIECE_STAGE='FINISHING',PR_WIP_STATUS='ASSEMBLED',PR_GIDC_STATUS='SDML',PR_SDF_ASSEMBLED_DATE='" + EITLERPGLOBAL.getCurrentDateDB() + "' WHERE PR_PIECE_NO='" + Piece + "'");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
            ...
            ObjFeltProductionApprovalFlow.ModuleID = 850; //Felt Mending Entry
    ...
    public boolean Update() {
        ...
        try {
            ...
            resultSetHistory = statementHistory.executeQuery("SELECT MAX(REVISION_NO) FROM PRODUCTION.FELT_PROD_DATA_H WHERE PROD_DEPT = 'SDF' AND PROD_DATE='" + EITLERPGLOBAL.formatDateDB(getAttribute("PRODUCTION_DATE").getString()) + "' AND PROD_DOC_NO='" + getAttribute("PRODUCTION_DOCUMENT_NO").getString() + "'");
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE PRODUCTION.`FELT_PROD_DATA` SET REJECTED=0,CHANGED=1 WHERE PROD_DEPT='SDF' AND PROD_DOC_NO='" + getAttribute("PRODUCTION_DOCUMENT_NO").getString() + "'");
                //Remove Old Records from FELT DOCUMENT APPROVAL TABLE
                //data.Execute("DELETE FROM PRODUCTION.`FELT_PROD_DOC_DATA` WHERE MODULE_ID=850 AND DOC_NO='"+getAttribute("PRODUCTION_DOCUMENT_NO").getString()+"'");
    ...
    public boolean CanDelete(String documentNo, String stringProductionDate, int userID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT = 'SDF' AND PROD_DOC_NO='" + documentNo + "' AND APPROVED=" + 1;
            ...
            if (rsTmp.getInt("COUNT") > 0) {  //Item is Approved
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.`FELT_PROD_DOC_DATA` WHERE MODULE_ID=850 AND USER_ID=" + userID + " AND DOC_NO='" + documentNo + "' AND STATUS='W'";
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();

                if (rsTmp.getInt("COUNT") > 0) {
                    strSQL = "DELETE FROM PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT = 'SDF'"
    ...
    public boolean Filter(String stringFindQuery) {
        Ready = false;
        try {
            String strSql = "SELECT DISTINCT PROD_DOC_NO FROM PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT = 'SDF' " + stringFindQuery + "ORDER BY PROD_DATE";
            connection = data.getConn();
            statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean setData() {
        ...
        try {
            ...
            totalWeight = data.getDoubleValueFromDB("SELECT SUM(WEIGHT) FROM PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT = 'SDF' AND MONTH(PROD_DATE)=" + documentNo.substring(6, 8) + " AND YEAR(PROD_DATE)=" + documentNo.substring(2, 6) + " AND APPROVED=1");
            setAttribute("TOTAL_WEIGHT", totalWeight);

            previousWeight = data.getDoubleValueFromDB("SELECT SUM(WEIGHT) FROM PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT = 'SDF' AND MONTH(PROD_DATE)=" + documentNo.substring(6, 8) + " AND YEAR(PROD_DATE)=" + documentNo.substring(2, 6) + " AND SUBSTRING(PROD_DOC_NO,3,8)<'" + documentNo.substring(2) + "' AND APPROVED=1");
            setAttribute("PREVIOUS_WEIGHT", previousWeight);

            //Now Populate the collection, first clear the collection
            hmFeltSpirallingDetails.clear();

            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT = 'SDF'  AND PROD_DOC_NO='" + documentNo + "' ORDER BY SR_NO");
            ...
            while (resultSetTemp.next()) {
                ...
                clsFeltSpirallingDetails ObjFeltMendingDetails = new clsFeltSpirallingDetails();
                ...
                hmFeltSpirallingDetails.put(Integer.toString(serialNoCounter), ObjFeltMendingDetails);
            }
            resultSetTemp.close();
            statementTemp.close();
            return true;
    ...
    public boolean setHistoryData(String pProductionDate, String pDocNo) {
        ...
        try {
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.`FELT_PROD_DATA_H` WHERE PROD_DEPT = 'SDF' AND PROD_DATE='" + pProductionDate + "' AND PROD_DOC_NO='" + pDocNo + "' AND REVISION_NO='" + resultSet.getInt("REVISION_NO") + "' ORDER BY REVISION_NO");
    ...
    public static HashMap getHistoryList(String stringProductionDate, String productionDocumentNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT * FROM PRODUCTION.FELT_PROD_DATA_H WHERE PROD_DEPT = 'SDF' AND PROD_DATE='" + stringProductionDate + "' AND PROD_DOC_NO='" + productionDocumentNo + "' GROUP BY REVISION_NO");

            while (rsTmp.next()) {
                clsFeltSpiralling ObjFeltMending = new clsFeltSpiralling();
    ...
    public boolean ShowHistory(String pProductionDate, String pDocNo) {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT DISTINCT REVISION_NO FROM PRODUCTION.FELT_PROD_DATA_H WHERE PROD_DEPT='SDF' AND PROD_DATE='" + pProductionDate + "' AND PROD_DOC_NO='" + pDocNo + "'");
            Ready = true;
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            ...
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT DISTINCT PROD_DOC_NO,PROD_DATE,RECEIVED_DATE FROM PRODUCTION.FELT_PROD_DATA,PRODUCTION.FELT_PROD_DOC_DATA WHERE PROD_DOC_NO=DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=850 AND CANCELED=0 ORDER BY RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT DISTINCT PROD_DOC_NO,PROD_DATE,RECEIVED_DATE FROM PRODUCTION.FELT_PROD_DATA,PRODUCTION.FELT_PROD_DOC_DATA WHERE PROD_DOC_NO=DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=850 AND CANCELED=0 ORDER BY PROD_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT DISTINCT PROD_DOC_NO,PROD_DATE,RECEIVED_DATE FROM PRODUCTION.FELT_PROD_DATA,PRODUCTION.FELT_PROD_DOC_DATA WHERE PROD_DOC_NO=DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=850 AND CANCELED=0 ORDER BY PROD_DOC_NO";
            }
            ...
            while (rsTmp.next()) {
                ...
                Counter = Counter + 1;
                clsFeltSpiralling ObjDoc = new clsFeltSpiralling();
    ...
    public boolean checkPieceNoInDB(String pPieceNo) {
        int count = data.getIntValueFromDB("SELECT COUNT(PROD_PIECE_NO) FROM PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT = 'SDF' AND PROD_PIECE_NO='" + pPieceNo + "' AND PROD_DATE>='" + EITLERPGLOBAL.FinFromDateDB + "' AND PROD_DATE<='" + EITLERPGLOBAL.FinToDateDB + "'");
        if (count > 0) {
            return true;
        } else {
            return false;
    ...
    public boolean checkPieceNoInDB(String pPieceNo, String pProdDate) {
        ...
        if (count >= 1) {
            ...
            try {
                ...
                ResultSet rsTmp = stTmp.executeQuery("SELECT PROD_DATE FROM PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT='SDF' AND PROD_PIECE_NO='" + pPieceNo + "' AND PROD_DATE>='" + EITLERPGLOBAL.FinFromDateDB + "' AND PROD_DATE<='" + EITLERPGLOBAL.FinToDateDB + "'");
                while (rsTmp.next()) {
    ...
    public boolean checkProductionDocumentNoInDB(String pProdDocNo) {
        int count = data.getIntValueFromDB("SELECT COUNT(PROD_DOC_NO) FROM PRODUCTION.`FELT_PROD_DATA` WHERE PROD_DEPT = 'SDF' AND PROD_DOC_NO='" + pProdDocNo + "'");
        if (count > 0) {
            return true;
        } else {
            return false;
