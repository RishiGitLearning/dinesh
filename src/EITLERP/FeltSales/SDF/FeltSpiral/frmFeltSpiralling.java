...
public class frmFeltSpiralling extends javax.swing.JApplet {
    ...
    private clsFeltSpiralling ObjFeltSpiralling;
    ...
    /**
     * Creates new form frmFeltSpiralling_New
     */
    public void init() {
        System.gc();
        setSize(830, 590);
        initComponents();
        cmdOpenDocument.setVisible(false);
        ...
        ObjFeltSpiralling = new clsFeltSpiralling();

        SetMenuForRights();
        GenerateHierarchyCombo();
        GenerateSendToCombo();
        GenerateUserNameCombo();
        if (ObjFeltSpiralling.LoadData()) {
            DisplayData();
        } else {
            JOptionPane.showMessageDialog(this, "Error occured while loading data. Error is " + ObjFeltSpiralling.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void initComponents() {
        ...
        lblTitle.setText(" FELT PRODUCTION SPIRAL DETAILS - ");
        ...
        txtFeltProductionDate.setBounds(150, 10, 93, 21);
        ...
        Tab1.add(jLabel4);
        jLabel4.setBounds(395, 12, 70, 15);
        ...
        txtProductionDocumentNo.setBounds(150, 40, 108, 21);

        lblRevNo.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        lblRevNo.setText("...");
        Tab1.add(lblRevNo);
        lblRevNo.setBounds(250, 10, 20, 18);

        jLabel6.setText("Previous Weight");
        Tab1.add(jLabel6);
        jLabel6.setBounds(10, 369, 130, 15);
        ...
        txtPreviousWeight.setBounds(133, 366, 100, 21);

        jLabel7.setText("Day Weight");
        Tab1.add(jLabel7);
        jLabel7.setBounds(256, 369, 90, 15);
        ...
        txtDayWeight.setBounds(350, 366, 100, 21);

        jLabel8.setText("Total Weight");
        Tab1.add(jLabel8);
        jLabel8.setBounds(256, 397, 100, 15);
        ...
        txtTotalWeight.setBounds(350, 394, 100, 21);
        ...
        txtMonthWeight.setBounds(133, 394, 100, 21);

        jLabel11.setText("Month Weight");
        Tab1.add(jLabel11);
        jLabel11.setBounds(10, 397, 120, 15);

        jLabel12.setText("User Name");
        Tab1.add(jLabel12);
        jLabel12.setBounds(395, 43, 80, 15);
    ...
    private void cmdOpenDocumentActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdOpenDocumentActionPerformed
        if (Table.getSelectedRow() != -1) {
            ...
            JOptionPane.showMessageDialog(this, "Select a row from Spiralling Details.", "WARNING", JOptionPane.INFORMATION_MESSAGE);
        }
    }//GEN-LAST:event_cmdOpenDocumentActionPerformed

    private void cmdPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdPrintActionPerformed
        new TReportWriter.TReportEngine().PreviewReport("http://" + EITLERPGLOBAL.HostIP + "/EITLERP/Reports/Production/rptFeltSpiralling.rpt", new HashMap(), ObjFeltSpiralling.getReportData(txtProductionDocumentNo.getText().trim()));
        EITLERPGLOBAL.PAGE_BREAK = true;
    }//GEN-LAST:event_cmdPrintActionPerformed

    private void cmdPreviewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdPreviewActionPerformed
        try {
            URL reportFile = new URL("http://" + EITLERPGLOBAL.HostIP + "/EITLERP/Reports/Production/rptFeltSpiralling.jsp?dbURL=" + EITLERPGLOBAL.DatabaseURL + "&DOC_NO=" + txtProductionDocumentNo.getText().trim());
    ...
    private void cmdBackToNormalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdBackToNormalActionPerformed
        ObjFeltSpiralling.HistoryView = false;
        ObjFeltSpiralling.LoadData();
        MoveLast();
    ...
    private void cmdViewRevisionsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdViewRevisionsActionPerformed
        ObjFeltSpiralling.ShowHistory(EITLERPGLOBAL.formatDateDB(txtFeltProductionDate.getText()), txtProductionDocumentNo.getText());
        MoveLast();
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            GenerateRejectedSendToCombo();
            if (clsFeltProductionApprovalFlow.IsOnceRejectedDoc(850, txtProductionDocumentNo.getText())) {
                cmbSendTo.setEnabled(true);
            } else {
                cmbSendTo.setEnabled(false);
            }
        ...
        if (evt.getKeyCode() == 112) //F1 Key pressed
        {
            if ((EditMode == 1) || (EditMode == 2 && clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID))) {
                if (Table.getSelectedColumn() == 1) {
                    LOV aList = new LOV();
                    aList.SQL = "SELECT WIP_PIECE_NO AS PIECE,WIP_PARTY_CODE AS PARTY_CODE,PARTY_NAME "
                            + "FROM PRODUCTION.FELT_WIP_PIECE_REGISTER R "
                            + " LEFT JOIN DINESHMILLS.D_SAL_PARTY_MASTER M "
                            + " ON R.WIP_PARTY_CODE=M.PARTY_CODE "
                            + "WHERE WIP_PIECE_STAGE='ASSEMBLY' "
                            + " AND WIP_PIECE_NO NOT IN (SELECT PROD_PIECE_NO FROM PRODUCTION.FELT_PROD_DATA "
                            + " WHERE PROD_DEPT='SDF' AND APPROVED=0 AND COALESCE(CANCELED,0)=0)";
                    aList.ReturnCol = 1;
                        ...
                        if (!data.getStringValueFromDB("SELECT PD.PIECE_NO FROM PRODUCTION.FELT_SALES_PIECE_AMENDMENT_HEADER_WIP PH, "
                            ...
                            try {
                                ...
                                if (trs.getRow() > 0) {
                                    ...
                                    Table.setValueAt(trs.getString("WIP_PRODUCT_CODE"), Table.getSelectedRow(), 3);
                                    Table.setValueAt(trs.getString("WIP_GROUP"), Table.getSelectedRow(), 4);
                                    Table.setValueAt(trs.getString("WIP_STYLE"), Table.getSelectedRow(), 7);
    ...
    private void jMenuItemWarpingActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItemWarpingActionPerformed
        ...
        try {
            ...
            if (mDocNo.trim().length() > 0) {
                ...
                aFrame.startAppletEx("EITLERP.FeltSales.FeltWarpingBeamOrder.frmWarpingBeamOrder", "Felt Warping Report");
                frmWarpingBeamOrder ObjFeltWarping = (frmWarpingBeamOrder) aFrame.ObjApplet;
    ...
    private void DisplayData() {
        ...
        if (EITLERPGLOBAL.gAuthorityUserID != EITLERPGLOBAL.gUserID) {
            if (clsAuthority.IsAuthorityGiven(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gUserID, EITLERPGLOBAL.gAuthorityUserID, 850)) {
        ...
        try {
            if (EditMode == 0) {
                if (ObjFeltSpiralling.getAttribute("APPROVED").getInt() == 1) {
                    lblTitle.setBackground(Color.BLUE);
                } else {
                    lblTitle.setBackground(Color.GRAY);
                }

                /*if(ObjFeltSpiralling.getAttribute("CANCELED").getInt()==1) {
                 lblTitle.setBackground(Color.RED);
                 }*/
            }
            //============================================//

            String productionDate = EITLERPGLOBAL.formatDate(ObjFeltSpiralling.getAttribute("PRODUCTION_DATE").getString());
            String documentNo = ObjFeltSpiralling.getAttribute("PRODUCTION_DOCUMENT_NO").getString();
            lblTitle.setText(" FELT PRODUCTION SPIRAL DETAILS - " + productionDate);
            lblRevNo.setText(Integer.toString((int) ObjFeltSpiralling.getAttribute("REVISION_NO").getVal()));
            txtMonthWeight.setText(Double.toString(EITLERPGLOBAL.round(ObjFeltSpiralling.getAttribute("TOTAL_WEIGHT").getVal(), 2)));
            txtPreviousWeight.setText(Double.toString(EITLERPGLOBAL.round(ObjFeltSpiralling.getAttribute("PREVIOUS_WEIGHT").getVal(), 2)));
            EITLERPGLOBAL.setComboIndex(cmbHierarchy, (int) ObjFeltSpiralling.getAttribute("HIERARCHY_ID").getVal());
            DoNotEvaluate = true;

            FormatGrid();
            txtFeltProductionDate.setText(productionDate);
            txtProductionDocumentNo.setText(documentNo);
            txtFormNo.setText(ObjFeltSpiralling.getAttribute("PRODUCTION_FORM_NO").getString());

            //Now Generate Table
            for (int i = 1; i <= ObjFeltSpiralling.hmFeltSpirallingDetails.size(); i++) {
                clsFeltSpirallingDetails ObjFeltSpirallingDetails = (clsFeltSpirallingDetails) ObjFeltSpiralling.hmFeltSpirallingDetails.get(Integer.toString(i));

                Object[] rowData = new Object[10];
                rowData[0] = Integer.toString(i);
                String PieceNo=ObjFeltSpirallingDetails.getAttribute("PRODUCTION_PIECE_NO").getString();
                rowData[1] = ObjFeltSpirallingDetails.getAttribute("PRODUCTION_PIECE_NO").getString();
                rowData[2] = ObjFeltSpirallingDetails.getAttribute("PRODUCTION_PARTY_CODE").getString();
                rowData[3] = data.getStringValueFromDB("SELECT WIP_PRODUCT_CODE FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_PIECE_NO='" + ObjFeltSpirallingDetails.getAttribute("PRODUCTION_PIECE_NO").getString() + "'"); //6
                rowData[4] = ObjFeltSpirallingDetails.getAttribute("GROUP").getString();
                rowData[5] = Double.toString(EITLERPGLOBAL.round(ObjFeltSpirallingDetails.getAttribute("WEIGHT").getVal(), 2));
                String ocYearMonth=data.getStringValueFromDB("SELECT WIP_OC_MONTHYEAR FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_PIECE_NO='"+PieceNo+"'");
                rowData[6] = ocYearMonth;
                rowData[7] = data.getStringValueFromDB("SELECT WIP_STYLE FROM PRODUCTION.FELT_WIP_PIECE_REGISTER WHERE WIP_PIECE_NO='" + ObjFeltSpirallingDetails.getAttribute("PRODUCTION_PIECE_NO").getString() + "'");  //8
                rowData[8] = ObjFeltSpirallingDetails.getAttribute("REMARKS").getString();
            ...
            for (int i = 0; i < Table.getRowCount(); i++) {
                if (Table.getValueAt(i, 5) != null) {
                    dayWeight = dayWeight + Double.parseDouble((String) Table.getValueAt(i, 5));
                }
            }
            ...
            hmList = clsFeltProductionApprovalFlow.getDocumentFlow(850, documentNo);
            ...
            FormatGridUpdateHistory();
            HashMap hmApprovalHistory = clsFeltSpiralling.getHistoryList(ObjFeltSpiralling.getAttribute("PRODUCTION_DATE").getString(), txtProductionDocumentNo.getText());
            for (int i = 1; i <= hmApprovalHistory.size(); i++) {
                clsFeltSpiralling ObjFeltSpiralling = (clsFeltSpiralling) hmApprovalHistory.get(Integer.toString(i));
                Object[] rowData = new Object[6];

                rowData[0] = Integer.toString((int) ObjFeltSpiralling.getAttribute("REVISION_NO").getVal());
                rowData[1] = FeltUser.getUserName((int) ObjFeltSpiralling.getAttribute("UPDATED_BY").getVal());
                rowData[2] = EITLERPGLOBAL.formatDate(ObjFeltSpiralling.getAttribute("ENTRY_DATE").getString()) + ObjFeltSpiralling.getAttribute("ENTRY_DATE").getString().substring(10, 19);

                String ApprovalStatus = "";

                if ((ObjFeltSpiralling.getAttribute("APPROVAL_STATUS").getString()).equals("H")) {
                    ApprovalStatus = "Hold";
                }

                if ((ObjFeltSpiralling.getAttribute("APPROVAL_STATUS").getString()).equals("A")) {
                    ApprovalStatus = "Approved";
                }

                if ((ObjFeltSpiralling.getAttribute("APPROVAL_STATUS").getString()).equals("F")) {
                    ApprovalStatus = "Final Approved";
                }

                if ((ObjFeltSpiralling.getAttribute("APPROVAL_STATUS").getString()).equals("W")) {
                    ApprovalStatus = "Waiting";
                }

                if ((ObjFeltSpiralling.getAttribute("APPROVAL_STATUS").getString()).equals("R")) {
                    ApprovalStatus = "Rejected";
                }

                if ((ObjFeltSpiralling.getAttribute("APPROVAL_STATUS").getString()).equals("P")) {
                    ApprovalStatus = "Pending";
                }

                if ((ObjFeltSpiralling.getAttribute("APPROVAL_STATUS").getString()).equals("C")) {
                    ApprovalStatus = "Skiped";
                }

                rowData[3] = ApprovalStatus;
                rowData[4] = ObjFeltSpiralling.getAttribute("APPROVER_REMARKS").getString();
                rowData[5] = ObjFeltSpiralling.getAttribute("FROM_IP").getString();
    ...
    private void FormatGrid() {
        try {
            ...
            DataModel.addColumn("Grey Weight");
    ...
    private void GenerateHierarchyCombo() {
        ...
        hmHierarchyList = clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=850 ");

        if (EditMode == EITLERPGLOBAL.EDIT) {
            hmHierarchyList = clsHierarchy.getList(" WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=850 ");
        }
    ...
    private void GenerateSendToCombo() {
        ...
        try {
            ...
            if (EditMode == EITLERPGLOBAL.ADD) {
                ...
                hmSendToList = clsFeltProductionApprovalFlow.getRemainingUsers(850, ObjFeltSpiralling.getAttribute("PRODUCTION_DOCUMENT_NO").getString());
                for (int i = 1; i <= hmSendToList.size(); i++) {
                    clsUser ObjUser = (clsUser) hmSendToList.get(Integer.toString(i));
                    ComboData aData = new ComboData();
                    aData.Code = (int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void GenerateRejectedSendToCombo() {
        ...
        for (int i = 1; i <= hmRejectedSendToList.size(); i++) {
            ...
            if (EditMode == EITLERPGLOBAL.EDIT) {
                if (OpgApprove.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInApproval(850, txtProductionDocumentNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }

                if (OpgReject.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInRejection(850, txtProductionDocumentNo.getText(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            int Creator = clsFeltProductionApprovalFlow.getCreator(850, txtProductionDocumentNo.getText());
    ...
    private void GenerateUserNameCombo() {
        ...
        hmUserNameList = ObjFeltSpiralling.getUserNameList(EITLERPGLOBAL.getComboCode(cmbHierarchy), EITLERPGLOBAL.gNewUserID, "SDF");
    ...
    private void SetupApproval() {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = clsFeltProductionApprovalFlow.getFromID(850, ObjFeltSpiralling.getAttribute("PRODUCTION_DOCUMENT_NO").getString());
            lnFromUserId = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = clsFeltProductionApprovalFlow.getFromRemarks(850, FromUserID, ObjFeltSpiralling.getAttribute("PRODUCTION_DOCUMENT_NO").getString());
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            cmbHierarchy.setEnabled(false);
            if (clsFeltProductionApprovalFlow.IsCreator(850, txtProductionDocumentNo.getText())) {
                OpgReject.setEnabled(false);
            }
        }
    ...
    private void SetMenuForRights() {
        // --- Add Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8923, 89231)) {
            cmdNew.setEnabled(true);
        } else {
            cmdNew.setEnabled(false);
        }
        ...
        // --- Edit Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8923, 89232)) {
            cmdEdit.setEnabled(true);
        } else {
            cmdEdit.setEnabled(false);
        }
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8923, 89233)) {
            cmdDelete.setEnabled(true);
        } else {
            cmdDelete.setEnabled(false);
        }
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8923, 89235)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        } else {
            cmdPreview.setEnabled(false);
...
//        jPopupMenu.show(cmdNew, 0, 35);
    ...
    private void Edit() {
        String productionDocumentNo = (String) ObjFeltSpiralling.getAttribute("PRODUCTION_DOCUMENT_NO").getObj();
        if (ObjFeltSpiralling.IsEditable(productionDocumentNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(850, productionDocumentNo)) {
                SetFields(true);
                txtFeltProductionDate.setEnabled(false);
                txtFormNo.setEnabled(false);
            } else {
    ...
    private void Delete() {
        if (ObjFeltSpiralling.CanDelete(txtProductionDocumentNo.getText(), txtFeltProductionDate.getText(), EITLERPGLOBAL.gNewUserID)) {
            DisplayData();
        } else {
            JOptionPane.showMessageDialog(this, ObjFeltSpiralling.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void Save() {
        ...
        productionDocumentNo = "FS" + productionDate.substring(6, 10) + productionDate.substring(3, 5) + productionDate.substring(0, 2);
        if (EditMode == EITLERPGLOBAL.EDIT) {
            productionDocumentNo = ObjFeltSpiralling.getAttribute("PRODUCTION_DOCUMENT_NO").getString();
        }

        // CHECK Document No already exist in database?
        if (EditMode == EITLERPGLOBAL.ADD) {
            if (ObjFeltSpiralling.checkProductionDocumentNoInDB(productionDocumentNo)) {
                JOptionPane.showMessageDialog(frmFeltSpiralling.this, "Production Date " + productionDate + " is Already Exists", "Production Date Already Exists", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
        ...
        if (Table.getRowCount() <= 0) {
            JOptionPane.showMessageDialog(this, "Enter Spiralling Details Before Saving.", "ERROR", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            ObjFeltSpiralling.hmFeltSpirallingDetails.clear();
            ...
            for (i = 0; i <= Table.getRowCount() - 1; i++) {
                ...
                if (EditMode == EITLERPGLOBAL.ADD) {
                    if (ObjFeltSpiralling.checkPieceNoInDB(pieceNo)) {
                        JOptionPane.showMessageDialog(frmFeltSpiralling.this, "Piece No." + pieceNo + " Already Mended.", " Piece No Already Exists", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
                ...
                if (EditMode == EITLERPGLOBAL.EDIT) {
                    if (ObjFeltSpiralling.checkPieceNoInDB(pieceNo, productionDate)) {
                        JOptionPane.showMessageDialog(frmFeltSpiralling.this, "Piece No." + pieceNo + " Already Mended.", " Piece No Already Exists", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
                ...
                clsFeltSpirallingDetails ObjFeltSpirallingDetails = new clsFeltSpirallingDetails();

                ObjFeltSpirallingDetails.setAttribute("PRODUCTION_PIECE_NO", pieceNo);
                ObjFeltSpirallingDetails.setAttribute("PRODUCTION_PARTY_CODE", partyCode);
                ObjFeltSpirallingDetails.setAttribute("WEIGHT", weight);
                ObjFeltSpirallingDetails.setAttribute("GROUP", group);
                ObjFeltSpirallingDetails.setAttribute("REMARKS", remarks);

                ObjFeltSpiralling.hmFeltSpirallingDetails.put(Integer.toString(ObjFeltSpiralling.hmFeltSpirallingDetails.size() + 1), ObjFeltSpirallingDetails);
            }
        ...
        ObjFeltSpiralling.setAttribute("PRODUCTION_DATE", productionDate);
        ObjFeltSpiralling.setAttribute("PRODUCTION_DOCUMENT_NO", productionDocumentNo);
        ObjFeltSpiralling.setAttribute("PRODUCTION_FORM_NO", formNo);
        SetData();
        lblType.setText("");

        if (EditMode == EITLERPGLOBAL.ADD) {
            if (ObjFeltSpiralling.Insert()) {
                DisplayData();
            } else {
                JOptionPane.showMessageDialog(this, "Error occured while saving. Error is " + ObjFeltSpiralling.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }

        if (EditMode == EITLERPGLOBAL.EDIT) {
            if (ObjFeltSpiralling.Update()) {
                DisplayData();
            } else {
                JOptionPane.showMessageDialog(this, "Error occured while saving editing. Error is " + ObjFeltSpiralling.LastError, "ERROR", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
    ...
    private void SetData() {
        ...
        ObjFeltSpiralling.setAttribute("HIERARCHY_ID", EITLERPGLOBAL.getComboCode(cmbHierarchy));
        ObjFeltSpiralling.setAttribute("FROM", EITLERPGLOBAL.gNewUserID);
        ObjFeltSpiralling.setAttribute("TO", EITLERPGLOBAL.getComboCode(cmbSendTo));
        ObjFeltSpiralling.setAttribute("FROM_REMARKS", txtToRemarks.getText().trim());
        ObjFeltSpiralling.setAttribute("UPDATED_BY", EITLERPGLOBAL.getComboCode(cmbUserName));

        if (OpgApprove.isSelected()) {
            ObjFeltSpiralling.setAttribute("APPROVAL_STATUS", "A");
        }

        if (OpgFinal.isSelected()) {
            ObjFeltSpiralling.setAttribute("APPROVAL_STATUS", "F");
        }

        if (OpgReject.isSelected()) {
            ObjFeltSpiralling.setAttribute("APPROVAL_STATUS", "R");
            ObjFeltSpiralling.setAttribute("SEND_DOC_TO", EITLERPGLOBAL.getComboCode(cmbSendTo));
        }

        if (OpgHold.isSelected()) {
            ObjFeltSpiralling.setAttribute("APPROVAL_STATUS", "H");
        }
        //-------------------------------------------------//

        if (EditMode == EITLERPGLOBAL.ADD) {
            ObjFeltSpiralling.setAttribute("CREATED_BY", EITLERPGLOBAL.gNewUserID);
        } else {
            ObjFeltSpiralling.setAttribute("CREATED_BY", (int) ObjFeltSpiralling.getAttribute("CREATED_BY").getVal());
            ObjFeltSpiralling.setAttribute("CREATED_DATE", ObjFeltSpiralling.getAttribute("CREATED_DATE").getString());
            ObjFeltSpiralling.setAttribute("MODIFIED_BY", EITLERPGLOBAL.gNewUserID);
        }
    }
    ...
    private void Find() {
        Loader ObjLoader = new Loader(this, "EITLERP.FeltSales.SDF.FeltSpiral.frmFindFeltSpiralling", true);
        frmFindFeltSpiralling ObjFindFeltSpiralling = (frmFindFeltSpiralling) ObjLoader.getObj();

        if (ObjFindFeltSpiralling.Cancelled == false) {
            if (!ObjFeltSpiralling.Filter(ObjFindFeltSpiralling.stringFindQuery)) {
                JOptionPane.showMessageDialog(frmFeltSpiralling.this, " No records found.", "Find Felt Spiralling Details", JOptionPane.YES_OPTION);
            }
            MoveLast();
        }
    }
    ...
    public void Find(String docNo) {
        ObjFeltSpiralling.Filter("AND PROD_DOC_NO='" + docNo + "'");
        SetMenuForRights();
        DisplayData();
    }
    ...
    public void Find(String pieceNo, String prodDate) {
        ObjFeltSpiralling.Filter("AND PROD_PIECE_NO='" + pieceNo + "'");
        SetMenuForRights();
        DisplayData();
    }
    ...
    public void FindWaiting() {
        ObjFeltSpiralling.Filter(" AND PROD_DOC_NO IN (SELECT DISTINCT PROD_DOC_NO FROM PRODUCTION.FELT_PROD_DATA, PRODUCTION.FELT_PROD_DOC_DATA WHERE PROD_DOC_NO=DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=850 AND CANCELED=0) ");
        SetMenuForRights();
        DisplayData();
    }

    private void MoveFirst() {
        ObjFeltSpiralling.MoveFirst();
        DisplayData();
    }

    private void MovePrevious() {
        ObjFeltSpiralling.MovePrevious();
        DisplayData();
    }

    private void MoveNext() {
        ObjFeltSpiralling.MoveNext();
        DisplayData();
    }

    private void MoveLast() {
        ObjFeltSpiralling.MoveLast();
        DisplayData();
    }
