...
public class FrmNRGP_SDML extends javax.swing.JApplet {

    private int EditMode = 0;
    private EITLTableModel DataModelL;
    private EITLTableCellRenderer Renderer = new EITLTableCellRenderer();

    private clsNRGP_SDML ObjNRGP;
    ...
    private void initComponents() {
        ...
        lblTitle.setText("NON RETURNABLE GATEPASS SDML To GIDC");
    ...
    private void txtGatepassNoFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtGatepassNoFocusLost
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            if (clsNRGP_SDML.IsNRGPExist(EITLERPGLOBAL.gCompanyID, txtGatepassNo.getText())) {
    ...
    private void NRGPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_NRGPActionPerformed
        ...
        if (ObjNRGP.getAttribute("APPROVED").getInt() == 1) {
            try {
                ...
                parameterMap.put("FROM", "Shri Dinesh Mill Ltd.");
                parameterMap.put("TO", "SDF Unit GIDC,Makarpura");
                parameterMap.put("MS", "SDF Unit GIDC,Makarpura");
        ...
        if (ObjNRGP.getAttribute("APPROVED").getInt() == 1) {
            try {
                ...
                String strSQL = "SELECT * FROM PRODUCTION.GIDC_FELT_NRGP G "
                        ...
                        + "WHERE G.GATEPASS_NO='" + txtGatepassNo.getText() + "'";
                System.out.println("Report :" + strSQL);
                rpt.setReportName("/EITLERP/FeltSales/GIDC_SDF/Reports/Delivery_Challan_SDML.jrxml", 1, strSQL); //productlist is the name of my jasper file.
                rpt.callReport();
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void DisplayData() {
        ...
        try {
            ...
            lblTitle.setText("NON RETURNABLE GATEPASS SDML To GIDC - " + txtGatepassNo.getText());
            ...
            for (int i = 1; i <= ObjNRGP.colLineItems.size(); i++) {
                ...
                clsNRGPItem_SDML ObjItem = (clsNRGPItem_SDML) ObjNRGP.colLineItems.get(Integer.toString(i));
                ...
                DataModelL.setValueByVariable("GATEPASS_ISSUE_NO", Double.toString(ObjItem.getAttribute("GATEPASS_ISSUE_NO").getVal()), NewRow);
                DataModelL.setValueByVariable("GATEPASS_ISSUE_SR_NO", Double.toString(ObjItem.getAttribute("GATEPASS_ISSUE_SR_NO").getVal()), NewRow);
            ...
            List = clsFeltProductionApprovalFlow.getDocumentFlow(794, DocNo);
            ...
            FormatGridHS();
            HashMap History = clsNRGP_SDML.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsNRGP_SDML ObjHistory = (clsNRGP_SDML) History.get(Integer.toString(i));
    ...
    private void SetData() {
        ...
        for (int i = 0; i < TableL.getRowCount(); i++) {
            ...
            ObjNRGPItem.setAttribute("GATEPASS_ISSUE_NO", DataModelL.getValueByVariable("GATEPASS_ISSUE_NO", i));
            ObjNRGPItem.setAttribute("GATEPASS_ISSUE_SR_NO", DataModelL.getValueByVariable("GATEPASS_ISSUE_SR_NO", i));
    ...
    private void GenerateCombos() {
        ...
        List = clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=794");

        if (EditMode == EITLERPGLOBAL.EDIT) {
            List = clsHierarchy.getList(" WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=794");
        }
        for (int i = 1; i <= List.size(); i++) {
    ...
    private void GenerateFromCombo() {
        ...
        try {
            if (EditMode == EITLERPGLOBAL.ADD) {
                ...
                List = clsFeltProductionApprovalFlow.getRemainingUsers(794, (String) ObjNRGP.getAttribute("GATEPASS_NO").getObj());
                for (int i = 1; i <= List.size(); i++) {
                    clsUser ObjUser = (clsUser) List.get(Integer.toString(i));
                    ComboData aData = new ComboData();
                    aData.Code = (int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void SetupApproval() {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = clsFeltProductionApprovalFlow.getFromID(794, (String) ObjNRGP.getAttribute("GATEPASS_NO").getObj());
            lnFromID = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = clsFeltProductionApprovalFlow.getFromRemarks(794, FromUserID, (String) ObjNRGP.getAttribute("GATEPASS_NO").getObj());
    ...
    private void FormatGrid() {
        try {
            ...
            DataModelL.addColumn("NRGP Desc"); //2
            DataModelL.addColumn("NRGP Qty"); //3
            DataModelL.addColumn("Issue No"); //4
            DataModelL.addColumn("Issue SrNo"); //5
            DataModelL.addColumn("Rate"); //6
            DataModelL.addColumn("Net Amount"); //7
            DataModelL.addColumn("Delivery Challan No"); //8
            DataModelL.addColumn("Delivery Challan Date"); //9

            DataModelL.SetVariable(0, "SR_NO");
            DataModelL.SetVariable(1, "ITEM_CODE");
            DataModelL.SetVariable(2, "NRGP_DESC");
            DataModelL.SetVariable(3, "QTY");
            DataModelL.SetVariable(4, "GATEPASS_ISSUE_NO");
            DataModelL.SetVariable(5, "GATEPASS_ISSUE_SR_NO");
            DataModelL.SetVariable(6, "RATE");
            DataModelL.SetVariable(7, "NET_AMOUNT");
            DataModelL.SetVariable(8, "DELIVERY_CHALLAN_NO");
            DataModelL.SetVariable(9, "DELIVERY_CHALLAN_DATE");
    ...
    private void SetMenuForRights() {
        // --- Add Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8928, 89281)) {
            cmdNew.setEnabled(true);
        } else {
            cmdNew.setEnabled(false);
        }
        ...
        // --- Edit Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8928, 89282)) {
            cmdEdit.setEnabled(true);
        } else {
            cmdEdit.setEnabled(false);
        }
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8928, 89283)) {
            cmdDelete.setEnabled(true);
        } else {
            cmdDelete.setEnabled(false);
        }
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8928, 89285)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        } else {
            cmdPreview.setEnabled(false);
    ...
    private void Add() {
        ...
        if (aList.ShowList()) {
            ...
            txtGatepassNo.setText(clsFirstFree.getNextFreeNo(EITLERPGLOBAL.gCompanyID, 794, SelPrefix, SelSuffix, false));
    ...
    private void Edit() {
        ...
        if (ObjNRGP.IsEditable(EITLERPGLOBAL.gCompanyID, lDocNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(794, lDocNo) || clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8928, 89282)) {
                SetFields(true);
            } else {
                EnableApproval();
            }
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            if (ObjNRGP.Insert(SelPrefix, SelNo)) {
                if (OpgFinal.isSelected()) {
                    ...
                    pSubject = "Non-Returnable Gatepass Final Approved";
                    pBody = "Non-Returnable Gatepass : " + txtGatepassNo.getText() + " has been Final Approved...";
                    pBody = pBody + "<br><br><br>**** This is an auto-generated email, please do not reply ****<br><br>";
                    recievers = "yrpatel@dineshmills.com,amitkanti@dineshmills.com,feltpp@dineshmills.com";
                    pcc = "brdfltfin@dineshmills.com,,sdmlerp@dineshmills.com";
                    String responce = MailNotification.sendNotificationMail(794, pSubject, pBody, recievers, pcc, EITLERPGLOBAL.getComboCode(cmbHierarchy));
    ...
    private void Find() {
        Loader ObjLoader = new Loader(this, "EITLERP.FeltSales.GIDC_SDF.NRGP.frmNRGPFind_SDML", true);
        frmNRGPFind_SDML ObjReturn = (frmNRGPFind_SDML) ObjLoader.getObj();
...
//        }
    ...
    public void FindEx(int pCompanyID, String pDocNo) {
        ObjNRGP.Filter(" WHERE COMPANY_ID=" + Integer.toString(pCompanyID) + " AND GATEPASS_TYPE='BS' AND GATEPASS_NO='" + pDocNo + "'", pCompanyID);
        ObjNRGP.MoveLast();
        DisplayData();
    }

    public void FindWaiting() {
        ObjNRGP.Filter(" WHERE GATEPASS_TYPE='BS' AND GATEPASS_NO IN(SELECT PRODUCTION.GIDC_FELT_NRGP.GATEPASS_NO FROM PRODUCTION.GIDC_FELT_NRGP,PRODUCTION.FELT_PROD_DOC_DATA WHERE PRODUCTION.GIDC_FELT_NRGP.GATEPASS_NO=PRODUCTION.FELT_PROD_DOC_DATA.DOC_NO AND PRODUCTION.FELT_PROD_DOC_DATA.USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND PRODUCTION.FELT_PROD_DOC_DATA.STATUS='W' AND PRODUCTION.FELT_PROD_DOC_DATA.MODULE_ID=794)", EITLERPGLOBAL.gCompanyID);
        ObjNRGP.MoveLast();
        DisplayData();
    }
