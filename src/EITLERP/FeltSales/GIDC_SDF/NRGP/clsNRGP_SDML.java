...
public class clsNRGP_SDML {
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            rsNRGP = Stmt.executeQuery("SELECT * FROM PRODUCTION.GIDC_FELT_NRGP WHERE GATEPASS_TYPE='BS' GROUP BY GATEPASS_NO ORDER BY GATEPASS_NO");//ORDER BY GATEPASS_NO");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
...
//            rsHeader=stHeader.executeQuery("SELECT * FROM PRODUCTION.GIDC_FELT_NRGP WHERE GATEPASS_NO='1'");
            ...
            if (UserDocNo) {
                ...
                setAttribute("GATEPASS_NO", clsFirstFree.getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 794, 284, true));
            }
            //-------------------------------------------------

            for (int i = 1; i <= colLineItems.size(); i++) {
                clsNRGPItem_SDML ObjNRGPItem = (clsNRGPItem_SDML) colLineItems.get(Integer.toString(i));
                ...
                resultSetTemp.updateString("ITEM_CODE", (String) ObjNRGPItem.getAttribute("ITEM_CODE").getObj());
                ...
                resultSetTemp.updateString("GATEPASS_ISSUE_NO", ObjNRGPItem.getAttribute("GATEPASS_ISSUE_NO").getString());
                resultSetTemp.updateString("GATEPASS_ISSUE_SR_NO", ObjNRGPItem.getAttribute("GATEPASS_ISSUE_SR_NO").getString());
                ...
                resultSetTemp.updateString("HSN", "5404");
                ...
                rsHistory.updateString("ITEM_CODE", (String) ObjNRGPItem.getAttribute("ITEM_CODE").getObj());
                ...
                rsHistory.updateString("GATEPASS_ISSUE_NO", (String) ObjNRGPItem.getAttribute("GATEPASS_ISSUE_NO").getObj());
                rsHistory.updateString("GATEPASS_ISSUE_SR_NO", (String) ObjNRGPItem.getAttribute("GATEPASS_ISSUE_SR_NO").getObj());
                rsHistory.updateString("UNIT", "KGS");
                rsHistory.updateString("HSN", "5404");
            ...
            ObjFlow.ModuleID = 794; //QUOTATION MODULE ID
...
//            System.out.println("ConCur : " +rsNRGP.getConcurrency());
            ...
            for (int i = 1; i <= colLineItems.size(); i++) {
                ...
                rsNRGP.updateString("ITEM_CODE", (String) ObjNRGPItem.getAttribute("ITEM_CODE").getObj());
                ...
                rsNRGP.updateString("GATEPASS_ISSUE_NO", ObjNRGPItem.getAttribute("GATEPASS_ISSUE_NO").getString());
                rsNRGP.updateString("GATEPASS_ISSUE_SR_NO", ObjNRGPItem.getAttribute("GATEPASS_ISSUE_SR_NO").getString());
                ...
                rsHistory.updateString("NRGP_DESC", (String) ObjNRGPItem.getAttribute("NRGP_DESC").getObj());
                rsHistory.updateDouble("QTY", ObjNRGPItem.getAttribute("QTY").getVal());
                rsHistory.updateString("GATEPASS_ISSUE_NO", ObjNRGPItem.getAttribute("GATEPASS_ISSUE_NO").getString());
                rsHistory.updateString("GATEPASS_ISSUE_SR_NO", ObjNRGPItem.getAttribute("GATEPASS_ISSUE_SR_NO").getString());
                rsHistory.updateDouble("RATE", ObjNRGPItem.getAttribute("RATE").getVal());
                rsHistory.updateDouble("NET_AMOUNT", ObjNRGPItem.getAttribute("NET_AMOUNT").getVal());
                rsHistory.updateString("NET_AMOUNT_IN_WORDS", rsInWord);
            ...
            if (ObjFlow.Status.equals("F") && ObjFlow.finalApproved) {
                ...
                try {
                    ...
                    while (rsTemp.next()) {
                        String challanNo = EITLERP.clsFirstFree.getNextFreeNo(2, 794, 285, true);
                        data.Execute("UPDATE PRODUCTION.GIDC_FELT_NRGP SET DELIVERY_CHALLAN_NO='" + challanNo + "',DELIVERY_CHALLAN_DATE=CURDATE() WHERE GATEPASS_NO='" + ObjFlow.DocNo + "' AND GATEPASS_ISSUE_NO = '" + rsTemp.getString("GATEPASS_ISSUE_NO") + "' AND GATEPASS_ISSUE_SR_NO = '" + rsTemp.getString("GATEPASS_ISSUE_SR_NO") + "'");
                    }
                    rsTemp.close();
                } catch (Exception e) {
                    LastError = e.getMessage();
    ...
    public boolean CanDelete(int pCompanyID, String pGatepassno, int userID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.GIDC_FELT_NRGP WHERE COMPANY_ID=" + Integer.toString(pCompanyID) + " AND GATEPASS_TYPE='BS' AND GATEPASS_NO='" + pGatepassno + "' AND APPROVED=1";
            ...
            if (rsTmp.getInt("COUNT") > 0) {  //Group is Approved
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=794 AND USER_ID=" + userID + " AND DOC_NO='" + pGatepassno + "' AND STATUS='W'";
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();

                if (rsTmp.getInt("COUNT") > 0) {
                    strSQL = "DELETE FROM PRODUCTION.GIDC_FELT_NRGP WHERE GATEPASS_NO='" + pGatepassno + "' AND GATEPASS_TYPE='BS' ";
    ...
    public boolean IsEditable(int pCompanyID, String pGatepassno, int pUserID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=794 AND USER_ID=" + Integer.toString(pUserID) + " AND DOC_NO='" + pGatepassno + "' AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean Delete(int pUserID) {
        try {
            ...
            if (CanDelete(lCompanyID, lGatepassno, pUserID)) {
                String strQry = "DELETE FROM PRODUCTION.GIDC_FELT_NRGP WHERE COMPANY_ID=" + Long.toString(EITLERPGLOBAL.gCompanyID) + " AND GATEPASS_TYPE='BS' AND GATEPASS_NO='" + lGatepassno.trim() + "'";
                data.Execute(strQry);
            }
    ...
    public Object getObject(int pCompanyID, String pDocNo) {
        String strCondition = " WHERE COMPANY_ID=" + Integer.toString(pCompanyID) + " AND GATEPASS_TYPE='BS' AND GATEPASS_NO='" + pDocNo + "'";
        clsNRGP_SDML ObjNRGP = new clsNRGP_SDML();
...
//                MoveLast();
    ...
    private boolean setData() {
        ...
        try {
            ...
            while (!rsTmp.isAfterLast()) {
                Counter = Counter + 1;
                clsNRGPItem_SDML ObjNRGPItem = new clsNRGPItem_SDML();
                ...
                ObjNRGPItem.setAttribute("GATEPASS_ISSUE_NO", rsTmp.getString("GATEPASS_ISSUE_NO"));
                ObjNRGPItem.setAttribute("GATEPASS_ISSUE_SR_NO", rsTmp.getString("GATEPASS_ISSUE_SR_NO"));
    ...
    public static HashMap getPendingApprovals(int pCompanyID, int pUserID, int pOrder) {
        ...
        try {
            ...
            while (!rsTmp.isAfterLast()) {
                Counter = Counter + 1;
                clsNRGP_SDML ObjDoc = new clsNRGP_SDML();

                //------------- Header Fields --------------------//
                ObjDoc.setAttribute("ITEM_CODE", rsTmp.getString("ITEM_CODE"));
    ...
    public boolean ShowHistory(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsNRGP = Stmt.executeQuery("SELECT * FROM PRODUCTION.GIDC_FELT_NRGP_H WHERE COMPANY_ID=" + pCompanyID + " AND GATEPASS_NO='" + pDocNo + "' AND GATEPASS_TYPE='BS' ");
            Ready = true;
            HistoryView = true;
            MoveFirst();
            return true;
    ...
    public static HashMap getHistoryList(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT * FROM PRODUCTION.GIDC_FELT_NRGP_H WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND GATEPASS_NO='" + pDocNo + "' AND GATEPASS_TYPE='BS' ");
    ...
    public static String getDocStatus(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsTmp = data.getResult("SELECT GATEPASS_NO,APPROVED,CANCELED FROM PRODUCTION.GIDC_FELT_NRGP WHERE COMPANY_ID=" + pCompanyID + " AND GATEPASS_NO='" + pDocNo + "' AND GATEPASS_TYPE='BS' ");
            rsTmp.first();
    ...
    public static boolean IsNRGPExist(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT GATEPASS_NO FROM PRODUCTION.GIDC_FELT_NRGP WHERE COMPANY_ID=" + pCompanyID + " AND GATEPASS_NO='" + pDocNo + "' AND GATEPASS_TYPE='BS' ");
            rsTmp.first();
    ...
    public static boolean CanCancel(int pCompanyID, String pDocNo) {
        ...
        try {
            rsTmp = data.getResult("SELECT GATEPASS_NO FROM PRODUCTION.GIDC_FELT_NRGP WHERE COMPANY_ID=" + pCompanyID + " AND GATEPASS_TYPE='BS' AND GATEPASS_NO='" + pDocNo + "' AND APPROVED=0 AND CANCELED=0");
            rsTmp.first();
    ...
    public static boolean CancelNRGP(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            if (CanCancel(pCompanyID, pDocNo)) {
                ...
                rsTmp = data.getResult("SELECT APPROVED FROM PRODUCTION.GIDC_FELT_NRGP WHERE COMPANY_ID=" + pCompanyID + " AND GATEPASS_NO='" + pDocNo + "' AND GATEPASS_TYPE='BS' ");
                rsTmp.first();
                ...
                if (ApprovedDoc) {
                    ...
                    //data.Execute("DELETE FROM D_COM_DOC_DATA WHERE COMPANY_ID=" + pCompanyID + " AND DOC_NO='" + pDocNo + "' AND MODULE_ID=794");
                    data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE DOC_NO='" + pDocNo + "' AND MODULE_ID=794");
                }

                //Now Update the header with cancel falg to true
                data.Execute("UPDATE PRODUCTION.GIDC_FELT_NRGP SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID=" + pCompanyID + " AND GATEPASS_NO='" + pDocNo + "' AND GATEPASS_TYPE='BS' ");
