...
public class frmGIDCInstructionAmend extends javax.swing.JApplet {

    private int EditMode = 0;

    //private clsGIDCInstructionAmend Obj;
    private clsGIDCInstructionAmend Obj;
...
//    private static PieChart pieChart;
     ...
     */
    public frmGIDCInstructionAmend() {
        //this.requestFocus();
...
//        }
        ...
        Obj = new clsGIDCInstructionAmend();
    ...
    private void GenerateCombos() {
        ...
        List = clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + clsGIDCInstructionAmend.ModuleID + "");

        if (EditMode == EITLERPGLOBAL.EDIT) {
            List = clsHierarchy.getList(" WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + clsGIDCInstructionAmend.ModuleID + "");
        }
    ...
    private void initComponents() {
        ...
        lblTitle.setText("Felt SDF Manufracturing Instruction Amendment");
        ...
        jTabbedPane1.addTab("Amendment Instruction Entry [SDF]", Tab1);
        ...
        jTabbedPane1.setBounds(0, 190, 1020, 380);
        ...
        jLabel20.setText("Amend Date");
        getContentPane().add(jLabel20);
        jLabel20.setBounds(240, 80, 90, 20);
        ...
        jLabel2.setBounds(460, 80, 100, 20);

        DOC_NO1.setEditable(false);
        DOC_NO1.setFont(new java.awt.Font("Tahoma", 1, 14)); // NOI18N
        DOC_NO1.setForeground(new java.awt.Color(0, 0, 255));
        DOC_NO1.setText("D000001");
        DOC_NO1.setDisabledTextColor(new java.awt.Color(0, 0, 255));
        getContentPane().add(DOC_NO1);
        DOC_NO1.setBounds(560, 70, 130, 30);

        jLabel21.setText("Date");
        getContentPane().add(jLabel21);
        jLabel21.setBounds(720, 80, 70, 20);

        txtdate1.setEditable(false);
        txtdate1.setFont(new java.awt.Font("Tahoma", 1, 14)); // NOI18N
        getContentPane().add(txtdate1);
        txtdate1.setBounds(780, 70, 120, 30);

        jLabel4.setText("Reason For Amend");
        getContentPane().add(jLabel4);
        jLabel4.setBounds(10, 120, 130, 14);
        ...
        jScrollPane5.setBounds(130, 110, 380, 70);
                ...
                if (TableDesc.getSelectedColumn() == 2) {
                    try {
                        ...
                        sql = "SELECT * FROM PRODUCTION.FELT_SALES_PIECE_REGISTER "
                                ...
                                + "AND (PR_PIECE_NO NOT IN (SELECT LEFT(PIECE_NO,5) FROM PRODUCTION.GIDC_INSTRUCTION_DETAIL_AMEND)) "
                                + "UNION ALL "
                                + "SELECT * FROM PRODUCTION.FELT_SALES_PIECE_REGISTER "
                                + "WHERE LEFT(PR_PIECE_NO,5) IN "
                                + "(SELECT LEFT(PIECE_NO,5) FROM PRODUCTION.GIDC_INSTRUCTION_DETAIL WHERE DOC_NO='" + DOC_NO1.getText() + "' AND LEFT(PIECE_NO,5)='" + PieceNo.substring(0, 5) + "')";
                    ...
                    if (trs.getRow() > 0) {
                        TableDesc.setValueAt(trs.getString("PR_PARTY_CODE"), TableDesc.getSelectedRow(), 9);
                        TableDesc.setValueAt(clsSales_Party.getPartyName(EITLERPGLOBAL.gCompanyID, trs.getString("PR_PARTY_CODE")), TableDesc.getSelectedRow(), 10);
                        TableDesc.setValueAt(trs.getString("PR_PRODUCT_CODE"), TableDesc.getSelectedRow(), 11);
                        TableDesc.setValueAt(trs.getString("PR_GROUP"), TableDesc.getSelectedRow(), 12);
                        TableDesc.setValueAt(trs.getString("PR_STYLE"), TableDesc.getSelectedRow(), 13);
                        TableDesc.setValueAt(df.format(trs.getDouble("PR_LENGTH")), TableDesc.getSelectedRow(), 14);
                        TableDesc.setValueAt(df.format(trs.getDouble("PR_WIDTH")), TableDesc.getSelectedRow(), 15);
                        TableDesc.setValueAt(trs.getString("PR_GSM"), TableDesc.getSelectedRow(), 16);
                        if (PieceNo.contains("B") || PieceNo.contains("b")) {
                            TableDesc.setValueAt("0.00", TableDesc.getSelectedRow(), 17);
                        } else {
                            TableDesc.setValueAt(df.format(trs.getDouble("PR_THORITICAL_WEIGHT")), TableDesc.getSelectedRow(), 17);
                        }
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            ...
            if (clsFeltProductionApprovalFlow.IsOnceRejectedDoc(clsGIDCInstructionAmend.ModuleID, DocNo)) {
                cmbSendTo.setEnabled(true);
            } else {
                cmbSendTo.setEnabled(false);
            }
    ...
    private void bexcelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bexcelActionPerformed
        ...
        try {
            ...
            exprt.fillData(TableDesc, new File(file1.getSelectedFile().toString() + ".xls"), "GIDC_INSTRUCTION_AMEND");
            ...
            if (clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID) && (EditMode == EITLERPGLOBAL.ADD || EditMode == EITLERPGLOBAL.EDIT)) {
                if (evt.getKeyCode() == 112) {
                        ...
                        if (aList.ShowLOV()) {
                            if (data.getStringValueFromDB("SELECT COALESCE(PR_OC_MONTHYEAR,'') FROM PRODUCTION.FELT_SALES_PIECE_REGISTER WHERE PR_PIECE_NO='" + aList.ReturnVal + "'").equalsIgnoreCase("")) {
                                ...
                                String sql = "SELECT * FROM PRODUCTION.FELT_SALES_PIECE_REGISTER "
                                        ...
                                        + "SELECT * FROM PRODUCTION.FELT_SALES_PIECE_REGISTER "
                                        + "WHERE LEFT(PR_PIECE_NO,5) IN "
                                        + "(SELECT LEFT(PIECE_NO,5) FROM PRODUCTION.GIDC_INSTRUCTION_DETAIL WHERE DOC_NO='" + DOC_NO1.getText() + "' AND LEFT(PIECE_NO,5)='" + aList.ReturnVal.substring(0, 5) + "')";
                    ...
                    if (TableDesc.getSelectedColumn() == 2 || TableDesc.getSelectedColumn() == 5) {
                        ...
                        LOV aList1 = new LOV();
                        aList1.SQL = "SELECT ITEM_ID,ITEM_DESCRIPTION FROM D_INV_ITEM_MASTER WHERE ITEM_ID LIKE 'RM1%' AND ITEM_ID IN (SELECT ITEM_CODE FROM PRODUCTION.GIDC_FELT_RM_STOCK WHERE AVAILABLE>0) ";
                        aList1.ReturnCol = 1;
    ...
    private void Add() {
        ...
        aList.SQL = "SELECT DISTINCT DOC_NO,DATE_FORMAT(DOC_DATE,'%d/%m/%Y') AS DOC_DATE FROM PRODUCTION.GIDC_INSTRUCTION_DETAIL WHERE APPROVED=1 AND CANCELED=0 ";
        aList.ReturnCol = 1;
        aList.ShowReturnCol = true;
        aList.DefaultSearchOn = 1;

        if (aList.ShowLOV()) {
            String DocNo = aList.ReturnVal;

            if (clsGIDCInstructionAmend.IsAmendmentUnderProcess(DocNo)) {
                JOptionPane.showMessageDialog(null, "One amendment of this Instruction is already under process. Cannot make another amendment");
                EditMode = 0;
                return;
            }
            ...
            int MaxNo = data.getIntValueFromDB("SELECT COALESCE(MAX(RIGHT(AMEND_DOC_NO,1)),0) FROM PRODUCTION.GIDC_INSTRUCTION_DETAIL_AMEND WHERE DOC_NO='" + DocNo + "'") + 1;
            DOC_NO.setText(DocNo + "/" + MaxNo);
            txtdate.setText(EITLERPGLOBAL.getCurrentDate());
            DisplayDataDoc(DocNo, MaxNo - 1);
            txtreason.requestFocus();

        } else {
            JOptionPane.showMessageDialog(null, "You must select Doc No.");
            EditMode = 0;
            Cancel();
        }
    ...
    private void SetupApproval() {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = clsFeltProductionApprovalFlow.getFromID(clsGIDCInstructionAmend.ModuleID, (String) Obj.getAttribute("PROFORMA_NO").getObj());
            lnFromID = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = clsFeltProductionApprovalFlow.getFromRemarks(clsGIDCInstructionAmend.ModuleID, FromUserID, (String) Obj.getAttribute("PROFORMA_NO").getObj());
    ...
    private void GenerateFromCombo() {
        ...
        try {
            if (EditMode == EITLERPGLOBAL.ADD) {
                ...
                String DocNo = (String) Obj.getAttribute("AMEND_DOC_NO").getObj();

                List = clsFeltProductionApprovalFlow.getRemainingUsers(clsGIDCInstructionAmend.ModuleID, DocNo);
                for (int i = 1; i <= List.size(); i++) {
                    clsUser ObjUser = (clsUser) List.get(Integer.toString(i));
                    ComboData aData = new ComboData();
                    aData.Code = (int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void Edit() {
        ...
        //----------------------------------//
        String lDocNo = Obj.getAttribute("AMEND_DOC_NO").getString();
        if (Obj.IsEditable(EITLERPGLOBAL.gCompanyID, lDocNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(clsGIDCInstructionAmend.ModuleID, lDocNo)) {  // || clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8912, 89122)) {
                SetFields(true);
            } else {
                EnableApproval();
            }
    ...
    private void Delete() {
        ...
        String lDocNo = (String) Obj.getAttribute("AMEND_DOC_NO").getObj();
...
//        }
        ...
        Obj.Filter(" AMEND_DOC_NO='" + DOC_NO.getText() + "'");
        DisplayData();
        //Mail();
        //Obj.LoadData(EITLERPGLOBAL.gCompanyID);
        EditMode = 0;
...
//        }
    ...
    private void DisplayData() {
        ...
        if (EITLERPGLOBAL.gAuthorityUserID != EITLERPGLOBAL.gUserID) {
            int ModuleID = clsGIDCInstructionAmend.ModuleID;
        ...
        try {
            ...
            DOC_NO.setText(Obj.getAttribute("AMEND_DOC_NO").getObj().toString());
            txtdate.setText(EITLERPGLOBAL.formatDate(Obj.getAttribute("AMEND_DOC_DATE").getObj().toString()));
            DOC_NO1.setText(Obj.getAttribute("DOC_NO").getObj().toString());
            txtdate1.setText(EITLERPGLOBAL.formatDate(Obj.getAttribute("DOC_DATE").getObj().toString()));
            txtreason.setText(Obj.getAttribute("AMEND_REASON").getObj().toString());
            ...
            for (int i = 1; i <= Obj.colMRItems.size(); i++) {
                ...
                clsGIDCInstructionAmendItem ObjItem = (clsGIDCInstructionAmendItem) Obj.colMRItems.get(Integer.toString(i));
...
//                rowData[8] = Integer.toString(ObjItem.getAttribute("SEQUANCE_NO").getInt());
                ...
                rowData[4] = data.getStringValueFromDB("SELECT PR_POSITION_NO FROM PRODUCTION.FELT_SALES_PIECE_REGISTER WHERE SUBSTRING(PR_PIECE_NO,1,5) = '"+(String) ObjItem.getAttribute("PIECE_NO").getObj()+"' ");
            ...
            String DocNo = Obj.getAttribute("AMEND_DOC_NO").getString();
            List = clsFeltProductionApprovalFlow.getDocumentFlow(clsGIDCInstructionAmend.ModuleID, DocNo);
            for (int i = 1; i <= List.size(); i++) {
            ...
            FormatGridHS();
            HashMap History = clsGIDCInstructionAmend.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsGIDCInstructionAmend ObjHistory = (clsGIDCInstructionAmend) History.get(Integer.toString(i));
    ...
    private void SetData() {
        ...
        Obj.setAttribute("DOC_NO", DOC_NO1.getText());
        Obj.setAttribute("DOC_DATE", EITLERPGLOBAL.formatDateDB(txtdate1.getText()));

        Obj.setAttribute("AMEND_DOC_NO", DOC_NO.getText());
        Obj.setAttribute("AMEND_DOC_DATE", EITLERPGLOBAL.formatDateDB(txtdate.getText()));
        Obj.setAttribute("AMEND_REASON", txtreason.getText());
        ...
        for (int i = 0; i < TableDesc.getRowCount(); i++) {
            clsGIDCInstructionAmendItem ObjItem = new clsGIDCInstructionAmendItem();
            String lItemID = (String) TableDesc.getValueAt(i, 1);

            //Add Only Valid Items
            ObjItem.setAttribute("AMEND_DOC_NO", DOC_NO.getText());
            ObjItem.setAttribute("AMEND_DOC_DATE", EITLERPGLOBAL.formatDateDB(txtdate.getText()));
            ObjItem.setAttribute("DOC_NO", DOC_NO1.getText());
            ObjItem.setAttribute("DOC_DATE", EITLERPGLOBAL.formatDateDB(txtdate1.getText()));
...
//            ObjItem.setAttribute("SEQUANCE_NO", Integer.parseInt(TableDesc.getValueAt(i, 8).toString()));
    ...
    private void SetMenuForRights() {
        // --- Add Rights --        
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8912, 89121)) {
            cmdNew.setEnabled(true);
        } else {
            cmdNew.setEnabled(false);
        }
        ...
        // --- Edit Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8912, 89122)) {
            cmdEdit.setEnabled(true);
        } else {
            cmdEdit.setEnabled(false);
        }
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8912, 89123)) {
            cmdDelete.setEnabled(true);
        } else {
            cmdDelete.setEnabled(false);
        }
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 8912, 89125)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        } else {
            cmdPreview.setEnabled(false);
    ...
    public void FindWaiting() {
        Obj.Filter(" AMEND_DOC_NO IN (SELECT DISTINCT H.AMEND_DOC_NO FROM PRODUCTION.GIDC_INSTRUCTION_DETAIL_AMEND H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=791 AND CANCELED=0)");
        Obj.MoveFirst();
        DisplayData();
    }

    public void FindEx(int pCompanyID, String pDocNo) {
        Obj.Filter(" AMEND_DOC_NO='" + pDocNo + "'");
        Obj.MoveFirst();
        DisplayData();
    }
    ...
    private void GenerateRejectedUserCombo() {
        ...
        for (int i = 1; i <= List.size(); i++) {
            ...
            if (EditMode == EITLERPGLOBAL.EDIT) {
                if (OpgApprove.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInApproval(clsGIDCInstructionAmend.ModuleID, DocNo, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }

                if (OpgReject.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInRejection(clsGIDCInstructionAmend.ModuleID, DocNo, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            ...
            int Creator = clsFeltProductionApprovalFlow.getCreator(clsGIDCInstructionAmend.ModuleID, DocNo);
...
//            DataModelDesc.addColumn("Sequence No.");   //8   
            DataModelDesc.addColumn("Party Code");//9
            DataModelDesc.addColumn("Name");//10
            DataModelDesc.addColumn("Position");//10
            DataModelDesc.addColumn("Product");//11
            DataModelDesc.addColumn("Group"); //12
            ...
            DataModelDesc.addColumn("Wt Kg");   //17
            DataModelDesc.addColumn("Grey Length");   //18
            DataModelDesc.addColumn("Grey Width");   //19
...
//            rpt.callReport();
    ...
    private void Mail() {
        ...
        if (Obj.getAttribute("APPROVAL_STATUS").getString().equalsIgnoreCase("F")) {
            ...
            try {
                ...
                HashMap hmApprovalHistory = clsGIDCInstructionAmend.getHistoryList(EITLERPGLOBAL.gCompanyID, DOC_NO.getText());
                for (int i = 1; i <= hmApprovalHistory.size(); i++) {
                    pBody += "<tr>";

                    clsGIDCInstructionAmend ObjWarping = (clsGIDCInstructionAmend) hmApprovalHistory.get(Integer.toString(i));
                    Object[] rowData = new Object[6];
                ...
                String responce = MailNotification.sendNotificationMail(791, pSubject, pBody, recievers, pcc, EITLERPGLOBAL.getComboCode(cmbHierarchy));
    ...
    private void DisplayDataDoc(String mDocno, int mRevisionNo) {
        try {
            ResultSet trs = null;
            FormatGrid();
            //Now Generate Table
            if (mRevisionNo == 0) {
                trs = data.getResult("SELECT * FROM PRODUCTION.GIDC_INSTRUCTION_DETAIL WHERE DOC_NO='" + mDocno + "' ORDER BY SR_NO");
            } else {
                trs = data.getResult("SELECT * FROM PRODUCTION.GIDC_INSTRUCTION_DETAIL_AMEND WHERE DOC_NO='" + mDocno + "' AND RIGHT(AMEND_DOC_NO,1)=" + mRevisionNo + "  ORDER BY SR_NO");
            }
            trs.first();
            double mtottpick = 0;
            DOC_NO1.setText(trs.getString("DOC_NO"));
            txtdate1.setText(EITLERPGLOBAL.formatDate(trs.getString("DOC_DATE")));
            while (!trs.isAfterLast()) {
                Object[] rowData = new Object[50];

                rowData[0] = trs.getInt("SR_NO");
                rowData[1] = trs.getString("PIECE_NO");
//                rowData[2] = trs.getString("RMCODE_LENGTH");
//                rowData[3] = data.getStringValueFromDB("SELECT ITEM_DESCRIPTION FROM D_INV_ITEM_MASTER WHERE ITEM_ID='" + rowData[2].toString() + "'");
//                rowData[4] = df.format(EITLERPGLOBAL.round(trs.getDouble("REQUIRE_QTY_LENGTH"), 2));
//                rowData[5] = trs.getString("RMCODE_WEFT");
//                rowData[6] = data.getStringValueFromDB("SELECT ITEM_DESCRIPTION FROM D_INV_ITEM_MASTER WHERE ITEM_ID='" + rowData[5].toString() + "'");
//                rowData[7] = df.format(EITLERPGLOBAL.round(trs.getDouble("REQUIRE_QTY_WEFT"), 2));
//                rowData[8] = trs.getInt("SEQUANCE_NO");
                rowData[2] = trs.getString("PARTY_CODE");  //9
                rowData[3] = clsSales_Party.getPartyName(EITLERPGLOBAL.gCompanyID, rowData[2].toString());  //10
                rowData[4] = data.getStringValueFromDB("SELECT PR_POSITION_NO FROM PRODUCTION.FELT_SALES_PIECE_REGISTER WHERE SUBSTRING(PR_PIECE_NO,1,5) = '"+trs.getString("PIECE_NO")+"' ");
                rowData[5] = trs.getString("PRODUCT_CODE");  //11
                rowData[6] = trs.getString("PRODUCT_GROUP");  //12
                rowData[7] = trs.getString("STYLE");  //13
                rowData[8] = df.format(EITLERPGLOBAL.round(trs.getDouble("LENGTH"), 2));  //14
                rowData[9] = df.format(EITLERPGLOBAL.round(trs.getDouble("WIDTH"), 2));  //15
                rowData[10] = trs.getInt("GSM");  //16
                rowData[11] = df.format(EITLERPGLOBAL.round(trs.getDouble("WEIGHT"), 2));  //17
                rowData[12] = df.format(EITLERPGLOBAL.round(trs.getDouble("GREY_LENGTH"), 2));  //18
                rowData[13] = df.format(EITLERPGLOBAL.round(trs.getDouble("GREY_WIDTH"), 2));  //19
    ...
    private boolean chkstk() {
        try {
            ...
            HashMap<String, Double> hmavailable = new HashMap<String, Double>();
            String itmcd = "", errmsg = "";
            double stk = 0;
            for (int i = 0; i < TableDesc.getRowCount(); i++) {
                //Length RMCode
                ...
                if (stk > 0) {
                    if (hm.containsKey(itmcd)) {
                        ...
                        r.first();
                        if (r.getDouble("AVAILABLE_QTY") >= stk) {
