...
public class clsFeltGSTAdvancePaymentEntryForm {
    ...
    public clsFeltGSTAdvancePaymentEntryForm() {
        LastError = "";
        props=new HashMap();
...
//            resultSet=statement.executeQuery("SELECT DOC_NO,PROCESSING_DATE,PARTY_CODE,PARTY_NAME,CHARGE_CODE_NEW,CRITICAL_LIMIT_NEW,REMARKS,INSURANCE_CODE,TRANSPORTER_CODE,WITHOUT_CRITICAL_LIMIT,VEHICLE_NO,ADV_DOC_NO,ADV_AGN_INV_AMT,ADV_AGN_IGST_AMT,ADV_AGN_SGST_AMT,ADV_AGN_CGST_AMT,ADV_AGN_GST_COMP_CESS_AMT,DOC_DATE,CREATED_BY,CREATED_DATE,MODIFIED_BY,MODIFIED_DATE,APPROVED,APPROVED_DATE,REJECTED,REJECTED_DATE,CANCELED,HIERARCHY_ID,CHANGED,CHANGED_DATE,F6 FROM PRODUCTION.FELT_INV_PROCESS_VAR_GST GROUP BY DOC_NO ORDER BY DOC_NO");
            resultSet=statement.executeQuery("SELECT DOC_NO,PROCESSING_DATE,PARTY_CODE,PARTY_NAME,CHARGE_CODE_NEW,CRITICAL_LIMIT_NEW,REMARKS,INSURANCE_CODE,TRANSPORTER_CODE,WITHOUT_CRITICAL_LIMIT,VEHICLE_NO,ADV_DOC_NO,ADV_AGN_INV_AMT,ADV_AGN_IGST_AMT,ADV_AGN_SGST_AMT,ADV_AGN_CGST_AMT,ADV_AGN_GST_COMP_CESS_AMT,DOC_DATE,CREATED_BY,CREATED_DATE,MODIFIED_BY,MODIFIED_DATE,APPROVED,APPROVED_DATE,REJECTED,REJECTED_DATE,CANCELED,HIERARCHY_ID,CHANGED,CHANGED_DATE,F6 FROM PRODUCTION.FELT_INV_PROCESS_VAR_GST WHERE DOC_NO NOT LIKE 'IPF6%' GROUP BY DOC_NO ORDER BY DOC_NO");
            HistoryView=false;
            Ready=true;
            MoveLast();
            return true;
...
//                baleDate = data.getStringValueFromDB("SELECT PKG_BALE_DATE FROM PRODUCTION.FELT_PKG_SLIP_HEADER WHERE PKG_BALE_NO='"+bno[i]+"' AND APPROVED=1 AND CANCELED=0 AND BALE_REOPEN_FLG=0 AND INVOICE_FLG=0 ORDER BY PKG_BALE_DATE DESC");
            ...
            clsFeltGSTAdvancePaymentEntryFormDetail ObjFeltPMDetail = (clsFeltGSTAdvancePaymentEntryFormDetail) hmFeltBaleNoDetails.get(Integer.toString(i));
...
//            resultSetDetail.insertRow();
    ...
    public boolean Update() {
        ...
        try {
            ...
            resultSetDetail.updateBoolean("INSURANCE_CODE", getAttribute("INSURANCE_CODE").getBool());
            resultSetDetail.updateBoolean("WITHOUT_CRITICAL_LIMIT", getAttribute("WITHOUT_CRITICAL_LIMIT").getBool());
            resultSetDetail.updateBoolean("F6", getAttribute("F6").getBool());
            resultSetDetail.updateString("TRANSPORTER_CODE", getAttribute("TRANSPORTER_CODE").getString());
            resultSetDetail.updateString("REMARKS", getAttribute("REMARKS").getString());
...
//            
        ...
        try {
          ...
          //  setAttribute("BALE_DATE",resultSet.getString("BALE_DATE"));
           ...
           // setAttribute("BILL_VALUE",resultSet.getString("BILL_VALUE"));
            ...
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.FELT_INV_PROCESS_VAR_GST WHERE DOC_NO='" + resultSet.getString("DOC_NO") + "'");
            
            while (resultSetTemp.next()) {
                serialNoCounter++;
                clsFeltGSTAdvancePaymentEntryFormDetail ObjFeltGSTAdvancePaymentEntryForm = new clsFeltGSTAdvancePaymentEntryFormDetail();
    ...
    public static HashMap getHistoryList(String dpDate, String dpNo) {
        ...
        try {
            ...
            while(rsTmp.next()) {
                clsFeltGSTAdvancePaymentEntryForm ObjFeltReopenBale=new clsFeltGSTAdvancePaymentEntryForm();
    ...
    public static HashMap getPendingApprovals(int pUserID,int pOrder) {
        ...
        try {
            ...
            if(pOrder==EITLERPGLOBAL.OnRecivedDate) {
                ...
                //strSQL="SELECT DISTINCT H.DOC_DATE,H.DOC_NO,D.RECEIVED_DATE FROM PRODUCTION.FELT_INV_PROCESS_VAR_GST H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=754 AND CANCELED=0 ORDER BY RECEIVED_DATE";
            }
            ...
            if(pOrder==EITLERPGLOBAL.OnDocDate) {
                ...
                //strSQL="SELECT DISTINCT H.DOC_DATE,H.DOC_NO,D.RECEIVED_DATE FROM PRODUCTION.FELT_INV_PROCESS_VAR_GST H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=754 AND CANCELED=0 ORDER BY DOC_DATE";
            }
            ...
            if(pOrder==EITLERPGLOBAL.OnDocNo) {
                ...
                //strSQL="SELECT DISTINCT H.DOC_DATE,H.DOC_NO,D.RECEIVED_DATE FROM PRODUCTION.FELT_INV_PROCESS_VAR_GST H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=754 AND CANCELED=0 ORDER BY DOC_NO";
            }
            ...
            while(rsTmp.next()) {
                ...
                Counter=Counter+1;
                clsFeltGSTAdvancePaymentEntryForm ObjDoc=new clsFeltGSTAdvancePaymentEntryForm();
 ...
 public static boolean CancelDoc(int pCompanyId, String pDocNo) {
        ...
        try {
            if (CanCancel(pCompanyId, pDocNo)) {
                ...
                if (ApprovedDoc) {
                    data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE DOC_NO='" + pDocNo + "' AND MODULE_ID=" + (clsFeltGSTAdvancePaymentEntryForm.ModuleID));                    
                }
