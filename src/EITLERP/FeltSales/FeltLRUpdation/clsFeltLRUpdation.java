...
public class clsFeltLRUpdation {
    ...
    public boolean LoadData() {
        try {
            ...
            resultSet=statement.executeQuery("SELECT DISTINCT DOC_NO FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL ORDER BY INVOICE_DATE");
            HistoryView=false;
            MoveLast();
            return true;
        }catch(Exception e) {
    ...
    public boolean Insert() {
        ...
        try {
            ...
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE DOC_NO=''");
            
            // Production data history connection
            statementHistory=connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            resultSetHistory=statementHistory.executeQuery("SELECT * FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL_H WHERE DOC_NO=''");
            ...
            ObjFeltProductionApprovalFlow.ModuleID=614;
            ...
            ObjFeltProductionApprovalFlow.TableName="PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL";
    ...
    public boolean Update() {
        ...
        try {
            // Production data connection
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE INVOICE_DATE=''");
            
            // Production data history connection
            statementHistory=connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            
            //Get the Maximum Revision No in History Table.
            resultSetHistory=statementHistory.executeQuery("SELECT MAX(REVISION_NO) FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL_H WHERE DOC_NO='"+getAttribute("DOC_NO").getString()+"'");
            ...
            statementHistory.execute("DELETE FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE DOC_NO='"+getAttribute("DOC_NO").getString()+"'");
            
            resultSetHistory=statementHistory.executeQuery("SELECT * FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL_H WHERE INVOICE_DATE=''");
            ...
            if(IsRejected) {
                ...
                data.Execute("UPDATE PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL SET REJECTED=0,CHANGED=1 WHERE DOC_NO='"+getAttribute("DOC_NO").getString()+"'");
                //Remove Old Records from FELT DOCUMET APPROVAL TABLE
                //data.Execute("DELETE FROM PRODUCTION.`FELT_PROD_DOC_DATA` WHERE MODULE_ID=614 AND DOC_NO='"+getAttribute("DOC_NO").getString()+"'");
    ...
    public static void CancelDoc(String pAmendNo) {
        ...
        if(CanCancel(pAmendNo)) {
            ...
            try {
                rsTmp=data.getResult("SELECT APPROVED FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE DOC_NO='"+pAmendNo+"' ");
                rsTmp.first();
                ...
                if(!Approved) {
                    data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE DOC_NO='"+pAmendNo+"' AND MODULE_ID=614");
                }
                data.Execute("UPDATE PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE DOC_NO='"+pAmendNo+"'");
    ...
    public static boolean CanCancel(String pDocNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT DOC_NO FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE DOC_NO='"+pDocNo+"'  AND APPROVED=0 AND CANCELED=0");
    ...
    public boolean CanDelete(String documentNo, int userID) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE DOC_NO='"+documentNo +"' AND APPROVED=1";
            ...
            if(rsTmp.getInt("COUNT")>0){  //Item is Approved
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=614 AND USER_ID="+userID+" AND DOC_NO='"+ documentNo +"' AND STATUS='W'";
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
                
                if(rsTmp.getInt("COUNT")>0) {
                    strSQL = "DELETE FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE DOC_NO='" + documentNo + "'";
    ...
    public boolean Filter(String stringFindQuery) {
        try {
            String strSql = "SELECT DISTINCT DOC_NO FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE " + stringFindQuery;
            connection=data.getConn();
            statement=connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean setData() {
        ...
        try {
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE DOC_NO='"+ resultSet.getString("DOC_NO") +"'");
    ...
    public boolean setHistoryData(String docNo) {
        ...
        try {
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL_H WHERE DOC_NO='"+docNo+"' AND REVISION_NO='"+ resultSet.getInt("REVISION_NO") +"' ORDER BY REVISION_NO");
    ...
    public static HashMap getHistoryList(String documentNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT * FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL_H WHERE DOC_NO='"+documentNo+"' GROUP BY REVISION_NO");
    ...
    public boolean ShowHistory(String docNo) {
        try {
            ...
            resultSet=statement.executeQuery("SELECT DISTINCT REVISION_NO FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL_H WHERE DOC_NO='"+docNo+"'");
    ...
    public HashMap getPendingLRData(String invoiceDate) {
        ...
        try {
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetTemp = statementTemp.executeQuery("SELECT INVOICE_NO, PARTY_CODE FROM PRODUCTION.FELT_SAL_INVOICE_HEADER WHERE INVOICE_DATE='"+invoiceDate+"' AND APPROVED=1 AND INVOICE_NO NOT IN(SELECT INVOICE_NO FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE INVOICE_DATE='"+invoiceDate+"')");
    ...
    public static HashMap getPendingApprovals(int pUserID,int pOrder) {
        ...
        try {
            ...
            if(pOrder==EITLERPGLOBAL.OnRecivedDate) {
                strSQL="SELECT H.DOC_NO,H.INVOICE_DATE,D.RECEIVED_DATE FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=614 ORDER BY RECEIVED_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocDate) {
                strSQL="SELECT H.DOC_NO,H.INVOICE_DATE,D.RECEIVED_DATE FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=614 ORDER BY INVOICE_DATE";
            }
            
            if(pOrder==EITLERPGLOBAL.OnDocNo) {
                strSQL="SELECT H.DOC_NO,H.INVOICE_DATE,D.RECEIVED_DATE FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID="+pUserID+" AND STATUS='W' AND MODULE_ID=614 ORDER BY DOC_NO";
            }
            ...
            while(rsTmp.next()) {
                ...
                Counter=Counter+1;
                clsFeltLRUpdation ObjDoc=new clsFeltLRUpdation();
    ...
    public boolean checkInvoiceNoInDB(String invoiceNo){
        int count=data.getIntValueFromDB("SELECT COUNT(INVOICE_NO) FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE INVOICE_NO='"+invoiceNo+"' AND INVOICE_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND INVOICE_DATE<='"+EITLERPGLOBAL.FinToDateDB+"'");
    ...
    public boolean checkInvoiceNoInDB(String invoiceNo, String invoiceDate){
        ...
        if(count>=1){
            ...
            try{
                ...
                ResultSet rsTmp=stTmp.executeQuery("SELECT INVOICE_DATE FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE INVOICE_NO='"+invoiceNo+"' AND INVOICE_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND INVOICE_DATE<='"+EITLERPGLOBAL.FinToDateDB+"'");
                while(rsTmp.next()){
    ...
    public String getDocumentNo(String invoiceDate){
        ...
        return "FLR"+docDate+data.getStringValueFromDB("SELECT COALESCE(MAX(CAST(SUBSTRING(DOC_NO,12) AS UNSIGNED)),0)+1 FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE SUBSTRING(DOC_NO,4,8)='"+docDate+"'"); 
    }
...
//            rsTmp=Stmt.executeQuery("SELECT * FROM(SELECT PARTY_CODE, TRIM(PARTY_NAME) PARTY_NAME FROM D_SAL_PARTY_MASTER  WHERE MAIN_ACCOUNT_CODE=210010 AND PARTY_CODE+0='"+partyCode+"' ) P LEFT JOIN (SELECT PARTY_CODE, TRIM(EMAIL) EMAIL FROM PRODUCTION.FELT_PARTY_EXTRA_INFO WHERE PARTY_CODE+0='"+partyCode+"') I ON P.PARTY_CODE=I.PARTY_CODE");
    ...
    public String createMailBody(String partyCode, String invoiceDate, String despatchNote){
        ...
        "</th>"+
        "<th width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
        "<p class='MsoNormal'>MACHINE NO</p>"+
        "</th>"+
        "<th width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
        "<p class='MsoNormal'>POSITION DISCRIPTION</p>"+
        "</th>"+
        "<th width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
        "<p class='MsoNormal'>PRODUCT GROUP</p>"+
        "</th>"+
        "<th width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
        "<p class='MsoNormal'>PARTY CODE</p>"+
        "</th>"+
        "<th width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
        "<p class='MsoNormal'>PARTY NAME</p>"+
        "</th>"+
        "<th width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
        "<p class='MsoNormal'>DISPATCH STATION</p>"+
        "</th>"+
        "<th width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
        ...
        try {
            tmpConn=data.getConn();
            stmt=tmpConn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
            rsTmp=stmt.executeQuery("SELECT * FROM (SELECT INVOICE_NO , INVOICE_DATE, LR_NO, LR_DATE, CARRIER FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE APPROVED =1 AND INVOICE_DATE='"+EITLERPGLOBAL.formatDateDB(invoiceDate)+"' AND PARTY_CODE+0='"+partyCode+"') D LEFT JOIN (SELECT PARTY_CODE, PARTY_NAME, INVOICE_NO , INVOICE_DATE, PIECE_NO, CONCAT(LENGTH ,'X',WIDTH) SIZE, MACHINE_NO, POSITION_DESC, PR_GROUP, INVOICE_AMT AS NET_AMOUNT,DISPATCH_STATION FROM PRODUCTION.FELT_SAL_INVOICE_HEADER,PRODUCTION.FELT_SALES_PIECE_REGISTER WHERE PIECE_NO=PR_PIECE_NO AND INVOICE_DATE='"+EITLERPGLOBAL.formatDateDB(invoiceDate)+"' AND PARTY_CODE+0='"+partyCode+"') I ON D.INVOICE_DATE=I.INVOICE_DATE AND D.INVOICE_NO=I.INVOICE_NO");
            ...
            while(rsTmp.next()) {
                ...
                "</td>"+
                "<td width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
                "<p class='MsoNormal'>"+rsTmp.getString("MACHINE_NO")+"</p>"+
                "</td>"+
                "<td width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
                "<p class='MsoNormal'>"+rsTmp.getString("POSITION_DESC")+"</p>"+
                "</td>"+
                "<td width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
                "<p class='MsoNormal'>"+rsTmp.getString("PR_GROUP")+"</p>"+
                "</td>"+
                "<td width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
                "<p class='MsoNormal'>"+rsTmp.getString("PARTY_CODE")+"</p>"+
                "</td>"+
                "<td width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
                "<p class='MsoNormal'>"+rsTmp.getString("PARTY_NAME")+"</p>"+
                "</td>"+
                "<td width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
                "<p class='MsoNormal'>"+rsTmp.getString("DISPATCH_STATION")+"</p>"+
                "</td>"+
                "<td width='12%' style='width:12.0%;border:solid black 1.0pt;padding:2.25pt 2.25pt 2.25pt 2.25pt'>"+
        ...
        "<p class='MsoNormal' style='mso-margin-top-alt:auto;mso-margin-bottom-alt:auto'><span style='color:black'><img src='http://200.0.0.223:8080/jmx-console/dinesh/EITLERP/Images/feltIcon.jpg' alt='Felt Icon' width='169' height='193'></span></p>"+
        "<p class='MsoNormal' style='mso-margin-top-alt:auto;mso-margin-bottom-alt:auto'><span style='color:black'>&nbsp;</span><span style='font-size:3.0pt;font-family:&quot;Verdana&quot;,&quot;sans-serif&quot;;color:#7F7F7F'>Office: +91 265 2960060/61/62/63/66 |</span></p>"+
        ...
        "<p><span><img border='0' width='28' height='25' src='http://200.0.0.223:8080/jmx-console/dinesh/EITLERP/Images/savetree.jpg' alt='Save Tree'></span><b><span style='font-size:4.0pt;color:green'>Please consider your environmental responsibility. Do not print this e-mail unless you really need to.</span></b></p>"+
        "</body>"+
        "</html>";
    ...
    public long saveMailDetail(String toEmailId, String message){
        ...
        try {
            ...
            ObjMailer.setAttribute("MODULE_ID",614);
...
//            ObjMailer.setAttribute("BCC","felts@dineshmills.com");
    ...
    private void updateLRNo(String docNo){
        data.Execute("UPDATE D_SAL_INVOICE_HEADER H, PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL D SET H.LR_NO=D.LR_NO, H.CHANGED=1, H.CHANGED_DATE=CURDATE() WHERE D.INVOICE_NO=H.INVOICE_NO AND D.INVOICE_DATE=H.INVOICE_DATE AND D.PARTY_CODE=H.PARTY_CODE AND D.DOC_NO='"+docNo+"' ");//D.APPROVED=1 AND 
    }
