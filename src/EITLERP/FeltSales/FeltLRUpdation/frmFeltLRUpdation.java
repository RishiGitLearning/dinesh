...
public class frmFeltLRUpdation extends javax.swing.JApplet {
    ...
    private void initComponents() {
        ...
        lblTitle.setText(" FELT SALES LR & CARRIER UPDATION DETAILS - ");
        ...
        cmbHierarchy.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cmbHierarchyActionPerformed(evt);
            }
        });
        ...
        cmdFromRemarksBig.setBounds(730, 80, 49, 21);
        ...
        EditorMessage.setContentType("text/html"); // NOI18N
    ...
    private void cmdEmailActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdEmailActionPerformed
        if(Table.getRowCount()>0 && Table.getSelectedRow()>=0) {
            ...
            if(EITLERP.clsSales_Party.IsPartyExistEx(EITLERPGLOBAL.gCompanyID,partyCode)){
                Loader ObjLoader=new Loader(this,"EITLERP.FeltSales.FeltLRUpdation.frmNoteFeltLRUpdation",true);
    ...
    private void cmdSendActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdSendActionPerformed
        ...
        try{
            ...
            if(mailNo!=0) {
//                URL MailDocument=new URL("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/Production/FeltLRDetailsMailer.jsp?dbURL="+EITLERPGLOBAL.DatabaseURL+"&DocNo="+mailNo);                
                ...
                URL MailDocument=new URL("http://"+EITLERPGLOBAL.HostIP+"/EITLERP/Reports/Production/FeltLRDetailsMailer.jsp?dbURL="+EITLERPGLOBAL.DatabaseURL+"&CompanyID="+EITLERPGLOBAL.gCompanyID+"&DocNo="+mailNo); 
            ...
            if(EditMode!=0) {//Only Process keys when in editing mode
                if(evt.getKeyCode()==112){ //F1 Key pressed
                    if(Table.getSelectedColumn()==1) {
                        LOV aList=new LOV();
                        aList.SQL="SELECT INVOICE_NO, PARTY_CODE FROM PRODUCTION.FELT_SAL_INVOICE_HEADER WHERE INVOICE_DATE='"+EITLERPGLOBAL.formatDateDB(txtInvoiceDate.getText().trim())+"' AND APPROVED=1 AND CONCAT(INVOICE_NO,INVOICE_DATE) NOT IN (SELECT CONCAT(INVOICE_NO,INVOICE_DATE) FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL WHERE INVOICE_DATE='"+EITLERPGLOBAL.formatDateDB(txtInvoiceDate.getText().trim())+"')";
                        System.out.println("lr query "+aList.SQL);
                        aList.ReturnCol=1;
                        aList.SecondCol=2;
                        ...
                        if(aList.ShowLOV()) {
                            ...
                            Table.setValueAt(clsSales_Party.getPartyName(EITLERPGLOBAL.gCompanyID, aList.SecondVal), Table.getSelectedRow(),4);
                            Table.setValueAt(txtInvoiceDate.getText().trim(), Table.getSelectedRow(),6);
                        }
                    }                    
                    ...
                    if (Table.getSelectedColumn() == 9) {
                        LOV aList = new LOV();
                        aList.SQL = "SELECT '1' AS SR_NO,'' AS MODE_OF_TRANSPORT FROM DUAL "
                                + "UNION ALL SELECT '2' AS SR_NO,'DOOR DELIVERY' FROM DUAL "
                                + "UNION ALL SELECT '3' AS SR_NO,'HAND DELIVERY' FROM DUAL "
                                + "UNION ALL SELECT '4' AS SR_NO,'GODOWN DELIVERY' FROM DUAL ";
                        aList.ReturnCol = 2;
                        aList.ShowReturnCol = true;
                        aList.DefaultSearchOn = 1;

                        if (aList.ShowLOV()) {
                            Table.setValueAt(aList.ReturnVal, Table.getSelectedRow(), 9);
                        }
                    }
                }
            }
...
//        }
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if(EditMode==EITLERPGLOBAL.EDIT) {
            GenerateRejectedSendToCombo();
            if(clsFeltProductionApprovalFlow.IsOnceRejectedDoc(614,ObjFeltLRUpdation.getAttribute("DOC_NO").getString())) {
                cmbSendTo.setEnabled(true);
            }
            else {
                cmbSendTo.setEnabled(false);
    ...
    private void cmbHierarchyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmbHierarchyActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_cmbHierarchyActionPerformed
    ...
    private void DisplayData() {
        ...
        if(EITLERPGLOBAL.gAuthorityUserID!=EITLERPGLOBAL.gUserID) {
            if(clsAuthority.IsAuthorityGiven(EITLERPGLOBAL.gCompanyID,EITLERPGLOBAL.gUserID,EITLERPGLOBAL.gAuthorityUserID,614)) {
        ...
        try {
            ...
            // --- Mail Rights --
            //if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6088,60884) && ObjFeltLRUpdation.getAttribute("APPROVED").getInt()==1) {
            if((clsHierarchy.CanFinalApprove(EITLERPGLOBAL.gCompanyID, ObjFeltLRUpdation.getAttribute("HIERARCHY_ID").getInt(), EITLERPGLOBAL.gNewUserID) || EITLERPGLOBAL.gNewUserID==237) && ObjFeltLRUpdation.getAttribute("APPROVED").getInt()==1) {
            ...
            lblTitle.setText(" FELT SALES LR UPDATION DETAILS - "+ invoiceDate);
            ...
            for(int i=1;i<=ObjFeltLRUpdation.hmFeltLRUpdationDetails.size();i++) {
                ...
                rowData[4]=clsSales_Party.getPartyName(EITLERPGLOBAL.gCompanyID, ObjFeltLRUpdationDetails.getAttribute("PARTY_CODE").getString());                
                rowData[5]=ObjFeltLRUpdationDetails.getAttribute("LR_NO").getString();
                rowData[6]=EITLERPGLOBAL.formatDate(ObjFeltLRUpdationDetails.getAttribute("LR_DATE").getString());
                rowData[7]=ObjFeltLRUpdationDetails.getAttribute("CARRIER").getString();
                rowData[8]=Double.toString(EITLERPGLOBAL.round(ObjFeltLRUpdationDetails.getAttribute("FREIGHT").getVal(),2));
                rowData[9]=ObjFeltLRUpdationDetails.getAttribute("REMARKS").getString();
            ...
            hmList=clsFeltProductionApprovalFlow.getDocumentFlow(614, ObjFeltLRUpdation.getAttribute("DOC_NO").getString());
            for(int i=1;i<=hmList.size();i++) {
                ...
                if(ObjFlow.getAttribute("RECEIVED_DATE").toString().startsWith("0000-00-00") || ObjFlow.getAttribute("RECEIVED_DATE").getString().equals("") || ObjFlow.getAttribute("RECEIVED_DATE")==null) {
                    rowData[4]="";
                } else {
                ...
                //rowData[4]=EITLERPGLOBAL.formatDate(ObjFlow.getAttribute("RECEIVED_DATE").getString())+ ObjFlow.getAttribute("RECEIVED_DATE").getString().substring(10,19);
                if(ObjFlow.getAttribute("ACTION_DATE").toString().startsWith("0000-00-00") || ObjFlow.getAttribute("ACTION_DATE").getString().equals("") || ObjFlow.getAttribute("ACTION_DATE")==null) {
                    rowData[5]="";
                } else {
    ...
    private void FormatGrid() {
        ...
        try {
            ...
            columnModel.getColumn(4).setMinWidth(130);
...
//            }
    ...
    private void GenerateHierarchyCombo() {
        ...
        hmHierarchyList=clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID=614 ");
        
        if(EditMode==EITLERPGLOBAL.EDIT) {
            hmHierarchyList=clsHierarchy.getList(" WHERE COMPANY_ID="+EITLERPGLOBAL.gCompanyID+" AND MODULE_ID=614 ");
        }
    ...
    private void GenerateSendToCombo() {
        ...
        try{
            ...
            if(EditMode==EITLERPGLOBAL.ADD) {
                ...
                hmSendToList= clsFeltProductionApprovalFlow.getRemainingUsers(614,ObjFeltLRUpdation.getAttribute("DOC_NO").getString());
                for(int i=1;i<=hmSendToList.size();i++) {
                    clsUser ObjUser=(clsUser) hmSendToList.get(Integer.toString(i));
                    ComboData aData=new ComboData();
                    aData.Code=(int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void GenerateRejectedSendToCombo() {
        ...
        for(int i=1;i<=hmRejectedSendToList.size();i++) {
            ...
            if(EditMode==EITLERPGLOBAL.EDIT) {
                if(OpgApprove.isSelected()) {
                    IncludeUser=clsFeltProductionApprovalFlow.IncludeUserInApproval(614, ObjFeltLRUpdation.getAttribute("DOC_NO").getString(), (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
                
                if(OpgReject.isSelected()) {
                    IncludeUser=clsFeltProductionApprovalFlow.IncludeUserInRejection(614, ObjFeltLRUpdation.getAttribute("DOC_NO").getString(), (int)ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if(EditMode==EITLERPGLOBAL.EDIT) {
            int Creator=clsFeltProductionApprovalFlow.getCreator(614,ObjFeltLRUpdation.getAttribute("DOC_NO").getString());
    ...
    private void SetupApproval() {
        ...
        if(EditMode==EITLERPGLOBAL.ADD) {
            ...
            int FromUserID=clsFeltProductionApprovalFlow.getFromID(614,ObjFeltLRUpdation.getAttribute("DOC_NO").getString());
            lnFromUserId=FromUserID;
            String strFromUser=clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks=clsFeltProductionApprovalFlow.getFromRemarks(614,FromUserID,ObjFeltLRUpdation.getAttribute("DOC_NO").getString());
    ...
    private void SetMenuForRights() {
        // --- Add Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6088,60881)) {
            cmdNew.setEnabled(true);
        }else {
            cmdNew.setEnabled(false);
        }
        ...
        // --- Edit Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6088,60882)) {
            cmdEdit.setEnabled(true);
        }else {
            cmdEdit.setEnabled(false);
        }
        ...
        // --- Delete Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6088,60883)) {
            cmdDelete.setEnabled(true);
        }else {
            cmdDelete.setEnabled(false);
        }
        ...
        // --- Print Rights --
        if(clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6088,0)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        }else {
            cmdPreview.setEnabled(false);
    ...
    private void Edit() {
        ...
        if(ObjFeltLRUpdation.IsEditable(documentNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if(clsFeltProductionApprovalFlow.IsCreator(614,documentNo)) {
                SetFields(true);
                txtInvoiceDate.setEnabled(false);
            }
            else {
    ...
    private void Save() {
        ...
        try{
            ...
            for(i=0;i<=Table.getRowCount()-1;i++) {
                ...
                lrNo = ((String)Table.getValueAt(i, 5)).trim().toUpperCase();
                lrDate = ((String)Table.getValueAt(i, 6)).trim();
                carrier = ((String)Table.getValueAt(i, 7)).trim().toUpperCase();
                freightString = ((String)Table.getValueAt(i, 8)).trim();
                remarks = ((String)Table.getValueAt(i, 9)).trim().toUpperCase();
        ...
        if(EditMode==EITLERPGLOBAL.ADD) {
            if(ObjFeltLRUpdation.Insert()) {
                ...
                if(OpgFinal.isSelected()) {
                        try{
                                String DOC_NO = txtInvoiceDate.getText();
                                String DOC_DATE = txtInvoiceDate.getText();
                                String Party_Code = "";
                                int Module_Id = 614;

                                String responce = JavaMail.sendFinalApprovalMail(Module_Id, DOC_NO, DOC_DATE, Party_Code, EITLERPGLOBAL.gNewUserID, EITLERPGLOBAL.getComboCode(cmbHierarchy), true);
    ...
    private void Find() {
        Loader ObjLoader=new Loader(this,"EITLERP.FeltSales.FeltLRUpdation.frmFindFeltLRUpdation",true);
    ...
    public void FindWaiting() {
        ObjFeltLRUpdation.Filter("DOC_NO IN (SELECT H.DOC_NO FROM PRODUCTION.FELT_SAL_INVOICE_EXTRA_DETAIL H, PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID="+EITLERPGLOBAL.gNewUserID+" AND STATUS='W' AND MODULE_ID=614 AND CANCELED=0) ");
        SetMenuForRights();
        DisplayData();
    }
    
    public void FindEx(int pCompanyID, String pdocNo) {
        ObjFeltLRUpdation.Filter(" DOC_NO='" + pdocNo + "'");
