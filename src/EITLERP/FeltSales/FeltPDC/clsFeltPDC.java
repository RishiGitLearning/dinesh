...
public class clsFeltPDC {
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT * FROM PRODUCTION.FELT_PDC_HEADER WHERE PDC_DOC_DATE >= '" + EITLERPGLOBAL.FinFromDateDB + "' AND PDC_DOC_DATE<='" + EITLERPGLOBAL.FinToDateDB + "' ORDER BY PDC_DOC_NO,PDC_DOC_DATE ");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
...
//            }
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHeader = stHeader.executeQuery("SELECT * FROM  PRODUCTION.FELT_PDC_HEADER WHERE PDC_DOC_NO=''");
            rsHeader.first();
            ...
            rsHHeader = stHHeader.executeQuery("SELECT * FROM  PRODUCTION.FELT_PDC_HEADER_H WHERE PDC_DOC_NO=''");
            rsHHeader.first();
            ...
            rsPieceDetail = stPieceDetail.executeQuery("SELECT * FROM  PRODUCTION.FELT_PDC_PIECE_DETAIL WHERE PDC_DOC_NO=''");
            rsPieceDetail.first();
            ...
            rsHPieceDetail = stHPieceDetail.executeQuery("SELECT * FROM  PRODUCTION.FELT_PDC_PIECE_DETAIL_H WHERE PDC_DOC_NO=''");
            rsHPieceDetail.first();

            // Felt PDC Last Free No, 
            setAttribute("PDC_DOC_NO", getNextFreeNo(625, true));
            ...
            for (int i = 1; i <= colPDCPieceItems.size(); i++) {
                clsFeltPDCPieceDetails ObjPieceItem = (clsFeltPDCPieceDetails) colPDCPieceItems.get(Integer.toString(i));
            ...
            ObjFeltProductionApprovalFlow.ModuleID = 625; //Felt Weaving Entry
            ObjFeltProductionApprovalFlow.DocNo = (String) getAttribute("PDC_DOC_NO").getObj();
            ObjFeltProductionApprovalFlow.DocDate = (String) getAttribute("PDC_DOC_DATE").getObj();
            ...
            ObjFeltProductionApprovalFlow.TableName = "PRODUCTION.FELT_PDC_HEADER";
            ...
            ObjFeltProductionApprovalFlow.FieldName = "PDC_DOC_NO";
            ...
            if (ObjFeltProductionApprovalFlow.Status.equals("F") && ObjFeltProductionApprovalFlow.finalApproved) {
                ...
                data.Execute("UPDATE PRODUCTION.FELT_PDC_PIECE_DETAIL SET APPROVED=1,APPROVED_DATE=CURRENT_TIMESTAMP,CHANGED=1 WHERE PDC_DOC_NO='"+ObjFeltProductionApprovalFlow.DocNo+"'");
            }
    ...
    public boolean Update() {
        ...
        try {
            ...
            revisionNo = EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM PRODUCTION.FELT_PDC_HEADER_H WHERE PDC_DOC_DATE='" + getAttribute("PDC_DOC_DATE").getString() + "' AND PDC_DOC_NO='" + getAttribute("PDC_DOC_NO").getString() + "'");
            revisionNo++;
                        
            rsHHeader.moveToInsertRow();
            rsHHeader.updateInt("REVISION_NO", revisionNo);
            rsHHeader.updateInt("UPDATED_BY", (int) getAttribute("UPDATED_BY").getVal());
            ...
            String DocNo = (String) getAttribute("PDC_DOC_NO").getObj();

            data.Execute("DELETE FROM PRODUCTION.FELT_PDC_BANK_DETAIL WHERE PDC_DOC_NO='" + DocNo + "' ");
            data.Execute("DELETE FROM PRODUCTION.FELT_PDC_PIECE_DETAIL WHERE PDC_DOC_NO='" + DocNo + "' ");
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE PRODUCTION.FELT_PDC_HEADER SET REJECTED=0,CHANGED=1 WHERE PDC_DOC_NO ='" + getAttribute("PDC_DOC_NO").getString() + "'");
    ...
    public boolean CanDelete(String documentNo, String stringProductionDate, int userID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PDC_HEADER WHERE PDC_DOC_NO='" + documentNo + "' AND APPROVED=" + 1;
            ...
            if (rsTmp.getInt("COUNT") > 0) {  //Item is Approved
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=625 AND USER_ID=" + userID + " AND DOC_NO='" + documentNo + "' AND STATUS='W'";
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();

                if (rsTmp.getInt("COUNT") > 0) {
                    strSQL = "DELETE FROM PRODUCTION.FELT_PDC_HEADER WHERE PDC_DOC_NO='" + documentNo + "'";
    ...
    public boolean IsEditable(String orderupdDocumentNo, int userID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PDC_HEADER WHERE PDC_DOC_NO='" + orderupdDocumentNo + "' AND APPROVED=1";
            rsTmp = tmpStmt.executeQuery(strSQL);
            rsTmp.first();
            ...
            if (rsTmp.getInt("COUNT") > 0) {  //Item is Approved
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=625 AND USER_ID=" + userID + " AND DOC_NO='" + orderupdDocumentNo + "' AND STATUS='W'";
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean Filter(String stringFindQuery) {
        Ready = false;
        try {
            String strSql = "SELECT * FROM PRODUCTION.FELT_PDC_HEADER WHERE " + stringFindQuery + "";
            connection = data.getConn();
            statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean setData() {
        ...
        try {
            ...
            stPieceDetail = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
            if (HistoryView) {
                rsPieceDetail = stPieceDetail.executeQuery("SELECT * FROM PRODUCTION.FELT_PDC_PIECE_DETAIL_H WHERE (PDC_PIECE_STATUS IS NULL OR PDC_PIECE_STATUS IN ('INSERT','ADD','')) AND PDC_DOC_NO='" + DocNo + "' AND REVISION_NO=" + RevNo + " ");
            } else {
                rsPieceDetail = stPieceDetail.executeQuery("SELECT * FROM PRODUCTION.FELT_PDC_PIECE_DETAIL WHERE (PDC_PIECE_STATUS IS NULL OR PDC_PIECE_STATUS IN ('INSERT','ADD','')) AND PDC_DOC_NO='" + DocNo + "' ");
            }
    ...
    public static HashMap getHistoryList(String stringProductionDate, String productionDocumentNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT * FROM PRODUCTION.FELT_PDC_HEADER_H WHERE PDC_DOC_NO='" + productionDocumentNo + "' GROUP BY REVISION_NO");

            while (rsTmp.next()) {
                clsFeltPDC ObjFeltOrderUpd = new clsFeltPDC();
    ...
    public boolean ShowHistory(String pProductionDate, String pDocNo) {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT DISTINCT REVISION_NO FROM PRODUCTION.FELT_PDC_HEADER_H WHERE PDC_DOC_DATE='" + pProductionDate + "' AND PDC_DOC_NO ='" + pDocNo + "'");
            Ready = true;
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            ...
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT DISTINCT PDC_DOC_NO,PDC_DOC_DATE,PDC_PARTY_CODE,RECEIVED_DATE FROM PRODUCTION.FELT_PDC_HEADER, PRODUCTION.FELT_PROD_DOC_DATA WHERE PDC_DOC_NO=DOC_NO AND STATUS='W' AND MODULE_ID=625 AND CANCELED=0  AND USER_ID=" + pUserID + " ORDER BY RECEIVED_DATE,PDC_DOC_NO";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT DISTINCT PDC_DOC_NO,PDC_DOC_DATE,PDC_PARTY_CODE,RECEIVED_DATE FROM PRODUCTION.FELT_PDC_HEADER, PRODUCTION.FELT_PROD_DOC_DATA WHERE PDC_DOC_NO=DOC_NO AND STATUS='W' AND MODULE_ID=625 AND CANCELED=0  AND USER_ID=" + pUserID + " ORDER BY PDC_DOC_DATE,PDC_DOC_NO";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT DISTINCT PDC_DOC_NO,PDC_DOC_DATE,PDC_PARTY_CODE,RECEIVED_DATE FROM PRODUCTION.FELT_PDC_HEADER, PRODUCTION.FELT_PROD_DOC_DATA WHERE PDC_DOC_NO=DOC_NO AND STATUS='W' AND MODULE_ID=625 AND CANCELED=0  AND USER_ID=" + pUserID + " ORDER BY PDC_DOC_NO";
            }
            ...
            while (rsTmp.next()) {
                ...
                Counter = Counter + 1;
                clsFeltPDC ObjDoc = new clsFeltPDC();

                //------------- Header Fields --------------------//
                ObjDoc.setAttribute("DOC_NO", rsTmp.getString("PDC_DOC_NO"));
                ObjDoc.setAttribute("DOC_DATE", rsTmp.getString("PDC_DOC_DATE"));
    ...
    public static String getNextFreeNo(int pModuleID, boolean UpdateLastNo) {
        ...
        try {
            ...
            strSQL = "SELECT LAST_USED_NO,PADDING_BY,NO_LENGTH,PREFIX_CHARS,SUFFIX_CHARS FROM D_COM_FIRSTFREE WHERE COMPANY_ID=2 AND MODULE_ID=625 AND FIRSTFREE_NO=272";
            rsTmp = tmpStmt.executeQuery(strSQL);
            ...
            if (rsTmp.getRow() > 0) {
                ...
                if (UpdateLastNo) {
                    ...
                    data.Execute("UPDATE D_COM_FIRSTFREE SET LAST_USED_NO='" + strNewNo.trim() + "',CHANGED=1,CHANGED_DATE=CURDATE() WHERE COMPANY_ID=2 AND MODULE_ID=625 AND FIRSTFREE_NO=272");
                }
    ...
    public static boolean CanCancel(int pCompanyID, String pDocNo) {
        ...
        try {
            rsTmp = data.getResult("SELECT PDC_DOC_NO,APPROVED FROM PRODUCTION.FELT_PDC_HEADER WHERE PDC_DOC_NO='" + pDocNo + "' AND CANCELED=0");
            rsTmp.first();
    ...
    public static boolean CancelDoc(int pCompanyID, String pDocNo) {
        ...
        try {
            if (CanCancel(pCompanyID, pDocNo)) {
                ...
                rsTmp = data.getResult("SELECT APPROVED FROM PRODUCTION.FELT_PDC_HEADER WHERE PDC_DOC_NO='" + pDocNo + "' ");
                rsTmp.first();
                ...
                data.Execute("UPDATE PRODUCTION.FELT_PDC_HEADER SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE PDC_DOC_NO='" + pDocNo + "' ");
                ...
                data.Execute("UPDATE PRODUCTION.FELT_PDC_PIECE_DETAIL SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE PDC_DOC_NO='" + pDocNo + "' ");
