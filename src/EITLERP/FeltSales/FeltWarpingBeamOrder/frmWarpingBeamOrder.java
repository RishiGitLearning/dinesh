...
public class frmWarpingBeamOrder extends javax.swing.JApplet {
...
//                                + "WHERE (SUBSTRING(PR_PIECE_NO,1,5) = '" + PieceNo.substring(0, 5) + "' AND COALESCE(PR_REJECTED_FLAG,0)=0 AND "
//                                + "PR_PIECE_STAGE='PLANNING' AND (PR_WARP_DATE='0000-00-00' OR PR_WARP_DATE IS NULL) "
//                                + "AND (PR_PIECE_NO NOT IN (SELECT LEFT(PIECE_NO,5) FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL)) "
//                                + "OR PR_PIECE_NO IN (SELECT DISTINCT LEFT(D.PIECE_NO,5) FROM ("
//                                + "SELECT  * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL "
//                                + "WHERE PIECE_NO='" + PieceNo + "' AND BEAM_NO IN (SELECT DISTINCT BEAM_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL_AMEND)) AS D "
//                                + "LEFT JOIN (SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL_AMEND) AS  A "
...
//                                + "WHERE A.PIECE_NO IS NULL))";
                        sql = "SELECT * FROM PRODUCTION.FELT_WIP_PIECE_REGISTER "
                                ...
                                + "AND (WIP_EXT_PIECE_NO NOT IN (SELECT PIECE_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL)) "
                    ...
                    if (trs.getRow() > 0) {
                        ...
                        if (PieceNo.contains("A") || PieceNo.contains("a")) {
                            trs = data.getResult("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL WHERE PIECE_NO = '" + PieceNo + "' AND INDICATOR!='DELETE'");
                            if (trs.getRow() > 0) {
                                JOptionPane.showMessageDialog(null, "Piece No. is Already Warped...");
                                cmdItemdeleteActionPerformed(null);
                            } else {
                                sql = "SELECT PIECE_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL "
                                ...
                                if (trs.getRow() > 0) {
                                    ...
                                    sql = "SELECT PROD_PIECE_NO FROM PRODUCTION.FELT_PROD_DATA  "
                                            + "SELECT PIECE_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL "
                                            + "WHERE PIECE_NO = '" + PieceNo + "'   AND INDICATOR!='DELETE'  "
                                            + "UNION ALL "
                                            + "WHERE PROD_PIECE_NO LIKE '" + PieceNo.substring(0, 5) + "%' AND "
                        ...
                        if (!data.getStringValueFromDB("SELECT PD.PIECE_NO FROM PRODUCTION.FELT_SALES_PIECE_AMENDMENT_HEADER_WIP PH, "
                            ...
                            try {
                                ...
                                String sql = "SELECT * FROM PRODUCTION.FELT_WIP_PIECE_REGISTER "
                                        + "WHERE WIP_EXT_PIECE_NO = '" + PieceNo + "' AND COALESCE(WIP_REJECTED_FLAG,0)=0 AND "
                                        + "WIP_PIECE_STAGE='PLANNING' AND (WIP_WARP_DATE='0000-00-00' OR WIP_WARP_DATE IS NULL) "
                                        + "AND (WIP_EXT_PIECE_NO NOT IN (SELECT PIECE_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL WHERE INDICATOR!='DELETE')) "
                                        + "OR WIP_EXT_PIECE_NO IN (SELECT DISTINCT D.PIECE_NO FROM ("
                                        + "SELECT  * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL "
                                        + "WHERE PIECE_NO='" + PieceNo + "' AND BEAM_NO IN (SELECT DISTINCT BEAM_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL_AMEND)) AS D "
                                        + "LEFT JOIN (SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL_AMEND) AS  A "
