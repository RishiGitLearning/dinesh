 ...
 * @author
 */
public class clsWarpingBeamOrderHDS {
    ...
    public static int ModuleID = 783; //
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER  ORDER BY DOC_NO");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHistory = stHistory.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER_H WHERE DOC_NO='1'"); // '1' for restricting all data retrieval
            rsHistory.first();
            rsHDetail = stHDetail.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL_H WHERE DOC_NO='1'");
            rsHDetail.first();
            //------------------------------------//

            //--------- Generate Gatepass requisition no.  ------------
            setAttribute("DOC_NO", clsFirstFree.getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 783, (int) getAttribute("FFNO").getVal(), true));
            //-------------------------------------------------
            ...
            rsTmp = tmpStmt.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL WHERE DOC_NO='1'");

            //Now Insert records into detail table
            for (int i = 1; i <= colMRItems.size(); i++) {
                clsWarpingBeamOrderHDSItem ObjMRItems = (clsWarpingBeamOrderHDSItem) colMRItems.get(Integer.toString(i));
            ...
            ObjFlow.ModuleID = clsWarpingBeamOrderHDS.ModuleID;
            ...
            String sql1 = "UPDATE PRODUCTION.FELT_SALES_PIECE_REGISTER P,PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL D "
            ...
            sql1 = "UPDATE PRODUCTION.FELT_WIP_PIECE_REGISTER P,PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL D "
    ...
    public boolean Update() {
        ...
        try {
            ...
            int RevNo = EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER_H WHERE DOC_NO='" + (String) getAttribute("DOC_NO").getObj() + "'");
            RevNo++;
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(ModuleID, (String) getAttribute("DOC_NO").getObj())) {
                ...
                data.Execute("DELETE FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL WHERE DOC_NO='" + mDocNo + "'");

                Statement tmpStmt;

                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
                ResultSet rsTmp1 = tmpStmt.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL WHERE DOC_NO='1'");
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE DOC_NO='" + ObjFlow.DocNo + "' ");

                //Remove Old Records from FELT_DOC_DATA
                data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=" + clsWarpingBeamOrderHDS.ModuleID + " AND DOC_NO='" + ObjFlow.DocNo + "'");
            ...
            if (ObjFlow.Status.equals("F")) {
                String sql = "UPDATE PRODUCTION.FELT_SALES_PIECE_REGISTER P,"
                        + "(SELECT PIECE_NO,DOC_NO,WEIGHT FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL "
                        ...
                        + "PR_WARPING_WEIGHT_A=D.WEIGHT,PR_WARPING_WEIGHT=D.WEIGHT,"
                ...
                data.Execute("UPDATE PRODUCTION.FELT_WIP_PIECE_REGISTER, "
                        ...
                        + "WIP_WARP_A_DATE = MAXDATE  WHERE LEFT(PIECE,5) =LEFT(WIP_EXT_PIECE_NO,5) AND COALESCE(WIP_REJECTED_FLAG,0)=0 ");
    ...
    public boolean setData() {
        ...
        try {
            ...
            if (HistoryView) {
                if (clsFeltProductionApprovalFlow.IsCreator(ModuleID, (String) getAttribute("DOC_NO").getObj())) {
                    rsTmp = tmpStmt.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL_H WHERE (INDICATOR IS NULL OR INDICATOR IN ('INSERT','ADD','')) AND DOC_NO='" + mDocNo + "' AND REVISION_NO=" + RevNo + " ORDER BY SR_NO");
                } else {
                    rsTmp = tmpStmt.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL_H WHERE (INDICATOR IS NULL OR INDICATOR IN ('INSERT','ADD','')) AND DOC_NO='" + mDocNo + "' AND REVISION_NO=" + RevNo + " ORDER BY SEQUANCE_NO");
                }
            } else {
                if (clsFeltProductionApprovalFlow.IsCreator(ModuleID, (String) getAttribute("DOC_NO").getObj())) {
                    rsTmp = tmpStmt.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL WHERE (INDICATOR IS NULL OR INDICATOR IN ('INSERT','ADD','')) AND DOC_NO='" + mDocNo + "' ORDER BY SR_NO");
                } else {
                    rsTmp = tmpStmt.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL WHERE (INDICATOR IS NULL OR INDICATOR IN ('INSERT','ADD','')) AND DOC_NO='" + mDocNo + "' ORDER BY SEQUANCE_NO");
                }
            }
            rsTmp.first();
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                Counter = Counter + 1;
                clsWarpingBeamOrderHDSItem ObjMRItems = new clsWarpingBeamOrderHDSItem();
    ...
    public boolean IsEditable(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER WHERE DOC_NO='" + pDocNo + "' AND (APPROVED=1)";
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=783 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getHistoryList(int CompanyID, String DocNo) {
        ...
        try {
            String strSQL = "SELECT DOC_NO,CREATED_DATE,REVISION_NO,UPDATED_BY,APPROVAL_STATUS,DATE_FORMAT(ENTRY_DATE,'%d/%m/%Y %H:%i:%S') AS ENTRY_DATE,REJECTED_REMARKS AS APPROVER_REMARKS,FROM_IP FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER_H WHERE DOC_NO='" + DocNo + "'";
            rsTmp = data.getResult(strSQL);
            rsTmp.first();

            if (rsTmp.getRow() > 0) {
                while (!rsTmp.isAfterLast()) {
                    clsWarpingBeamOrderHDS objParty = new clsWarpingBeamOrderHDS();
    ...
    public boolean Filter(String Condition) {
        ...
        try {
            String strSQL = "";
            strSQL += "SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER WHERE " + Condition;
            Conn = data.getConn();
            Stmt = Conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rsResultSet = Stmt.executeQuery(strSQL);

            if (!rsResultSet.first()) {
                strSQL = "SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER WHERE CREATED_DATE>='" + EITLERPGLOBAL.FinFromDateDB + "' AND CREATED_DATE<='" + EITLERPGLOBAL.FinToDateDB + "' ORDER BY DOC_NO";
                rsResultSet = Stmt.executeQuery(strSQL);
                Ready = true;
                MoveLast();
                return false;
    ...
    public boolean CanDelete(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=783 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND TYPE='C' AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.DOC_NO,PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.CREATED_DATE,RECEIVED_DATE,BEAM_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER,PRODUCTION.FELT_PROD_DOC_DATA WHERE PRODUCTION.FELT_PROD_DOC_DATA.DOC_NO=PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.DOC_NO AND PRODUCTION.FELT_PROD_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND PRODUCTION.FELT_PROD_DOC_DATA.STATUS='W' AND MODULE_ID=783 ORDER BY PRODUCTION.FELT_PROD_DOC_DATA.RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.DOC_NO,PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.CREATED_DATE,RECEIVED_DATE,BEAM_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER,PRODUCTION.FELT_PROD_DOC_DATA WHERE PRODUCTION.FELT_PROD_DOC_DATA.DOC_NO=PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.DOC_NO AND PRODUCTION.FELT_PROD_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND PRODUCTION.FELT_PROD_DOC_DATA.STATUS='W' AND MODULE_ID=783 ORDER BY PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.CREATED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.DOC_NO,PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.CREATED_DATE,RECEIVED_DATE,BEAM_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER,PRODUCTION.FELT_PROD_DOC_DATA WHERE PRODUCTION.FELT_PROD_DOC_DATA.DOC_NO=PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.DOC_NO AND PRODUCTION.FELT_PROD_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND PRODUCTION.FELT_PROD_DOC_DATA.STATUS='W' AND MODULE_ID=783 ORDER BY PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER.DOC_NO";
            }
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                ...
                Counter = Counter + 1;
                clsWarpingBeamOrderHDS ObjItem = new clsWarpingBeamOrderHDS();
    ...
    public boolean ShowHistory(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER_H WHERE DOC_NO='" + pDocNo + "' ORDER BY REVISION_NO");
            Ready = true;
            HistoryView = true;
            MoveFirst();
            return true;
