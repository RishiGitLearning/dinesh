...
public class frmWarpingBeamOrderAmend extends javax.swing.JApplet {
...
//    private static PieChart pieChart;
     ...
     */
    public frmWarpingBeamOrderAmend() {
        // this.requestFocus();
...
//        //chart1.add(dataPaneel);
    ...
    private void initComponents() {
        ...
        jLabel4.setBounds(10, 230, 130, 17);
    ...
    private void TableDescKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_TableDescKeyReleased
        ...
        try {
            ...
            try {
                mthep10 = Double.parseDouble(TableDesc.getValueAt(TableDesc.getSelectedRow(), 14).toString());
            } catch (Exception b) {
                mthep10 = 0.0;
            }
            try {
            ...
            TableDesc.setValueAt(df.format(EITLERPGLOBAL.round((mtpclen * mthep10) * 10, 2)), TableDesc.getSelectedRow(), 15);
            TableDesc.setValueAt(df.format(EITLERPGLOBAL.round((mtpclen * mreedspc), 2)), TableDesc.getSelectedRow(), 16);
    ...
    private void Save() {
        ...
        for (int k = 0; k <= TableDesc.getRowCount() - 1; k++) {
            ...
            if (TableDesc.getValueAt(k, 14).toString().equalsIgnoreCase("")) {
        ...
        for (int k = 0; k <= TableDesc.getRowCount() - 1; k++) {
            for (int l = k; l <= TableDesc.getRowCount() - 1; l++) {
                ...
                if (l != k && (TableDesc.getValueAt(k, 11).toString().trim().equals(TableDesc.getValueAt(l, 11).toString().trim()))) {
...
//                DisplayData();
    ...
    private void DisplayData() {
        ...
        try {
            ...
            for (int i = 1; i <= Obj.colMRItems.size(); i++) {
                ...
                rowData[14] = df.format(EITLERPGLOBAL.round(ObjItem.getAttribute("THEORICAL_PICKS_10_CM").getDouble(), 2));
                rowData[15] = df.format(EITLERPGLOBAL.round(ObjItem.getAttribute("TOTAL_PICKS").getDouble(), 2));
                rowData[16] = df.format(EITLERPGLOBAL.round(ObjItem.getAttribute("EXPECTED_GREV_SQ_MTR").getDouble(), 2));
                mtottpick = mtottpick + Double.parseDouble(rowData[13].toString());
                DataModelDesc.addRow(rowData);
            }
    ...
    private void SetData() {
        ...
        for (int i = 0; i < TableDesc.getRowCount(); i++) {
            ...
            ObjItem.setAttribute("THEORICAL_PICKS_10_CM", Double.parseDouble(TableDesc.getValueAt(i, 14).toString()));
            ObjItem.setAttribute("TOTAL_PICKS", Double.parseDouble(TableDesc.getValueAt(i, 15).toString()));
            ObjItem.setAttribute("EXPECTED_GREV_SQ_MTR", Double.parseDouble(TableDesc.getValueAt(i, 16).toString()));
    ...
    private void UpdateWovenLength() {
        ...
        for (int i = 0; i < TableDesc.getRowCount(); i++) {
            try {
                mtotlen = mtotlen + Double.parseDouble(TableDesc.getValueAt(i, 13).toString());
            } catch (Exception u) {
                mtotlen = mtotlen;
            }
            try {
                mexp_grev = mexp_grev + Double.parseDouble(TableDesc.getValueAt(i, 16).toString());
            } catch (Exception q) {
                mexp_grev = mexp_grev;
            }
        }
        ...
        try {
            mwrplen = Double.parseDouble(txtwarplen.getText());
        } catch (Exception a) {
            mwrplen = 0;
        }
        if (mexp_grev > 0) {
    ...
    private void FormatGrid() {
        ...
        try {
            ...
            DataModelDesc.addColumn("Theo Picks/10 CM");  //14
            DataModelDesc.addColumn("Total Picks");  //15
            DataModelDesc.addColumn("Expected Grev SqMtr");  //16
    ...
    private void DisplayDataBeam(String mBeamno, int mRevisionNo) {
        try {
            ...
            while (!trs.isAfterLast()) {
                ...
                rowData[11] = trs.getInt("SEQUANCE_NO");
                rowData[12] = df.format(EITLERPGLOBAL.round(trs.getDouble("READ_SPACE"), 2));
                rowData[13] = df.format(EITLERPGLOBAL.round(trs.getDouble("THEORICAL_LENGTH_MTR"), 2));
                rowData[14] = df.format(EITLERPGLOBAL.round(trs.getDouble("THEORICAL_PICKS_10_CM"), 2));
                rowData[15] = df.format(EITLERPGLOBAL.round(trs.getDouble("TOTAL_PICKS"), 2));
                rowData[16] = df.format(EITLERPGLOBAL.round(trs.getDouble("EXPECTED_GREV_SQ_MTR"), 2));
                mtottpick = mtottpick + Double.parseDouble(rowData[13].toString());
                DataModelDesc.addRow(rowData);
                trs.next();
            }
