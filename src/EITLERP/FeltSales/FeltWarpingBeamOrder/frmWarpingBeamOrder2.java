...
public class frmWarpingBeamOrder2 extends javax.swing.JApplet {

    private int EditMode = 0;

    //private clsWarpingBeamOrder Obj;
    private clsWarpingBeamOrder Obj;
...
//    private static PieChart pieChart;
    ...
    private void GenerateCombos() {
        ...
        List = clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + clsWarpingBeamOrder.ModuleID + "");

        if (EditMode == EITLERPGLOBAL.EDIT) {
            List = clsHierarchy.getList(" WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + clsWarpingBeamOrder.ModuleID + "");
        }
    ...
    private void initComponents() {
        ...
        lblTitle.setText("Felt Warping Beam Order");
        ...
        jLabel23.setBounds(250, 230, 110, 20);

        jLabel24.setText("Length");
        getContentPane().add(jLabel24);
        jLabel24.setBounds(250, 240, 90, 20);
        ...
        txtrwvglen.setBounds(350, 230, 120, 30);

        jLabel25.setText("Weaving");
        getContentPane().add(jLabel25);
        jLabel25.setBounds(30, 230, 90, 20);

        jLabel26.setText("Length");
        getContentPane().add(jLabel26);
        jLabel26.setBounds(30, 240, 70, 20);
        ...
        txtwvglen.setBounds(130, 230, 120, 30);
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            ...
            if (clsFeltProductionApprovalFlow.IsOnceRejectedDoc(clsWarpingBeamOrder.ModuleID, DocNo)) {
                cmbSendTo.setEnabled(true);
            } else {
                cmbSendTo.setEnabled(false);
            }
    ...
    private void txtselewarpdtlKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtselewarpdtlKeyPressed
        if (evt.getKeyCode() == 112) //F1 Key pressed
        {
            ...
            aList.SQL = "SELECT * FROM PRODUCTION.FELT_WARPING_SELECTION_DATA WHERE TYPE NOT IN ('SDF','HDS')";
            aList.ReturnCol = 1;
            ...
            if ((EditMode == 1) || (EditMode == 2 && clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID))) {
                if (TableDesc.getSelectedColumn() == 1) {
                    ...
                    aList.SQL = "SELECT WIP_EXT_PIECE_NO AS PIECE_NO,WIP_PIECE_NO AS PIECE,DATE_FORMAT(WIP_ORDER_DATE,'%d/%m/%Y') AS ORDER_DATE,WIP_UPN AS UPN,WIP_MACHINE_NO AS MACHINE_NO,"
                            ...
                            + " WHERE WIP_PIECE_STAGE='PLANNING' AND WIP_GROUP NOT IN ('HDS') AND WIP_PRIORITY_HOLD_CAN_FLAG=0 AND COALESCE(WIP_REJECTED_FLAG,0)=0 AND "
...
//        wp.WarpSort(TableDesc);
    ...
    private void Add() {
        ...
        if (aList.ShowList()) {
            ...
            DOC_NO.setText(clsFirstFree.getNextFreeNo(EITLERPGLOBAL.gCompanyID, 777, FFNo, false));
    ...
    private void SetupApproval() {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = clsFeltProductionApprovalFlow.getFromID(clsWarpingBeamOrder.ModuleID, (String) Obj.getAttribute("PROFORMA_NO").getObj());
            lnFromID = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = clsFeltProductionApprovalFlow.getFromRemarks(clsWarpingBeamOrder.ModuleID, FromUserID, (String) Obj.getAttribute("PROFORMA_NO").getObj());
    ...
    private void GenerateFromCombo() {
        ...
        try {
            if (EditMode == EITLERPGLOBAL.ADD) {
                ...
                List = clsFeltProductionApprovalFlow.getRemainingUsers(clsWarpingBeamOrder.ModuleID, DocNo);
                for (int i = 1; i <= List.size(); i++) {
                    clsUser ObjUser = (clsUser) List.get(Integer.toString(i));
                    ComboData aData = new ComboData();
                    aData.Code = (int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void ClearFields() {
        ...
        txtwvglen.setText("0.00");
        txtrwvglen.setText("0.00");
        FormatGrid();
        FormatGridA();
        FormatGridHS();
    }
    ...
    private void Edit() {
        ...
        if (Obj.IsEditable(EITLERPGLOBAL.gCompanyID, lDocNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(clsWarpingBeamOrder.ModuleID, lDocNo)) {  // || clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6070, 60702)) {
                SetFields(true);
            } else {
                EnableApproval();
            }
...
//                        String Party_Code = "";
//
//                        String responce = JavaMail.sendFinalApprovalMail(clsWarpingBeamOrder.ModuleID, DOC_NO, DOC_DATE, Party_Code, EITLERPGLOBAL.gNewUserID, SelHierarchyID, true);
...
//                DisplayData();
    ...
    private void DisplayData() {
        ...
        if (EITLERPGLOBAL.gAuthorityUserID != EITLERPGLOBAL.gUserID) {
            int ModuleID = clsWarpingBeamOrder.ModuleID;
        ...
        try {
            ...
            for (int i = 1; i <= Obj.colMRItems.size(); i++) {
                ...
                clsWarpingBeamOrderItem ObjItem = (clsWarpingBeamOrderItem) Obj.colMRItems.get(Integer.toString(i));
            ...
            try {
                ResultSet t;
                t = data.getResult("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DETAIL WHERE INDICATOR IN ('ADD','DELETE') AND DOC_NO='" + DOC_NO.getText() + "' ORDER BY INDICATOR");
                t.first();
                if (t.getRow() > 0) {
                    while (!t.isAfterLast()) {
                        Object[] rowData = new Object[50];
            ...
            List = clsFeltProductionApprovalFlow.getDocumentFlow(clsWarpingBeamOrder.ModuleID, DocNo);
            for (int i = 1; i <= List.size(); i++) {
            ...
            FormatGridHS();
            HashMap History = clsWarpingBeamOrder.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsWarpingBeamOrder ObjHistory = (clsWarpingBeamOrder) History.get(Integer.toString(i));
    ...
    private void SetData() {
        ...
        for (int i = 0; i < TableDesc.getRowCount(); i++) {
            clsWarpingBeamOrderItem ObjItem = new clsWarpingBeamOrderItem();
    ...
    public void FindWaiting() {
        Obj.Filter(" DOC_NO IN (SELECT H.DOC_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_HEADER H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=777 AND CANCELED=0)");
        Obj.MoveFirst();
        DisplayData();
    }
    ...
    private void GenerateRejectedUserCombo() {
        ...
        for (int i = 1; i <= List.size(); i++) {
            ...
            if (EditMode == EITLERPGLOBAL.EDIT) {
                if (OpgApprove.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInApproval(clsWarpingBeamOrder.ModuleID, DocNo, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }

                if (OpgReject.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInRejection(clsWarpingBeamOrder.ModuleID, DocNo, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            ...
            int Creator = clsFeltProductionApprovalFlow.getCreator(clsWarpingBeamOrder.ModuleID, DocNo);
    ...
    private boolean chkbeamno() {
        int a = data.getIntValueFromDB("SELECT COUNT(*) FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_HEADER WHERE BEAM_NO='" + txtbeam.getText() + "' AND APPROVED=1");
        if (a > 0) {
    ...
    private void Mail() {
        ...
        if (Obj.getAttribute("APPROVAL_STATUS").getString().equalsIgnoreCase("F")) {
            ...
            try {
                ...
                HashMap hmApprovalHistory = clsWarpingBeamOrder.getHistoryList(EITLERPGLOBAL.gCompanyID, DOC_NO.getText());
                for (int i = 1; i <= hmApprovalHistory.size(); i++) {
                    pBody += "<tr>";

                    clsWarpingBeamOrder ObjWarping = (clsWarpingBeamOrder) hmApprovalHistory.get(Integer.toString(i));
                    Object[] rowData = new Object[6];
                ...
                String responce = MailNotification.sendNotificationMail(777, pSubject, pBody, recievers, pcc, EITLERPGLOBAL.getComboCode(cmbHierarchy));
                System.out.println("Send Mail Responce : " + responce);
