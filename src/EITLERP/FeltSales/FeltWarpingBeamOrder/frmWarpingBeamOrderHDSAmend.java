...
public class frmWarpingBeamOrderHDSAmend extends javax.swing.JApplet {

    private int EditMode = 0;

    //private clsWarpingBeamOrderHDSAmend Obj;
    private clsWarpingBeamOrderHDSAmend Obj;
...
//    private static PieChart pieChart;
     ...
     */
    public frmWarpingBeamOrderHDSAmend() {
        // this.requestFocus();
...
//        //chart1.add(dataPaneel);
        ...
        Obj = new clsWarpingBeamOrderHDSAmend();
    ...
    private void GenerateCombos() {
        ...
        List = clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + clsWarpingBeamOrderHDSAmend.ModuleID + "");

        if (EditMode == EITLERPGLOBAL.EDIT) {
            List = clsHierarchy.getList(" WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + clsWarpingBeamOrderHDSAmend.ModuleID + "");
        }
    ...
    private void initComponents() {
        ...
        lblTitle.setText("Felt Warping Beam Order Amend [HDS]");
        ...
        jTabbedPane1.setBounds(5, 310, 750, 340);
        ...
        jScrollPane5.setBounds(130, 220, 380, 70);
                ...
                if (TableDesc.getSelectedColumn() == 2) {
                    try {
                        sql = "SELECT * FROM PRODUCTION.FELT_WIP_PIECE_REGISTER "
                                ...
                                + "(SELECT PIECE_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL WHERE BEAM_NO='" + txtbeam.getText() + "' AND PIECE_NO='" + PieceNo + "')";
                        trs = data.getResult(sql);

                        //trs = data.getResult("SELECT * FROM PRODUCTION.FELT_SALES_PIECE_REGISTER WHERE SUBSTRING(PR_PIECE_NO,1,5) = '" + PieceNo.substring(0, 5) + "' AND PR_PIECE_STAGE='PLANNING' AND (PR_WARP_DATE='0000-00-00' OR PR_WARP_DATE IS NULL) AND PR_PIECE_NO NOT IN (SELECT LEFT(PIECE_NO,5) FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL_AMEND)");
                    } catch (Exception a) {
                    ...
                    if (trs.getRow() > 0) {
                        ...
                        if (PieceNo.contains("A") || PieceNo.contains("a")) {
                            ...
                            sql = "SELECT PIECE_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL_AMEND "
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            ...
            if (clsFeltProductionApprovalFlow.IsOnceRejectedDoc(clsWarpingBeamOrderHDSAmend.ModuleID, DocNo)) {
                cmbSendTo.setEnabled(true);
            } else {
                cmbSendTo.setEnabled(false);
            }
            ...
            if ((EditMode == 1) || (EditMode == 2 && clsHierarchy.IsCreator(EITLERPGLOBAL.gCompanyID, SelHierarchyID, EITLERPGLOBAL.gUserID))) {
                if (TableDesc.getSelectedColumn() == 1) {
                    ...
                    aList.SQL = "SELECT WIP_EXT_PIECE_NO AS PIECE_NO,WIP_PIECE_NO AS PIECE,DATE_FORMAT(WIP_ORDER_DATE,'%d/%m/%Y') AS ORDER_DATE,WIP_UPN AS UPN,WIP_MACHINE_NO AS MACHINE_NO,"
                            ...
                            + " UNION ALL "
                            + " SELECT WIP_EXT_PIECE_NO AS PIECE_NO,WIP_PIECE_NO,DATE_FORMAT(WIP_ORDER_DATE,'%d/%m/%Y') AS ORDER_DATE,WIP_UPN AS UPN,WIP_MACHINE_NO AS MACHINE_NO,"
                            + " WIP_POSITION_NO AS POSITION_NO,WIP_PARTY_CODE AS PARTY_CODE,M.PARTY_NAME,WIP_GROUP AS PRODUCT_GROUP,WIP_PRODUCT_CODE AS PRODUCT_CODE,"
                            + " WIP_STYLE AS STYLE,"
                            + "COALESCE(AMEND_WIP.PIECE_AMEND_NO,'') AS PENDING_WIP_AMEND "
                            + ",CASE WHEN PARTY_LOCK=1 THEN 'LOCKED' ELSE '' END AS PARTY_LOCK "
                            ...
                            + " WIP_PIECE_STAGE='PLANNING' AND WIP_PRIORITY_HOLD_CAN_FLAG=0 AND COALESCE(WIP_REJECTED_FLAG,0)=0 AND "
                            + " WIP_EXT_PIECE_NO IN "
                            + "(SELECT PIECE_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL WHERE BEAM_NO='" + txtbeam.getText() + "' AND COALESCE(INDICATOR,'')!='DELETE' )"
                        ...
                        if (!data.getStringValueFromDB("SELECT PD.PIECE_NO FROM PRODUCTION.FELT_SALES_PIECE_AMENDMENT_HEADER_WIP PH, "
                            ...
                            try {
                                ...
                                String sql = "SELECT * FROM PRODUCTION.FELT_WIP_PIECE_REGISTER "
                                        ...
                                        + "WIP_PIECE_STAGE='PLANNING' AND (WIP_WARP_DATE='0000-00-00' OR WIP_WARP_DATE IS NULL) "
                                        + "AND (WIP_EXT_PIECE_NO NOT IN (SELECT PIECE_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL)) "
                                        + "OR WIP_EXT_PIECE_NO IN (SELECT DISTINCT D.PIECE_NO FROM ("
    ...
    private void Add() {
        ...
        aList.SQL = "SELECT BEAM_NO,DOC_NO,DATE_FORMAT(CREATED_DATE,'%d/%m/%Y') AS DOC_DATE FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER WHERE APPROVED=1 AND CANCELED=0  AND COALESCE(BEAM_CLOSURE_IND,0)=0 ";
        aList.ReturnCol = 1;
        aList.ShowReturnCol = true;
        aList.DefaultSearchOn = 1;
        ...
        if (aList.ShowLOV()) {
            ...
            if (clsWarpingBeamOrderHDSAmend.IsAmendmentUnderProcess(BeamNo)) {
            ...
            int MaxNo = data.getIntValueFromDB("SELECT COALESCE(MAX(RIGHT(DOC_NO,1)),0) FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER_AMEND WHERE BEAM_NO='" + BeamNo + "'") + 1;
    ...
    private void SetupApproval() {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = clsFeltProductionApprovalFlow.getFromID(clsWarpingBeamOrderHDSAmend.ModuleID, (String) Obj.getAttribute("PROFORMA_NO").getObj());
            lnFromID = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = clsFeltProductionApprovalFlow.getFromRemarks(clsWarpingBeamOrderHDSAmend.ModuleID, FromUserID, (String) Obj.getAttribute("PROFORMA_NO").getObj());
    ...
    private void GenerateFromCombo() {
        ...
        try {
            if (EditMode == EITLERPGLOBAL.ADD) {
                ...
                List = clsFeltProductionApprovalFlow.getRemainingUsers(clsWarpingBeamOrderHDSAmend.ModuleID, DocNo);
                for (int i = 1; i <= List.size(); i++) {
                    clsUser ObjUser = (clsUser) List.get(Integer.toString(i));
                    ComboData aData = new ComboData();
                    aData.Code = (int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void Edit() {
        ...
        if (Obj.IsEditable(EITLERPGLOBAL.gCompanyID, lDocNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(clsWarpingBeamOrderHDSAmend.ModuleID, lDocNo)) {  // || clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 60032, 600322)) {
                SetFields(true);
            } else {
                EnableApproval();
            }
...
//                        String Party_Code = "";
//
//                        String responce = JavaMail.sendFinalApprovalMail(clsWarpingBeamOrderHDSAmend.ModuleID, DOC_NO, DOC_DATE, Party_Code, EITLERPGLOBAL.gNewUserID, SelHierarchyID, true);
...
//                DisplayData();
    ...
    private void DisplayData() {
        ...
        if (EITLERPGLOBAL.gAuthorityUserID != EITLERPGLOBAL.gUserID) {
            int ModuleID = clsWarpingBeamOrderHDSAmend.ModuleID;
        ...
        try {
            ...
            String msql = "SELECT H.DOC_NO,ACTUAL_WARP_RELISATION,WARP_LENGTH,FABRIC_REALISATION_PER,SUM(THEORICAL_LENGTH_MTR) AS TOTAL_PC_LEN "
                    + "FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER H,PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL D "
            ...
            List = clsFeltProductionApprovalFlow.getDocumentFlow(clsWarpingBeamOrderHDSAmend.ModuleID, DocNo);
            for (int i = 1; i <= List.size(); i++) {
            ...
            FormatGridHS();
            HashMap History = clsWarpingBeamOrderHDSAmend.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsWarpingBeamOrderHDSAmend ObjHistory = (clsWarpingBeamOrderHDSAmend) History.get(Integer.toString(i));
    ...
    private void SetMenuForRights() {
        // --- Add Rights --        
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 60032, 600321)) {
            cmdNew.setEnabled(true);
        } else {
            cmdNew.setEnabled(false);
        }
        ...
        // --- Edit Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 60032, 600322)) {
            cmdEdit.setEnabled(true);
        } else {
            cmdEdit.setEnabled(false);
        }
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 60032, 600323)) {
            cmdDelete.setEnabled(true);
        } else {
            cmdDelete.setEnabled(false);
        }
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 60032, 600325)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        } else {
            cmdPreview.setEnabled(false);
    ...
    public void FindWaiting() {
        Obj.Filter(" DOC_NO IN (SELECT H.DOC_NO FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER_AMEND H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=784 AND CANCELED=0)");
        Obj.MoveFirst();
        DisplayData();
    }
    ...
    private void GenerateRejectedUserCombo() {
        ...
        for (int i = 1; i <= List.size(); i++) {
            ...
            if (EditMode == EITLERPGLOBAL.EDIT) {
                if (OpgApprove.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInApproval(clsWarpingBeamOrderHDSAmend.ModuleID, DocNo, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }

                if (OpgReject.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInRejection(clsWarpingBeamOrderHDSAmend.ModuleID, DocNo, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            ...
            int Creator = clsFeltProductionApprovalFlow.getCreator(clsWarpingBeamOrderHDSAmend.ModuleID, DocNo);
    ...
    private void UpdateWovenLength() {
        ...
        int mseries = 1, i = 0;
        //mseries=Integer.parseInt(TableDesc.getValueAt(0, 14).toString());
    ...
    private void PreviewReport() {
        ...
        try {
            ...
            String strSQL = "SELECT * FROM (SELECT H.DOC_NO,H.BEAM_NO,H.LOOM_NO,H.REED_SPACE,H.WARP_DETAIL,H.WARP_TEX,(H.REED_SPACE*H.ENDS_10_CM)*10 AS WARP_END_TOTAL,H.ENDS_10_CM,H.ACTUAL_WARP_RELISATION,H.WARP_LENGTH,H.REED_COUNT, "
                    ...
                    + " FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL_AMEND D,PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER_AMEND H "
                    + "WHERE H.DOC_NO=D.DOC_NO AND "
                    + "H.DOC_NO='" + DOC_NO.getText() + "') AS A "
                    + "LEFT JOIN (SELECT DOC_NO,SUM(THEORICAL_LENGTH_MTR) AS TOTAL_PC_LENGTH FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL_AMEND WHERE DOC_NO='" + DOC_NO.getText() + "') AS B "
    ...
    private boolean chkbeamno() {
        int a = data.getIntValueFromDB("SELECT COUNT(*) FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER_AMEND WHERE BEAM_NO='" + txtbeam.getText() + "'");
        if (a > 0) {
    ...
    private void DisplayDataBeam(String mBeamno, int mRevisionNo) {
        try {
            ResultSet trs = null;
            //if (mRevisionNo == 0) {
            trs = data.getResult("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER WHERE BEAM_NO='" + mBeamno + "'");
            //} else {
            //    trs = data.getResult("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_HEADER_AMEND WHERE BEAM_NO='" + mBeamno + "' AND RIGHT(DOC_NO,1)=" + mRevisionNo);
            //}
            trs.first();
            ...
            //Now Generate Table
            if (mRevisionNo == 0) {
                trs = data.getResult("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL WHERE BEAM_NO='" + mBeamno + "' ORDER BY SR_NO");
            } else {
                trs = data.getResult("SELECT * FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL_AMEND WHERE BEAM_NO='" + mBeamno + "' AND RIGHT(DOC_NO,1)=" + mRevisionNo + "  ORDER BY SR_NO");
            }
            trs.first();
            double mtottpick = 0;
            while (!trs.isAfterLast()) {
                ...
                rowData[14] = trs.getInt("SERIES");
                rowData[15] = trs.getString("SUB_SERIES");
                if (trs.getString("TOTAL").equalsIgnoreCase("0.00")) {
                    rowData[16] = "";
                } else {
                    rowData[16] = df.format(EITLERPGLOBAL.round(trs.getDouble("TOTAL"), 2));;
                }
                if (trs.getString("MAX_THEORICAL_LENGTH").equalsIgnoreCase("0.00")) {
                    rowData[17] = "";
                } else {
                    rowData[17] = df.format(EITLERPGLOBAL.round(trs.getDouble("MAX_THEORICAL_LENGTH"), 2));;
                }
                ...
                try {
                    rowData[21] = EITLERPGLOBAL.formatDate(trs.getString("WEAVING_DATE"));
                } catch (Exception e) {
                    rowData[21] = "";
                }
                mtottpick = mtottpick + Double.parseDouble(trs.getString("MAX_THEORICAL_LENGTH"));
                DataModelDesc.addRow(rowData);
                trs.next();
            }
            txttotpclen.setText(df.format(EITLERPGLOBAL.round(mtottpick, 2)));
    ...
    private void Mail() {
        ...
        if (Obj.getAttribute("APPROVAL_STATUS").getString().equalsIgnoreCase("F")) {
            ...
            try {
                ...
                tmrsData = tmstmt.executeQuery("SELECT  D.*,PARTY_NAME FROM PRODUCTION.FELT_WARPING_BEAM_ORDER_DRYER_DETAIL_AMEND D LEFT JOIN DINESHMILLS.D_SAL_PARTY_MASTER P ON D.PARTY_CODE=P.PARTY_CODE WHERE DOC_NO='" + DOC_NO.getText() + "' ORDER BY SEQUANCE_NO ");
                tmrsData.first();
                pBody += "<br>";
                ...
                HashMap hmApprovalHistory = clsWarpingBeamOrderHDSAmend.getHistoryList(EITLERPGLOBAL.gCompanyID, DOC_NO.getText());
                for (int i = 1; i <= hmApprovalHistory.size(); i++) {
                    pBody += "<tr>";

                    clsWarpingBeamOrderHDSAmend ObjWarping = (clsWarpingBeamOrderHDSAmend) hmApprovalHistory.get(Integer.toString(i));
                    Object[] rowData = new Object[6];
                ...
                String responce = MailNotification.sendNotificationMail(784, pSubject, pBody, recievers, pcc, EITLERPGLOBAL.getComboCode(cmbHierarchy));
                System.out.println("Send Mail Responce : " + responce);
