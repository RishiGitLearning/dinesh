 ...
 */
public class clsFeltEvaluationClosure {
     ...
     */
    public clsFeltEvaluationClosure() {
        LastError = "";
        props = new HashMap();
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT * FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE GROUP BY DOC_NO ORDER BY DOC_NO");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE DOC_NO='1'");

            // Production data history connection
            statementHistory = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetHistory = statementHistory.executeQuery("SELECT * FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE_H WHERE DOC_NO='1'");

            setAttribute("DOC_NO", getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 611, 261, true));
            ...
            for (int i = 1; i <= hmFeltEvaluationDetails.size(); i++) {
                ...
                resultSetTemp.updateString("CANCEL_FLAG", ObjFeltEvaluationDetails.getAttribute("CANCEL_FLAG").getString());
                ...
                resultSetHistory.updateString("CANCEL_FLAG", ObjFeltEvaluationDetails.getAttribute("CANCEL_FLAG").getString());
            ...
            ObjFeltProductionApprovalFlow.ModuleID = 611;
            ...
            ObjFeltProductionApprovalFlow.TableName = "PRODUCTION.FELT_SALES_EVALUATION_CLOSURE";
...
//            resultSetHistory=statementHistory.executeQuery("SELECT MAX(REVISION_NO) FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE_H WHERE DOC_NO='"+getAttribute("DOC_NO").getString()+"'");
...
//            }
//            
            int RevNo = EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE_H WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
            RevNo++;

            //delete records from production data table before insert
            statementHistory.execute("DELETE FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE PRODUCTION.FELT_SALES_EVALUATION_CLOSURE SET REJECTED=0,CHANGED=1 WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
...
//                updatePieceRegisterS(getAttribute("PRODUCTION_DOCUMENT_NO").getString());
            ...
            if (ObjFeltProductionApprovalFlow.Status.equals("F") && ObjFeltProductionApprovalFlow.finalApproved) {
                ...
                try {
                    ...
                    ResultSet rsTemp = data.getConn().createStatement().executeQuery("SELECT * FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE DOC_NO = '" + getAttribute("DOC_NO").getString() + "' AND CANCEL_FLAG='1' ");
                    System.out.println("PIECE UPDATION ON FINAL APPROVAL");
                    while (rsTemp.next()) {
                        data.Execute("UPDATE PRODUCTION.FELT_SALES_PIECE_REGISTER P,PRODUCTION.FELT_SALES_EVALUATION_CLOSURE D SET P.PR_PIECE_STAGE='CANCELED',P.PR_WIP_STATUS='CANCELED',P.PR_PRIORITY_HOLD_CAN_FLAG=9 WHERE P.PR_PIECE_NO = '" + rsTemp.getString("PIECE_NO") + "' ");
                        System.out.println("PIECE UPDATED ON PIECE NO : " + rsTemp.getString("PIECE_NO"));
                    }
    ...
    public boolean CanDelete(String docNo, int userID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE DOC_NO='" + docNo + "' AND  APPROVED=1";
            ...
            if (rsTmp.getInt("COUNT") > 0) {  //Group is Approved
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=611 AND USER_ID=" + userID + " AND DOC_NO='" + getAttribute("DOC_NO").getString() + "' AND STATUS='W'";
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();

                if (rsTmp.getInt("COUNT") > 0) {
                    strSQL = "DELETE FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE DOC_NO='" + docNo + "'";
    ...
    public boolean IsEditable(String docNo, int userID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) {  //Item is Approved
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=611 AND USER_ID=" + userID + " AND DOC_NO='" + docNo + "' AND STATUS='W'";
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean Filter(String stringFindQuery) {
        Ready = false;
        try {
            String strSql = "SELECT * FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE " + stringFindQuery;
            connection = data.getConn();
            statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean setData() {
        ...
        try {
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            if (HistoryView) {
                resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.FELT_SALES_EVALUATION_CLOSURE_H WHERE DOC_NO='" + resultSet.getString("DOC_NO") + "'  AND REVISION_NO=" + RevNo);
            } else {
                resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE DOC_NO='" + resultSet.getString("DOC_NO") + "' ORDER BY DOC_NO,SR_NO ");
            }
            resultSetTemp.first();
            ...
            while (!resultSetTemp.isAfterLast()) {
                ...
                ObjFeltEvaluationDetails.setAttribute("CANCEL_FLAG", resultSetTemp.getString("CANCEL_FLAG"));
    ...
    public boolean ShowHistory(String docNo) {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT * FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE_H WHERE DOC_NO='" + docNo + "'");
            Ready = true;
            HistoryView = true;
            MoveFirst();
            return true;
    ...
    public static HashMap getHistoryList(String dpDate, String dpNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT * FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE_H WHERE DOC_NO='" + dpNo + "' GROUP BY REVISION_NO ");

            while (rsTmp.next()) {
                clsFeltEvaluationClosure ObjFeltGroup = new clsFeltEvaluationClosure();
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            ...
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT DISTINCT H.DOC_NO,H.DOC_DATE,RECEIVED_DATE FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=611 AND CANCELED=0 ORDER BY RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT DISTINCT H.DOC_NO,H.DOC_DATE,RECEIVED_DATE FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=611 AND CANCELED=0 ORDER BY DOC_NO";
            }
            ...
            while (rsTmp.next()) {
                ...
                Counter = Counter + 1;
                clsFeltEvaluationClosure ObjDoc = new clsFeltEvaluationClosure();
    ...
    public static void CancelDoc(String pDocNo) {
        ...
        if (CanCancel(pDocNo)) {
            ...
            try {
                rsTmp = data.getResult("SELECT APPROVED FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE DOC_NO='" + pDocNo + "' ");
                rsTmp.first();
                ...
                if (!Approved) {
                    data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE DOC_NO='" + pDocNo + "' AND MODULE_ID=611");
                }
                data.Execute("UPDATE PRODUCTION.FELT_SALES_EVALUATION_CLOSURE SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE DOC_NO='" + pDocNo + "'");
    ...
    public static boolean CanCancel(String pDocNo) {
        ...
        try {
            ...
            System.out.println("SELECT DOC_NO FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
            rsTmp = stTmp.executeQuery("SELECT DOC_NO FROM PRODUCTION.FELT_SALES_EVALUATION_CLOSURE WHERE DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
    ...
    public HashMap getPieceList(String pDocNo, String pPartyCode) {
        ...
        try {
            ...
            while (resultSet.next()) {
                clsFeltEvaluationClosure piece = new clsFeltEvaluationClosure();
                boolean flag = true;
