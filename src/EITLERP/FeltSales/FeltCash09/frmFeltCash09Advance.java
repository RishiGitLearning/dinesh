...
public class frmFeltCash09Advance extends javax.swing.JApplet {
    ...
    private void initComponents() {
        ...
        lblTitle.setText("FELT CASH 09 ADVANCE CREDIT NOTE PROCESS & PRINTING FORM");
        ...
        jLabel2.setText("Felt Cash 09 Advance Credit Note Process");
        ...
        Processcash.setText("1. Process Felt Cash 09 Discount Advance");
        ...
        jButton7.setBounds(20, 470, 200, 25);
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        ...
        if (txtDraftDate.getText().trim().equals("")) {
            ...
            try {
                ...
                if (data.IsRecordExist("SELECT VOUCHER_NO FROM D_FIN_VOUCHER_HEADER WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + "  AND LEFT(VOUCHER_NO,4) = 'CN21'", FinanceGlobal.FinURL)) {
                    ...
                    rsResultSet = Stmt.executeQuery("SELECT VOUCHER_NO,SUBSTRING(VOUCHER_NO,1,6) AS CNV,RIGHT(VOUCHER_NO,5) AS CNSRNO FROM (SELECT MAX(VOUCHER_NO) AS VOUCHER_NO FROM FINANCE.D_FIN_VOUCHER_HEADER WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + "  AND LEFT(VOUCHER_NO,4) = 'CN21' AND SUBSTRING(VOUCHER_NO,5,2)=14 ) AS A");
                    String LastDocNo = "";
                    String LastcnvNo = "";
                    int draftSrNo = 0;
...
//                String draftDate="2016-01-31";
                    ...
                    ResultSet rsSchemeHeader = Stmt.executeQuery("SELECT CNH_NO,CNH_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE  CNH_YEAR_MON_ID =SUBSTRING('" + txtInterval.getText() + "',1,15) AND CNH_TYPE ='FELTCASHADV' ORDER BY CNH_SUB_ACCOUNT_CODE,CNH_NO");
                    ...
                    if (rsSchemeHeader.getRow() > 0) {
                        while (!rsSchemeHeader.isAfterLast()) {
                            Stmt.executeUpdate("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_DRAFT_CR_NOTE_NO='" + draftNo + (draftSrNo + counter) + "',CNH_LINK_NO='" + linkNo + "',CNH_DRAFT_CR_NOTE_DATE='" + draftDate + "' WHERE CNH_TYPE ='FELTCASHADV' AND CNH_YEAR_MON_ID =SUBSTRING('" + txtInterval.getText() + "',1,15) AND  CNH_NO='" + rsSchemeHeader.getString("CNH_NO") + "'");
                            Stmt.execute("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER  SET CNH_FIN_VOUCHER_NO = CONCAT('CN',211400000 +ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))),CNH_LEGACY_NO = ABS(REPLACE(RIGHT( CNH_DRAFT_CR_NOTE_NO,4),' ',''))  WHERE CNH_TYPE ='FELTCASHADV' AND CNH_YEAR_MON_ID =SUBSTRING('" + txtInterval.getText() + "',1,15) AND  CNH_NO='" + rsSchemeHeader.getString("CNH_NO") + "'");
//                        rsSchemeHeader.next();
//                        counter++;

                            int counter1 = 1;
                            ResultSet rsSchemedetail = Stmt.executeQuery("SELECT CONCAT(CND_NO,CND_SUB_ACCOUNT_CODE,CND_EFFECT,CND_INVOICE_NO,CND_INVOICE_DATE) AS CND_PO FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE  CND_YEAR_MON_ID =SUBSTRING('" + txtInterval.getText() + "',1,15) AND CND_TYPE ='FELTCASHADV' AND  CND_NO='" + rsSchemeHeader.getString("CNH_NO") + "' ORDER BY CND_NO,CND_EFFECT");
                            rsSchemedetail.first();
                            if (rsSchemedetail.getRow() > 0) {
                                while (!rsSchemedetail.isAfterLast()) {
                                    Stmt.executeUpdate("UPDATE PRODUCTION.D_CREDIT_NOTE_DETAIL SET CND_SRNO='" + counter1 + "' WHERE CND_TYPE ='FELTCASHADV' AND CND_YEAR_MON_ID =SUBSTRING('" + txtInterval.getText() + "',1,15) AND  CONCAT(CND_NO,CND_SUB_ACCOUNT_CODE,CND_EFFECT,CND_INVOICE_NO,CND_INVOICE_DATE)='" + rsSchemedetail.getString("CND_PO") + "'");
                ...
                String SQL = "INSERT INTO TEMP_DATABASE.L_FIN_VOUCHER_HEADER (COMPANY_ID,VOUCHER_NO,BOOK_CODE,BANK_NAME,HIERARCHY_ID,REMARKS,VOUCHER_DATE,VOUCHER_TYPE,EXCLUDE_IN_ADJ) SELECT 2,CNH_FIN_VOUCHER_NO,14,'DISCOUNT JOURNAL',2034,CNH_REMARKS,CNH_DRAFT_CR_NOTE_DATE,9,1 FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_TYPE ='FELTCASHADV' AND CNH_YEAR_MON_ID = SUBSTRING('" + txtInterval.getText() + "',1,15)";
                System.out.println(SQL);
                Stmt.execute("INSERT INTO TEMP_DATABASE.L_FIN_VOUCHER_HEADER (COMPANY_ID,VOUCHER_NO,BOOK_CODE,BANK_NAME,HIERARCHY_ID,REMARKS,VOUCHER_DATE,VOUCHER_TYPE,EXCLUDE_IN_ADJ,LEGACY_NO,LEGACY_DATE) SELECT 2,CNH_FIN_VOUCHER_NO,14,'DISCOUNT JOURNAL',2034,CNH_REMARKS,CNH_DRAFT_CR_NOTE_DATE,9,1,CNH_LEGACY_NO,CNH_DRAFT_CR_NOTE_DATE FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_TYPE ='FELTCASHADV' AND CNH_YEAR_MON_ID = SUBSTRING('" + txtInterval.getText() + "',1,15)");

                Stmt.execute("TRUNCATE TABLE TEMP_DATABASE.L_FIN_VOUCHER_DETAIL");

                Stmt.execute("INSERT INTO TEMP_DATABASE.L_FIN_VOUCHER_DETAIL (COMPANY_ID,VOUCHER_NO,EFFECT,MAIN_ACCOUNT_CODE,SUB_ACCOUNT_CODE,AMOUNT,MODULE_ID,REF_COMPANY_ID,IS_DEDUCTION,VALUE_DATE,LINK_NO,PO_NO,PO_DATE,INVOICE_AMOUNT,SR_NO) SELECT 2,CNH_FIN_VOUCHER_NO,'C' AS CND_EFFECT,CNH_MAIN_CODE,CNH_SUB_ACCOUNT_CODE,CND_CREDIT_AMOUNT+CND_DEBIT_AMOUNT,66,2,0,CNH_DRAFT_CR_NOTE_DATE,CND_LINK_NO,CND_INVOICE_NO,CND_INVOICE_DATE,CND_INVOICE_AMOUNT,CND_SRNO  FROM PRODUCTION.D_CREDIT_NOTE_HEADER H,PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CNH_TYPE ='FELTCASHADV' AND CNH_YEAR_MON_ID = SUBSTRING('" + txtInterval.getText() + "',1,15) AND CNH_TYPE = CND_TYPE AND CND_NO = CNH_NO AND CNH_YEAR_MON_ID = CND_YEAR_MON_ID  UNION SELECT 2,CNH_FIN_VOUCHER_NO,CNH_EFFECT,CNH_MAIN_ACCOUNT_CODE,'',CNH_CREDIT_AMOUNT+CNH_DEBIT_AMOUNT,0,0,0,CNH_DRAFT_CR_NOTE_DATE,'','','0000-00-00',0,CNH_SRNO  FROM PRODUCTION.D_CREDIT_NOTE_HEADER H WHERE CNH_TYPE ='FELTCASHADV' AND CNH_YEAR_MON_ID = SUBSTRING('" + txtInterval.getText() + "',1,15) AND  CNH_MAIN_ACCOUNT_CODE = '435077' ");
    ...
    private void generateIntervalCombo() {
        ...
        try {
            ...
            ResultSet rs = data.getResult("SELECT DISTINCT CND_YEAR_MON_ID,CND_INTERVAL_CAPTION FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_TYPE ='FELTCASHADV' ORDER BY CND_YEAR_MON_ID DESC ");
            while (!rs.isAfterLast()) {
    ...
    private void processLCDiscount() {
        ...
        try {
            ...
            stmt.execute("TRUNCATE TABLE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE");

            stmt.execute("INSERT INTO PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SELECT A.VOUCHER_NO,VOUCHER_DATE,MAIN_ACCOUNT_CODE,SUB_ACCOUNT_CODE,EFFECT,INVOICE_NO,INVOICE_DATE,INVOICE_AMOUNT,AMOUNT,GRN_NO,GRN_DATE, '0000-00-00',A.LINK_NO,'0000-00-00','0000-00-00','0000-00-00',MATCHED_DATE,MATCHED,  '','','',    '','','',    '','','',    '','','',    '','','',    '','','',    '','','',  BOOK_CODE,'',0,0,0,0,0,SUB_ACCOUNT_CODE,'','','','','','','','' FROM FINANCE.D_FIN_VOUCHER_DETAIL A,FINANCE.D_FIN_VOUCHER_HEADER B WHERE A.VOUCHER_NO = B.VOUCHER_NO AND B.CANCELLED =0 AND B.APPROVED =1 AND VOUCHER_DATE >='" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "'  AND  VOUCHER_DATE <='" + EITLERPGLOBAL.formatDateDB(txtToDate.getText()) + "' AND SUBSTRING(A.VOUCHER_NO,1,2) = 'SJ' AND MAIN_ACCOUNT_CODE =210010 LIMIT 10000000000");

            stmt.execute("UPDATE PRODUCTION.FELT_CREDIT_NOTE_PARTY_EXCEPTION, PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_VOUCHER_NO ='DELETED' WHERE  EXCP_TYPE ='FELTCASH' AND LCN_INVOICE_DATE  >= EXCP_FROM_DATE AND LCN_INVOICE_DATE <= EXCP_TO_DATE AND EXCP_PARTY_CODE = LCN_SUB_ACCOUNT_CODE");

            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE LCN_VOUCHER_NO ='DELETED'");
            ...
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE,TEMP_DATABASE.TM80  SET LCN_UNADJ = COL04 WHERE COL01 = LCN_INVOICE_NO AND COL02 =LCN_INVOICE_DATE AND COL03 = LCN_SUB_ACCOUNT_CODE");
            ...
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE,TEMP_DATABASE.TM80  SET LCN_OEM = COL04 WHERE COL01 = LCN_INVOICE_NO AND COL02 =LCN_INVOICE_DATE AND COL03 = LCN_SUB_ACCOUNT_CODE");
            ...
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE,TEMP_DATABASE.TM80  SET LCN_YEAR_END = COL04 WHERE COL01 = LCN_INVOICE_NO AND COL02 =LCN_INVOICE_DATE AND COL03 = LCN_SUB_ACCOUNT_CODE");
            ...
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE,TEMP_DATABASE.TM80  SET LCN_INS_CLAIM = COL04 WHERE COL01 = LCN_INVOICE_NO AND COL02 =LCN_INVOICE_DATE AND COL03 = LCN_SUB_ACCOUNT_CODE");
            ...
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE,TEMP_DATABASE.TM80  SET LCN_COMP = COL04 WHERE COL01 = LCN_INVOICE_NO AND COL02 =LCN_INVOICE_DATE AND COL03 = LCN_SUB_ACCOUNT_CODE");
            ...
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE,TEMP_DATABASE.TM80  SET LCN_GOODS_RETURN = COL04 WHERE COL01 = LCN_INVOICE_NO AND COL02 =LCN_INVOICE_DATE AND COL03 = LCN_SUB_ACCOUNT_CODE");

            //------------------------------------------------------------------------
            //--------------------------------EXTRA ADDITIONS----------------------------------
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET EXT12=LCN_UNADJ+LCN_OEM+LCN_INS_CLAIM+LCN_COMP+LCN_GOODS_RETURN+LCN_YEAR_END");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET EXT13=LCN_AMOUNT-EXT12");

            //---------------------------------------------------------------------------------------------------
            //NEW QUERY (DX)
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE ,PRODUCTION.FELT_GROUP_MASTER_DETAIL SET LCN_LC_OPENER = GROUP_CODE WHERE LCN_SUB_ACCOUNT_CODE = PARTY_CODE");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE ,DINESHMILLS.D_SAL_INVOICE_HEADER SET LCN_CHARGE_CODE = PAYMENT_TERM_CODE WHERE LCN_INVOICE_NO  = INVOICE_NO AND  LCN_INVOICE_DATE  = INVOICE_DATE");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE ,DINESHMILLS.D_SAL_PARTY_MASTER SET EXT4 = CHARGE_CODE WHERE PARTY_CODE = LCN_SUB_ACCOUNT_CODE");
/////LOOK AND COMPAIR
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE A ,(SELECT CN_INVOICE_NO FROM PRODUCTION.FELT_CN_TEMP_DETAIL D,PRODUCTION.FELT_CN_TEMP_HEADER H WHERE D.CN_ID = H.CN_ID AND H.CN_ID LIKE 'F6%' AND H.CN_FROM_DATE >= '" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "' AND H.CN_TO_DATE <= '" + EITLERPGLOBAL.formatDateDB(txtToDate.getText()) + "' AND D.CN_INVOICE_DATE >= '" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "' AND D.CN_INVOICE_DATE <= '" + EITLERPGLOBAL.formatDateDB(txtToDate.getText()) + "' AND H.APPROVED =1 AND H.CANCELED =0) AS F6 SET A.EXT2 = 'F6'  WHERE CN_INVOICE_NO = LCN_INVOICE_NO");
...
//            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE EXT2 IN ('SDF')");            
            ////
            
////////////////////////////////////
            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE LCN_CHARGE_CODE =''");

//            //25-05-2020
//            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN ('9','7') AND EXT2 NOT IN ('F6','D6','DH')");
//            ////stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN ('9','7') AND EXT2 NOT IN ('F6','D6')");
            
            //28-04-2021
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE A ,(SELECT INVOICE_NO FROM PRODUCTION.FELT_F6_REPORT_DETAIL A WHERE COALESCE(F6,'')='' AND SUBSTRING(PAYMENT_TERM,1,1)='1' AND INVOICE_DATE>= '" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "' AND INVOICE_DATE<='" + EITLERPGLOBAL.formatDateDB(txtToDate.getText()) + "' AND ROUND(AMOUNT,2)>=ROUND(TOTAL_NET_AMOUNT,2)) AS F6 SET A.EXT2 = 'PU'  WHERE INVOICE_NO = LCN_INVOICE_NO");
            //29-04-2022
            ...
            //28-04-2021
            //stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN ('9','7') AND EXT2 NOT IN ('F6','D6','DH','SP','SDF','PU')");
            ////stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN ('9','7') AND EXT2 NOT IN ('F6','D6')");
            //29-04-2022
            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN ('9','7') AND EXT2 NOT IN ('F6','D6','DH','SP','SDF','PU','UP')");

            //-------------------------------F6 ----------------------------------------------
            // -------------- F6 GR ELIGIBLE LIST -----------------
//28-04-2022            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE CONCAT(LCN_INVOICE_NO,SUBSTRING(LCN_INVOICE_DATE,1,10)) IN ( SELECT CONCAT(INVOICE_NO,INVOICE_DATE) FROM PRODUCTION.FELT_SALES_RETURNS_HEADER H, PRODUCTION.FELT_SALES_RETURNS_DETAIL D WHERE H.DOC_NO=D.DOC_NO AND H.APPROVED=1 AND H.CANCELED=0 AND D.INVOICE_DATE>='" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "' AND D.INVOICE_DATE<='" + EITLERPGLOBAL.formatDateDB(txtToDate.getText()) + "' )");

//28-04-2021            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE CONCAT(LCN_INVOICE_NO,SUBSTRING(LCN_INVOICE_DATE,1,10)) IN ( SELECT CONCAT(I.INVOICE_NO,I.INVOICE_DATE) FROM PRODUCTION.FELT_SALES_RETURNS_HEADER H, PRODUCTION.FELT_SALES_RETURNS_DETAIL D, PRODUCTION.FELT_SAL_INVOICE_HEADER I WHERE H.DOC_NO=D.DOC_NO AND H.APPROVED=1 AND H.CANCELED=0 AND I.INVOICE_DATE>='" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "' AND I.INVOICE_DATE<='" + EITLERPGLOBAL.formatDateDB(txtToDate.getText()) + "' AND D.PARTY_CODE=I.PARTY_CODE AND D.NEW_PIECE_NO=I.PIECE_NO AND I.APPROVED=1 AND I.CANCELLED=0 AND D.RETURN_CATEGORY='F6' AND I.PIECE_NO NOT IN (SELECT INVOICE_PIECE_NO FROM PRODUCTION.FELT_F6GR_ELIGIBLE_INV_LIST WHERE CANCELED=0) )");
            
            //ADDED ON 28-04-2022 AS PER POLICY
//            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE CONCAT(LCN_INVOICE_NO,SUBSTRING(LCN_INVOICE_DATE,1,10)) IN ( SELECT CONCAT(I.INVOICE_NO,I.INVOICE_DATE) FROM PRODUCTION.FELT_SALES_RETURNS_HEADER H, PRODUCTION.FELT_SALES_RETURNS_DETAIL D, PRODUCTION.FELT_SAL_INVOICE_HEADER I WHERE H.DOC_NO=D.DOC_NO AND H.APPROVED=1 AND H.CANCELED=0 AND I.INVOICE_DATE>='" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "' AND I.INVOICE_DATE<='" + EITLERPGLOBAL.formatDateDB(txtToDate.getText()) + "' AND D.PARTY_CODE=I.PARTY_CODE AND D.NEW_PIECE_NO=I.PIECE_NO AND I.APPROVED=1 AND I.CANCELLED=0 AND D.RETURN_CATEGORY IN ('F6','GR') AND I.PIECE_NO NOT IN (SELECT INVOICE_PIECE_NO FROM PRODUCTION.FELT_F6GR_ELIGIBLE_INV_LIST WHERE CANCELED=0) )");
            //29-04-2022 AS PER POLICY
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE A ,(SELECT I.INVOICE_NO FROM PRODUCTION.FELT_SALES_RETURNS_HEADER H, PRODUCTION.FELT_SALES_RETURNS_DETAIL D, PRODUCTION.FELT_SAL_INVOICE_HEADER I WHERE H.DOC_NO=D.DOC_NO AND H.APPROVED=1 AND H.CANCELED=0 AND I.INVOICE_DATE>='" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "' AND I.INVOICE_DATE<='" + EITLERPGLOBAL.formatDateDB(txtToDate.getText()) + "' AND D.PARTY_CODE=I.PARTY_CODE AND D.NEW_PIECE_NO=I.PIECE_NO AND I.APPROVED=1 AND I.CANCELLED=0 AND D.RETURN_CATEGORY IN ('F6','GR') AND I.PIECE_NO NOT IN (SELECT INVOICE_PIECE_NO FROM PRODUCTION.FELT_F6GR_ELIGIBLE_INV_LIST WHERE CANCELED=0)) AS F6 SET A.EXT2 = 'GR'  WHERE INVOICE_NO = LCN_INVOICE_NO");
            ...
            stmt.execute("UPDATE TEMP_DATABASE.TM82,PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE  SET LCN_RC_VOUCHER = COL05, LCN_RC_VALUE_DATE = COL04,EXT1=COL06,LCN_RC_VOU_DATE= COL03 WHERE COL01 = LCN_INVOICE_NO AND COL08= LCN_INVOICE_DATE");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_DAY = EXTRACT(DAY FROM LCN_VOUCHER_DATE), LCN_MONTH =EXTRACT(MONTH FROM LCN_VOUCHER_DATE), LCN_YEAR = EXTRACT(YEAR FROM LCN_VOUCHER_DATE)");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_DAY_DIFF  = DATEDIFF(LCN_RC_VALUE_DATE,LCN_INVOICE_DATE) WHERE LCN_RC_VALUE_DATE != '0000-00-00'");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_RC_VOUCHER_1  = LCN_AMOUNT - EXT12");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_INVOICE_AMOUNT =LCN_AMOUNT");

//            //25-05-2020
//            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_AMOUNT =0  WHERE EXT2 NOT IN ('F6','D6','DH') AND SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN (9,7)");
//            ////stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_AMOUNT =0  WHERE EXT2 NOT IN ('F6','D6') AND SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN (9,7)");
            
            //07-04-2021
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_AMOUNT =0  WHERE EXT2 NOT IN ('F6','D6','DH','SP','SDF') AND SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN (9,7)");
            ////stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_AMOUNT =0  WHERE EXT2 NOT IN ('F6','D6') AND SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN (9,7)");

            //stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE  WHERE EXT4='7' AND EXT2 !='F6' AND LCN_DAY_DIFF>15");
            //--UPDATE TMP_LC_CREDITNOTE_ADVANCE  SET LCN_AMOUNT =0  WHERE SUBSTRING(LCN_CHARGE_CODE,1,1) NOT IN (1,9) AND EXT4 !=9;
            //--UPDATE TMP_LC_CREDITNOTE_ADVANCE  SET LCN_AMOUNT =0  WHERE SUBSTRING(LCN_CHARGE_CODE,1,1)  IN (1,2)  AND LCN_DAY_DIFF >=6  AND SUBSTRING(EXT1,1,2) != 'AR';
            //--------------------------------------------------------
            //----------------------------------------------------------------------------------------------------------------------
            stmt.execute("TRUNCATE TABLE TEMP_DATABASE.TM80");

            stmt.execute("INSERT INTO TEMP_DATABASE.TM80 SELECT LCN_LC_OPENER,SUM(EXT13),'','' FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE GROUP BY LCN_LC_OPENER");
            ...
            stmt.execute("UPDATE TEMP_DATABASE.TM80,PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE  SET LCN_PER = COL03  WHERE COL01 = LCN_LC_OPENER");
            ...
            //28-04-2021
            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE CONCAT(LCN_INVOICE_NO,SUBSTRING(LCN_INVOICE_DATE,1,10)) IN ( SELECT CONCAT(I.INVOICE_NO,I.INVOICE_DATE) FROM PRODUCTION.FELT_SALES_RETURNS_HEADER H, PRODUCTION.FELT_SALES_RETURNS_DETAIL D, PRODUCTION.FELT_SAL_INVOICE_HEADER I WHERE H.DOC_NO=D.DOC_NO AND H.APPROVED=1 AND H.CANCELED=0 AND I.INVOICE_DATE>='" + EITLERPGLOBAL.formatDateDB(txtFromDate.getText()) + "' AND I.INVOICE_DATE<='" + EITLERPGLOBAL.formatDateDB(txtToDate.getText()) + "' AND D.PARTY_CODE=I.PARTY_CODE AND D.NEW_PIECE_NO=I.PIECE_NO AND I.APPROVED=1 AND I.CANCELLED=0 AND D.RETURN_CATEGORY='F6' AND I.PIECE_NO NOT IN (SELECT INVOICE_PIECE_NO FROM PRODUCTION.FELT_F6GR_ELIGIBLE_INV_LIST WHERE CANCELED=0) )");
//            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE EXT2 IN ('PU')");     
            //29-04-2022
            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE EXT2 IN ('PU','UP','GR')");     
            ////

            //1819 POLICY AMENDMENT
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET EXT3 = LCN_PER ");
            stmt.execute("UPDATE PRODUCTION.FELT_CREDIT_NOTE_PARTY_DISC_DEDUCTION,PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE  SET LCN_PER = (LCN_PER - DISC_DEDUCTION)  WHERE DISC_PARTY_CODE = LCN_SUB_ACCOUNT_CODE AND DISC_TYPE='FELTCASH' AND LCN_INVOICE_DATE  >= DISC_FROM_DATE AND LCN_INVOICE_DATE <= DISC_TO_DATE ");

            //1920 POLICY AMENDMENT 30-05-2020
            
            stmt.execute("UPDATE PRODUCTION.FELT_CREDIT_NOTE_PARTY_DISC_DEDUCTION_PERCENT,PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE  SET LCN_PER = ROUND((LCN_PER * DISC_DEDUCTION_PERCENT)/100,2)  WHERE DISC_PARTY_CODE = LCN_SUB_ACCOUNT_CODE AND DISC_TYPE='FELT_CASH' AND LCN_INVOICE_DATE  >= DISC_FROM_DATE AND LCN_INVOICE_DATE <= DISC_TO_DATE ");

//            //F6 8-15 DAYS AMENDMENT
//            
//            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_PER = ROUND(LCN_PER/2,2),EXT2='H6' WHERE LCN_DAY_DIFF>=8 AND LCN_DAY_DIFF<=15 AND EXT2='F6' ");
//            
//            //----------------------
            //F6 31-61 DAYS AMENDMENT AS PER NEW POLICY w.e.f 01/07/2019 till then 7-15 DAYS POLICY till 30/06/2019
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_PER = ROUND(LCN_PER/2,2),EXT2='H6' WHERE LCN_DAY_DIFF>=8 AND LCN_DAY_DIFF<=15 AND EXT2='F6' AND LCN_INVOICE_DATE<='2019-06-30' ");

//            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_PER = ROUND(LCN_PER/2,2),EXT2='H6' WHERE LCN_DAY_DIFF>=32 AND LCN_DAY_DIFF<=61 AND EXT2='F6' AND LCN_INVOICE_DATE>='2019-07-01' ");
            
            //AS PER POLICY REVISED FOR MONTH OF 01 AND 02 OF 2020
            
//            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_PER = ROUND(LCN_PER/2,2),EXT2='H6' WHERE LCN_DAY_DIFF>=32 AND LCN_DAY_DIFF<=61 AND EXT2='F6' AND LCN_INVOICE_DATE>='2019-07-01' AND LCN_INVOICE_DATE<='2020-03-31' AND EXTRACT(MONTH FROM LCN_INVOICE_DATE) NOT IN (1,2) AND EXTRACT(YEAR FROM LCN_INVOICE_DATE) NOT IN (2020)");
//            
//            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_PER = ROUND(LCN_PER/2,2),EXT2='H6' WHERE LCN_RC_VALUE_DATE>='2020-03-03' AND LCN_RC_VALUE_DATE<='2020-05-15' AND EXT2='F6' AND EXTRACT(MONTH FROM LCN_INVOICE_DATE) IN (1) AND EXTRACT(YEAR FROM LCN_INVOICE_DATE) IN (2020)");
//            
//            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_PER = ROUND(LCN_PER/2,2),EXT2='H6' WHERE LCN_RC_VALUE_DATE>='2020-05-16' AND LCN_RC_VALUE_DATE<='2020-06-15' AND EXT2='F6' AND EXTRACT(MONTH FROM LCN_INVOICE_DATE) IN (2) AND EXTRACT(YEAR FROM LCN_INVOICE_DATE) IN (2020)");
            
            //25-05-2020
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_PER = ROUND(LCN_PER/2,2) WHERE EXT2='DH' ");
            ////

            //07-04-2021
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_PER = ROUND(LCN_PER-1,2) WHERE EXT2='SP' ");
            ////
            
            //----------------------
            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_CREDIT_AMOUNT = ROUND(((LCN_PER/100)*EXT13)+.001,0)");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_BANK_TR_CODE = SUBSTRING(LCN_RC_VOUCHER,5,2)");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_BANK_TR_CODE = 89 WHERE SUBSTRING(LCN_RC_VOUCHER,1,2) ='CN'");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "')");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_INTERVAL_CAPTION = CONCAT('FROM ','" + txtFromDate.getText().trim() + "',' TO ','" + txtToDate.getText().trim() + "')");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_INTERVAL = CONCAT('" + toyr + "','-','" + frmyr + "')");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_NO = CONCAT(LCN_SUB_ACCOUNT_CODE,LCN_YEAR_MON_ID)");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE SET LCN_LINK_NO = CONCAT(RIGHT(LCN_INVOICE_NO,4),'/',LCN_INTERVAL)");

            stmt.execute("UPDATE PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE,DINESHMILLS.D_SAL_PARTY_MASTER SET LCN_LC_NAME = SUBSTRING(PARTY_NAME,1,18)  WHERE PARTY_CODE = LCN_SUB_ACCOUNT_CODE");

            stmt.execute("DELETE FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE WHERE LCN_PER =0");
...
//            stmt.execute("INSERT INTO PRODUCTION.D_CREDIT_NOTE_DETAIL (CND_ID,CND_NO,CND_TYPE,CND_MAIN_ACCOUNT_CODE,CND_SUB_ACCOUNT_CODE,CND_EFFECT,CND_LINK_NO,CND_LINK_YEAR,CND_INVOICE_NO,CND_INVOICE_DATE,CND_INVOICE_AMOUNT,CND_CHARGE_CODE,CND_PERCENT,CND_BOOK_CODE,CND_RC_VOUCHER_NO,CND_RC_VOUCHER_DATE,CND_DAYS,CND_CREDIT_AMOUNT,CND_FIN_VOUCHER_NO,CND_FIN_VOUCHER_DATE,CND_FIN_ELIGIBLE,CND_LC_OPENER,CND_VALUE_DATE,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_EXT2,CND_EXT3,CND_EXT4,CND_EXT5,CND_EXT6,CND_EXT7,CND_EXT8,CND_EXT9,CND_EXT10,CND_CATG) SELECT 1,LCN_NO,'FELTCASH',LCN_MAIN_ACCOUNT_CODE,LCN_SUB_ACCOUNT_CODE,LCN_EFFECT,LCN_LINK_NO,LCN_INTERVAL,LCN_INVOICE_NO,LCN_INVOICE_DATE,LCN_INVOICE_AMOUNT,LCN_CHARGE_CODE,LCN_PER,LCN_BANK_TR_CODE,LCN_RC_VOUCHER,LCN_RC_VALUE_DATE,LCN_DAY_DIFF,LCN_CREDIT_AMOUNT,EXT1,LCN_VOUCHER_DATE,'Y',LCN_LC_OPENER,LCN_RC_VALUE_DATE,LCN_LC_NAME,LCN_YEAR_MON_ID,LCN_INTERVAL_CAPTION,EXT12,EXT13,LCN_INVOICE_AMOUNT,LCN_UNADJ,LCN_OEM,LCN_INS_CLAIM,LCN_COMP,LCN_GOODS_RETURN,LCN_YEAR_END,EXT2 FROM PRODUCTION.TMP_LC_CREDITNOTE_ADVANCE  WHERE LCN_YEAR_MON_ID = CONCAT('FELTCASH','"+fyr+"','"+tyr+"') ORDER BY LCN_SUB_ACCOUNT_CODE,LCN_INVOICE_DATE,LCN_INVOICE_NO LIMIT 10000000");
            ...
            stmt.execute("DELETE FROM PRODUCTION.D_CREDIT_NOTE_HEADER  WHERE CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "')");
            ...
            stmt.execute("INSERT INTO PRODUCTION.D_CREDIT_NOTE_HEADER (CNH_ID,CNH_NO,CNH_TYPE,CNH_MAIN_ACCOUNT_CODE,CNH_MAIN_CODE,CNH_SUB_ACCOUNT_CODE,CNH_PARTY_NAME,CNH_EFFECT,CNH_INVOICE_AMOUNT,CNH_PERCENT,CNH_CREDIT_AMOUNT,CNH_YEAR_MON_ID,CNH_INTERVAL_CAPTION,CNH_BOOK_CODE,CNH_EXT2,CNH_EXT3,CNH_EXT5,CNH_EXT6,CNH_EXT7,CNH_EXT8,CNH_EXT9,CNH_EXT10) SELECT CND_ID,CND_NO,CND_TYPE,435077,210010,CND_SUB_ACCOUNT_CODE,CND_MAIN_ACCOUNT_NAME,'D',SUM(CND_INVOICE_AMOUNT),MAX(CND_PERCENT),SUM(CND_CREDIT_AMOUNT),CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,'TR-14',SUM(CND_EXT2),SUM(CND_EXT3),SUM(CND_EXT5),SUM(CND_EXT6),SUM(CND_EXT7),SUM(CND_EXT8),SUM(CND_EXT9),SUM(CND_EXT10) FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "')  GROUP BY CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION LIMIT 1000000000000");
...
//            //25-05-2020
//            stmt.execute("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER AS CNH, (SELECT SUM(CND_EXT3) AS INVOICE_AMOUNT,MAX(CND_PERCENT) AS PERCENT,SUM(CND_CREDIT_AMOUNT) AS CREDIT_AMOUNT,CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CND_CATG NOT IN ('H6','DH') GROUP BY CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_PERCENT ) AS CND SET CNH.CNH_INVOICE_AMOUNT_1=CND.INVOICE_AMOUNT,CNH.CNH_PERCENT_1=CND.PERCENT,CNH.CNH_CREDIT_AMOUNT_1=CND.CREDIT_AMOUNT WHERE CNH.CNH_YEAR_MON_ID=CND.CND_YEAR_MON_ID AND CNH.CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CNH.CNH_NO=CND.CND_NO ");
//            stmt.execute("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER AS CNH, (SELECT SUM(CND_EXT3) AS INVOICE_AMOUNT,MAX(CND_PERCENT) AS PERCENT,SUM(CND_CREDIT_AMOUNT) AS CREDIT_AMOUNT,CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CND_CATG IN ('H6','DH') GROUP BY CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_PERCENT ) AS CND SET CNH.CNH_INVOICE_AMOUNT_2=CND.INVOICE_AMOUNT,CNH.CNH_PERCENT_2=CND.PERCENT,CNH.CNH_CREDIT_AMOUNT_2=CND.CREDIT_AMOUNT WHERE CNH.CNH_YEAR_MON_ID=CND.CND_YEAR_MON_ID AND CNH.CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CNH.CNH_NO=CND.CND_NO ");
//            ////stmt.execute("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER AS CNH, (SELECT SUM(CND_EXT3) AS INVOICE_AMOUNT,MAX(CND_PERCENT) AS PERCENT,SUM(CND_CREDIT_AMOUNT) AS CREDIT_AMOUNT,CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CND_CATG!='H6' GROUP BY CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_PERCENT ) AS CND SET CNH.CNH_INVOICE_AMOUNT_1=CND.INVOICE_AMOUNT,CNH.CNH_PERCENT_1=CND.PERCENT,CNH.CNH_CREDIT_AMOUNT_1=CND.CREDIT_AMOUNT WHERE CNH.CNH_YEAR_MON_ID=CND.CND_YEAR_MON_ID AND CNH.CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CNH.CNH_NO=CND.CND_NO ");
//            ////stmt.execute("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER AS CNH, (SELECT SUM(CND_EXT3) AS INVOICE_AMOUNT,MAX(CND_PERCENT) AS PERCENT,SUM(CND_CREDIT_AMOUNT) AS CREDIT_AMOUNT,CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CND_CATG='H6' GROUP BY CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_PERCENT ) AS CND SET CNH.CNH_INVOICE_AMOUNT_2=CND.INVOICE_AMOUNT,CNH.CNH_PERCENT_2=CND.PERCENT,CNH.CNH_CREDIT_AMOUNT_2=CND.CREDIT_AMOUNT WHERE CNH.CNH_YEAR_MON_ID=CND.CND_YEAR_MON_ID AND CNH.CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CNH.CNH_NO=CND.CND_NO ");
            
            //07-04-2021
            stmt.execute("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER AS CNH, (SELECT SUM(CND_EXT3) AS INVOICE_AMOUNT,MAX(CND_PERCENT) AS PERCENT,SUM(CND_CREDIT_AMOUNT) AS CREDIT_AMOUNT,CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CND_CATG NOT IN ('H6','DH','SP') GROUP BY CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_PERCENT ) AS CND SET CNH.CNH_INVOICE_AMOUNT_1=CND.INVOICE_AMOUNT,CNH.CNH_PERCENT_1=CND.PERCENT,CNH.CNH_CREDIT_AMOUNT_1=CND.CREDIT_AMOUNT WHERE CNH.CNH_YEAR_MON_ID=CND.CND_YEAR_MON_ID AND CNH.CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CNH.CNH_NO=CND.CND_NO ");
            stmt.execute("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER AS CNH, (SELECT SUM(CND_EXT3) AS INVOICE_AMOUNT,MAX(CND_PERCENT) AS PERCENT,SUM(CND_CREDIT_AMOUNT) AS CREDIT_AMOUNT,CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CND_CATG IN ('H6','DH','SP') GROUP BY CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_PERCENT ) AS CND SET CNH.CNH_INVOICE_AMOUNT_2=CND.INVOICE_AMOUNT,CNH.CNH_PERCENT_2=CND.PERCENT,CNH.CNH_CREDIT_AMOUNT_2=CND.CREDIT_AMOUNT WHERE CNH.CNH_YEAR_MON_ID=CND.CND_YEAR_MON_ID AND CNH.CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CNH.CNH_NO=CND.CND_NO ");
            ////stmt.execute("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER AS CNH, (SELECT SUM(CND_EXT3) AS INVOICE_AMOUNT,MAX(CND_PERCENT) AS PERCENT,SUM(CND_CREDIT_AMOUNT) AS CREDIT_AMOUNT,CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CND_CATG!='H6' GROUP BY CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_PERCENT ) AS CND SET CNH.CNH_INVOICE_AMOUNT_1=CND.INVOICE_AMOUNT,CNH.CNH_PERCENT_1=CND.PERCENT,CNH.CNH_CREDIT_AMOUNT_1=CND.CREDIT_AMOUNT WHERE CNH.CNH_YEAR_MON_ID=CND.CND_YEAR_MON_ID AND CNH.CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CNH.CNH_NO=CND.CND_NO ");
            ////stmt.execute("UPDATE PRODUCTION.D_CREDIT_NOTE_HEADER AS CNH, (SELECT SUM(CND_EXT3) AS INVOICE_AMOUNT,MAX(CND_PERCENT) AS PERCENT,SUM(CND_CREDIT_AMOUNT) AS CREDIT_AMOUNT,CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_SUB_ACCOUNT_CODE FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_YEAR_MON_ID  = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CND_CATG='H6' GROUP BY CND_ID,CND_NO,CND_TYPE,CND_LC_OPENER,CND_MAIN_ACCOUNT_NAME,CND_YEAR_MON_ID,CND_INTERVAL_CAPTION,CND_PERCENT ) AS CND SET CNH.CNH_INVOICE_AMOUNT_2=CND.INVOICE_AMOUNT,CNH.CNH_PERCENT_2=CND.PERCENT,CNH.CNH_CREDIT_AMOUNT_2=CND.CREDIT_AMOUNT WHERE CNH.CNH_YEAR_MON_ID=CND.CND_YEAR_MON_ID AND CNH.CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "') AND CNH.CNH_NO=CND.CND_NO ");
            ...
            stmt.execute("UPDATE DINESHMILLS.D_SAL_PARTY_MASTER,PRODUCTION.D_CREDIT_NOTE_HEADER SET CNH_PARTY_NAME = PARTY_NAME,CNH_CITY = CITY_ID WHERE CNH_SUB_ACCOUNT_CODE = PARTY_CODE AND CNH_YEAR_MON_ID = CONCAT('FELTCASHADV','" + fyr + "','" + tyr + "')");
    ...
    private void LCCreditDraftReport() {
	    ...
	    objEngine.PreviewReport("http://" + EITLERPGLOBAL.HostIP + "/EITLERP/Reports/Production/rpt_CN_Draft_2.rpt", Parameters, objReportData);
    ...
    private void DraftReport() {
        try {
            ...
            String strSQL = "SELECT * FROM ( SELECT CND_QUALITY,CND_NO,CND_MAIN_ACCOUNT_NAME,CND_MAIN_ACCOUNT_CODE,CND_SUB_ACCOUNT_CODE,CND_LINK_NO, CND_LINK_YEAR, CND_INVOICE_DATE, CND_INVOICE_AMOUNT , CND_CHARGE_CODE , CND_PERCENT,CND_BOOK_CODE, CND_DEBIT_AMOUNT,CND_CREDIT_AMOUNT,CND_VALUE_DATE,CND_INTERVAL_CAPTION,CND_RC_VOUCHER_NO,CND_RC_VOUCHER_DATE,CND_DAYS,CND_FIN_VOUCHER_NO,RIGHT(CND_INTERVAL_CAPTION,10)AS VL,LEFT(CND_LINK_NO,2)AS LL,CND_CATG,CND_EXT2,CND_EXT3,CND_EXT4    FROM PRODUCTION.D_CREDIT_NOTE_DETAIL WHERE CND_TYPE ='FELTCASHADV' AND CND_YEAR_MON_ID = SUBSTRING('" + txtInterval.getText() + "',1,15)  AND CND_CREDIT_AMOUNT !=0 UNION ALL SELECT '',CNH_NO,'CASH/LC DISCOUNT',CNH_MAIN_ACCOUNT_CODE,'','', '','', '' , '' , 0.00,'', CNH_CREDIT_AMOUNT,CNH_DEBIT_AMOUNT,CNH_VALUE_DATE,CNH_INTERVAL_CAPTION,'','','','',RIGHT(CNH_INTERVAL_CAPTION,10),'','','','',''   FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_TYPE ='FELTCASHADV' AND CNH_YEAR_MON_ID = SUBSTRING('" + txtInterval.getText() + "',1,15)  ORDER BY  CND_NO,CND_LINK_NO  ) AS A LEFT JOIN (SELECT CNH_NO,CNH_PERCENT_1,CNH_CREDIT_AMOUNT_1,CNH_INVOICE_AMOUNT_1,CNH_PERCENT_2,CNH_CREDIT_AMOUNT_2,CNH_INVOICE_AMOUNT_2 FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_TYPE ='FELTCASHADV' AND CNH_YEAR_MON_ID = SUBSTRING('" + txtInterval.getText() + "',1,15) ) AS B ON A.CND_NO = B.CNH_NO ";
    ...
    private void GenerateCreditNoteSummary() {
        ...
        try {
            ...
            String strSQL = "SELECT CNH_FIN_VOUCHER_NO,CNH_ID,CNH_NO,CNH_TYPE,CNH_MAIN_ACCOUNT_CODE,CNH_MAIN_CODE,CNH_SUB_ACCOUNT_CODE,CNH_CITY,CONCAT(CNH_PARTY_NAME,',',CNH_CITY) AS CNH_PARTY_NAME,CNH_REASON_CODE,CNH_BOOK_CODE,CNH_PERCENT,CNH_CREDIT_AMOUNT,CNH_LINK_NO,CNH_YEAR_MON_ID,CNH_INTERVAL_CAPTION,CNH_INVOICE_AMOUNT,CNH_EXT3,CNH_EXT5,CNH_EXT6,CNH_EXT7,CNH_EXT8,CNH_EXT9,CNH_EXT10,CNH_PERCENT_1,CNH_CREDIT_AMOUNT_1,CNH_INVOICE_AMOUNT_1,CNH_PERCENT_2,CNH_CREDIT_AMOUNT_2,CNH_INVOICE_AMOUNT_2 FROM PRODUCTION.D_CREDIT_NOTE_HEADER WHERE CNH_YEAR_MON_ID='" + str + "' AND CNH_TYPE='FELTCASHADV' ORDER BY CNH_SUB_ACCOUNT_CODE";
