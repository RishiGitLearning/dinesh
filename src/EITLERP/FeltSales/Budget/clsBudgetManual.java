...
public class clsBudgetManual {
    ...
    public static int ModuleID = 768; //72
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM PRODUCTION.FELT_BUDGET_DETAIL WHERE  LEFT(DOC_NO,2)='MB' GROUP BY DOC_NO,YEAR_FROM  ORDER BY CREATED_DATE,DOC_NO ");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            setAttribute("DOC_NO", clsFirstFree.getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 768, (int) getAttribute("FFNO").getVal(), true));
            //-------------------------------------------------
            ...
            for (int i = 1; i <= colMRItems.size(); i++) {
                clsBudgetManualItem ObjMRItems = (clsBudgetManualItem) colMRItems.get(Integer.toString(i));
            ...
            ObjFlow.ModuleID = clsBudgetManual.ModuleID;
    ...
    public boolean Update() {
        ...
        try {
            ...
            for (int i = 1; i <= colMRItems.size(); i++) {
                ...
                pstmh.setInt(45, (int) getAttribute("MODIFIED_BY").getVal());
                pstmh.setString(46, getAttribute("APPROVAL_STATUS").getString());
                pstmh.setString(47, data.getStringValueFromDB("SELECT CURRENT_TIMESTAMP AS DATE FROM DUAL"));
                pstmh.setString(48, getAttribute("APPROVER_REMARKS").getString());
                pstmh.setString(49, str_split[1]);
            ...
            if (IsRejected) {
                ...
                data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=" + clsBudgetManual.ModuleID + " AND DOC_NO='" + ObjFlow.DocNo + "'");
    ...
    public boolean setData() {
        ...
        try {
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                Counter = Counter + 1;
                clsBudgetManualItem ObjMRItems = new clsBudgetManualItem();
    ...
    public boolean IsEditable(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=768 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
        ...
        try {
            ...
            if (rsTmp.getRow() > 0) {
                while (!rsTmp.isAfterLast()) {
                    clsBudgetManual objParty = new clsBudgetManual();
    ...
    public boolean CanDelete(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=768 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND TYPE='C' AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
...
public static void CancelDoc(String pAmendNo) {
        ...
        if (CanCancel(pAmendNo)) {
            ...
            try {
                ...
                if (!Approved) {
                    data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE DOC_NO='" + pAmendNo + "' AND MODULE_ID=768");
                }
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                ...
                Counter = Counter + 1;
                clsBudgetManual ObjItem = new clsBudgetManual();

                //------------- Header Fields --------------------//
                ObjItem.setAttribute("DOC_NO", rsTmp.getString("DOC_NO"));
                ObjItem.setAttribute("DOC_DATE", rsTmp.getString("PARTY_NAME"));
                ObjItem.setAttribute("RECEIVED_DATE", rsTmp.getString("PARTY_GROUP"));
