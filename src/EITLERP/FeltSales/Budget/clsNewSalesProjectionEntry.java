 ...
 * @author ashutosh
 */
public class clsNewSalesProjectionEntry {
     ...
     */
    public clsNewSalesProjectionEntry() {
        LastError = "";
        props = new HashMap();
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL  "
                    + " WHERE DOC_NO LIKE 'B%' AND YEAR_FROM=" + EITLERPGLOBAL.FinYearFrom + " "
    ...
    public boolean Update() {
        ...
        try {
            ...
            ObjFlow.ModuleID = clsNewSalesProjectionEntry.ModuleID;
            ...
            if (IsRejected) {
                ...
                data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=" + clsNewSalesProjectionEntry.ModuleID + " AND DOC_NO='" + ObjFlow.DocNo + "'");
        ...
        try {
            ...
            if (rsTmp.getRow() > 0) {
                while (!rsTmp.isAfterLast()) {
                    clsNewSalesProjectionEntry objParty = new clsNewSalesProjectionEntry();
    ...
    public boolean Filter(String Condition) {
        ...
        try {
            ...
            if (!rsResultSet.first()) {
                strSQL = "SELECT * FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL WHERE CREATED_DATE>='" + EITLERPGLOBAL.FinFromDateDB + "' AND CREATED_DATE<='" + EITLERPGLOBAL.FinToDateDB + "'  AND LEFT(DOC_NO,2)='B2' GROUP BY DOC_NO,YEAR_FROM  ORDER BY DOC_NO";
                rsResultSet = Stmt.executeQuery(strSQL);
                Ready = true;
                MoveLast();
                return false;
    ...
    public boolean CanDelete(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=" + clsNewSalesProjectionEntry.ModuleID + " AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND TYPE='C' AND STATUS='W'"; //825
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                ...
                Counter = Counter + 1;
                clsNewSalesProjectionEntry ObjItem = new clsNewSalesProjectionEntry();
