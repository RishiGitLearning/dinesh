 ...
 */
public class rptBudgetReviewSales2021_03022021 extends javax.swing.JApplet {
...
//        
        ...
        s = "UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL D, "
                ...
                + "SET AVG_LIFE  = M, SELLING_PRICE = ROUND(G,0)  WHERE YEAR_FROM =2020 AND YEAR_TO = 2021 "
        ...
        s = "UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL D,"
                + "(SELECT H.MM_PARTY_CODE,H.MM_MACHINE_NO,MM_MACHINE_POSITION,COALESCE(MM_AVG_LIFE,0) AS M,ceiling((340/COALESCE(MM_AVG_LIFE,0))) AS V,MM_FELT_VALUE_WITH_GST AS G FROM PRODUCTION.FELT_MACHINE_MASTER_DETAIL D,PRODUCTION.FELT_MACHINE_MASTER_HEADER H "
                + "WHERE H.MM_DOC_NO = D.MM_DOC_NO AND H.APPROVED =1 AND H.CANCELED =0) AS C "
                + "SET AVG_LIFE  = M, SELLING_PRICE = G  WHERE    YEAR_FROM =2020 AND YEAR_TO = 2021 "
                + "AND MM_PARTY_CODE = PARTY_CODE AND MACHINE_NO+0 = MM_MACHINE_NO+0 AND MM_MACHINE_POSITION+0 = POSITION_NO+0";
        ...
        data.Execute("UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL B,(SELECT G.PARTY_CODE, DISC_PER,PRODUCT_CODE "
                ...
                + "WHERE G.PARTY_CODE = B.PARTY_CODE AND QUALITY_NO = PRODUCT_CODE");
        data.Execute("UPDATE  PRODUCTION.FELT_RATE_DISC_MASTER_DETAIL R, PRODUCTION.FELT_BUDGET_REVIEW_DETAIL B "
                + "SET SPL_DISCOUNT = DISC_PER "
                + "WHERE SUBSTRING(PRODUCT_CODE,1,3) IN ('729','719') "
...
//                + "SET OC_STOCK=OCSTK "
//                + "WHERE D.UPN=OC.OCUPN  AND LEFT(D.DOC_NO,7)=CONCAT('B2021',RIGHT(100+(MONTH(CURDATE())),2))");
        data.Execute("UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL D,(SELECT UPN AS OCUPN,COUNT(*) AS OCSTK FROM PRODUCTION.FELT_WH_MTH_CLOSING_PIECE_STOCK F "
...
//                + "WHERE  LEFT(DOC_NO,7)=CONCAT('B2021',RIGHT(100+(MONTH(CURDATE())),2))");
        data.Execute("UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL "
                + "SET ACESS_QTY=0 "
                + "WHERE ACESSD_QTY<0 AND LEFT(DOC_NO,7)=CONCAT('B2021',RIGHT(100+(MONTH(CURDATE())),2))");
        data.Execute("UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL "
                + "SET ACESS_QTY=0 "
                + "WHERE ACESSD_QTY<0 AND LEFT(DOC_NO,7)=CONCAT('B2021',RIGHT(100+MONTH(DATE_ADD(LAST_DAY(CURDATE()),INTERVAL 1 DAY)),2))");
...
//                + "WHERE LEFT(DOC_NO,7)=CONCAT('B2021',RIGHT(100+MONTH(DATE_ADD(LAST_DAY(CURDATE()),INTERVAL 1 DAY)),2))");
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Budget Sales Projection Report for Year 2020-2021");
        ...
        ytmdbtn.setText("Realistic");
        jPanel15.add(ytmdbtn);
        ytmdbtn.setBounds(180, 10, 110, 30);

        YTMGROUP.add(ytmrbtn);
        ytmrbtn.setSelected(true);
        ytmrbtn.setText("Aspirational");
        jPanel15.add(ytmrbtn);
        ytmrbtn.setBounds(10, 10, 150, 30);
        ...
        canpieced.setText("Realistic");
        jPanel14.add(canpieced);
        canpieced.setBounds(180, 10, 170, 30);

        CANEXCESSGRUP.add(canupn);
        canupn.setSelected(true);
        canupn.setText("Aspirational");
...
//            aList.SQL = "SELECT DISTINCT PRODUCT_CODE,PRODUCT_DESC,GROUP_NAME FROM PRODUCTION.FELT_QLT_RATE_MASTER ORDER BY PRODUCT_CODE ";
            aList.SQL = "SELECT DISTINCT QUALITY_NO,GROUP_NAME FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL WHERE YEAR_FROM = 2020 AND YEAR_TO = 2021 ";
            aList.ReturnCol = 1;
...
//            aList.SQL = "SELECT PARTY_CODE,PARTY_NAME,PARTY_CLOSE_IND FROM DINESHMILLS.D_SAL_PARTY_MASTER WHERE MAIN_ACCOUNT_CODE=210010 AND APPROVED=1 AND CANCELLED=0";
            aList.SQL = "SELECT DISTINCT PARTY_CODE,PARTY_NAME FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL WHERE YEAR_FROM = 2020 AND YEAR_TO = 2021 ";
            aList.ReturnCol = 1;
            aList.ShowReturnCol = true;
            aList.DefaultSearchOn = 2;
    ...
    private void GenerateZAS() {
        ...
        try {
            ...
            strSQL = "SELECT INCHARGE,INCHARGE_NAME,COUNT(*) AS TOTAL , "
                    ...
                    + "(SELECT DISTINCT INCHARGE,INCHARGE_NAME,DOC_NO,COALESCE(APPROVED,0) AS APPROVED ,COALESCE(CANCELED,0) AS CANCELED FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL,PRODUCTION.FELT_INCHARGE  WHERE YEAR_FROM = 2020 AND YEAR_TO = 2021 "
                    + "AND INCHARGE = INCHARGE_CD  AND  LEFT(DOC_NO,7)=CONCAT('B2021',RIGHT(100+" + cmbmonth.getSelectedItem().toString() + ",2))"
                    + ") AS V "
                    + "LEFT JOIN "
                    + "(SELECT DOC_NO, "
                    ...
                    + "FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID = 834 AND DOC_DATE >='2019-12-19' AND STATUS = 'W'  AND  LEFT(DOC_NO,7)=CONCAT('B2021',RIGHT(100+" + cmbmonth.getSelectedItem().toString() + ",2)))  AS D "
    ...
    private void GenerateZPM() {
        ...
        try {
            ...
            strSQL = strSQL + "sum(coalesce(" + cmnth + "_budget,0)) as cbudget,"
                    ...
                    + "WHERE YEAR_FROM = 2020 AND YEAR_TO = 2021 "
                    + "AND INCHARGE = INCHARGE_CD and left(doc_no,7)=CONCAT('B2021',RIGHT(100+" + curmnth + ",2)) "
...
//            strSQL = strSQL + "coalesce(" + cmnth + "_budget,0)>0)  ";
            ...
            strSQL = strSQL + cndtn
                    ...
                    //                    + "left join "
                    //                    + "(select party_code as pcd,round(sum(invoice_amt)/100000,2) as amt from PRODUCTION.FELT_SAL_INVOICE_HEADER "
                    //                    + "where finyr='2020-2021' and ";
                    //            //if (pmnth.equalsIgnoreCase("")) {
                    //            //    strSQL = strSQL + "left(upper(monthname(invoice_date)),3)='APR' and ";
                    //            //} else {
                    //            //    strSQL = strSQL + "left(upper(monthname(invoice_date)),3)='" + pmnth + "' and ";
                    //            //}
                    //            strSQL = strSQL + " INVOICE_DATE>='2020-04-01' AND INVOICE_DATE<='" + lstdate + "' AND ";
                    //            strSQL = strSQL + "PIECE_NO NOT IN (SELECT PIECE_NO FROM PRODUCTION.FELT_SALES_RETURNS_DETAIL D,PRODUCTION.FELT_SALES_RETURNS_HEADER H WHERE H.DOC_NO=D.DOC_NO AND COALESCE(H.APPROVED,0)=1 AND COALESCE(H.CANCELED,0)=0 ) "
                    //                    + "group by party_code) as I "
                    //                    + "ON party_code=pcd "
...
//            System.out.println("Row no." + rsTmp.getRow());
            if (rsTmp.getRow() > 0) {
                ...
                while (!rsTmp.isAfterLast()) {
                    ...
                    }catch(Exception e){rowData[9] = 0.00;
                    }
                    try{
        ...
        try {
            ...
            if (canupn.isSelected()) {
                ...
                strSQL = strSQL + "LEFT JOIN "
                        + "(SELECT UPN AS SPUPN,SUM(CURRENT_PROJECTION) AS CURRENT_PROJECTION,SUM(WIP_QTY) AS WIP_QTY,SUM(STOCK_QTY) AS STOCK_QTY,SUM(DISPATCH_QTY) AS DISPATCH_QTY,SUM(ACESS_QTY) AS ACESS_QTY "
                        + " FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL D WHERE LEFT(D.DOC_NO,7)=CONCAT('B2021',RIGHT(100+" + cmbmonth.getSelectedItem().toString() + ",2)) AND ACESS_QTY>0 GROUP BY UPN) D "
                        + "ON F.CNE_UPN=D.SPUPN "
                        + " WHERE LEFT(CNE_DOC_NO,7)=CONCAT('B2021',RIGHT(100+" + cmbmonth.getSelectedItem().toString() + ",2))"
                        + "GROUP BY CNE_UPN "
            ...
            if (canpieced.isSelected()) {
                ...
                strSQL = strSQL + "LEFT JOIN "
                        + "(SELECT UPN AS SPUPN,SUM(CURRENT_PROJECTION-coalesce(" + cmnth + "_DOUBTFUL_QTY,0)) AS CURRENT_PROJECTION,SUM(WIP_QTY) AS WIP_QTY,SUM(STOCK_QTY) AS STOCK_QTY,SUM(DISPATCH_QTY) AS DISPATCH_QTY,SUM(ACESSD_QTY) AS ACESS_QTY "
                        + " FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL D WHERE LEFT(D.DOC_NO,7)=CONCAT('B2021',RIGHT(100+" + cmbmonth.getSelectedItem().toString() + ",2)) AND ACESSD_QTY>0 GROUP BY UPN) D "
                        + "ON F.CNE_UPN=D.SPUPN "
                        + " WHERE LEFT(CNE_DOC_NO,7)=CONCAT('B2021',RIGHT(100+" + cmbmonth.getSelectedItem().toString() + ",2))"
                        + "GROUP BY CNE_UPN "
                        + "HAVING COUNT(CNE_PIECE_NO)>0 "
...
//                    }
    ...
    private void GenerateYTM() {
        ...
        data.Execute("UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL  "
               ...
               + "WHERE  LEFT(DOC_NO,7)=CONCAT('B2021',RIGHT(100+"+cmbmonth.getSelectedItem().toString()+",2))");
       data.Execute("UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL D,"
               ...
               + "SET CANCELABLE_PIECE_NO = CANCELPIECE, CANCELABLE_PIECES = CAN_PIECE, "
               + "NONCANCELABLE_PIECE_NO = NONCANCELPIECE, NONCANCELABLE_PIECES = NON_CAN_PIECE "
               + "WHERE D.UPN = N.CNE_UPN AND LEFT(D.DOC_NO,7)=CONCAT('B2021',RIGHT(100+"+cmbmonth.getSelectedItem().toString()+",2))");
       data.Execute("UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL D,"
               + "(SELECT CNE_UPN,COUNT(CASE WHEN CNE_PIECE_STAGE IN ('PLANNING','BOOKING') THEN CNE_PIECE_NO END) AS CANCELPIECE,"
               + "COUNT(CASE WHEN CNE_PIECE_STAGE NOT IN('PLANNING','BOOKING') THEN CNE_PIECE_NO END) AS NONCANCELPIECE,"
               + "COALESCE(GROUP_CONCAT(CASE WHEN CNE_PIECE_STAGE IN('PLANNING','BOOKING') THEN CONCAT(CNE_PIECE_NO,'(',CNE_PIECE_STAGE,')') END),'') AS CAN_PIECE,"
        ...
        try {
            ...
            if (ytmrbtn.isSelected()) {
                strSQL = "SELECT *,ROUND(YTM1*WEIGHT,2) AS YTMKG,ROUND(YTM1*SQMTR,2) AS YTMSQMTR,"
                        ...
                        + cndtn
                        + " AND LEFT(DOC_NO,7)=CONCAT('B2021',RIGHT(100+" + cmbmonth.getSelectedItem().toString() + ",2))) AS D "
...
//            System.out.println("Row no." + rsTmp.getRow());
            if (rsTmp.getRow() > 0) {
                ...
                if (ytmrbtn.isSelected()) {
                    strSQL = "SELECT PROD_GROUP,SIZE_CRITERIA,(PRODCAPACITY) AS PRODCAPACITY,SUM(SP) AS SP,SUM(OC) AS OC,SUM(DISP) AS DISP,SUM(NCAN) AS NCAN,SUM(PGR) AS PGR,"
                            + "SUM(YTM) AS YTM,SUM(YTM1) AS YTM1,SUM(YTMKG) AS YTMKG,"
    ...
    private void GenerateSFUP() {
        ...
        String grp_cndtn = "";
        try {
            double gobqty, gobkg, gobval, gppqty, gppkg, gppval, gcpqty, gcpkg, gcpval, gp, gfupp, gfus,
                    pobqty, pobkg, pobval, pppqty, pppkg, pppval, pcpqty, pcpkg, pcpval, pp, pfupp, pfus,
                    zobqty, zobkg, zobval, zppqty, zppkg, zppval, zcpqty, zcpkg, zcpval, zp, zfupp, zfus;
            gobqty = gobkg = gobval = gppqty = gppkg = gppval = gcpqty = gcpkg = gcpval = gp = gfupp = gfus
                    = pobqty = pobkg = pobval = pppqty = pppkg = pppval = pcpqty = pcpkg = pp = pcpval = pfupp = pfus
                    = zobqty = zobkg = zobval = zppqty = zppkg = zppval = zcpqty = zcpkg = zp = zcpval = zfupp = zfus = 0;
            ...
            String strSQL = "SELECT *,"
                    + "COALESCE(CASE WHEN FOLLOWUP='YES' AND PRIORITY>(STOCKQTY+DISPATCHQTY) THEN CASE WHEN WIPQTY>0 THEN CASE WHEN PRIORITY-(STOCKQTY+DISPATCHQTY)>WIPQTY THEN WIPQTY ELSE PRIORITY-(STOCKQTY+DISPATCHQTY) END END END,0) AS FOLLOWUPPP,"
                    + "COALESCE(CASE WHEN FOLLOWUP='YES' AND PRIORITY>(STOCKQTY+DISPATCHQTY) THEN CASE WHEN PRIORITY>(WIPQTY+STOCKQTY+DISPATCHQTY) THEN PRIORITY-(WIPQTY+STOCKQTY+DISPATCHQTY) END END,0) AS FOLLOWUPSALE "
                    ...
                    + " AND COALESCE(PR_DELINK,'')!='OBSOLETE' AND UPPER(PR_OC_MONTHYEAR)='" + cmnth + " - 2020' "
                    + "AND "
                    + "((PR_REQ_MTH_LAST_DDMMYY>='2020-04-01' AND PR_REQ_MTH_LAST_DDMMYY<='2021-03-31') "
                    + "OR "
                    + "(PR_CURRENT_SCH_LAST_DDMMYY>='2020-04-01' AND PR_CURRENT_SCH_LAST_DDMMYY<='2021-03-31')) "
                    + "GROUP BY PR_UPN) AS WIP "
                    + "ON WIP.PR_UPN=UPN "
                    + "LEFT JOIN (SELECT * FROM PRODUCTION.FELT_INCHARGE) AS INCHRG "
                    + "ON INCHARGE=INCHARGE_CD "
                    + "WHERE LEFT(DOC_NO,7)=CONCAT('B2021',RIGHT(100+" + cmbmonth.getSelectedItem().toString() + ",2)) "
            ...
            if (rsTmp.getRow() > 0) {
                ...
                while (!rsTmp.isAfterLast()) {
                    ...
                    if (!p.equalsIgnoreCase(rsTmp.getString("PARTY_CODE")) || !z.equalsIgnoreCase(rsTmp.getString("INCHARGE_NAME"))) {
                        ...
                        DataModel.addRow(rowData);
                        p = rsTmp.getString("PARTY_CODE");
                        pobqty = pobkg = pobval = pppqty = pppkg = pppval = pcpqty = pcpkg = pcpval = pfupp = pfus = 0;
                    }
                    if (!z.equalsIgnoreCase(rsTmp.getString("INCHARGE_NAME"))) {
                        Object[] rowData = new Object[100];
                        DataModel.addRow(rowData);
                        ...
                        DataModel.addRow(rowData);
                        z = rsTmp.getString("INCHARGE_NAME");
                        zobqty = zobkg = zobval = zppqty = zppkg = zppval = zcpqty = zcpkg = zcpval = zfupp = zfus = 0;
                    }
                    Object[] rowData = new Object[100];

                    rowData[0] = Integer.toString(cnt);
                    rowData[1] = rsTmp.getString("INCHARGE_NAME");
                    rowData[2] = rsTmp.getString("UPN");
                    ...
                    rowData[7] = rsTmp.getString("CURRENT_PROJECTION");
                    rowData[8] = rsTmp.getString("PREV_GR_QTY");
                    rowData[9] = rsTmp.getString("CP_PGR");
                    rowData[10] = rsTmp.getString("WIPQTY");
                    rowData[11] = rsTmp.getString("STOCKQTY");
                    rowData[12] = rsTmp.getString("DISPATCHQTY");
                    rowData[13] = rsTmp.getString("PRIORITY");
                    rowData[14] = rsTmp.getString("FOLLOWUP");
                    rowData[15] = rsTmp.getString("FOLLOWUPPP");
                    rowData[16] = rsTmp.getString("FOLLOWUPSALE");
                    rowData[17] = EITLERPGLOBAL.getCurrentDate() + " " + EITLERPGLOBAL.getCurrentTime();
