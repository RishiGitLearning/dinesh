 ...
 * @author ashutosh
 */
public class clsBudgetReviewEntry {
    ...
    public static int ModuleID = 834; //825 // //72
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL    GROUP BY DOC_NO,YEAR_FROM  ORDER BY CREATED_DATE,DOC_NO ");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            setAttribute("DOC_NO", clsFirstFree.getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 834, (int) getAttribute("FFNO").getVal(), true));   //7
            //-------------------------------------------------
            ...
            sql = "INSERT INTO PRODUCTION.FELT_BUDGET_REVIEW_DETAIL "
            ...
            ObjFlow.ModuleID = clsBudgetReviewEntry.ModuleID;
    ...
    public boolean Update() {
        ...
        try {
            ...
            sql = "UPDATE PRODUCTION.FELT_BUDGET_REVIEW_DETAIL "
                    + "SET " + mmonth.get(mcurmonth) + "_BUDGET=?," + mmonth.get(mcurmonth) + "_NET_AMOUNT=?,"
                    + "" + mmonth.get(mcurmonth) + "_DOUBTFUL_QTY=?,"
                    + "" + mmonth.get(mcurmonth) + "_REMARK=?,"
                    + "" + mmonth.get(mcurmonth) + "_REMARK_DOUBTFUL=?,"
                    + "" + mmonth.get(mcurmonth) + "_PROPOSED_ACTION_BY_IC=?,"
                    + "" + mmonth.get(mcurmonth) + "_PROPOSED_ACTION_DOUBTFUL=?,"
                    + "CANCEL_QTY=?,CANCEL_VALUE=?,ACESS_QTY=?,ACESS_VALUE=?,"
                    + "CURRENT_PROJECTION=?,CURRENT_PROJECTION_VALUE=?,PROJ_OF_ORDER_QTY=?,PROJ_OF_ORDER_VALUE=?,"
                    + "MODIFIED_BY=?,MODIFIED_DATE=?,"
                    + "CE_PC=?,NCE_WIP=?,NCE_STK=?,MRF_PROJECTION=?,NCE_WIP_P_YEAR=?,NCE_STK_P_YEAR=?,GOAL=?,ADDTIONAL_SCOPE=?,CURRENT_POTENTIAL=?,"
                    + "MARK_AS_COMPLETE=?,CHANGE_QTY=?,SCOPE=? "
                    + "WHERE UPN=? AND DOC_NO=? AND YEAR_FROM=?";

            pstm = Conn.prepareStatement(sql);
            sqlh = "INSERT INTO PRODUCTION.FELT_BUDGET_REVIEW_DETAIL_H "
                    + "(DOC_NO,YEAR_FROM,YEAR_TO,PARTY_CODE,PARTY_NAME,MACHINE_NO,POSITION_NO,POSITION_DESC,"
                    + mmonth.get(mcurmonth) + "_BUDGET," + mmonth.get(mcurmonth) + "_NET_AMOUNT,"
                    + mmonth.get(mcurmonth) + "_DOUBTFUL_QTY,"
                    + mmonth.get(mcurmonth) + "_REMARK,"
                    + "" + mmonth.get(mcurmonth) + "_REMARK_DOUBTFUL,"
                    + "" + mmonth.get(mcurmonth) + "_PROPOSED_ACTION_BY_IC,"
                    + "" + mmonth.get(mcurmonth) + "_PROPOSED_ACTION_DOUBTFUL,"
                    + "CANCEL_QTY,CANCEL_VALUE,ACESS_QTY,ACESS_VALUE,CURRENT_PROJECTION,"
                    + "CURRENT_PROJECTION_VALUE,PROJ_OF_ORDER_QTY,PROJ_OF_ORDER_VALUE,"
                    + "HIERARCHY_ID,MODIFIED_BY,MODIFIED_DATE,REJECTED_REMARKS,REVISION_NO,APPROVAL_STATUS,ENTRY_DATE,"
                    + "APPROVER_REMARKS,FROM_IP,UPDATED_BY,UPN,"
                    + "CE_PC,NCE_WIP,NCE_STK,MRF_PROJECTION,NCE_WIP_P_YEAR,NCE_STK_P_YEAR,GOAL,ADDTIONAL_SCOPE,CURRENT_POTENTIAL,NEW_GOAL,"
                    + "MARK_AS_COMPLETE,CHANGE_QTY,SCOPE)";
            ...
            for (int i = 1; i <= colMRItems.size(); i++) {
                ...
                setAttribute("DOC_NO", ObjMRItems.getAttribute("DOC_NO").getObj().toString());
                pstm.setDouble(1, (double) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_BUDGET").getVal());
                pstm.setDouble(2, (double) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_NET_AMOUNT").getVal());
                pstm.setDouble(3, (double) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_DOUBTFUL_QTY").getVal());
                pstm.setString(4, (String) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_REMARK").getObj());

                pstm.setString(5, (String) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_REMARK_DOUBTFUL").getObj());
                pstm.setString(6, (String) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_PROPOSED_ACTION_BY_IC").getObj());
                pstm.setString(7, (String) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_PROPOSED_ACTION_DOUBTFUL").getObj());

                pstm.setDouble(8, (double) ObjMRItems.getAttribute("CANCEL_QTY").getVal());
                pstm.setDouble(9, (double) ObjMRItems.getAttribute("CANCEL_VALUE").getVal());
                pstm.setDouble(10, (double) ObjMRItems.getAttribute("ACESS_QTY").getVal());
                pstm.setDouble(11, (double) ObjMRItems.getAttribute("ACESS_VALUE").getVal());
                pstm.setDouble(12, (double) ObjMRItems.getAttribute("CURRENT_PROJECTION").getVal());
                pstm.setDouble(13, (double) ObjMRItems.getAttribute("CURRENT_PROJECTION_VALUE").getVal());
                pstm.setDouble(14, (double) ObjMRItems.getAttribute("PROJ_OF_ORDER_QTY").getVal());
                pstm.setDouble(15, (double) ObjMRItems.getAttribute("PROJ_OF_ORDER_VALUE").getVal());
                pstm.setInt(16, (int) getAttribute("MODIFIED_BY").getVal());
                pstm.setString(17, EITLERPGLOBAL.getCurrentDateTimeDB());
                pstm.setDouble(18, (double) ObjMRItems.getAttribute("CE_PC").getVal());
                pstm.setDouble(19, (double) ObjMRItems.getAttribute("NCE_WIP").getVal());
                pstm.setDouble(20, (double) ObjMRItems.getAttribute("NCE_STK").getVal());
                pstm.setDouble(21, (double) ObjMRItems.getAttribute("MRF_PROJECTION").getVal());
                pstm.setDouble(22, (double) ObjMRItems.getAttribute("NCE_WIP_P_YEAR").getVal());
                pstm.setDouble(23, (double) ObjMRItems.getAttribute("NCE_STK_P_YEAR").getVal());
                pstm.setDouble(24, (double) ObjMRItems.getAttribute("GOAL").getVal());
                pstm.setDouble(25, (double) ObjMRItems.getAttribute("ADDTIONAL_SCOPE").getVal());
                pstm.setDouble(26, (double) ObjMRItems.getAttribute("CURRENT_POTENTIAL").getVal());
                pstm.setInt(27, (int) getAttribute("MARK_AS_COMPLETE").getVal());
                pstm.setDouble(28, (double) ObjMRItems.getAttribute("CHANGE_QTY").getVal());
                pstm.setDouble(29, (double) ObjMRItems.getAttribute("SCOPE").getVal());
                pstm.setString(30, (String) ObjMRItems.getAttribute("UPN").getObj());
                pstm.setString(31, ObjMRItems.getAttribute("DOC_NO").getObj().toString());
                pstm.setString(32, (String) ObjMRItems.getAttribute("YEAR_FROM").getObj());
                ...
                pstmh.setString(7, (String) ObjMRItems.getAttribute("POSITION_NO").getObj());
                pstmh.setString(8, (String) ObjMRItems.getAttribute("POSITION_DESC").getObj());
                pstmh.setDouble(9, (double) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_BUDGET").getVal());
                pstmh.setDouble(10, (double) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_NET_AMOUNT").getVal());
                pstmh.setDouble(11, (double) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_DOUBTFUL_QTY").getVal());
                pstmh.setString(12, (String) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_REMARK").getObj());

                pstmh.setString(13, (String) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_REMARK_DOUBTFUL").getObj());
                pstmh.setString(14, (String) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_PROPOSED_ACTION_BY_IC").getObj());
                pstmh.setString(15, (String) ObjMRItems.getAttribute(mmonth.get(mcurmonth) + "_PROPOSED_ACTION_DOUBTFUL").getObj());

                pstmh.setDouble(16, (double) ObjMRItems.getAttribute("CANCEL_QTY").getVal());
                pstmh.setDouble(17, (double) ObjMRItems.getAttribute("CANCEL_VALUE").getVal());
                pstmh.setDouble(18, (double) ObjMRItems.getAttribute("ACESS_QTY").getVal());
                pstmh.setDouble(19, (double) ObjMRItems.getAttribute("ACESS_VALUE").getVal());
                pstmh.setDouble(20, (double) ObjMRItems.getAttribute("CURRENT_PROJECTION").getVal());
                pstmh.setDouble(21, (double) ObjMRItems.getAttribute("CURRENT_PROJECTION_VALUE").getVal());
                pstmh.setDouble(22, (double) ObjMRItems.getAttribute("PROJ_OF_ORDER_QTY").getVal());
                pstmh.setDouble(23, (double) ObjMRItems.getAttribute("PROJ_OF_ORDER_VALUE").getVal());
                pstmh.setInt(24, (int) getAttribute("HIERARCHY_ID").getVal());
                pstmh.setInt(25, (int) getAttribute("MODIFIED_BY").getVal());
                pstmh.setString(26, (String) getAttribute("MODIFIED_DATE").getObj());
                pstmh.setString(27, getAttribute("REJECTED_REMARKS").getString());
                pstmh.setInt(28, RevNo);
                pstmh.setString(29, getAttribute("APPROVAL_STATUS").getString());
                pstmh.setString(30, data.getStringValueFromDB("SELECT CURRENT_TIMESTAMP AS DATE FROM DUAL"));
                pstmh.setString(31, getAttribute("APPROVER_REMARKS").getString());
                pstmh.setString(32, str_split[1]);
                pstmh.setInt(33, (int) getAttribute("MODIFIED_BY").getVal());
                pstmh.setString(34, (String) ObjMRItems.getAttribute("UPN").getObj());
                pstmh.setDouble(35, (double) ObjMRItems.getAttribute("CE_PC").getVal());
                pstmh.setDouble(36, (double) ObjMRItems.getAttribute("NCE_WIP").getVal());
                pstmh.setDouble(37, (double) ObjMRItems.getAttribute("NCE_STK").getVal());
                pstmh.setDouble(38, (double) ObjMRItems.getAttribute("MRF_PROJECTION").getVal());
                pstmh.setDouble(39, (double) ObjMRItems.getAttribute("NCE_WIP_P_YEAR").getVal());
                pstmh.setDouble(40, (double) ObjMRItems.getAttribute("NCE_STK_P_YEAR").getVal());
                pstmh.setDouble(41, (double) ObjMRItems.getAttribute("GOAL").getVal());
                pstmh.setDouble(42, (double) ObjMRItems.getAttribute("ADDTIONAL_SCOPE").getVal());
                pstmh.setDouble(43, (double) ObjMRItems.getAttribute("CURRENT_POTENTIAL").getVal());
                pstmh.setDouble(44, (double) ObjMRItems.getAttribute("NEW_GOAL").getVal());
                pstmh.setInt(45, (int) getAttribute("MARK_AS_COMPLETE").getVal());
                pstmh.setDouble(46, (double) ObjMRItems.getAttribute("CHANGE_QTY").getVal());
                pstmh.setDouble(47, (double) ObjMRItems.getAttribute("SCOPE").getVal());
                pstmh.addBatch();
                if ((i + 1) % 1000 == 0) {
                    pstm.executeBatch();
                    pstmh.executeBatch();
            ...
            if (IsRejected) {
                ...
                data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=" + clsBudgetReviewEntry.ModuleID + " AND DOC_NO='" + ObjFlow.DocNo + "'");
    ...
    public boolean IsEditable(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=834 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND STATUS='W'"; //825
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
        ...
        try {
            ...
            if (rsTmp.getRow() > 0) {
                while (!rsTmp.isAfterLast()) {
                    clsBudgetReviewEntry objParty = new clsBudgetReviewEntry();
    ...
    public boolean Filter(String Condition) {
        ...
        try {
            String strSQL = "";
            strSQL += "SELECT * FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL WHERE LEFT(DOC_NO,2)='B2' AND " + Condition + " GROUP BY DOC_NO";
            ...
            if (!rsResultSet.first()) {
                strSQL = "SELECT * FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL WHERE CREATED_DATE>='" + EITLERPGLOBAL.FinFromDateDB + "' AND CREATED_DATE<='" + EITLERPGLOBAL.FinToDateDB + "'  AND LEFT(DOC_NO,2)='BU' GROUP BY DOC_NO,YEAR_FROM  ORDER BY DOC_NO";
                rsResultSet = Stmt.executeQuery(strSQL);
                Ready = true;
                MoveLast();
                return false;
    ...
    public static void CancelDoc(String pAmendNo) {
        ...
        if (CanCancel(pAmendNo)) {
            ...
            try {
                ...
                if (!Approved) {
                    data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE DOC_NO='" + pAmendNo + "' AND MODULE_ID=834"); //825
                }
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            ...
            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT DISTINCT case when received_date='0000-00-00 00:00:00' then date_format(doc_date,'%d/%m/%Y') else date_format(received_date,'%d/%m/%Y %H:%i') end as received_date,case when action_date='0000-00-00 00:00:00' then '' else date_format(action_date,'%d/%m/%Y %H:%i') end as action_date,PRODUCTION.FELT_BUDGET_REVIEW_DETAIL.DOC_NO,PRODUCTION.FELT_BUDGET_REVIEW_DETAIL.PARTY_NAME,PARTY_GROUP FROM PRODUCTION.FELT_BUDGET_REVIEW_DETAIL,PRODUCTION.FELT_PROD_DOC_DATA WHERE PRODUCTION.FELT_PROD_DOC_DATA.DOC_NO=PRODUCTION.FELT_BUDGET_REVIEW_DETAIL.DOC_NO AND PRODUCTION.FELT_PROD_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND PRODUCTION.FELT_PROD_DOC_DATA.STATUS='W' AND MODULE_ID=834 ORDER BY PRODUCTION.FELT_BUDGET_REVIEW_DETAIL.DOC_NO"; //825 
            }
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                ...
                Counter = Counter + 1;
                clsBudgetReviewEntry ObjItem = new clsBudgetReviewEntry();
