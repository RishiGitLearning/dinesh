 ...
 *
 */
public class FrmNewBudgetEntry extends javax.swing.JApplet {
    ...
    private int FFNo = 0;
    private int ModuleId = 872;
    private String DOC_NO = "";
    private clsNewBudgetEntry BudgetEntry;
...
//        FormatGrid();
        ...
        BudgetEntry = new clsNewBudgetEntry();
...
//                        }
        ...
        try {
            ...
            HashMap History = clsNewBudgetEntry.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsNewBudgetEntry ObjHistory = (clsNewBudgetEntry) History.get(Integer.toString(i));
                ...
                if (ke.getKeyCode() == 112) //F1 Key pressed
                {
                    if (EditMode == EITLERPGLOBAL.EDIT) {
                        ...
                        sch.SQL = "SELECT SCH AS PRS,COALESCE(SUM(BUDGET),0) AS PRS_QTY,COALESCE(CPIECE,0) AS CPRS_QTY,COALESCE(PIECES,'') AS PIECES,OCPIECE "
                                ...
                                + "where selected_date between '" + txtyearfrom.getText() + "-04-01' and '" + lblyearto.getText() + "-03-31' "
                                + "AND SELECTED_DATE>CURDATE()) AS D "
