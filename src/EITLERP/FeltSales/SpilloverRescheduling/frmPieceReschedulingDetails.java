...
public class frmPieceReschedulingDetails extends javax.swing.JApplet {
    ...
    private void FormatGrid() {
        try {
            ...
            DataModel.addColumn("Date of reschedule"); //1
            DataModel.addColumn("Original OC month"); //1
            DataModel.addColumn("Finishing date"); //1
            DataModel.addColumn("Rescheduled month"); //7
            ...
            DataModel.SetVariable(5, "RESCHEDULED_MONTH"); //7
            DataModel.SetVariable(6, "DATE_OF_COMMUNICATION"); //7
            DataModel.SetVariable(7, "MODE_OF_COMMUNICATION"); //7
            DataModel.SetVariable(8, "CONTACT_PERSON"); //7
            DataModel.SetVariable(9, "PARTY_JUSTIFICATION"); //7
            DataModel.SetVariable(10, "AREA_MANAGER_COMMENT"); //7
            ...
            tblReport.getColumnModel().getColumn(5).setMinWidth(140);
            tblReport.getColumnModel().getColumn(6).setMinWidth(160);
            tblReport.getColumnModel().getColumn(7).setMinWidth(160);
            tblReport.getColumnModel().getColumn(8).setMinWidth(140);
            tblReport.getColumnModel().getColumn(9).setMinWidth(140);
            tblReport.getColumnModel().getColumn(10).setMinWidth(200);
    ...
    public void GenerateData(String PieceNo)
    {
         try{
            ...
            if(data.IsRecordExist("SELECT D.*,H.APPROVED_DATE FROM PRODUCTION.SPILLOVER_RESCHEDULING_DETAIL D,PRODUCTION.SPILLOVER_RESCHEDULING_HEADER H   where D.PIECE_NO='"+PieceNo+"' AND D.DOC_NO=H.DOC_NO GROUP BY D.PIECE_NO,D.RE_SCH_MONTH,D.DATE_OF_COMMUNICATION,D.MODE_OF_COMMUNICATION,D.CONTACT_PERSON,D.PARTY_JUSTIFICATION,D.AREA_MANAGER_COMMENT ORDER BY D.PIECE_NO,D.DOC_NO"))
            {
               rsData.first();
               int i=1;
               while(!rsData.isAfterLast())
               {
                   Object[] rowData = new Object[100];
                        ...
                        rowData[2] = EITLERPGLOBAL.formatDate(rsData.getString("APPROVED_DATE"));//DATE_OF_RESCHEDULE
                        ...
                        rowData[5] = rsData.getString("RE_SCH_MONTH");//RESCHEDULED_MONTH
                        rowData[6] = EITLERPGLOBAL.formatDate(rsData.getString("DATE_OF_COMMUNICATION"));//DATE_OF_COMMUNICATION
                        rowData[7] = rsData.getString("MODE_OF_COMMUNICATION");//MODE_OF_COMMUNICATION
                        rowData[8] = rsData.getString("CONTACT_PERSON");//CONTACT_PERSON
                        rowData[9] = rsData.getString("PARTY_JUSTIFICATION");//PARTY_JUSTIFICATION
                        rowData[10] = rsData.getString("AREA_MANAGER_COMMENT");//AREA_MANAGER_COMMENT
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Rescheduling Details");
