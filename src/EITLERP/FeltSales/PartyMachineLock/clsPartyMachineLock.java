...
package EITLERP.FeltSales.PartyMachineLock;
...
public class clsPartyMachineLock {
    ...
    public static int ModuleID=627;   //627
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE WHERE DOC_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND DOC_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' AND STATUS_TYPE='LOCK' ORDER BY DOC_DATE");
            Ready=true;
            MoveLast();
            return true;
        }
    ...
    public boolean ShowHistory(String pDocNo) {
        ...
        try {
            ...
            rsResultSet=Stmt.executeQuery("SELECT * FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE_H WHERE STATUS_TYPE='LOCK' AND DOC_NO='"+pDocNo+"'");
            Ready=true;
            HistoryView=true;
            MoveFirst();
            return true;
    ...
    public static HashMap getHistoryList(int pCompanyID,String pDocNo) {
        ...
        try {
            ...
            rsTmp=stTmp.executeQuery("SELECT * FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE_H WHERE STATUS_TYPE='LOCK' AND DOC_NO='"+pDocNo+"'");
            
            while(rsTmp.next()) {
                clsPartyMachineLock ObjDoc=new clsPartyMachineLock();
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHistory=stHistory.executeQuery("SELECT * FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE_H WHERE STATUS_TYPE='LOCK' AND DOC_NO='1'"); // '1' for restricting all data retrieval
            rsHistory.first();
            //------------------------------------//
            ...
            setAttribute("DOC_NO",clsFirstFree.getNextFreeNo((int)EITLERPGLOBAL.gCompanyID, clsPartyMachineLock.ModuleID, (int)getAttribute("FFNO").getVal(), true));
            //-------------------------------------------------
            ...
            rsResultSet.updateString("STATUS_TYPE","LOCK");
            ...
            rsHistory.updateString("STATUS_TYPE","LOCK");
            ...
            ObjFlow.ModuleID=clsPartyMachineLock.ModuleID;
...
//          }
            ...
            if(AppStatus.equals("F")) {
                if(getAttribute("DOC_NO").getObj().toString().startsWith("FPL")){  //FPC
                    data.Execute("UPDATE DINESHMILLS.D_SAL_PARTY_MASTER SET PARTY_LOCK=1 WHERE PARTY_CODE='"+getAttribute("PARTY_CODE").getObj().toString()+"' AND MAIN_ACCOUNT_CODE=210010");
                    data.Execute("UPDATE PRODUCTION.FELT_MACHINE_MASTER_HEADER SET MACHINE_LOCK_IND=1 WHERE MM_PARTY_CODE='"+getAttribute("PARTY_CODE").getObj().toString()+"'");
                    data.Execute("UPDATE PRODUCTION.FELT_MACHINE_MASTER_DETAIL SET POSITION_LOCK_IND=1 WHERE MM_PARTY_CODE='"+getAttribute("PARTY_CODE").getObj().toString()+"'");
                }
                
                if(getAttribute("DOC_NO").getObj().toString().startsWith("PML")){   //PMC
                    data.Execute("UPDATE PRODUCTION.FELT_MACHINE_MASTER_HEADER SET MACHINE_LOCK_IND=1 WHERE MM_PARTY_CODE='"+getAttribute("PARTY_CODE").getObj().toString()+"' AND MM_MACHINE_NO='"+getAttribute("MACHINE_NO").getObj().toString()+"' ");
                    data.Execute("UPDATE PRODUCTION.FELT_MACHINE_MASTER_DETAIL SET POSITION_LOCK_IND=1 WHERE MM_PARTY_CODE='"+getAttribute("PARTY_CODE").getObj().toString()+"' AND MM_MACHINE_NO='"+getAttribute("MACHINE_NO").getObj().toString()+"' ");
                }
                
                if(getAttribute("DOC_NO").getObj().toString().startsWith("MPL")){  //MPC
                    data.Execute("UPDATE PRODUCTION.FELT_MACHINE_MASTER_DETAIL SET POSITION_LOCK_IND=1 WHERE MM_PARTY_CODE='"+getAttribute("PARTY_CODE").getObj().toString()+"' AND MM_MACHINE_NO='"+getAttribute("MACHINE_NO").getObj().toString()+"' AND MM_MACHINE_POSITION='"+getAttribute("POSITION_NO").getObj().toString()+"' ");
                }
            }
    ...
    public boolean Update() {
        ...
        try {
            ...
            //========= Inserting Into History =================//
            //Get the Maximum Revision No.
            int RevNo=EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE_H WHERE STATUS_TYPE='LOCK' AND DOC_NO='"+(String)getAttribute("DOC_NO").getObj()+"'");
            RevNo++;
            ...
            if(IsRejected) {
                //Remove the Rejected Flag First
                data.Execute("UPDATE PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE STATUS_TYPE='LOCK' AND DOC_NO='"+ObjFlow.DocNo+"'");
                //Remove Old Records from PRODUCTION.FELT_PROD_DOC_DATA
    ...
    public boolean Delete(int pUserID) {
        try {
            ...
            if(CanDelete(lCompanyID,lDocNo,pUserID)) {
                String strQry = "DELETE FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE WHERE STATUS_TYPE='LOCK' AND DOC_NO='"+lDocNo+"'";
                data.Execute(strQry);
    ...
    public boolean Filter(String pCondition,long pCompanyID) {
        ...
        try {
            ...
            if(rsResultSet.getRow()<=0) {
                strSql = "SELECT * FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE WHERE DOC_DATE>='"+EITLERPGLOBAL.FinFromDateDB+"' AND DOC_DATE<='"+EITLERPGLOBAL.FinToDateDB+"' AND STATUS_TYPE='LOCK' ORDER BY DOC_NO";
                rsResultSet=Stmt.executeQuery(strSql);
                Ready=true;
                MoveLast();
    ...
    public boolean CanDelete(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            strSQL="SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE WHERE STATUS_TYPE='LOCK' AND DOC_NO='"+pDocNo+"' AND (APPROVED=1)";
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID='"+clsPartyMachineLock.ModuleID+"' AND DOC_NO='"+pDocNo+"' AND USER_ID="+Integer.toString(pUserID)+" AND TYPE='C' AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public boolean IsEditable(int pCompanyID,String pDocNo,int pUserID) {
        ...
        try {
            ...
            if(rsTmp.getInt("COUNT")>0)  //Item is Approved
            {
                ...
                strSQL="SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID='"+clsPartyMachineLock.ModuleID+"' AND DOC_NO='"+pDocNo+"' AND USER_ID="+Integer.toString(pUserID)+" AND STATUS='W'";
                tmpStmt=Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                rsTmp=tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static boolean CanCancel(int pCompanyID,String pDocNo) {
        ...
        try {
            rsTmp=data.getResult("SELECT DOC_NO,APPROVED FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE WHERE STATUS_TYPE='LOCK' AND DOC_NO='"+pDocNo+"' AND CANCELLED=0");
            rsTmp.first();
    ...
    public static boolean CancelDoc(int pCompanyID,String pDocNo) {
        ...
        try {
            if(CanCancel(pCompanyID,pDocNo)) {
                ...
                rsTmp=data.getResult("SELECT APPROVED FROM PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE WHERE STATUS_TYPE='LOCK' AND DOC_NO='"+pDocNo+"'");
                rsTmp.first();
                ...
                if(ApprovedDoc) {
                    ...
                    int ModuleID=clsPartyMachineLock.ModuleID;
                ...
                data.Execute("UPDATE PRODUCTION.FELT_PARTY_MACHINE_POSITION_CLOSURE SET CANCELLED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE STATUS_TYPE='LOCK' AND DOC_NO='"+pDocNo+"'");
