 ...
 * @author root
 */
public class ObsoleteUnMapped_30 {
    ...
    private static void Auto_UnMapped_30() {
        ...
        try {
            ...
            rsData1 = stmt1.executeQuery("SELECT * FROM PRODUCTION.OBSOLETE_MAPPED_UNMAPPED_SCRAP "
                    ...
                    + "AND DATEDIFF(CURDATE(),ENTRY_DATE)>30 AND COALESCE(AUTO_UNMAPPED_IND,0)=0 "
                    + "UNION ALL "
                    + "SELECT * FROM PRODUCTION.OBSOLETE_MAPPED_UNMAPPED_SCRAP "
                    + "WHERE UPN_ASSIGN_STATUS='UNMAPPED' AND COALESCE(MAPPING_DOC_NO,'') NOT IN ('') "
                    + "AND PIECE_STAGE NOT IN ('DIVERTED','INVOICED','EXP-INVOICE','SCRAP','DIVIDED','RETURN','JOINED') "
                    + "AND DATEDIFF(CURDATE(),ENTRY_DATE)>60 AND COALESCE(AUTO_UNMAPPED_IND,0)=1 "
                    + "UNION ALL "
                    + "SELECT * FROM PRODUCTION.OBSOLETE_MAPPED_UNMAPPED_SCRAP "
                    + "WHERE UPN_ASSIGN_STATUS='UNMAPPED' AND COALESCE(MAPPING_DOC_NO,'') NOT IN ('') "
                    + "AND PIECE_STAGE NOT IN ('DIVERTED','INVOICED','EXP-INVOICE','SCRAP','DIVIDED','RETURN','JOINED') "
                    + "AND DATEDIFF(CURDATE(),ENTRY_DATE)>90 AND COALESCE(AUTO_UNMAPPED_IND,0)=2 "
                    + "UNION ALL "
                    + "SELECT * FROM PRODUCTION.OBSOLETE_MAPPED_UNMAPPED_SCRAP "
                    + "WHERE UPN_ASSIGN_STATUS='UNMAPPED' AND COALESCE(MAPPING_DOC_NO,'') NOT IN ('') "
                    + "AND PIECE_STAGE NOT IN ('DIVERTED','INVOICED','EXP-INVOICE','SCRAP','DIVIDED','RETURN','JOINED') "
                    + "AND DATEDIFF(CURDATE(),ENTRY_DATE)>120 AND COALESCE(AUTO_UNMAPPED_IND,0)=3 "
                    + "UNION ALL "
                    + "SELECT * FROM PRODUCTION.OBSOLETE_MAPPED_UNMAPPED_SCRAP "
                    + "WHERE UPN_ASSIGN_STATUS='UNMAPPED' AND COALESCE(MAPPING_DOC_NO,'') NOT IN ('') "
                    + "AND PIECE_STAGE NOT IN ('DIVERTED','INVOICED','EXP-INVOICE','SCRAP','DIVIDED','RETURN','JOINED') "
                    + "AND DATEDIFF(CURDATE(),ENTRY_DATE)>150 AND COALESCE(AUTO_UNMAPPED_IND,0)=4 "
                    + "");
            rsData1.first();
            ...
            if (rsData1.getRow() > 0) {
                ...
                sql = "INSERT INTO PRODUCTION.FELT_OBSOLETE_MAPPING_HEADER "
                        ...
                        + "VALUES('" + pDocNo + "', '" + pDocDate + "', 'UNMAPPED ENTRY', 'AUTO GENERATED', "
                ...
                sql = "INSERT INTO PRODUCTION.FELT_OBSOLETE_MAPPING_HEADER_H "
                        ...
                        + "VALUES(1, '" + pDocNo + "', '" + pDocDate + "', 'UNMAPPED ENTRY', 'AUTO GENERATED', "
                        + "338, '" + pDocDate + "', 0, '0000-00-00', 1, '" + pDocDate + "', 0, '', "
                        + "'0000-00-00', 0, '', '0000-00-00', 0, 4439, "
                ...
                while (!rsData1.isAfterLast()) {
                    ...
                    sql = "INSERT INTO PRODUCTION.FELT_OBSOLETE_MAPPING_DETAIL "
                            ...
                            + "UPN1, UPN2, UPN3, UPN4, UPN5, UPN6, UPN7, UPN8, UPN9, UPN10, UPN11, UPN12, UPN13, UPN14, UPN15, UPN16, UPN17, UPN18, UPN19, UPN20, "
                            + "'UNMAPPED ENTRY', '" + pPieceStage + "', UPN_ASSIGN_STATUS "
                    ...
                    sql = "INSERT INTO PRODUCTION.FELT_OBSOLETE_MAPPING_DETAIL_H "
                            ...
                            + "UPN1, UPN2, UPN3, UPN4, UPN5, UPN6, UPN7, UPN8, UPN9, UPN10, UPN11, UPN12, UPN13, UPN14, UPN15, UPN16, UPN17, UPN18, UPN19, UPN20, "
                            + "'UNMAPPED ENTRY', UPN_ASSIGN_STATUS "
                            + "FROM PRODUCTION.FELT_OBSOLETE_MAPPING_DETAIL "
                            + "WHERE DOC_NO='" + pMapDocNo + "' AND PIECE_NO='" + pPieceNo + "' AND PARTY_CODE='" + pPartyCode + "' ";
                    System.out.println("Insert Into Detail History Data :" + sql);
                    data.Execute(sql);
                    
                    data.Execute("UPDATE PRODUCTION.OBSOLETE_MAPPED_UNMAPPED_SCRAP SET AUTO_UNMAPPED_IND=(COALESCE(AUTO_UNMAPPED_IND,0)+1) "
                ...
                sql = "INSERT INTO PRODUCTION.FELT_PROD_DOC_DATA "
                        ...
                        + "CASE WHEN CREATOR =0 THEN 'A' ELSE 'C'  END,'UNMAPPED ENTRY',SR_NO,0,'','" + pDocDate + "',"
