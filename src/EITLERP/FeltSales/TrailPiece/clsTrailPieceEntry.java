 ...
 * @author
 */
public class clsTrailPieceEntry {
    ...
    public static int ModuleID = 799; //72
    ...
    public boolean LoadData(long pCompanyID) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION  GROUP BY DOC_NO");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
    ...
    public boolean Insert() {
        ...
        try {
            ...
            rsHistory = stHistory.executeQuery("SELECT * FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION_H WHERE DOC_NO='1'"); // '1' for restricting all data retrieval
            rsHistory.first();

            //------------------------------------//
            //--------- Generate Gatepass requisition no.  ------------
            setAttribute("DOC_NO", clsFirstFree.getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 799, (int) getAttribute("FFNO").getVal(), true));
            //-------------------------------------------------
            ...
            for (int i = 1; i <= colMRItems.size(); i++) {
                clsTrailPieceEntryItem ObjMRItems = (clsTrailPieceEntryItem) colMRItems.get(Integer.toString(i));
                ...
                rsResultSet.updateString("PIECETRIAL_CATEGORY", ObjMRItems.getAttribute("PIECETRIAL_CATEGORY").getString());
                ...
                rsHistory.updateString("PIECETRIAL_CATEGORY", ObjMRItems.getAttribute("PIECETRIAL_CATEGORY").getString());
            ...
            ObjFlow.ModuleID = clsTrailPieceEntry.ModuleID;
            ...
            ObjFlow.TableName = "PRODUCTION.FELT_TRAIL_PIECE_SELECTION";
            ...
            if (ObjFlow.Status.equals("F")) {
                String sql = "UPDATE PRODUCTION.FELT_SALES_PIECE_REGISTER R,PRODUCTION.FELT_TRAIL_PIECE_SELECTION S "
                        + "SET PR_PIECETRIAL_FLAG=1,PR_PIECETRIAL_CATEGORY=PIECETRIAL_CATEGORY "
                        + "WHERE S.FT_PIECE_NO=R.PR_PIECE_NO AND "
                        + "S.DOC_NO='" + (String) getAttribute("DOC_NO").getObj() + "'";
                data.Execute(sql);
            }
    ...
    public boolean Update() {
        ...
        try {
            ...
            int RevNo = EITLERPGLOBAL.GetQueryResult("SELECT MAX(REVISION_NO) FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION_H WHERE DOC_NO='" + (String) getAttribute("DOC_NO").getObj() + "'");
            RevNo++;
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(ModuleID, (String) getAttribute("DOC_NO").getObj())) {
                ...
                data.Execute("DELETE FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION WHERE DOC_NO='" + mDocNo + "'");
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE PRODUCTION.FELT_TRAIL_PIECE_SELECTION SET REJECTED=0,CHANGED=1,CHANGED_DATE=CURDATE() WHERE DOC_NO='" + ObjFlow.DocNo + "' ");

                //Remove Old Records from FELT_DOC_DATA
                data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=" + clsTrailPieceEntry.ModuleID + " AND DOC_NO='" + ObjFlow.DocNo + "'");
    ...
    public boolean setData() {
        ...
        try {
            ...
            if (HistoryView) {
                ...
                rsTmp = tmpStmt.executeQuery("SELECT * FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION_H WHERE  DOC_NO='" + mDocNo + "' AND REVISION_NO=" + RevNo + " ");

            } else {
                rsTmp = tmpStmt.executeQuery("SELECT * FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION WHERE  DOC_NO='" + mDocNo + "' ");
            }
            rsTmp.first();
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                Counter = Counter + 1;
                clsTrailPieceEntryItem ObjMRItems = new clsTrailPieceEntryItem();
                ObjMRItems.setAttribute("FT_PIECE_NO", rsTmp.getString("FT_PIECE_NO"));
                ObjMRItems.setAttribute("PIECETRIAL_CATEGORY", rsTmp.getString("PIECETRIAL_CATEGORY"));
    ...
    public boolean IsEditable(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION WHERE DOC_NO='" + pDocNo + "' AND (APPROVED=1)";
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=799 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getHistoryList(int CompanyID, String DocNo) {
        ...
        try {
            String strSQL = "SELECT DOC_NO,DOC_DATE,REVISION_NO,UPDATED_BY,APPROVAL_STATUS,DATE_FORMAT(ENTRY_DATE,'%d/%m/%Y %H:%i:%S') AS ENTRY_DATE,REJECTED_REMARKS AS APPROVER_REMARKS,FROM_IP FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION_H WHERE DOC_NO='" + DocNo + "' GROUP BY REVISION_NO,DOC_NO";
            rsTmp = data.getResult(strSQL);
            rsTmp.first();

            if (rsTmp.getRow() > 0) {
                while (!rsTmp.isAfterLast()) {
                    clsTrailPieceEntry objParty = new clsTrailPieceEntry();
    ...
    public boolean Filter(String Condition) {
        ...
        try {
            String strSQL = "";
            strSQL += "SELECT * FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION WHERE " + Condition + " GROUP BY DOC_NO";
            ...
            if (!rsResultSet.first()) {
                strSQL = "SELECT * FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION GROUP BY DOC_NO ";
                rsResultSet = Stmt.executeQuery(strSQL);
                Ready = true;
                MoveLast();
                return false;
    ...
    public boolean CanDelete(int pCompanyID, String pDocNo, int pUserID) {
        ...
        try {
            ...
            if (rsTmp.getInt("COUNT") > 0) //Item is Approved
            {
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=799 AND DOC_NO='" + pDocNo + "' AND USER_ID=" + Integer.toString(pUserID) + " AND TYPE='C' AND STATUS='W'";
                tmpStmt = Conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT DISTINCT PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_NO,PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_DATE,RECEIVED_DATE FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION,PRODUCTION.FELT_PROD_DOC_DATA WHERE PRODUCTION.FELT_PROD_DOC_DATA.DOC_NO=PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_NO AND PRODUCTION.FELT_PROD_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND PRODUCTION.FELT_PROD_DOC_DATA.STATUS='W' AND MODULE_ID=799 ORDER BY PRODUCTION.FELT_PROD_DOC_DATA.RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT DISTINCT PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_NO,PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_DATE,RECEIVED_DATE FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION,PRODUCTION.FELT_PROD_DOC_DATA WHERE PRODUCTION.FELT_PROD_DOC_DATA.DOC_NO=PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_NO AND PRODUCTION.FELT_PROD_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND PRODUCTION.FELT_PROD_DOC_DATA.STATUS='W' AND MODULE_ID=799 ORDER BY PRODUCTION.FELT_TRAIL_PIECE_SELECTION.CREATED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT DISTINCT PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_NO,PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_DATE,RECEIVED_DATE FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION,PRODUCTION.FELT_PROD_DOC_DATA WHERE PRODUCTION.FELT_PROD_DOC_DATA.DOC_NO=PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_NO AND PRODUCTION.FELT_PROD_DOC_DATA.USER_ID=" + Integer.toString(pUserID) + " AND PRODUCTION.FELT_PROD_DOC_DATA.STATUS='W' AND MODULE_ID=799 ORDER BY PRODUCTION.FELT_TRAIL_PIECE_SELECTION.DOC_NO";
            }
            ...
            while (!rsTmp.isAfterLast() && rsTmp.getRow() > 0) {
                ...
                Counter = Counter + 1;
                clsTrailPieceEntry ObjItem = new clsTrailPieceEntry();
    ...
    public boolean ShowHistory(int pCompanyID, String pDocNo) {
        ...
        try {
            ...
            rsResultSet = Stmt.executeQuery("SELECT * FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION_H WHERE DOC_NO='" + pDocNo + "' ORDER BY REVISION_NO");
            Ready = true;
            HistoryView = true;
            MoveFirst();
            return true;
    ...
    public static boolean CanCancel(int pCompanyID, String pDocNo) {
        ...
        try {
            rsTmp = data.getResult("SELECT DOC_NO FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION WHERE  DOC_NO='" + pDocNo + "' AND CANCELED=0");
            rsTmp.first();
    ...
    public static boolean CancelDoc(int pCompanyID, String pDocNo) {
        ...
        try {
            if (CanCancel(pCompanyID, pDocNo)) {
                ...
                rsTmp = data.getResult("SELECT APPROVED FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION WHERE DOC_NO='" + pDocNo + "' ");
                rsTmp.first();
                ...
                data.Execute("UPDATE PRODUCTION.FELT_TRAIL_PIECE_SELECTION SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE DOC_NO='" + pDocNo + "' ");
                Cancelled = true;
            }
