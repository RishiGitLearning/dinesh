...
public class frmTrailPieceEntry extends javax.swing.JApplet {
...
//    private static PieChart pieChart;
    ...
    private void GenerateCombos() {
        ...
        List = clsHierarchy.getListEx(" WHERE D_COM_HIERARCHY.COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + clsTrailPieceEntry.ModuleID + "");

        if (EditMode == EITLERPGLOBAL.EDIT) {
            List = clsHierarchy.getList(" WHERE COMPANY_ID=" + EITLERPGLOBAL.gCompanyID + " AND MODULE_ID=" + clsTrailPieceEntry.ModuleID + "");
        }
    ...
    private void initComponents() {
        ...
        lblTitle.setText("Felt Trial Piece Selection");
        ...
        jTabbedPane1.addTab("Trial/Sample Piece Selection", Tab1);
        ...
        jScrollPane7.setViewportView(TableDesc1);

        Tab2.add(jScrollPane7);
        jScrollPane7.setBounds(0, 0, 740, 360);

        jTabbedPane1.addTab("Trial/Sample UPN Selection", Tab2);
    ...
    private void OpgApproveMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_OpgApproveMouseClicked
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            GenerateRejectedUserCombo();
            if (clsFeltProductionApprovalFlow.IsOnceRejectedDoc(clsTrailPieceEntry.ModuleID, DocNo)) {
                cmbSendTo.setEnabled(true);
            } else {
                cmbSendTo.setEnabled(false);
            }
    ...
    private void btnShowDataActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnShowDataActionPerformed
        ...
        try {
            ...
            SQL = "SELECT DISTINCT PR_PIECE_NO,"
                    ...
                    + "WHERE COALESCE(PR_DELINK,'')!='OBSOLETE' AND COALESCE(PR_PIECETRIAL_FLAG,0)=0 AND COALESCE(PARTY_LOCK,0)=0 AND "
            ...
            SQL = "SELECT DISTINCT MM_PARTY_CODE AS PARTY_CODE,PARTY_NAME,MM_UPN_NO AS UPN,MM_GRUP AS GRUP,MM_MACHINE_NO AS MACHINE_NO,"
                    + "MM_MACHINE_POSITION_DESC AS POSITION_DESC,INCHARGE_NAME AS ZONE,M.INCHARGE_CD  "
                    + "FROM PRODUCTION.FELT_MACHINE_MASTER_DETAIL  "
                    + "LEFT JOIN DINESHMILLS.D_SAL_PARTY_MASTER M ON MM_PARTY_CODE=PARTY_CODE "
                    + "LEFT JOIN PRODUCTION.FELT_INCHARGE I ON M.INCHARGE_CD=I.INCHARGE_CD  "
                    + "WHERE  COALESCE(PARTY_LOCK,0)=0 AND "
                    + "COALESCE(POSITION_LOCK_IND,0)=0 AND "
                    + "COALESCE(PARTY_MILL_CLOSED_IND,0)=0 "
                    + "AND MM_UPN_NO NOT IN (SELECT FT_PIECE_NO FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION WHERE COALESCE(CANCELED,0)=0)";

            if (txtPartyCode.getText().trim().length() >= 6) {
                SQL = SQL + " AND MM_PARTY_CODE='" + txtPartyCode.getText() + "'";
            }
            if (txtUPN.getText().trim().length() >= 6) {
                SQL = SQL + " AND MM_UPN_NO='" + txtUPN.getText() + "'";
            }
            ...
            if (tdata1.getRow() > 0) {
                ...
                while (!tdata1.isAfterLast()) {
                    ...
                    rowData[0] = srNo;
                    rowData[1] = false;
                    rowData[2] = tdata1.getString("UPN");
                    rowData[3] = tdata1.getString("PARTY_CODE");
                    rowData[4] = tdata1.getString("PARTY_NAME");
                    rowData[5] = tdata1.getString("ZONE");
                    rowData[6] = tdata1.getString("GRUP");
                    rowData[7] = tdata1.getString("MACHINE_NO");
    ...
    private void Add() {
        ...
        if (aList.ShowList()) {
            ...
            DOC_NO.setText(clsFirstFree.getNextFreeNo(EITLERPGLOBAL.gCompanyID, 799, FFNo, false));
    ...
    private void SetupApproval() {
        ...
        if (EditMode == EITLERPGLOBAL.ADD) {
            ...
            int FromUserID = clsFeltProductionApprovalFlow.getFromID(clsTrailPieceEntry.ModuleID, (String) Obj.getAttribute("PROFORMA_NO").getObj());
            lnFromID = FromUserID;
            String strFromUser = clsUser.getUserName(EITLERPGLOBAL.gCompanyID, FromUserID);
            String strFromRemarks = clsFeltProductionApprovalFlow.getFromRemarks(clsTrailPieceEntry.ModuleID, FromUserID, (String) Obj.getAttribute("PROFORMA_NO").getObj());
    ...
    private void GenerateFromCombo() {
        ...
        try {
            if (EditMode == EITLERPGLOBAL.ADD) {
                ...
                List = clsFeltProductionApprovalFlow.getRemainingUsers(clsTrailPieceEntry.ModuleID, DocNo);
                for (int i = 1; i <= List.size(); i++) {
                    clsUser ObjUser = (clsUser) List.get(Integer.toString(i));
                    ComboData aData = new ComboData();
                    aData.Code = (int) ObjUser.getAttribute("USER_ID").getVal();
    ...
    private void Edit() {
        ...
        if (Obj.IsEditable(EITLERPGLOBAL.gCompanyID, lDocNo, EITLERPGLOBAL.gNewUserID)) {
            ...
            if (clsFeltProductionApprovalFlow.IsCreator(clsTrailPieceEntry.ModuleID, lDocNo)) {  // || clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 6070, 60702)) {
                SetFields(true);
            } else {
                EnableApproval();
            }
    ...
    private void Save() {
        ...
        for (int k = 0; k <= TableDesc.getRowCount() - 1; k++) {
            if (TableDesc.getValueAt(k, 1).toString().equalsIgnoreCase("TRUE")) {
                mcntpc++;
            }
        }
...
//        }
    ...
    private void DisplayData() {
        ...
        if (EITLERPGLOBAL.gAuthorityUserID != EITLERPGLOBAL.gUserID) {
            int ModuleID = clsTrailPieceEntry.ModuleID;
        ...
        try {
            ...
            double mtottpick = 0;
            int srNo = 0;
            String sql = "SELECT DISTINCT PR_PIECE_NO,"
                    + "PR_PIECE_STAGE,PR_WIP_STATUS,PR_PARTY_CODE,PARTY_NAME,PR_DOC_NO,"
                    + "COALESCE(DATE_FORMAT(CASE WHEN COALESCE(PR_ORDER_DATE,'0000-00-00')='0000-00-00' THEN '' ELSE PR_ORDER_DATE END,'%d/%m/%Y'),'') AS ORDER_DATE,"
                    + "PR_UPN,PR_PRODUCT_CODE,PR_GROUP,PR_MACHINE_NO,POSITION_DESC,PR_STYLE,"
                    + "PR_LENGTH,PR_WIDTH,PR_GSM,PR_THORITICAL_WEIGHT,INCHARGE_NAME AS ZONE "
                    ...
                    + "LEFT JOIN (SELECT MM_PARTY_CODE,MM_MACHINE_NO,MM_MACHINE_POSITION,POSITION_LOCK_IND FROM PRODUCTION.FELT_MACHINE_MASTER_DETAIL) AS MD "
                    + "ON PR_PARTY_CODE=MM_PARTY_CODE AND PR_MACHINE_NO=MM_MACHINE_NO AND PR_POSITION_NO=MM_MACHINE_POSITION "
                    + "WHERE PR_PIECE_NO IN ("
                    + "SELECT FT_PIECE_NO FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION WHERE DOC_NO='" + DOC_NO.getText() + "'  AND "
                    + "PIECETRIAL_CATEGORY='PIECE' )";
            ...
            if (disdata.getRow() > 0) {
                while (!disdata.isAfterLast()) {
                    ...
                    rowData[0] = srNo;
                    rowData[1] = true;
                    rowData[2] = disdata.getString("PR_PIECE_NO");
                    rowData[3] = disdata.getString("PR_PIECE_STAGE");
                    rowData[4] = disdata.getString("PR_WIP_STATUS");
                    rowData[5] = disdata.getString("PR_DOC_NO");
                    rowData[6] = disdata.getString("ORDER_DATE");
                    rowData[7] = disdata.getString("PR_PARTY_CODE");
                    rowData[8] = disdata.getString("PARTY_NAME");
                    rowData[9] = disdata.getString("PR_PRODUCT_CODE").trim();
                    rowData[10] = disdata.getString("PR_GROUP");
                    rowData[11] = disdata.getString("ZONE");
                    rowData[13] = disdata.getString("PR_MACHINE_NO");
                    rowData[14] = disdata.getString("POSITION_DESC");
                    rowData[15] = disdata.getString("PR_STYLE");
                    rowData[16] = df.format(Double.parseDouble(disdata.getString("PR_LENGTH")));
                    rowData[17] = df.format(Double.parseDouble(disdata.getString("PR_WIDTH")));
                    rowData[18] = disdata.getString("PR_GSM");
                    rowData[19] = df.format(Double.parseDouble(disdata.getString("PR_THORITICAL_WEIGHT")));
                    DataModelDesc.addRow(rowData);
                    disdata.next();
                }
            }
            sql = "SELECT DISTINCT MM_PARTY_CODE AS PARTY_CODE,PARTY_NAME,MM_UPN_NO AS UPN,MM_GRUP AS GRUP,MM_MACHINE_NO AS MACHINE_NO,"
                    ...
                    + "AND MM_UPN_NO IN ("
                    + "SELECT FT_PIECE_NO FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION WHERE DOC_NO='" + DOC_NO.getText() + "'  AND "
                    + "PIECETRIAL_CATEGORY='UPN')";
            disdata = data.getResult(sql);
            disdata.first();
            if (disdata.getRow() > 0) {
                srNo = 0;
                while (!disdata.isAfterLast()) {
                    ...
                    rowData[0] = srNo;
                    rowData[1] = true;
                    rowData[2] = disdata.getString("UPN");
                    rowData[3] = disdata.getString("PARTY_CODE");
                    rowData[4] = disdata.getString("PARTY_NAME");
                    rowData[5] = disdata.getString("ZONE");
                    rowData[6] = disdata.getString("GRUP");
                    rowData[7] = disdata.getString("MACHINE_NO");
                    rowData[8] = disdata.getString("POSITION_DESC");
                    DataModelDesc1.addRow(rowData);
                    disdata.next();
                }
            }
            ...
            List = clsFeltProductionApprovalFlow.getDocumentFlow(clsTrailPieceEntry.ModuleID, DocNo);
            for (int i = 1; i <= List.size(); i++) {
            ...
            FormatGridHS();
            HashMap History = clsTrailPieceEntry.getHistoryList(EITLERPGLOBAL.gCompanyID, DocNo);
            for (int i = 1; i <= History.size(); i++) {
                clsTrailPieceEntry ObjHistory = (clsTrailPieceEntry) History.get(Integer.toString(i));
    ...
    private void SetData() {
        ...
        try {
            ...
            for (int i = 0; i < TableDesc1.getRowCount(); i++) {
                ...
                if (TableDesc1.getValueAt(i, 1).toString().equalsIgnoreCase("TRUE")) {
                    addpc = data.getResult("SELECT PR_PIECE_NO FROM PRODUCTION.FELT_SALES_PIECE_REGISTER WHERE PR_UPN='" + TableDesc1.getValueAt(i, 2).toString() + "'");
                    addpc.first();
                    clsTrailPieceEntryItem ObjItem = new clsTrailPieceEntryItem();
                    ObjItem.setAttribute("FT_PIECE_NO", TableDesc1.getValueAt(i, 2).toString());
                    ObjItem.setAttribute("PIECETRIAL_CATEGORY", "UPN");
                    Obj.colMRItems.put(Integer.toString(Obj.colMRItems.size() + 1), ObjItem);
                    if (addpc.getRow() > 0) {
                        while (!addpc.isAfterLast()) {
                            //clsTrailPieceEntryItem ObjItem = new clsTrailPieceEntryItem();
                            ObjItem = new clsTrailPieceEntryItem();
                            tp.put(addpc.getString("PR_PIECE_NO"), "1");
                            ObjItem.setAttribute("FT_PIECE_NO", addpc.getString("PR_PIECE_NO"));
                            ObjItem.setAttribute("PIECETRIAL_CATEGORY", "UPN");
                            Obj.colMRItems.put(Integer.toString(Obj.colMRItems.size() + 1), ObjItem);
                            addpc.next();
                        }
        ...
        for (int i = 0; i < TableDesc.getRowCount(); i++) {
            ...
            if (TableDesc.getValueAt(i, 1).toString().equalsIgnoreCase("TRUE")) {
                if (!tp.containsKey(TableDesc.getValueAt(i, 2).toString())) {
                    ObjItem.setAttribute("FT_PIECE_NO", TableDesc.getValueAt(i, 2).toString());
                    ObjItem.setAttribute("PIECETRIAL_CATEGORY", "PIECE");
                    Obj.colMRItems.put(Integer.toString(Obj.colMRItems.size() + 1), ObjItem);
                }
            }
        }
    ...
    private void SetMenuForRights() {
        // --- Add Rights --        
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 99001, 990011)) {
            cmdNew.setEnabled(true);
        } else {
            cmdNew.setEnabled(false);
        }
        ...
        // --- Edit Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 99001, 990012)) {
            cmdEdit.setEnabled(true);
        } else {
            cmdEdit.setEnabled(false);
        }
        ...
        // --- Delete Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 99001, 990013)) {
            cmdDelete.setEnabled(true);
        } else {
            cmdDelete.setEnabled(false);
        }
        ...
        // --- Print Rights --
        if (clsUser.IsFunctionGranted(EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gNewUserID, 99001, 990015)) {
            cmdPreview.setEnabled(true);
            cmdPrint.setEnabled(true);
        } else {
            cmdPreview.setEnabled(false);
    ...
    public void FindWaiting() {
        Obj.Filter(" DOC_NO IN (SELECT H.DOC_NO FROM PRODUCTION.FELT_TRAIL_PIECE_SELECTION H,PRODUCTION.FELT_PROD_DOC_DATA D WHERE H.DOC_NO=D.DOC_NO AND USER_ID=" + EITLERPGLOBAL.gNewUserID + " AND STATUS='W' AND MODULE_ID=799 AND CANCELED=0)");
        Obj.MoveFirst();
        DisplayData();
    }
    ...
    private void GenerateRejectedUserCombo() {
        ...
        for (int i = 1; i <= List.size(); i++) {
            ...
            if (EditMode == EITLERPGLOBAL.EDIT) {
                if (OpgApprove.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInApproval(clsTrailPieceEntry.ModuleID, DocNo, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }

                if (OpgReject.isSelected()) {
                    IncludeUser = clsFeltProductionApprovalFlow.IncludeUserInRejection(clsTrailPieceEntry.ModuleID, DocNo, (int) ObjUser.getAttribute("USER_ID").getVal(), EITLERPGLOBAL.gNewUserID);
                }
        ...
        if (EditMode == EITLERPGLOBAL.EDIT) {
            ...
            int Creator = clsFeltProductionApprovalFlow.getCreator(clsTrailPieceEntry.ModuleID, DocNo);
    ...
    private void FormatGrid() {
        ...
        try {
            ...
            DataModelDesc.addColumn("Trial/Sample");  //1 - Read Only
            ...
            DataModelDesc1.addColumn("Trial/Sample");  //1 - Read Only
...
//        }
    ...
    private void Mail() {
        System.out.println("Felt Trial Piece Entry approved = " + Obj.getAttribute("APPROVAL_STATUS").getString());
        ...
        if (Obj.getAttribute("APPROVAL_STATUS").getString().equalsIgnoreCase("F")) {
            ...
            pSubject = "Notification : Felt Trial Piece Entry  :" + DOC_NO.getText() + " Final Approved";
            pBody = "Felt Trial Piece Entry. " + DOC_NO.getText() + " has been final approved  by " + clsUser.getUserName((int) EITLERPGLOBAL.gCompanyID, EITLERPGLOBAL.gUserID) + "<br><br><br>";

            try {
                pBody += "Document Name : Felt Trial Piece Entry  <br>";
                ...
                HashMap hmApprovalHistory = clsTrailPieceEntry.getHistoryList(EITLERPGLOBAL.gCompanyID, DOC_NO.getText());
                for (int i = 1; i <= hmApprovalHistory.size(); i++) {
                    pBody += "<tr>";

                    clsTrailPieceEntry ObjWarping = (clsTrailPieceEntry) hmApprovalHistory.get(Integer.toString(i));
                    Object[] rowData = new Object[6];
                ...
                String responce = MailNotification.sendNotificationMail(799, pSubject, pBody, recievers, pcc, EITLERPGLOBAL.getComboCode(cmbHierarchy));
