 ...
 * @author root
 */
public class clsDocTravelVoucher {
    ...
    public String getDoc_pcd() {
        return Doc_pcd;
    }

    public void setDoc_pcd(String Doc_pcd) {
        this.Doc_pcd = Doc_pcd;
    }

    public String getDoc_pname() {
        return Doc_pname;
    }

    public void setDoc_pname(String Doc_pname) {
        this.Doc_pname = Doc_pname;
    }
    ...
    public void saveDocumentFile(int length) {
        try {
            ...
            PreparedStatement statement = data.getConn().prepareStatement("INSERT INTO DOC_MGMT.TRAVEL_VOUCHER "
                    + "(DOC_NAME,DOCUMENT,DOCUMENT_DOC_NO,DOCUMENT_SR_NO,DOC_REMARK,DOC_TYPE,DOC_PARTY_CODE,DOC_PARTY_NAME) "
                    + "values (?,?,?,?,?,?,?,?)");
            ...
            statement.setString(7, Doc_pcd);
            statement.setString(8, Doc_pname);
    ...
    public ArrayList<clsDocTravelVoucher> getStatus(String DOC_NO) {
        ArrayList<clsDocTravelVoucher> dataList = new ArrayList<clsDocTravelVoucher>();
        try {
            ResultSet rs;
            rs = data.getResult("SELECT * FROM DOC_MGMT.TRAVEL_VOUCHER where DOCUMENT_DOC_NO='" + DOC_NO + "'");
            ...
            if (rs.getRow() > 0) {
                ...
                while (!rs.isAfterLast()) {
                    ...
                    clsDocTravelVoucher Obj = new clsDocTravelVoucher();
                    ...
                    Obj.setDoc_pcd(rs.getString("DOC_PARTY_CODE"));
                    Obj.setDoc_pname(rs.getString("DOC_PARTY_NAME"));
