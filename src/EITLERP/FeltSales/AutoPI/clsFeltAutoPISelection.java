 ...
 * @author Vivek Kumar
 */
public class clsFeltAutoPISelection {
    ...
    public boolean LoadData() {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT * FROM PRODUCTION.FELT_AUTO_PI_DATA GROUP BY DOC_NO ORDER BY DOC_NO");
            HistoryView = false;
            Ready = true;
            MoveLast();
            return true;
...
//            else setData();
    ...
    public boolean Insert() {
        ...
        try {
            ...
            resultSetTemp = statementTemp.executeQuery("SELECT * FROM  PRODUCTION.FELT_AUTO_PI_DATA WHERE DOC_NO='1'");

            // Production data history connection
            statementHistory = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
            resultSetHistory = statementHistory.executeQuery("SELECT * FROM PRODUCTION.FELT_AUTO_PI_DATA_H WHERE DOC_NO='1'");

            setAttribute("DOC_NO", getNextFreeNo((int) EITLERPGLOBAL.gCompanyID, 630, 305, true));

            //Now Insert records into production tables
            for (int i = 1; i <= hmFeltFinishingDetails.size(); i++) {
                clsFeltAutoPISelectionDetails ObjFeltFinishingDetails = (clsFeltAutoPISelectionDetails) hmFeltFinishingDetails.get(Integer.toString(i));
                ...
                resultSetTemp.updateString("PI_SELECTED_FLAG",ObjFeltFinishingDetails.getAttribute("PI_SELECTED_FLAG").getString());
                ...
                resultSetTemp.updateDouble("LENGTH", (double) EITLERPGLOBAL.round(ObjFeltFinishingDetails.getAttribute("LENGTH").getVal(), 2));
                resultSetTemp.updateDouble("WIDTH", (double) EITLERPGLOBAL.round(ObjFeltFinishingDetails.getAttribute("WIDTH").getVal(), 2));
                resultSetTemp.updateDouble("WEIGHT", (double) EITLERPGLOBAL.round(ObjFeltFinishingDetails.getAttribute("WEIGHT").getVal(), 2));
                resultSetTemp.updateDouble("PI_VALUE", (double) EITLERPGLOBAL.round(ObjFeltFinishingDetails.getAttribute("PI_VALUE").getVal(), 2));
                resultSetTemp.updateString("PI_GENERATED_FLAG", "0");
                resultSetTemp.updateString("PI_MAILED_FLAG", "0");
                ...
                resultSetHistory.updateString("PI_SELECTED_FLAG",ObjFeltFinishingDetails.getAttribute("PI_SELECTED_FLAG").getString());
                ...
                resultSetHistory.updateDouble("LENGTH", (double) EITLERPGLOBAL.round(ObjFeltFinishingDetails.getAttribute("LENGTH").getVal(), 2));
                resultSetHistory.updateDouble("WIDTH", (double) EITLERPGLOBAL.round(ObjFeltFinishingDetails.getAttribute("WIDTH").getVal(), 2));
                resultSetHistory.updateDouble("WEIGHT", (double) EITLERPGLOBAL.round(ObjFeltFinishingDetails.getAttribute("WEIGHT").getVal(), 2));
                resultSetHistory.updateDouble("PI_VALUE", (double) EITLERPGLOBAL.round(ObjFeltFinishingDetails.getAttribute("PI_VALUE").getVal(), 2));
                resultSetHistory.updateString("PI_GENERATED_FLAG", "0");
                resultSetHistory.updateString("PI_MAILED_FLAG", "0");
            ...
            ObjFeltProductionApprovalFlow.ModuleID = 630; 
            ...
            ObjFeltProductionApprovalFlow.TableName = "PRODUCTION.FELT_AUTO_PI_DATA";
    ...
    public boolean Update() {
        ...
        try {
            ...
            resultSetHistory = statementHistory.executeQuery("SELECT MAX(REVISION_NO) FROM PRODUCTION.FELT_AUTO_PI_DATA_H WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
            ...
            statementHistory.execute("DELETE FROM PRODUCTION.FELT_AUTO_PI_DATA WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
            ...
            if (IsRejected) {
                ...
                data.Execute("UPDATE PRODUCTION.FELT_AUTO_PI_DATA SET REJECTED=0,CHANGED=1 WHERE DOC_NO='" + getAttribute("DOC_NO").getString() + "'");
            ...
            if (ObjFeltProductionApprovalFlow.Status.equals("F") && ObjFeltProductionApprovalFlow.finalApproved) {
                ...
                try {
                    data.Execute("UPDATE PRODUCTION.FELT_AUTO_PI_DATA SET FINAL_APPROVAL_DATE='" + EITLERPGLOBAL.getCurrentDateDB() + "' WHERE DOC_NO = '" + getAttribute("DOC_NO").getString() + "'");
                } catch (Exception e) {
    ...
    public boolean CanDelete(String documentNo, String stringProductionDate, int userID) {
        ...
        try {
            ...
            strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_AUTO_PI_DATA WHERE DOC_NO='" + documentNo + "' AND APPROVED=" + 1;
            ...
            if (rsTmp.getInt("COUNT") > 0) {  //Item is Approved
                ...
                strSQL = "SELECT COUNT(*) AS COUNT FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE MODULE_ID=630 AND USER_ID=" + userID + " AND DOC_NO='" + documentNo + "' AND STATUS='W'";
                rsTmp = tmpStmt.executeQuery(strSQL);
                rsTmp.first();

                if (rsTmp.getInt("COUNT") > 0) {
                    strSQL = "DELETE FROM PRODUCTION.FELT_AUTO_PI_DATA WHERE DOC_NO='" + documentNo + "'";
    ...
    public boolean Filter(String stringFindQuery) {
        Ready = false;
        try {
            String strSql = "SELECT * FROM PRODUCTION.FELT_AUTO_PI_DATA WHERE 1=1 " + stringFindQuery + " GROUP BY DOC_NO ORDER BY DOC_NO ";
            connection = data.getConn();
            statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    public boolean setData() {
        ...
        try {
            ...
            statementTemp = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            String str = "SELECT * FROM PRODUCTION.FELT_AUTO_PI_DATA WHERE DOC_NO = '" + docNo + "' AND DOC_DATE = '" + docDate + "' ORDER BY PARTY_CODE,PIECE_NO";
            ...
            while (!resultSetTemp.isAfterLast()) {
                ...
                ObjFeltFinishingDetails.setAttribute("PI_SELECTED_FLAG", resultSetTemp.getString("PI_SELECTED_FLAG"));                        
                ...
                ObjFeltFinishingDetails.setAttribute("LENGTH", resultSetTemp.getDouble("LENGTH"));
                ObjFeltFinishingDetails.setAttribute("WIDTH", resultSetTemp.getDouble("WIDTH"));
                ObjFeltFinishingDetails.setAttribute("WEIGHT", resultSetTemp.getDouble("WEIGHT"));
                ObjFeltFinishingDetails.setAttribute("PI_VALUE", resultSetTemp.getDouble("PI_VALUE"));
    ...
    public static HashMap getHistoryList(String DocumentNo) {
        ...
        try {
            ...
            rsTmp = stTmp.executeQuery("SELECT * FROM PRODUCTION.FELT_AUTO_PI_DATA_H WHERE DOC_NO='" + DocumentNo + "' GROUP BY REVISION_NO");

            while (rsTmp.next()) {
                clsFeltAutoPISelection ObjFeltFinishing = new clsFeltAutoPISelection();
    ...
    public boolean ShowHistory(String pProductionDate, String pDocNo) {
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT * FROM PRODUCTION.FELT_AUTO_PI_DATA_H WHERE DOC_NO='" + pDocNo + "'");
            Ready = true;
    ...
    public static HashMap getPendingApprovals(int pUserID, int pOrder) {
        ...
        try {
            ...
            if (pOrder == EITLERPGLOBAL.OnRecivedDate) {
                strSQL = "SELECT DISTINCT PI.DOC_NO,PI.DOC_DATE,SUBSTRING(RECEIVED_DATE,1,10) AS RECEIVED_DATE FROM PRODUCTION.FELT_AUTO_PI_DATA PI,PRODUCTION.FELT_PROD_DOC_DATA D WHERE PI.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=630 AND PI.CANCELED=0 ORDER BY RECEIVED_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocDate) {
                strSQL = "SELECT DISTINCT PI.DOC_NO,PI.DOC_DATE,SUBSTRING(RECEIVED_DATE,1,10) AS RECEIVED_DATE FROM PRODUCTION.FELT_AUTO_PI_DATA PI,PRODUCTION.FELT_PROD_DOC_DATA D WHERE PI.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=630 AND PI.CANCELED=0 ORDER BY PI.DOC_DATE";
            }

            if (pOrder == EITLERPGLOBAL.OnDocNo) {
                strSQL = "SELECT DISTINCT PI.DOC_NO,PI.DOC_DATE,SUBSTRING(RECEIVED_DATE,1,10) AS RECEIVED_DATE FROM PRODUCTION.FELT_AUTO_PI_DATA PI,PRODUCTION.FELT_PROD_DOC_DATA D WHERE PI.DOC_NO=D.DOC_NO AND USER_ID=" + pUserID + " AND STATUS='W' AND MODULE_ID=630 AND PI.CANCELED=0 ORDER BY PI.DOC_NO";
            }
            ...
            while (rsTmp.next()) {
                ...
                Counter = Counter + 1;
                clsFeltAutoPISelection ObjDoc = new clsFeltAutoPISelection();
    ...
    public static void CancelDoc(String pDocNo) {
        ...
        if (CanCancel(pDocNo)) {
            ...
            try {
                rsTmp = data.getResult("SELECT APPROVED FROM PRODUCTION.FELT_AUTO_PI_DATA WHERE DOC_NO='" + pDocNo + "' ");
                rsTmp.first();
                ...
                if (!Approved) {
                    data.Execute("DELETE FROM PRODUCTION.FELT_PROD_DOC_DATA WHERE DOC_NO='" + pDocNo + "' AND MODULE_ID=630");
                }
                data.Execute("UPDATE PRODUCTION.FELT_AUTO_PI_DATA SET CANCELED=1,CHANGED=1,CHANGED_DATE=CURDATE() WHERE DOC_NO='" + pDocNo + "'");
    ...
    public static boolean CanCancel(String pDocNo) {
        ...
        try {
            ...
            System.out.println("SELECT DOC_NO FROM PRODUCTION.FELT_AUTO_PI_DATA WHERE DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
            rsTmp = stTmp.executeQuery("SELECT DOC_NO FROM PRODUCTION.FELT_AUTO_PI_DATA WHERE DOC_NO='" + pDocNo + "'  AND APPROVED=0 AND CANCELED=0");
