...
public class clsUser {
    ...
    public boolean Insert() {
        try {
            ...
            long SrNo=data.getMaxID(EITLERPGLOBAL.gCompanyID, "D_COM_USER_RIGHTS", "SR_NO");
            ...
            rsUser.updateString("PASSWORD_EXPIRY_DATE",EITLERPGLOBAL.formatDateDB(getAttribute("PASSWORD_EXPIRY_DATE").getString()));
            ...
            rsUser.updateString("CHANGED_DATE",EITLERPGLOBAL.getCurrentDateDB());
            rsUser.insertRow();
            ...
            for(cnt=1;cnt<=lnSize;cnt++) {
                ...
                rsRights.updateLong("MENU_ID",clsMenu.getMenuIDFromFunctionID(EITLERPGLOBAL.gCompanyID,(int)ObjRights.getAttribute("FUNCTION_ID").getVal(),(long)getAttribute("USER_ID").getVal()));
                ...
                rsRights.updateString("CHANGED_DATE",EITLERPGLOBAL.getCurrentDateDB());
                rsRights.insertRow();
            }
    ...
    public boolean Update() {
        try {
            ...
            for(cnt=1;cnt<=lnSize;cnt++) {
                ...
                rsRights.updateLong("MENU_ID",clsMenu.getMenuIDFromFunctionID(EITLERPGLOBAL.gCompanyID,(int)ObjRights.getAttribute("FUNCTION_ID").getVal(),lUserID) );
                rsRights.updateLong("FUNCTION_ID",(long)ObjRights.getAttribute("FUNCTION_ID").getVal());
                rsRights.updateLong("SR_NO",SrNo);
                rsRights.updateBoolean("CHANGED",true);
                rsRights.updateString("CHANGED_DATE",EITLERPGLOBAL.getCurrentDateDB());
...
//            System.out.println(SQL);
    ...
    public static boolean userCanProcessDept(int pCompanyID,int pDeptID) {
        ...
        try {
            tmpConn=data.getCreatedConn();
            tmpStmt=tmpConn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rsTmp=tmpStmt.executeQuery("SELECT * FROM D_COM_DEPT_BUYERS WHERE COMPANY_ID="+Integer.toString(pCompanyID)+" AND BUYER="+EITLERPGLOBAL.gUserID+" AND DEPT_ID="+pDeptID);
            rsTmp.first();
