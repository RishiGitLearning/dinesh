...
public class clsItemMapping {
    ...
    public clsItemMapping() {
        props=new HashMap();
        props.put("COMPANY_ID",new Variant(0));
        props.put("SR_NO",new Variant(0));
        props.put("FIRST_ITEM_ID",new Variant(""));
        props.put("SECOND_COMPANY_ID",new Variant(0));
        props.put("SECOND_ITEM_ID",new Variant(""));
    ...
    public static String getItemCode(int pCurrentCompanyID, String pCurrentItemCode,int pOtherCompanyID) {
        Connection tmpConn;
        Statement stTmp;
        ResultSet rsTmp;
        String ItemID="";
        ...
        try {
            tmpConn=data.getConn();
            stTmp=tmpConn.createStatement();
            rsTmp=stTmp.executeQuery("SELECT * FROM D_COM_ITEM_MAPPING WHERE COMPANY_ID="+pOtherCompanyID+" AND SECOND_ITEM_ID='"+pCurrentItemCode+"'");
            rsTmp.first();
            
            if(rsTmp.getRow()>0) {
                ItemID=rsTmp.getString("FIRST_ITEM_ID");
            }
            rsTmp.close();
            stTmp.close();
        }
    ...
    public static boolean Delete(int pCompanyID,int pSrNo) {
        try {
            data.Execute("DELETE FROM D_COM_ITEM_MAPPING WHERE COMPANY_ID="+pCompanyID+" AND SR_NO="+pSrNo);
        }
        catch(Exception e) {
    ...
    public static boolean Insert(Object pObjItemMapping) {
        ...
        try {
            ...
            long NextNo=data.getMaxID(EITLERPGLOBAL.gCompanyID, "D_COM_ITEM_MAPPING", "SR_NO")+1;
            
            rsTmp.moveToInsertRow();
            rsTmp.updateInt("COMPANY_ID",EITLERPGLOBAL.gCompanyID);
            rsTmp.updateLong("SR_NO",NextNo);
            rsTmp.updateString("FIRST_ITEM_ID",(String)ObjItem.getAttribute("FIRST_ITEM_ID").getObj());
            rsTmp.updateInt("SECOND_COMPANY_ID",(int)ObjItem.getAttribute("SECOND_COMPANY_ID").getVal());
            rsTmp.updateString("SECOND_ITEM_ID",(String)ObjItem.getAttribute("SECOND_ITEM_ID").getObj());
