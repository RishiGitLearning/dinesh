 ...
 * @author  root
 */
public class clsCheckIssueValue_1 {
            ...
            while(!rsData.isAfterLast()) {
                ...
                while(!rsDetail.isAfterLast()) {
                    ...
                    if(Math.abs(EITLERPGLOBAL.round(TotalIssueValue,3)-EITLERPGLOBAL.round(IssueValue,3))>1 || IssueValue==0) {
                        ...
                        String UpdateQry = "UPDATE D_INV_ISSUE_DETAIL " +
                        "SET RATE="+TotalLandedRate+", ISSUE_VALUE="+EITLERPGLOBAL.round(TotalIssueValue,3)+" "+
                        "WHERE ISSUE_NO='"+IssueNo+"' AND ITEM_CODE='"+ItemCode+"' AND SR_NO="+SrNo;
                        data.Execute(UpdateQry,DataURL);
                         
                        UpdateQry = "UPDATE D_INV_ISSUE_HEADER SET CHANGED=1 WHERE ISSUE_NO='"+IssueNo+"' ";
                        data.Execute(UpdateQry,DataURL);
                         */
                        ...
                        ObjItem.setAttribute("ITEM_QTY",Qty);
    ...
    public static void checkSTMValue(String FromDate,String ToDate) {
        try {
            ...
            String DataURL = "jdbc:mysql://200.0.0.227:3306/DINESHMILLS";
            ResultSet rsData = data.getResult(SQL,DataURL);
            rsData.first();
            int Counter = 0;
            if(rsData.getRow()>0) {
                while(!rsData.isAfterLast()) {
                    ...
                    while(!rsDetail.isAfterLast()) {
                        ...
                        //if(Math.abs(EITLERPGLOBAL.round(TotalIssueValue,3)-EITLERPGLOBAL.round(IssueValue,3))>1) {
                            if(EITLERPGLOBAL.round(TotalIssueValue,3)!=EITLERPGLOBAL.round(IssueValue,3)) {
                                System.out.println("STM NO: " + IssueNo + "STM Value: " + IssueValue + " Issue Value: " + TotalIssueValue);
                            }
                            else
                            {
                                //System.out.println("STM NO: " + IssueNo + "STM Value: " + IssueValue + " Issue Value: " + TotalIssueValue);
    ...
    {
        checkSTMValue("2010-04-01","2011-03-31");
    }
}
